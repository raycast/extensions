...
| model | The AI model to use to answer to the prompt.    Pass an `AI.Model` value to use a Raycast-hosted model, or `{ id: "..." }`  to use a model provided by this extension through `ai.modelProvider`.  The ID is the local model `id` returned by `getModels()`. | <code>[AI.ModelSelector](ai.md#ai.modelselector)</code> |
