...
export async function saveInvoice(record: InvoiceRecord): Promise<void> {
  ...
  const updated = [record, ...existing.filter((i) => i.invoiceId !== record.invoiceId)];
  await LocalStorage.setItem(INVOICES_KEY, JSON.stringify(updated));
}

export async function updateInvoiceRecord(invoiceId: string, changes: Partial<InvoiceRecord>): Promise<void> {
  const existing = await loadInvoices();
  const updated = existing.map((i) => (i.invoiceId === invoiceId ? { ...i, ...changes } : i));
  await LocalStorage.setItem(INVOICES_KEY, JSON.stringify(updated));
}
...
export async function loadListPreferences(): Promise<ListPreferences> {
  ...
  try {
    return { ...DEFAULT_PREFS, ...(JSON.parse(raw) as Partial<ListPreferences>) };
  } catch {
    return DEFAULT_PREFS;
  }
}

export async function saveListPreferences(prefs: ListPreferences): Promise<void> {
