...
function newItem(): LineItem {
  return { id: String(++itemIdCounter), name: "", description: "", quantity: "1", price: "" };
}
...
export default function CreateInvoiceCommand(props: LaunchProps<{ draftValues: DraftValues }>) {
  const { draftValues } = props;

  const [recipientName, setRecipientName] = useState(draftValues?.recipientName ?? "");
  const [recipientEmail, setRecipientEmail] = useState(draftValues?.recipientEmail ?? "");
  const [recipientNameError, setRecipientNameError] = useState<string | undefined>();
  const [recipientEmailError, setRecipientEmailError] = useState<string | undefined>();
  ...
  const [allowPartialPayment, setAllowPartialPayment] = useState(draftValues?.allowPartialPayment ?? false);
  ...
  async function handleCreateLink() {
    ...
    try {
      await showToast({ style: Toast.Style.Animated, title: "Creating invoice…" });
      ...
      await showToast({ style: Toast.Style.Animated, title: "Generating shareable link…" });

      const hasValidEmail = recipientEmail.trim().length > 0 && isValidEmail(recipientEmail.trim());
      ...
      await showHUD(
        hasValidEmail ? `Invoice sent to ${recipientEmail} — link copied ✓` : "Invoice link copied to clipboard ✓",
      );
      popToRoot();
    } catch (err) {
      await showToast({
  ...
  return (
    ...
    >
      ...
      <Form.TextField
        ...
        onBlur={() => {
          if (!recipientName.trim()) setRecipientNameError("Customer name is required");
      ...
      />
      <Form.Dropdown id="currency" title="Currency" value={currency} onChange={setCurrency}>
      ...
      <Form.Description title="Tax" text="Apply a tax to the entire invoice. Leave blank for no tax." />
      <Form.TextField
        id="taxName"
        title="Tax Name"
