...
export function usePaystack() {
  const { liveSecretKey, testSecretKey, mode } = getPreferenceValues<Preferences>();
  ...
  async function paystackFetch<T>(endpoint: string, options?: RequestInit): Promise<PaystackResponse<T>> {
    if (!secretKey) {
      throw new Error("No API key provided. Please add your Paystack secret key in preferences.");
    }
  ...
  return {
    ...
    post: <T>(endpoint: string, body: unknown) =>
      paystackFetch<T>(endpoint, { method: "POST", body: JSON.stringify(body) }),
    put: <T>(endpoint: string, body: unknown) =>
      paystackFetch<T>(endpoint, { method: "PUT", body: JSON.stringify(body) }),
    delete: <T>(endpoint: string) => paystackFetch<T>(endpoint, { method: "DELETE" }),
  };
}
