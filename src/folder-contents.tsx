...
export default function FolderContentsView({ folderId, folderName, parentPath }: FolderContentsViewProps) {
  ...
  const handleRemoveItem = useCallback(
    async (item: FolderItem) => {
      ...
      await showToast({ title: "Removed", message: `${itemName} removed from ${folderName}`, style: Toast.Style.Success });
      await handleSave();
    },
    ...
    async (item: FolderItem) => {
      try {
        ...
        if (item.type === "application" && item.path) {
          ...
          await showToast({ title: "Opened", message: getItemDisplayName(item, appMap, allFolders), style: Toast.Style.Success });
  ...
  const handleOpenAllWeb = useCallback(
    () => openAllWebsites(websiteItems, folderName),
    [websiteItems, folderName],
  );
  ...
  const bulkProps = useMemo(
    ...
    [hasApps, hasWebsites, hasRunningApps, hasUrls, handleOpenAllApps, handleOpenAllWeb, handleQuitAllApplications, copyAsMarkdown, copyAsList],
  );
  ...
  const renderActions = useCallback(
    ...
    [folder, folderId, displayPath, allFolders, bulkProps, handleOpenItem, handleSave, handleRemoveItem, handleDuplicateItem, handleRemoveDuplicates, duplicateInfo, revalidate],
  );
