...
function getBaseUrl(): string {
  const { sandboxMode } = getPreferenceValues<Preferences>();
...
export async function createDraftInvoice(params: CreateInvoiceParams): Promise<{
  ...
  const selfLink: string = data.href ?? data.links?.find((l) => l.rel === "self")?.href ?? "";
  const invoiceId = selfLink.split("/").pop() ?? "";

  const detailResponse = await fetch(`${getBaseUrl()}/v2/invoicing/invoices/${invoiceId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });

  const detail = (await detailResponse.json()) as PayPalInvoiceResponse;
  const invoicerViewUrl: string =
    detail.detail?.metadata?.invoicer_view_url ?? `https://www.paypal.com/invoice/details/${invoiceId}`;
...
export async function updateDueDate(invoiceId: string, dueDate: string | null): Promise<void> {
  const token = await getAccessToken();
...
export async function getFullInvoice(invoiceId: string): Promise<PayPalInvoiceResponse> {
  const token = await getAccessToken();

  const response = await fetch(`${getBaseUrl()}/v2/invoicing/invoices/${invoiceId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export async function sendInvoice(invoiceId: string, sendToRecipient: boolean): Promise<string> {
  const token = await getAccessToken();

  const response = await fetch(`${getBaseUrl()}/v2/invoicing/invoices/${invoiceId}/send`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
  ...
  const data = (await response.json()) as PayPalInvoiceResponse;
  const payerViewUrl: string = data.href ?? data.links?.find((l) => l.rel === "payer-view")?.href ?? "";

  return payerViewUrl;
}

export async function getInvoiceStatus(invoiceId: string): Promise<{ status: string; invoicerViewUrl: string }> {
  const token = await getAccessToken();
  ...
  const invoicerViewUrl: string =
    data.detail?.metadata?.invoicer_view_url ?? `https://www.paypal.com/invoice/details/${invoiceId}`;
