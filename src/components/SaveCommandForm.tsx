...
export default function SaveCommandForm(props: SaveCommandFormProps) {
  console.log(
    "SaveCommandForm: Rendering with props:",
    JSON.stringify({
      method: props.method,
      url: props.url,
  ...
  const { method = "GET", url = "", headers = "", queryParams = "", body = "" } = props;
  ...
  return (
    ...
    >
      ...
      <Form.Separator />
      <Form.TextField id="name" title="Command Name" placeholder="e.g., Trigger Daily Report" autoFocus={true} />
    </Form>
  );
}
