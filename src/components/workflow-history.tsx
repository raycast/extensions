...
function EventItem({ event, icon, color }: { event: HistoryEvent; icon: Icon; color: Color }) {
  ...
  const subtitle = contextName
    ? `${contextName} - Event #${event.eventId}`
    : `Event #${event.eventId}`;
  ...
  return (
    <List navigationTitle="All Events">
      {sortedEvents.map((event) => {
        ...
        const activityName = event.details?.activityTypeName as string | undefined;
        const subtitle = activityName
          ? `${activityName} - Event #${event.eventId}`
          : `Event #${event.eventId}`;
      ...
      metadata={
        <Detail.Metadata>
          ...
          {activity.scheduledEvent && (
            <Detail.Metadata.Label
              title="Scheduled"
              text={activity.scheduledEvent.eventTime.toLocaleString()}
            />
          )}
          {activity.startedEvent && (
            <Detail.Metadata.Label
              title="Started"
              text={activity.startedEvent.eventTime.toLocaleString()}
            />
          )}
          {activity.completedEvent && (
            <Detail.Metadata.Label
              title="Completed"
              text={activity.completedEvent.eventTime.toLocaleString()}
            />
          )}
          {activity.failedEvent && (
            <Detail.Metadata.Label
              title="Failed"
              text={activity.failedEvent.eventTime.toLocaleString()}
            />
          )}
        </Detail.Metadata>
      }
