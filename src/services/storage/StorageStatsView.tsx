...
export default function StorageStatsView({ projectId, gcloudPath, bucketName }: StorageStatsViewProps) {
    ...
    try {
      ...
      if (bucketName) {
        ...
        if (Array.isArray(objects) && objects.length > 0) {
          ...
          storageStats.recentlyModified.sort((a, b) => 
      ...
      setError(`Failed to analyze storage: ${error.message}`);
      showToast({
  ...
  function generateStorageOverview(): string {
    ...
    return `# Storage Overview ${bucketName ? `for ${bucketName}` : ''}\n\n` +
  ...
  return (
    <List
      ...
      actions={
        <ActionPanel>
          <Action title="Refresh" icon={Icon.ArrowClockwise} shortcut={{ modifiers: ["cmd"], key: "r" }} onAction={fetchStorageStats} />
            ...
            <List.Item.Detail
              ...
              metadata={
                <List.Item.Detail.Metadata>
                  ...
                  <List.Item.Detail.Metadata.Label title="Average Size" text={formatBytes(stats.objectCount > 0 ? stats.totalSize / stats.objectCount : 0)} />
                  <List.Item.Detail.Metadata.Separator />
                  <List.Item.Detail.Metadata.Label title="Storage Classes" text={Object.keys(stats.storageClassDistribution).length.toString()} />
                  <List.Item.Detail.Metadata.Label title="Regions" text={Object.keys(stats.regionDistribution).length.toString()} />
                </List.Item.Detail.Metadata>
              }
            />
          }
        ...
        {stats.largestObjects.map((obj, index) => (
          <List.Item
            ...
            accessories={[
              ...
              { text: new Date(obj.updated).toLocaleDateString() }
            ]}
            detail={
              <List.Item.Detail
