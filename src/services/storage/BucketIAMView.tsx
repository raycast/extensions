    ...
    push(
      ...
      >
        <Form.Dropdown id="role" title="Role" defaultValue="roles/storage.objectViewer" info="The IAM role to grant">
          ...
          <Form.Dropdown.Item value="roles/storage.legacyBucketWriter" title="Legacy Bucket Writer" icon={Icon.Pencil} />
        ...
        <Form.Dropdown id="memberType" title="Member Type" defaultValue="user" info="The type of member to grant the role to">
...
• Legacy Object Reader: Read specific objects`}
  ...
  async function createBinding(formValues: any) {
  ...
  function viewRoleDetails(role: string, binding: IAMBinding) {
    ...
    const membersText = binding.members.map(member => {
    ...
    const detailsMarkdown = `# Role: ${formattedRole}\n\n` +
      `## Members\n\n` +
      membersText +
      (conditionText ? `\n\n${conditionText}` : '') +
      `\n\n## Permissions\n\n` +
  ...
  function getRoleIcon(role: string) {
    ...
    } else if (role.includes('viewer') || role.includes('Viewer') || role.includes('reader') || role.includes('Reader')) {
      return { source: Icon.Eye, tintColor: Color.Blue };
    } else if (role.includes('creator') || role.includes('Creator') || role.includes('writer') || role.includes('Writer')) {
  ...
  return (
    ...
    >
      {!policy || policy.bindings.length === 0 && !isLoading ? (
        ...
        policy && policy.bindings.map((binding) => (
