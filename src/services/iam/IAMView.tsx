import { ActionPanel, Action, List, showToast, Toast, useNavigation, Icon, Detail, Color, confirmAlert, Form } from "@raycast/api";
import { useState, useEffect, useMemo } from "react";
import { IAMService, IAMPrincipal, IAMRole } from "./IAMService";
import { predefinedRoles, getRoleInfo, formatRoleName } from "../../utils/iamRoles";
...
export default function IAMView({ projectId, gcloudPath, resourceName, resourceType }: IAMViewProps) {
        ...
        actions={
          <ActionPanel>
            <Action
              title="Add Role"
              icon={Icon.Plus}
              onAction={() => showAddRoleForm(principal)}
            />
            <Action
              title="Refresh"
  ...
  async function showAddRoleForm(principal: IAMPrincipal) {
    push(
      ...
      >
        <Form.Description
          title={`Adding role to ${principal.displayName}`}
          text={`Default User ID: ${principal.id}`}
        />
...
- Principal Types: ${principalTypes.join(', ')}
    ...
    push(
      <Detail
        navigationTitle="IAM Debug Information"
        markdown={debugInfo}
      />
    );
  }
