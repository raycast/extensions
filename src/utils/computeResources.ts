...
export const diskTypes: DiskTypeInfo[] = [
  ...
  { name: "pd-extreme", title: "Extreme persistent disk", description: "High-performance block storage for critical workloads" },
  { name: "hyperdisk-balanced", title: "Hyperdisk Balanced", description: "Next-generation balanced block storage" },
  { name: "hyperdisk-extreme", title: "Hyperdisk Extreme", description: "Next-generation high-performance block storage" }
];
...
export function getMachineTypeDescription(machineType: string): string {
  for (const series in machineTypes) {
    const found = machineTypes[series].find((mt: MachineTypeInfo) => mt.name === machineType);
    if (found) {
