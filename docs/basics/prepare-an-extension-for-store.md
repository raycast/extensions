...
- With each modification, furnish clear and descriptive details regarding the latest update, accompanied by a title formatted as an h2 header followed by **{PR_MERGE_DATE}**. This tag will be automatically updated upon the merging of the pull request. While you may still use the date timestamp format YYYY-MM-DD, it is often more practical to use **{PR_MERGE_DATE}** since the merging of a pull request can take several days.
