...
Often items are context-aware, e.g. based on the selected list item. Actions can be grouped into semantic section and can have keyboard shortcuts assigned.

The first and second action become the primary and secondary action. They get automatically the default keyboard shortcuts assigned. In list and details, this is `↵` for the primary and `⌘` `↵` for the secondary action. In forms it's `⌘` `↵` for the primary and `⌘` `⇧` `↵` for the secondary.
...
Use sections if the [ActionPanel](action-panel.md#actionpanel) contains a lot of actions to help guide the user to related actions. For example, create a section for all copy actions.
