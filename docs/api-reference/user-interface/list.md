...
| onSearchTextChange   | <code>(text: string) => void</code>                                                                                                                                                               | No       | -                                                             | If provided, the callback will be called when the search bar text changes. Note: Specifying this implicitly toggles `enableFiltering` to false. To enable native filtering when using `onSearchTextChange`, explicitly set `enableFiltering` to true.                                |
| onSelectionChange    | <code>(id: string) => void</code>                                                                                                                                                                 | No       | -                                                             | If provided, the callback will be called when the selection changes. The `id` argument it receives will be set to the newly-selected [List.Item](#list.item)'s id.                                                                                                                   |
