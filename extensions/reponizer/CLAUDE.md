...
- **Offload safety**: offload trashes `.git`, so anything a fresh clone of origin would not reproduce has to be caught first. `offload.checkOffloadReady` fetches origin, then returns blocking `problems` — uncommitted/untracked changes, stashes, any branch that is ahead, upstream-less, or tracking a gone upstream, local tags missing from `git ls-remote --tags origin` (unverifiable tags fail closed), and, as a catch-all for a detached HEAD, `rev-list HEAD --branches --tags --not --remotes` — alongside `ignored` paths from `status --ignored`. Ignored files are deliberately outside git, so they are confirmed in the UI rather than blocking; every other entry blocks. `offloadRepo` re-runs the check unless the caller hands back the same preflight. The working copy is renamed aside, the placeholder (`reponizer-offloaded.json`, schema `reponizer/offloaded`, includes origin + all remotes with their push URLs + branch + size) is written, then the copy is trashed; failures roll back. Restore refuses non-empty folders, re-creates the placeholder if the clone fails, and reapplies every recorded push URL — `git clone`/`git remote add` set one URL for both directions, so a push URL that differs from the fetch URL is lost otherwise.
