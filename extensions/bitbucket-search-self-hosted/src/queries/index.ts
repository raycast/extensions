...
export async function getRepositories(key: string, start = 0, repositories = []): Promise<any[]> {
  const data = await fetch(
    `${preferences.baseURL}/rest/api/latest/repos?start=${start}`, {
    headers: {
 ...
 */
export async function pullRequestsGetQuery(repository: { project: { key: string }, slug: string }, start = 0, pullRequests = []): Promise<any[]> {
  const data = await fetch(
