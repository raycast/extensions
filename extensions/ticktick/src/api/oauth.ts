...
export const provider = new OAuthService({
  ...
  // TickTick doesn't always return expires_in — default to 30 days so token isn't immediately treated as expired
  tokenResponseParser: (response) => {
    const r = response as OAuth.TokenResponse & { expires_in?: number };
    return { ...r, expires_in: r.expires_in ?? 60 * 60 * 24 * 30 };
  },
});
