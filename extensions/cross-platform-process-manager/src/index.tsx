...
// FIX #3: Tri-state validation result to distinguish network errors from invalid licenses
type ValidationResult = "valid" | "invalid" | "network_error";
...
// ============================================================================
// FIX #3: Returns "network_error" instead of "invalid" when offline,
// so the background check does NOT revoke cached access during outages.
async function validateLicense(key: string, showToasts: boolean = true): Promise<ValidationResult> {
  try {
    ...
    if (!response.ok) {
      if (showToasts) {
        await showToast({ style: Toast.Style.Failure, title: "Validation Failed", message: `Server returned status ${response.status}.` });
      }
    ...
    if (!data.success || !data.purchase) {
      if (showToasts) {
        await showToast({ style: Toast.Style.Failure, title: "Invalid License", message: data.message || "Key not found." });
      }
      return "invalid";
    }
    ...
    if (refunded || chargebacked) {
      if (showToasts) {
        await showToast({ style: Toast.Style.Failure, title: "License Revoked", message: "Subscription refunded or charged back." });
      }
      return "invalid";
    }
    ...
    if (!subscription || subscription.ended_at || subscription.failed_at || subscription.user_requested_cancellation_at) {
      if (showToasts) {
        await showToast({ style: Toast.Style.Failure, title: "Subscription Inactive", message: "Subscription canceled, failed, or expired." });
      }
      return "invalid";
    }
    ...
    if (showToasts) {
      await showToast({ style: Toast.Style.Success, title: "License Verified", message: "Premium features unlocked." });
    }
    return "valid";
  } catch (error) {
    console.error("License validation error:", error);
    if (showToasts) {
      await showToast({ style: Toast.Style.Failure, title: "Network Error", message: "Failed to connect to Gumroad. Check your internet." });
    }
    // FIX #3: Return "network_error" so cached license is NOT revoked during outages
    return "network_error";
  }
}
      ...
      try {
        ...
        if (cachedKey && cachedTimestampStr) {
          ...
          if (!isExpired) {
            ...
            // FIX #3: Only revoke cache if the API explicitly says "invalid".
            // If the user is offline ("network_error"), keep the cached access.
            validateLicense(cachedKey, false).then((result) => {
              if (result === "invalid") {
                SecureStorage.remove("licenseKey");
                SecureStorage.remove("lastValidatedAt");
                setIsValidated(false);
                showToast({ style: Toast.Style.Failure, title: "Subscription Inactive", message: "License revoked. Please renew." });
              }
              // If result is "network_error", do nothing — preserve cached access
            });
          } else {
            // Cache expired: must re-validate. If network is down, deny access
            // because we cannot confirm the subscription is still active.
            const result = await validateLicense(cachedKey, true);
            if (result === "valid") {
              setIsValidated(true);
            } else {
              setIsValidated(false);
          ...
          <ActionPanel>
            <Action.SubmitForm
              ...
              onSubmit={async (values: { licenseKey: string }) => {
                setIsVerifying(true);
                const result = await validateLicense(values.licenseKey, true);
                if (result === "valid") {
                  setIsValidated(true);
                  setView("list");

                  // FIX #4: Show confirmation dialog before killing, even after license unlock
                  if (pendingKill) {
                    const confirmed = await confirmAlert({
                      title: `Terminate ${pendingKill.name}?`,
                      message: `Are you sure you want to force kill PID ${pendingKill.pid}? This action cannot be undone.`,
                      icon: Icon.Warning,
                      primaryAction: { title: "Kill Process", style: Alert.ActionStyle.Destructive },
                    });
                    if (confirmed) {
            ...
            <Action
              ...
              shortcut={{ modifiers: ["esc"], key: "escape" }}
            />
          </ActionPanel>
        }
      >
