...
- **Preferences**: open issues in a chosen app (`appPicker` — Issue actions and **Open Issue from Clipboard**); configurable Git branch name template; optional copy of new issue URL after create.
- **Actions**: Copy Markdown Link; Git branch name uses the template.
- **Epic/parent autocomplete**: prefer “Current Search” section when the API returns both history and current sections.
- **AI Search Issues tool**: uses a slimmer search (`getIssuesForAI`, max 50 issues, fewer fields).
- **Dependencies / manifest**: bump lodash for prototype pollution fix (CVE-2025-13465); declare support for macOS and Windows.
