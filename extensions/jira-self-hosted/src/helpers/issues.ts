...
export function getCustomFieldsForDetail(issue?: IssueDetail | null) {
  ...
  const isTextareaField = (key: string) => issue.schema[key].custom === CustomFieldSchema.textarea;

  // Jira's textareas are shown in the markdown field of the Detail screen (optional via extension preferences)
  const markdownFieldsKeys = showTextareaCustomFields ? customFieldsWithValueKeys.filter(isTextareaField) : [];

  const metadataFieldsKeys = customFieldsWithValueKeys.filter((key) => !isTextareaField(key));
