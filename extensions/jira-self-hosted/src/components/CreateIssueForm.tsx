...
export default function CreateIssueForm({ draftValues, enableDrafts = false }: CreateIssueFormProps) {
  ...
  const { handleSubmit, itemProps, values, setValue, reset, focus } = useForm<IssueFormValues>({
    async onSubmit(values) {
      ...
      try {
        ...
        if (issue) {
          const issuePageUrl = getIssueUrl(issue.key);
          ...
          if (copyURLtoClipboard) {
            await Clipboard.copy(issuePageUrl);
          }
  ...
  const { data: issueTypesSummary } = usePromise(
  ...
  const issueTypes = issueMetadata ?? [];

  const selectedIssueType: IssueTypeWithCustomFields | undefined = issueTypes?.find(
    (issueType) => issueType.id === values.issueTypeId,
  );
  ...
  const { unknownCustomFields, customFields } = useMemo(() => {
    if (selectedIssueType) {
      ...
      // console.log("selectedIssueType not found")
    }
  ...
  return (
    ...
    >
      ...
      <Form.TextField {...itemProps.summary} title="Title" placeholder="Short summary for the issue" />
