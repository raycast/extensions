...
import { getAuthenticatedUri, getHostUrl } from "../api/request";
...
export default function IssueDetail({ initialIssue, issueKey }: IssueDetailProps) {
  const {
    ...
    async (issueId) => {
      ...
      // Jira images use site-root paths; prepend host origin (not the Jira base path) so paths like /jira/secure/... resolve
      const resolvedDescription = await replaceAsync(description, /src="(.*?)"/g, async (_, uri) => {
        const dataUri = await getAuthenticatedUri(`${hostUrl}${uri}`, "image/jpeg");
        return `src="${dataUri}"`;
      });
    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label
            title="Assignee"
            text={issue?.fields.assignee ? issue?.fields.assignee.displayName : getAvatarIcon("-")}
          ...
          {issue?.fields.labels && issue?.fields.labels.length > 0 && (
            <Detail.Metadata.TagList title="Labels">
              {issue?.fields.labels.map((label, index) => <Detail.Metadata.TagList.Item key={index} text={label} />)}
          ...
          {issue?.fields.components && issue?.fields.components.length > 0 && (
            <Detail.Metadata.TagList title="Components">
              {issue.fields.components.map((component) => (
                <Detail.Metadata.TagList.Item key={component.id} text={component.name} />
              ))}
            </Detail.Metadata.TagList>
          )}
          ...
          {issue?.fields.fixVersions && issue?.fields.fixVersions.length > 0 && (
            <Detail.Metadata.TagList title="Fix Versions">
              {issue.fields.fixVersions.map((version) => (
                <Detail.Metadata.TagList.Item key={version.id} text={version.name} />
              ))}
            </Detail.Metadata.TagList>
          )}
            ...
            <>
              ...
              {issue.fields.timetracking?.originalEstimate && (
                <Detail.Metadata.Label
                  ...
                  text={issue.fields.timetracking?.originalEstimate ?? "None"}
              ...
              {issue.fields.timetracking?.remainingEstimate && (
                <Detail.Metadata.Label
                  title="Remaining"
                  text={issue.fields.timetracking?.remainingEstimate ?? "None"}
              ...
              {issue.fields.timetracking?.timeSpent && (
                <Detail.Metadata.Label
                  title="Time Spent"
                  text={issue.fields.timetracking?.timeSpent ?? "None"}
                  icon={{ source: Icon.Hourglass, tintColor: Color.Green }}
                />
              )}
            </>
          ) : null}
