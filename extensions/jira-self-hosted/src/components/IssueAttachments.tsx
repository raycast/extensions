...
import { jiraFetch } from "../api/httpClient";
...
export default function IssueAttachments({ attachments }: IssueAttachmentsProps) {
    ...
    return Promise.all(
      attachments.map((attachment) => {
        ...
        if (attachment.thumbnail) {
          return jiraFetch(attachment.thumbnail, { headers: { Authorization: authorizationHeader } }).then(
            (result) => ({
              ...attachment,
  ...
  async function downloadFile(attachment: Attachment) {
    const response = await jiraFetch(attachment.content, {
