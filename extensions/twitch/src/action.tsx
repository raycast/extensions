import { ActionPanel, getPreferenceValues, Action, Icon } from "@raycast/api";
import { PrimaryAction } from "./interfaces/Preferences";
import { watchStream } from "./helper/streamlink";
...
export const action = (name: string, isLive = true) => {
  ...
  return (
    <ActionPanel>
      {!isLive && <Action.OpenInBrowser title="Open Channel in Browser" url={`twitch.tv/${name}`} />}
      {primary === PrimaryAction.Browser && isLive && (
        <Action.OpenInBrowser title="Open Stream in Browser" url={`twitch.tv/${name}`} />
      )}
      {primary === PrimaryAction.Browser && isLive && (
        <Action
          icon={Icon.Eye}
