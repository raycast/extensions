...
import type { Position, Account, DeepDiveData, PulseData, SearchResult, Quote } from "./types";
...
async function fetchApi<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (res.status === 401) {
    throw new Error("Invalid API token — generate a new one in Sonar Settings → Connections → Raycast");
  }
...
export const api = {
  positions: cached<Position[]>("positions", ONE_MIN, () => fetchApi("/api/positions")),
  deepdive: cached<DeepDiveData>("deepdive", FIVE_MIN, () => fetchApi("/api/deepdive")),
  ...
  accounts: cached<Account[]>("accounts", TEN_MIN, () => fetchApi("/api/accounts")),
  search: (q: string) => fetchApi<SearchResult[]>(`/api/search?q=${encodeURIComponent(q)}`),
  quote: (symbol: string) => fetchApi<Quote>(`/api/quotes/${encodeURIComponent(symbol)}`),
};
