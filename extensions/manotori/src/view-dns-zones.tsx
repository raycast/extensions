          ...
          actions={
            <ActionPanel>
              <Action.Push
                ...
                target={<ViewDNSRecords zoneId={item.zone_id} />}
              />
            </ActionPanel>
          }
        />
...
function ViewDNSRecords({ zoneId }: { zoneId: number }) {
  ...
  return <List isLoading={isLoading} searchBarPlaceholder="Search dns record" isShowingDetail>
    {data.map(item => <List.Item key={item.dns_record_id} icon={Icon.Text} title={item.name} detail={<List.Item.Detail markdown={item.content} metadata={<List.Item.Detail.Metadata>
      ...
      {item.prio ? <List.Item.Detail.Metadata.Label title="Priority" text={item.prio.toString()} /> : undefined}
    </List.Item.Detail.Metadata>} />} actions={<ActionPanel>
      {/* eslint-disable-next-line @raycast/prefer-title-case */}
      <Action.Push icon={Icon.Plus} title="Create DNS Record" target={<CreateDNSRecord zoneId={item.zone_id} />} onPop={revalidate} />
    </ActionPanel>} />)}
  </List>
}

function CreateDNSRecord({zoneId}: {zoneId: number}) {
...
toast.message = `${error}`;
  ...
  return <Form actions={<ActionPanel>
    ...
    <Form.Dropdown title="Type" {...itemProps.type}>
      {Object.keys(DNS_RECORD_TYPE_TO_PLACEHOLDER).map(type => <Form.Dropdown.Item key={type} title={type} value={type} />)}
    </Form.Dropdown>
    ...
    <Form.Dropdown title="TTL" {...itemProps.ttl}>
      {["60", "300", "900", "1800", "3600", "7200"].map(ttl => <Form.Dropdown.Item key={ttl} title={ttl} value={ttl} />)}
    </Form.Dropdown>
  </Form>
}