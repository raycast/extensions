...
export default function Characters() {
    ...
    const { isLoading, data: characters, pagination } = useCachedPromise(
...
await showToast(Toast.Style.Success, `Fetched ${data.length} Characters`);
    ...
    return <List isLoading={isLoading} pagination={pagination} isShowingDetail throttle searchBarPlaceholder="Search character name" onSearchTextChange={setSearchName}>
        {characters?.map((character, characterIndex) => {
            ...
            return <List.Item key={characterIndex} icon={{ source, tintColor: character.status==="Alive" ? Color.Green : character.status==="Dead" ? Color.Red : Color.Yellow }} title={character.name} subtitle={character.type} detail={<List.Item.Detail markdown={`![${character.name}](${character.image})`} metadata={<List.Item.Detail.Metadata>
                ...
                <List.Item.Detail.Metadata.Label title="Type" text={character.type} icon={!character.type ? Icon.Minus : undefined} />
                <List.Item.Detail.Metadata.TagList title="Gender">
                    <List.Item.Detail.Metadata.TagList.Item text={character.gender} />
                </List.Item.Detail.Metadata.TagList>
                <List.Item.Detail.Metadata.Link title="Origin" text={character.origin.name} target={character.origin.url} />
<List.Item.Detail.Metadata.Link title="Location" text={character.location.name} target={character.location.url} />
