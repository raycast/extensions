...
export default function Episodes() {
    ...
    const { isLoading, data: episodes, pagination } = useCachedPromise(
...
await showToast(Toast.Style.Success, `Fetched ${data.length} Episodes`);
    ...
    return <List isLoading={isLoading} pagination={pagination} throttle searchBarPlaceholder="Search episode name" onSearchTextChange={setSearchName}>
        {episodes?.map((episode, episodeIndex) => <List.Item key={episodeIndex} icon={Icon.FilmStrip} title={episode.name} subtitle={episode.episode} accessories={[
