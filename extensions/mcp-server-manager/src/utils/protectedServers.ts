...
export async function getUserLockedServers(
  editorType?: EditorType,
): Promise<string[]> {
  try {
    const lockedData = await LocalStorage.getItem<string>(
      USER_LOCKED_SERVERS_KEY,
    );
    ...
    return allLocked
      ...
      .map((key) => key.substring(editorPrefix.length));
  } catch (error) {
...
export async function isProtectedServer(
  serverName: string,
  editorType: EditorType,
): Promise<boolean> {
...
export async function getUnlockedServers(
  editorType?: EditorType,
): Promise<string[]> {
  try {
    const unlockedData =
      await LocalStorage.getItem<string>(UNLOCKED_SERVERS_KEY);
...
export async function isServerUnlocked(
  serverName: string,
  editorType: EditorType,
): Promise<boolean> {
...
export async function unlockServer(
  ...
  const allUnlockedServers: string[] = unlockedData
    ? JSON.parse(unlockedData)
    : [];

  if (!allUnlockedServers.includes(serverKey)) {
    allUnlockedServers.push(serverKey);
    await LocalStorage.setItem(
      UNLOCKED_SERVERS_KEY,
      JSON.stringify(allUnlockedServers),
    );
  }
}
...
export async function lockServer(
  ...
  if (isDefaultProtected) {
    ...
    const allUnlockedServers: string[] = unlockedData
      ? JSON.parse(unlockedData)
      : [];
    const filteredServers = allUnlockedServers.filter(
      (key) => key !== serverKey,
    );
    await LocalStorage.setItem(
      UNLOCKED_SERVERS_KEY,
      JSON.stringify(filteredServers),
    );
  } else {
    const lockedData = await LocalStorage.getItem<string>(
      USER_LOCKED_SERVERS_KEY,
    );
    const allUserLockedServers: string[] = lockedData
    ...
    if (!allUserLockedServers.includes(serverKey)) {
      ...
      await LocalStorage.setItem(
        USER_LOCKED_SERVERS_KEY,
        JSON.stringify(allUserLockedServers),
      );
    }
    ...
    const allUnlockedServers: string[] = unlockedData
      ? JSON.parse(unlockedData)
      : [];
    const filteredUnlocked = allUnlockedServers.filter(
      (key) => key !== serverKey,
    );
    await LocalStorage.setItem(
      UNLOCKED_SERVERS_KEY,
      JSON.stringify(filteredUnlocked),
    );
  }
}

export async function unlockUserLockedServer(
  serverName: string,
  editorType: EditorType,
): Promise<void> {
  const serverKey = createServerKey(editorType, serverName);
  ...
  const allUserLockedServers: string[] = lockedData
    ? JSON.parse(lockedData)
    : [];
  const filteredServers = allUserLockedServers.filter(
    (key) => key !== serverKey,
  );
  await LocalStorage.setItem(
    USER_LOCKED_SERVERS_KEY,
...
export async function getAllProtectedServers(
  editorType: EditorType,
): Promise<string[]> {
  const userLockedServers = await getUserLockedServers(editorType);
...
export async function validateLockedServersPresent(
  ...
  const lockedServers = protectedServers.filter(
    (name) => !unlockedServers.includes(name),
  );
  ...
  const missingServers = relevantLockedServers.filter(
