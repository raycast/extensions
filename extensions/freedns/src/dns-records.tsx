...
export default function DNSRecords() {
    ...
    const sha = crypto.createHash('sha1').update(username + '|' + password).digest('hex');
    ...
    return <List isLoading={isLoading}>
        {data.map(record => <List.Item key={record.domain} icon={getFavicon(`https://${record.domain}`, { fallback: Icon.Globe })} title={record.domain} subtitle={record.address} actions={<ActionPanel>
            {!isLoading && <Action.Push icon={Icon.Pencil} title="Update" target={<Update record={record} onUpdate={revalidate} />} />}
        </ActionPanel>} />)}
    </List>
}
interface Record {
...
function Update({record, onUpdate}: {record: Record, onUpdate: () => void}) {
    ...
    return <Form isLoading={isLoading} actions={<ActionPanel><Action.SubmitForm icon={Icon.Check} title="Update" onSubmit={handleSubmit} /></ActionPanel>}>
        <Form.Description title="Domain" text={record.domain} />
        <Form.TextField title="Address" placeholder={record.address} {...itemProps.address} info="Leave empty to use your own IP" />
    </Form>
}