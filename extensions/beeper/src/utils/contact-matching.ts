...
/**
 * Calculate a normalized similarity score between 0 and 1
 * Uses multiple matching strategies with Levenshtein fallback
 */
export function calculateSimilarity(
  ...
  // Target starts with query (e.g., "John" matches "John Smith")
  if (t.startsWith(q)) {
  ...
  // Word-level match (e.g., "Smith" matches "John Smith")
  ...
  // Boost score if first letters match
  if (q[0] === t[0]) {
...
/**
 * Rank and filter chat matches based on query and optional service filter
 */
export function rankChatMatches(
  ...
  // Pre-filter by service if specified
  if (service) {
...
/**
 * Get suggested contacts when no exact match is found
 * Returns a user-friendly error message with alternatives
 */
