...
const NEW_API_RESPONSE: MinimaxCNModelRemain[] = [
  {
    start_time: 0,
    end_time: 0,
...
test("getIntervalPercent falls back to counts when percent fields are missing (legacy API)", () => {
  const legacy: MinimaxCNModelRemain = {
    start_time: 0,
    end_time: 0,
    remains_time: 0,
    current_interval_total_count: 100,
    current_interval_usage_count: 30,
    model_name: "MinimaxCN-M1",
...
test("getCodingModelRemain falls back to MinimaxCN-M* model name (backward compat)", () => {
  const only: MinimaxCNModelRemain[] = [
    {
      start_time: 0,
      end_time: 0,
      remains_time: 0,
  ...
  assert.equal(getCodingModelRemain(only)!.model_name, "MinimaxCN-M1");
});
