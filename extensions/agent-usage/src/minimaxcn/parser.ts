import type { MinimaxCNModelRemain } from "./types.ts";

export function getCodingModelRemain(remains: MinimaxCNModelRemain[]): MinimaxCNModelRemain | null {
  const active = remains.find((r) => r.current_interval_status === 1 || r.current_weekly_status === 1);
  return active || remains.find((r) => r.model_name.startsWith("MinimaxCN-M")) || remains[0] || null;
}

export function getIntervalPercent(model: MinimaxCNModelRemain): number | null {
...
export function getWeeklyPercent(model: MinimaxCNModelRemain): number | null {
