...
function editLink(name, category) {
  const linkCategory = ['FileFunc', 'LogicLib', 'Memento', 'StrFunc', 'TextFunc', 'WinVer', 'WordFunc', 'x64'].includes(category)
    ? `Includes/${category}`
    : category;
      ...
      {Object.entries(reference).map(([category, item]) => (
        Object.entries(item).map(([key, value]) => (
          <List.Item
            ...
            actions={
              <ActionPanel>
                <PushAction title={`Show Documentation`} target={<Detail markdown={`${value.content}${editLink(value.name, category)}`} />} />
              </ActionPanel>
            }
          />
        ))
