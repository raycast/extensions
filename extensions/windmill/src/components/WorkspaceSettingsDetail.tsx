...
import { fetchWorkspaces } from "../hooks/useFetchWorkspaces";
...
export function WorkspaceSettingsDetail({
  ...
  const { isLoading, data } = useFetch<Record<string, boolean | string | object | null>>(`${workspace.remoteURL}api/w/${workspace.workspaceId}/workspaces/get_settings`, {
    headers: {
  ...
  return (
    <Detail isLoading={isValidating || isLoading} markdown={!data ? "" : `
| - | - |
|---|---|
