...
export function WorkspaceForm({ workspace: currentWorkspace, name, onCreate, onDelete }: { workspace?: WorkspaceConfig, name?: string, onCreate?: () => void, onDelete?: () => void }) {
    ...
    let workspaceToken = currentWorkspace?.workspaceToken
    let remoteURL = currentWorkspace?.remoteURL || 'https://app.windmill.dev/'
    ...
    const handleDelete = async () => {
        if (currentWorkspace?.id && await confirmAlert({ title: `You are about to remove the "${currentWorkspace?.workspaceName}" workspace from Raycast` })) {
