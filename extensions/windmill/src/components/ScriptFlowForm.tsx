...
export function ScriptFlowForm({ path, kind, starred, workspace }: { path: string; kind: Kind; starred: boolean; workspace: WorkspaceConfig }) {
    ...
    if (!properties || Object.keys(properties).length === 0) {
        ...
        return <Detail markdown={markdown}
            actions={<ScriptActionPanel kind={kind} path={path} setSubmitError={setSubmitError} setSubmitting={setSubmitting} starred={starred} properties={properties} resources={resources} workspace={workspace} />}
        />
    }
    ...
    >
        <FormItemsComponent properties={properties} required={required} kind={kind} resources={resources} workspace={workspace} />
    </Form>;
}

function ScriptActionPanel({ kind, path, setSubmitError, setSubmitting, starred, properties, resources, workspace }: { kind: Kind; path: string; setSubmitError: React.Dispatch<React.SetStateAction<string | null>>; setSubmitting: React.Dispatch<React.SetStateAction<boolean>>; starred: boolean; properties: any; resources: Resource[]; workspace: WorkspaceConfig }) {
    ...
    return <ActionPanel>
        <Action.SubmitForm
            ...
            onSubmit={async (values) => {
                ...
                setSubmitting(true)
                SubmitForm(values, path, kind, properties, resources, workspace).then((jobId) => {
...
async function SubmitForm(values: any, path: string, kind: Kind, properties: any, resources: Resource[], workspace: WorkspaceConfig) {
