...
async function fetchSnapshot(mrrType: MrrType): Promise<Snapshot | null> {
...
export default function MenuBar() {
    const { mrrType } = getPreferenceValues<{ mrrType: MrrType }>();
    ...
    return (
        <MenuBarExtra icon={Icon.LineChart} isLoading={isLoading} title={title}>
            {data ? (
                <>
                    <MenuBarExtra.Section title={data.companyName}>
                        <MenuBarExtra.Item
                            ...
                            subtitle={
                                data.mrr !== null ? formatCurrency(data.mrr, data.currency) : '—'
                            }
                        />
                        <MenuBarExtra.Item
                            icon={Icon.Coin}
                            title={`Cash balance`}
                            subtitle={
                                data.cash !== null ? formatCurrency(data.cash, data.currency) : '—'
                            }
                        />
                ...
                <MenuBarExtra.Item
                    title={isLoading ? 'Loading…' : 'No company found'}
                    icon={Icon.Warning}
                />
            )}
            <MenuBarExtra.Section>
