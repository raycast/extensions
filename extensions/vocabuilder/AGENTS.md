...
- Use the Node version in `.nvmrc` — **Node 20.15.1 / npm 10.7.0** — which matches the Raycast store CI. Run `nvm use` before installing dependencies. Do not regenerate `package-lock.json` under npm 11.
...
The store CI runs `npm ci` with **npm 10.7.0 / Node 20.15.1** (see `.nvmrc`). npm 11 and npm 10 disagree on how to record optional peer dependencies (e.g. `gcp-metadata`, an optional peer of `mongodb` pulled in transitively by `promptfoo`): npm 11 omits a lockfile node that npm 10 requires. So a `package-lock.json` generated by npm 11 passes `npm ci` locally but fails CI with `Missing: <pkg> from lock file`.

- Always `nvm use` (→ npm 10.7.0) before any `npm install`. A stray `npm install` under npm 11 silently strips the node back out and re-breaks CI.
- If the lockfile was touched on the wrong npm, regenerate it: `npx -y npm@10.7.0 install --package-lock-only`, then verify with `npx -y npm@10.7.0 ci --dry-run`.
- Do **not** add `engine-strict` to force this — it enforces *dependency* engines too, and `@raycast/api` declares `node >=22.14.0` while CI runs Node 20.15.1, so it would turn that tolerated warning into a fatal `npm ci` error.
