...
- Tool calling — `web_search` via SearXNG, `rag_search` against a private
  ...
  its URL; left blank, it returns a clear "not configured" message instead of
  a confusing failure. `rag_search` never runs when the turn is answered by a
  cloud fallback provider, so private knowledge-base content stays local.
