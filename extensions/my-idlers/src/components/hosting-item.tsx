...
export default function HostingItem({ host }: { host: Reseller | Shared }) {
  function getIconByType(type: HostingType) {
    switch (type) {
      ...
      case "cPanel": return "cpanel@dark.png";
  ...
  return <List.Item icon={getFavicon(`https://${host.main_domain}`, { fallback: icon })} title={host.main_domain} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
