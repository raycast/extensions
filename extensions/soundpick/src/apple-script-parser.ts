...
export default class AppleScriptParser implements AppleScriptParser {
  parse(input: AppleScriptResponse): Array<SoundOutputDevice> {
    ...
    let response = Array<SoundOutputDevice>()

    if (deviceNames.length != deviceConnections.length) {
      console.log(`[ERROR]: the number of devices ${deviceNames.length} is not equal to the number of connections ${deviceConnections.length}`)
      return response
    }

    for (let index = 0; index < deviceNames.length; index++) {
      let deviceName = deviceNames[index]
      let deviceConnection = deviceConnections[index]
  ...
  parseDeviceNames(input: AppleScriptResponse): Array<string> {
    ...
    const deviceNames = splittedInput.filter(
    ...
    splitInput(input: AppleScriptResponse): Array<string> {
      ...
      if (splittedInput.length % 2 != 0) {
        console.log(`[ERROR]: The number of elements in the apple script response is not equal to an even number : ${splittedInput.length}`)
