 ...
 * @param value
 */
const toDate = (value?: string) => value && !isNaN(Date.parse(value)) ? new Date(value).toLocaleDateString() : "Unknown";
 ...
 * @param value
 */
const toEuro = (value?: number) => value && !isNaN(value) ? new Intl.NumberFormat("nl-NL", {
  style: "currency",
  currency: "EUR"
...
export default function Command() {
  ...
  const url: string = useMemo(() => {
    ...
    return `https://opendata.rdw.nl/resource/m9d7-ebf2.json?`
  ...
  return (
    <List isLoading={isLoading} searchText={searchText} onSearchTextChange={setSearchText} isShowingDetail={true}
          searchBarPlaceholder="Search on licence plate, brand or type" throttle>
              ...
              <List.Item.Detail
                metadata={
                  <List.Item.Detail.Metadata>
                    ...
                    <List.Item.Detail.Metadata.Label
                      title="Color"
                      text={item.eerste_kleur ?? "Unknown"}
                    />
                    ...
                    <List.Item.Detail.Metadata.Label
                      title="Length"
                      text={toCentimeters(item.lengte)}
                    />
                    <List.Item.Detail.Metadata.Label
                      title="Width"
                      text={toCentimeters(item.breedte)}
                    />
                    <List.Item.Detail.Metadata.Label
                    ...
                    <List.Item.Detail.Metadata.Label
                      title="Catalog price"
                      text={toEuro(item.catalogusprijs)}
                    />
                    <List.Item.Detail.Metadata.Label
                      title="Gross BPM"
                      text={toEuro(item.bruto_bpm)}
                    />
                  </List.Item.Detail.Metadata>
                }
              />
