...
export class TinkererCommunity {
  ...
  async thread(postId: string, signal?: AbortSignal): Promise<{ comments: JsonValue; post: JsonValue }> {
    const [post, comments] = await Promise.all([
      this.client.call({ path: "post.byId", type: "query" }, { id: postId }, signal),
      this.client.call({ path: "post.listComments", type: "query" }, { limit: 50, postId }, signal),
    ]);
    return { comments: unwrapPayload(comments), post: unwrapPayload(post) };
  }
