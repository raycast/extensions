...
export default function GPUEstimates(props: LaunchProps<{ arguments: Arguments.GpuEstimates }>) {
    ...
    const { isLoading, data: estimates, revalidate } = useHashrate<GPUEstimate>("gpuEstimates", {
        params: {
    ...
    return <List isLoading={isLoading} isShowingDetail={isShowingDetail}>
        <List.Section title={`${Object.keys(estimates).length} estimates | cost: ${power_cost} | last_update: ${lastUpdated.toLocaleDateString()}`}>
            {Object.entries(estimates).map(([key, estimate]) => <List.Item key={key} icon={Icon.Dot} title={estimate.device.name} subtitle={estimate.device.brand} actions={<ActionPanel>
                <Action icon={Icon.AppWindowSidebarRight} title="Toggle Details" onAction={() => setIsShowingDetail(prev => !prev)} />
                ...
                <List.Item.Detail.Metadata.Label title="Revenue" text={formatAsUSD(estimate.profit24.revenueUSD24)} />
                ...
                <List.Item.Detail.Metadata.Label title="Revenue" text={formatAsUSD(estimate.revenue24.revenueUSD24)} />
                <List.Item.Detail.Metadata.Label title="Profit" text={formatAsUSD(estimate.revenue24.profitUSD24)} />
