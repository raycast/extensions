...
// export function useHashrate<T>(endpoint: string, params?: {[key: string]: string, initialData: Record<string, never> | []}) {
    ...
    const { isLoading, data, revalidate } = useFetch(API_URL + endpoint + `?${PARAMS}`, {
        ...
        mapResult(result: ErrorResponse | NonNullable<T>) {
            if (typeof result === "object")
                if ("title" in result) throw new Error(result.detail, { cause: result.title });
