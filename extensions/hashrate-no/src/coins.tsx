...
export default function Coins() {
    ...
    const { isLoading, data: coins, revalidate } = useHashrate<Coin[]>("coins", {
        initialData: []
    });
    ...
    return <List isLoading={isLoading} isShowingDetail={isShowingDetail}>
        <List.Section title={`${coins.length} coins | last_update: ${lastUpdated.toLocaleDateString()}`}>
            {coins.map(coin => <List.Item key={coin.coin} icon={Icon.Coin} title={coin.coin + (isShowingDetail ? "" : ` - ${coin.name}`)} subtitle={coin.algo} accessories={[
                { text: `$${coin.usdPrice}` }
            ]} actions={<ActionPanel>
                ...
                <List.Item.Detail.Metadata.Label title="networkHashrate" text={coin.networkHashrate} />
                <List.Item.Detail.Metadata.Label title="networkDifficulty" text={coin.networkDifficulty} />
