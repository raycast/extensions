...
export function ItemDetail({ item }: { item: SearchItem | Item }) {
  const markdown = `## Description \n\n --- \n ${NodeHtmlMarkdown.translate(item.description_html ?? item.description)}`;
  return (
    <List.Item.Detail
      markdown={markdown}
      metadata={
        <List.Item.Detail.Metadata>
          ...
          <List.Item.Detail.Metadata.Link title="Classification" text={item.classification} target={item.classification_url} />
          ...
          {item.attributes.map(({name, label, value}) => {
            ...
            if (value==="Yes" || value==="No") return <List.Item.Detail.Metadata.Label key={name} title={title} icon={value==="Yes" ? Icon.Check : Icon.Xmark} />;
            if (value instanceof Array) return <List.Item.Detail.Metadata.TagList key={name} title={title}>
            {value.map(tag => <List.Item.Detail.Metadata.TagList.Item key={tag} text={tag} />)}
          </List.Item.Detail.Metadata.TagList>;
            if (value.startsWith("http")) return <List.Item.Detail.Metadata.Link key={name} title={title} text={value} target={value} />;
          })}
        </List.Item.Detail.Metadata>
      }
    />
