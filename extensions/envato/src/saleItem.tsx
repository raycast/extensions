...
function price(price: "", support: "") {
	let support_out = "";
	if (parseInt(support) != 0) { support_out = " + $" + support + ""; }
...
export function SaleItemDetail(props: { sale: saleItem; todey: Boolean}) { 
  ...
  ${description}`;
	...
	return <Detail markdown={markdown} navigationTitle={item.name} 
		metadata={ <MetadataSale sale={props.sale} /> }
		actions={ <ActionPanel><OpenInBrowserAction url={`${item.url}`} /></ActionPanel>}
	/>;
}
...
export function MetadataSale(props: { sale: saleItem; }) { 
	...
	if (props.sale.support_amount !== undefined) {
		SupportAmount = <Detail.Metadata.TagList title="Support Amount"> 
  		<Detail.Metadata.TagList.Item text={`$ ${props.sale.support_amount}`} color={Color.Blue} />
		</Detail.Metadata.TagList>;
	} else { SupportAmount = null; }
	...
	if (props.sale.supported_until !== undefined) {
		SupportUntil = <Detail.Metadata.Label title="Support Until" text={String(dateFormat(props.sale.supported_until, "dd.mm.yyyy"))} />  ;
	} else { SupportUntil = null; }
