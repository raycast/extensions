...
Cities and the rest of the IP record come from ip-api.com. The menu bar deliberately does not use it: the country comes from the source's own `cdn-cgi/trace` response, so a background refresh every few minutes never eats into that quota. Lookups are cached for 24 hours, and reserved addresses are recognized locally — they never leave your machine.
