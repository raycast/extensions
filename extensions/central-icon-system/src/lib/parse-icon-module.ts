 ...
 *
 * Shared by the runtime installer (`install-style.ts`) and the development
 * build script (`scripts/build-manifest.mjs`), so both produce byte-identical
 * output — a Store user installing from within the extension gets exactly what
 * `npm run build:icons` would have written.
 *
 ...
 */
function matchDelimiter(src: string, i: number, open: string, close: string): number {
  let depth = 0;
  while (i < src.length) {
    const ch = src[i];
    if (ch === '"' || ch === "'") {
...
function readStringLike(src: string, i: number, quote: string): [string, number] {
  let out = "";
  i += 1;
  while (i < src.length) {
    if (src[i] === "\\") {
 ...
 */
function parseProps(src: string, start: number): [Map<string, string>, number] {
 ...
 */
function renderCall(src: string, start: number): [string, number] {
  const end = matchDelimiter(src, start, "(", ")");
...
function renderChildren(expr: string): string {
  let out = "";
  let i = 0;
  while (i < expr.length) {
 ...
 */
export function parseIconModule(source: string, { name }: { name?: string } = {}): ParsedIcon {
