...
export default function Command() {
    ...
    const items = (
        <List.Section title="Search">
            {valueText.length >= 1 &&
                valueText.length < 10 &&
                searchOption("Block ID / Hash", Color.Magenta, "blocks", valueText)}
            {valueText.length == 64 &&
                searchOption("Vault", Color.Magenta, "vaults", valueText)}
            {valueText.length >= 30 &&
                searchOption("Address", Color.Magenta, "address", valueText)}
            {valueText.length == 64 &&
                searchOption("Transaction", Color.Magenta, "transactions", valueText)}
        </List.Section>
    );
