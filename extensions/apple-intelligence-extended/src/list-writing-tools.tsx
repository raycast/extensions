...
import { executeWritingToolCommand, executeComposeWithPrompt } from "./api";
...
const commands = [
  ...
  {
    title: CommandTitle.COMPOSE_FRENCH,
    number: 15,
    icon: Icon.Globe,
    category: "OpenAI",
    isCustomCompose: true,
    composePromptKey: "FRENCH",
  },
  {
    title: CommandTitle.COMPOSE_GERMAN_AUSTRIA,
    number: 15,
    icon: Icon.Globe,
    category: "OpenAI",
    isCustomCompose: true,
    composePromptKey: "GERMAN_AUSTRIA",
  },
  {
    title: CommandTitle.COMPOSE_ITALIAN,
    number: 15,
    icon: Icon.Globe,
    category: "OpenAI",
    isCustomCompose: true,
    composePromptKey: "ITALIAN",
  },
  {
    title: CommandTitle.COMPOSE_PORTUGUESE,
    number: 15,
    icon: Icon.Globe,
    category: "OpenAI",
    isCustomCompose: true,
    composePromptKey: "PORTUGUESE",
  },
  {
    title: CommandTitle.COMPOSE_SPANISH,
    number: 15,
    icon: Icon.Globe,
    category: "OpenAI",
    isCustomCompose: true,
    composePromptKey: "SPANISH",
  },
];
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={() => {
                  const cmdWithCompose = cmd as { isCustomCompose?: boolean; composePromptKey?: string };
                  if (cmdWithCompose.isCustomCompose && cmdWithCompose.composePromptKey) {
                    const promptConfig = COMPOSE_PROMPTS[cmdWithCompose.composePromptKey];
                    if (promptConfig) {
                      executeComposeWithPrompt(promptConfig.prompt, cmd.title);
                    }
                  } else {
                    executeWritingToolCommand(cmd.number, cmd.title);
                  }
                }}
                icon={cmd.icon}
              />
