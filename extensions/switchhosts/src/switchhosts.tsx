...
function SwitchHostsList() {
  ...
  * Please check the port(50761) is opened in the application <SwitchHosts>`;
    ...
    return <List isLoading={isLoading}>
      {switchHostsList.data.map(
        (item: switchHosts, index: number) => (
          <List.Item key={index} title={item.title} icon={{
            source: item.on ? Icon.Checkmark : Icon.Circle,
            tintColor: item.on ? Color.Green : Color.Blue
          }} actions={
            <ActionPanel>
              <ActionPanel.Item title={item.on ? "Close" : "Open"} onAction={async () => {
                ...
                if (isSwitchSuccess === "ok") {
                  setTimeout(async function() {
                    const result = await getSwitchHostsList();
                    setSwitchHostsList(await result.json());
                    await showToast(ToastStyle.Success, "Success", `${item.on ? "Close" : "Open"} Host ${item.title} Success`);
                  }, 300);
                } else {
                  await showToast(ToastStyle.Failure, "Failed", `${item.on ? "Close" : "Open"} Host ${item.title} Failed`);
                }
              }} />
            </ActionPanel>
          } />
