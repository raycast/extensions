...
const source: SourceSection[] = [
    {
        ...
        items: [
            ...
            [`S`, `English ordinal suffix for the day of the month, 2 characters`, () => format(now, 'do').replace(/^\d+/, '')],
            [`w`, `Numeric representation of the day of the week`, () => formatAndConvertToZeroIndex(now, 'i')],
            [`z`, `The day of the year (starting from 0)`, () => formatAndConvertToZeroIndex(now, 'D', {useAdditionalDayOfYearTokens: true})],
        ],
    },
    {
        section: `Week`,
        items: [
            [`W`, `ISO 8601 week number of year, weeks starting on Monday`, 'w'],
        ],
    },
    {
        section: `Month`,
    ...
    },
    {
        ...
        items: [
            ...
            [`o`, `ISO 8601 week-numbering year. This has the same value as "Y", except that if the ISO week number ("W") belongs to the previous or next year, that year is used instead.`, 'yyyy'],
...
export function stackToTemplate(stack?: (FormatItem | FavoriteItem)[]): string {
    return stack
        ? stack.reduce((template: string, item) => `${template}${item.character}`, ``)
        : '';
}

export function stackToExample(stack?: (FormatItem | FavoriteItem)[]): string {
    return stack
        ? stack.reduce((template: string, item) => `${template}${getExample(item.character)}`, ``)
        : '';
}
