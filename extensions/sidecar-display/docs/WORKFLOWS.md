...
| **BetterDisplay** _(optional)_ | only for Fix Mirroring, if you use its virtual screens                 | `betterdisplaycli version` |
| **An iPad** _(optional)_       | hardware tests only                                                    | —                          |
...
| The engine (TS side)                            | `src/lib/native.ts`                            |
| The engine (Swift side)                         | `swift/Sources/Sidecar/`                       |
...
| the engine, or before a release                      | `+ test:hardware`                             |
| the Swift helper                                     | `+ test:hardware` (needs `swift build` first) |
...
### Running the hardware suites

They drive the real Swift helper, so build it first:

```sh
cd swift && swift build && cd ..
npm run test:safety      # needs the iPad DISCONNECTED (tests the no-display case)
npm run test:hardware    # needs the iPad CONNECTED; changes display topology
```

`safety` refuses to run with a Sidecar display present and says so, rather than
failing obscurely — "absent" means "no Sidecar display", not "unknown name".
...
> on the branch itself blocks _every_ direct push — the check cannot run until
...
hidden, so a version that _can_ move still shows up as a PR.
...
Git tags are therefore for _this repo only_: they mark what you submitted and
...
4. Hardware suites pass locally.
5. Run it:
...
**Known review risk:** the Swift helper uses a private Apple framework
...
| `native <action> failed: …`                                  | The Swift helper threw. A macOS update may have changed the private SidecarCore selectors — patch `SidecarBridge.swift`. There is no fallback engine.              |
...
> it. Rename freely _before_ the store release; after, a rename is a breaking
> change.

**Add a backend**

Implement `SidecarBackend` (`src/lib/backend.ts`), wire it into `getBackend()`
in `src/lib/preferences.ts`, wire it into `getBackend()` in `preferences.ts`. The orchestration needs no changes — that is the point of the
interface.
