# Hijri Converter Changelog

## [1.0.2] - {PR_MERGE_DATE}

- Expanded valid Hijri year range (removed upper cap; now supports any year ≥ 1).
- Strengthened date validation and error handling for Hijri and Gregorian inputs.
- Refined the "Show Today's Date" layout with clearer presentation and shortcuts.
- Updated menu bar refresh interval to 1 hour.
- Updated extension icon and refreshed metadata screenshots.

## [1.0.1] - 2025-12-22

- Improved Menu Bar: Added 1-minute automatic refresh interval to ensure the date stays accurate without manual clicks.

## [Initial Version] - 2025-12-14

- Added Convert Hijri to Gregorian command with live preview
- Added Convert Gregorian to Hijri command with live preview
- Added Show Today's Date command with side-by-side comparison
- Added View Hijri Calendar command showing 14 upcoming days
- Added Menu Bar command for quick Hijri date access
- Auto-copy results to clipboard on submit
- Form defaults to today's date for convenience
