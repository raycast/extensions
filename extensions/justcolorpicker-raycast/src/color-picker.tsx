...
type ColorFormat = 'hsl' | 'hwb' | 'rgb' | 'rgba' | 'lab' | 'lch' | 'device-cmyk' | 'hex'
  ...
  try {
    if (parts.length === 3 || parts.length === 4) {
      ...
      if (parts.every(part => part.endsWith('%'))) {
        ...
        format = 'hsl'
      } else if (parts.every(part => !isNaN(parseFloat(part)) && parseFloat(part) <= 255)) {
...
export default function Command() {
  ...
  function parseColor(input: string) {
    ...
    // 解析 HEX
    if (input.startsWith('#') || /^[0-9A-Fa-f]{3}$|^[0-9A-Fa-f]{6}$/.test(input)) {
    ...
    if (parsedColor) {
      ...
      if (color.isValid()) {
        setColorValue(color.toHex())
        updateColorItems(color)
      }
    }
  }

  function updateColorItems(color: any) {
