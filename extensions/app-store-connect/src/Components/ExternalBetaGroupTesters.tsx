...
export default function ExternalBetaGroupTesters({ group, app, didUpdateNewTesters }: ExternalBetaGroupTestersProps) {
    ...
    return (
        ...
        >
            ...
            <Form.TextArea {...itemProps.externalTesters} placeholder="New testers in CSV format" info="External testers must be in the format: first name, last name, and email address. Example: John,Doe,john@example.com,Jane,Doe,jane@example.com" />
            <Form.FilePicker title="CSV File" allowMultipleSelection={false} info="Import testers from a CSV file. The CSV file must be in the format: first name, last name, and email address"  {...itemProps.csvFile} />
        </Form>
    )
}