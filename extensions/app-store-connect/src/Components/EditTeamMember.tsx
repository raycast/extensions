...
export default function EditTeamMember({ userChanged, user }: Props) {
    const { data, isLoading: isLoadingUser } = useAppStoreConnectApi(`/users/${user.id}?include=visibleApps`, (response) => {
    ...
    useEffect(() => {
        if (data) {
            setValue("visibleApps", (data?.included || []).map((app) => app.id));
    ...
    return (
        ...
        >
            ...
            <Form.Checkbox label="All apps visible" {...itemProps.allAppsVisible} info="Invitee will be able to see all apps" />
            ...
            <Form.Checkbox label="Provisioning allowed" {...itemProps.provisioningAllowed} info="Invitee will be able to access the provisioning functionality on the Apple Developer website" />
        </Form>
    );
    return null;
}
