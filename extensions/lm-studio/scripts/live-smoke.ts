...
const chatModel = requestedChatModel
  ...
  : (chatModels.find((model) => model.loadedInstances.length > 0) ??
    chatModels[0]);
const embeddingModel = requestedEmbeddingModel
  ? embeddingModels.find((model) => model.key === requestedEmbeddingModel)
  : (embeddingModels.find((model) => model.loadedInstances.length > 0) ??
    embeddingModels[0]);

if (!chatModel) {
  throw new Error(
    requestedChatModel
      ? `Chat model not found: ${requestedChatModel}`
      : "No LM Studio language model is available.",
  );
}
if (!embeddingModel) {
  throw new Error(
...
const reasoning = chatModel.capabilities?.reasoning?.allowedOptions.includes(
  "off",
)
  ? "off"
  : undefined;
