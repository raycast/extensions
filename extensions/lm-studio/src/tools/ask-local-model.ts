...
export default async function askLocalModel(input: Input) {
  ...
  if (!prompt)
    throw new Error("Provide a non-empty prompt for the local model.");

  const client = createClient();
  const [availableModels, defaultModelKey] = await Promise.all([
    client.listModels(),
  ...
  if (!model) {
    throw new Error(
      "No language model is available. Download a model in LM Studio and start its local server.",
    );
  }
