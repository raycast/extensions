...
async function temporaryDirectory() {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "lm-studio-knowledge-"),
  );
...
describe("cosine ranking", () => {
  ...
  it("sorts by similarity and obeys the result limit", () => {
    expect(
      rankKnowledgeChunks(chunks, [1, 0], 2).map(({ chunk }) => chunk.id),
    ).toEqual(["best", "middle"]);
    expect(
      rankKnowledgeChunks(chunks, [1, 0], 3).map(({ chunk }) => chunk.id),
    ).toEqual(["best", "middle", "opposite"]);
  });
});
...
describe("buildKnowledgeIndex", () => {
  it("skips hidden content and symlinks, deduplicates chunks, and reuses vectors", async () => {
    ...
    expect(
      first.index.chunks.every((chunk) => chunk.sources.length === 2),
    ).toBe(true);
    expect(
      first.index.chunks.some((chunk) => chunk.text.includes("do not index")),
    ).toBe(false);
  ...
  it("isolates indexes when the same model changes vector dimension", async () => {
    ...
    await writeFile(
      ...
      "A useful note about local language models.",
    );
