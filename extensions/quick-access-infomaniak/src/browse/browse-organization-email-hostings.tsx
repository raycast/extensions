...
export default function BrowseOrganizationEmailHostings(props: MetadataProps) {
  ...
  const { isLoading, data, pagination } = usePromise(
    (organization: Organization) => async (options: { page: number }) => {
      let result;
      try {
        result = await API.get<InfomaniakResponse<EmailHosting>>(
          "/1/products?account_id=" +
            organization.id.toString() +
            "&order_by=customer_name&per_page=100&order=asc&service_name=email_hosting&page=" +
          ...
          actions={
            <ActionPanel>
              <Action.OpenInBrowser
                url={
                  "https://manager.infomaniak.com/v3/mail/" +
                  props.organization.id.toString() +
                  "/mail-service/" +
                  mailService.id.toString() +
                  "/mailboxes/list"
                }
              />
            </ActionPanel>
