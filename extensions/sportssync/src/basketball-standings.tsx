...
export default function scoresAndSchedule() {
  ...
  const { isLoading: nbaStandingsStats, data: nbaStandingsData } = useFetch(
  ...
  const nbaEasternTeams = items1.map((team1, index) => {
    return (
      <List.Item
        ...
        accessoryTitle={`${team1.stats[15].displayValue} | Pct: ${(team1.stats[13].displayValue * 100).toFixed(1)}% | PF: ${team1.stats[11].displayValue} | PA: ${team1.stats[10].displayValue} | Dif: ${team1.stats[8].displayValue} `}
  ...
  const nbaWesternTeams = items2.map((team2, index) => {
    return (
      <List.Item
        ...
        accessoryTitle={`${team2.stats[15].displayValue} | Pct: ${(team2.stats[13].displayValue * 100).toFixed(1)}% | PF: ${team2.stats[11].displayValue} | PA: ${team2.stats[10].displayValue} | Dif: ${team2.stats[8].displayValue} `}
  ...
  const { isLoading: wnbaStandingsStats, data: wnbaStandingsData } = useFetch(
  ...
  const wnbaEasternTeams = items3.map((team3, index) => {
    return (
      <List.Item
        ...
        accessoryTitle={`${team3.stats[16].displayValue} | Pct: ${(team3.stats[14].displayValue * 100).toFixed(1)}% | PF: ${team3.stats[12].displayValue} | PA: ${team3.stats[11].displayValue} | Dif: ${team3.stats[9].displayValue} `}
  ...
  const wnbaWesternTeams = items4.map((team4, index) => {
    return (
      <List.Item
        ...
        accessoryTitle={`${team4.stats[16].displayValue} | Pct: ${(team4.stats[14].displayValue * 100).toFixed(1)}% | PF: ${team4.stats[12].displayValue} | PA: ${team4.stats[11].displayValue} | Dif: ${team4.stats[9].displayValue} `}
  ...
  return (
    ...
    >
      {currentLeague === "NBA" && (
        <>
          <List.Section title={`${nbaStandingsData.children[0].name}`}>{nbaEasternTeams}</List.Section>
          <List.Section title={`${nbaStandingsData.children[1].name}`}>{nbaWesternTeams}</List.Section>
        </>
      )}

      {currentLeague === "WNBA" && (
        <>
          <List.Section title={`${wnbaStandingsData.children[0].name}`}>{wnbaEasternTeams}</List.Section>
          <List.Section title={`${wnbaStandingsData.children[1].name}`}>{wnbaWesternTeams}</List.Section>
        </>
      )}
    </List>
  );
