...
export default function SelectModel() {
  ...
  const load = useCallback(() => {
    try {
      ...
      setCurrentId(tryReadJSONPref<string>("transcriptionModelID") ?? "");
    } catch (err) {
      showToast({
  ...
  async function handleSelect(model: ModelInfo) {
    try {
      writeJSONPref("transcriptionModelID", model.id);
      setCurrentId(model.id);
      await showHUD(`Dictation model: ${model.label} (active next dictation)`);
    } catch (err) {
      await showToast({
        style: Toast.Style.Failure,
