...
export class HomeAssistant {
    ...
    async getStates(params: { domain: string, query: string }): Promise<State[]> {
        ...
        if (params) {
            ...
            if (params.query) {
                result = result.filter((e) => e.entity_id.toLowerCase().includes(params.query.toLowerCase()) || (e.attributes.friendly_name.toLowerCase() || "").includes(params.query.toLowerCase()));
            }
            return result;
        }
        return items;
