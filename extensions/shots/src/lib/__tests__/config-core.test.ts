...
import { ConfigValidationError, formatPreferenceFieldList, parsePreferences, RawPreferences } from "../config-core";
...
describe("formatPreferenceFieldList", () => {
  it("formats preference field names for user-facing errors", () => {
    expect(formatPreferenceFieldList(["endpoint", "publicBaseUrl", "maxRetries"])).toBe(
      "S3 Endpoint, Public Base URL, Upload Retries",
    );
  });
});
