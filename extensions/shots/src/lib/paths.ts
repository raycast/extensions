...
const TEMP_DIR = join(tmpdir(), "raycast-shots");
const FAILED_DIR = join(homedir(), "Pictures", "Shots", "failed");

export type UploadImageExtension = "webp" | "jpg";
export type ImageExtension = "png" | "tiff" | UploadImageExtension;
...
export function buildObjectKey(prefix?: string, date = new Date(), extension: UploadImageExtension = "webp"): string {
...
export function buildTestObjectKey(prefix?: string, date = new Date()): string {
  const year = date.getFullYear().toString();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  const filename = `${uuidv4()}.txt`;
  const keyBody = `shots-test/${year}/${month}/${day}/${filename}`;
  return prefix ? `${normalizePrefix(prefix)}/${keyBody}` : keyBody;
}
