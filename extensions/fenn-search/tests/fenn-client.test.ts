...
test("search sends credentials only to loopback with mode, filters, and request generation", async (context) => {
  ...
  const result = await searchFenn(request, "test-token", new AbortController().signal);
  assert.equal(result[0].results[0].filename, "a.pdf");
  assert.equal(calls[0].url, "http://127.0.0.1:5001/api/mcp/search-files");
  assert.equal(calls[0].options.redirect, "error");
  assert.equal((calls[0].options.headers as Record<string, string>)["X-Fenn-MCP-Token"], "test-token");
...
test("HTTP failures are actionable and never expose the authentication token", async (context) => {
  context.mock.method(
    globalThis,
    "fetch",
    async () => new Response(JSON.stringify({ license_required: true }), { status: 403 }),
  );
  await assert.rejects(searchFenn(request, "private-token", new AbortController().signal), /license verification/);
  assert.match(responseError(423, { index_optimization: true }, "hybrid").message, /optimizing/);
  assert.match(responseError(400, { error: "mode must be one of: hybrid" }, "discover").message, /Update Fenn/);
...
test("connection failure, malformed JSON, and empty searches have distinct behavior", async (context) => {
  ...
  await assert.rejects(searchFenn(request, "token", new AbortController().signal), /Cannot connect/);
