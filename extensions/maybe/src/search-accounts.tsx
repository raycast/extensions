...
export default function SearchAccounts() {
  ...
  return <List isLoading={isLoading} pagination={pagination}>
    {!isLoading && !accounts.length ? <List.EmptyView icon={Icon.Layers} title="No accounts yet" description="Add accounts to display net worth data" /> : Object.entries(grouped).map(([group, accounts]) => <List.Section key={group} title={group}>
{accounts.map(account => <List.Item key={account.id} icon={getAvatarIcon(account.name)} title={account.name} subtitle={account.balance} actions={<ActionPanel>
