...
const execute = async (
  command: string,
  opts: { mask?: string; log?: { stdout: boolean } } = { log: { stdout: false } }
) => {
  ...
  return new Promise<string>((res, rej) =>
    exec(wrappedCommand, (error: ExecException | null, stdout: string, stderr: string) => {
      ...
      if (error) {
        ...
        rej({
          ...error,
          message: !opts.mask ? error.message : error.message.replace(opts.mask, "*".repeat(opts.mask.length)),
        });
      }
...
export const lastPass = (email: string, password: string) => ({
  ...
  login: () => execute(`echo "${password}" | LPASS_DISABLE_PINENTRY=1 lpass login ${email}`, { mask: password }),
  ...
  show: (id: string, opts: { sync: "auto" | "now" | "no" } = { sync: "auto" }): Promise<Account> =>
    execute(`echo "${password}" | LPASS_DISABLE_PINENTRY=1 lpass show --sync=${opts.sync} --json ${id}`, {
      mask: password,
    }).then((stdout) => serializeFromJson(stdout)[0]),
