...
export default function ListDomains() {
  ...
  const handleDelete = async (domain: string) => {
    if (await confirmAlert({ title: `Delete ${domain}?`, message: 'You will not be able to recover it', primaryAction: { title: 'Delete', style: Alert.ActionStyle.Destructive } })) {
  ...
  return state.error ? (
    ...
    <List isLoading={state.domains === undefined || state.isLoading} searchBarPlaceholder="Search for domain..." isShowingDetail={state.showDetails} navigationTitle={`${state.domains?.length || 0} domains`}>
      <List.Section title="Domains">
        {(state.domains || []).filter(domain => state.listSharedDomains ? domain : !domain.isShared).map((domain) => (
          <List.Item
            key={domain.name}
              ...
              <List.Item.Detail
                metadata={
                  <List.Item.Detail.Metadata>
                    <List.Item.Detail.Metadata.Label title="Name" icon={getFavicon(`https://${domain.name}`)} text={domain.name} />
                    <List.Item.Detail.Metadata.Label title="Allow Account Reset" icon={domain.allowAccountReset ? Icon.Check : Icon.Multiply} />
                    <List.Item.Detail.Metadata.Label title="Symbolic Subaddressing" icon={domain.symbolicSubaddressing ? Icon.Check : Icon.Multiply} />
                    <List.Item.Detail.Metadata.Label title="Is Shared" icon={domain.isShared ? Icon.Check : Icon.Multiply} />
