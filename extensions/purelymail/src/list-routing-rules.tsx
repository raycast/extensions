import { ActionPanel, Action, List, Icon, launchCommand, LaunchType, Color, Alert, confirmAlert, showToast, Toast, popToRoot, Detail, openExtensionPreferences } from "@raycast/api";
...
export default function ListRoutingRules() {
  ...
  const handleDelete = async (ruleId: number) => {
    if (await confirmAlert({ title: `Delete rule with id ${ruleId}?`, message: 'You will not be able to recover it', primaryAction: { title: 'Delete', style: Alert.ActionStyle.Destructive } })) {
  ...
  return state.error ? (
    ...
    <List isLoading={state.rules === undefined || state.isLoading} searchBarPlaceholder="Search for rule..." isShowingDetail={state.showDetails == true} navigationTitle={`${state.rules?.length || 0} rules`}>
      <List.Section title="Rules">
        {(state.rules || []).map((rule) => (
          <List.Item
            ...
            subtitle={state.showDetails ? undefined : `${rule.matchUser || (rule.catchall && '*')} => ${rule.targetAddresses.toString()}`}
            accessories={state.showDetails ? undefined : [
              ...
              <List.Item.Detail
                ...
                metadata={
                  <List.Item.Detail.Metadata>
                    ...
                    <List.Item.Detail.Metadata.Label title="Domain Name" text={rule.domainName} icon={getFavicon(`https://${rule.domainName}`)} />
                    <List.Item.Detail.Metadata.Label title="Prefix" icon={rule.prefix ? Icon.Check : Icon.Multiply} />
                    <List.Item.Detail.Metadata.Label title="Catch All" icon={rule.catchall ? Icon.Check : Icon.Multiply} />
                    <List.Item.Detail.Metadata.Separator />
                    <List.Item.Detail.Metadata.Label title="Match User" text={rule.matchUser} icon={!rule.matchUser ? Icon.Minus : null} />
                    ...
                    <List.Item.Detail.Metadata.Separator />
                    <List.Item.Detail.Metadata.Label title={`${rule.matchUser || (rule.catchall && '*')}@${rule.domainName} => ${rule.targetAddresses.toString()}`} />
                  </List.Item.Detail.Metadata>
                }
              />
            }
