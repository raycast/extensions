...
function buildTooltip(icon: HugeIcon & Partial<SearchResultMeta> & { resolvedStyle?: HugeiconsIconStyle }): string {
  ...
  lines.push(`Preview: ${getSearchStyleLabel(icon.resolvedStyle ?? "default")}`);
...
function IconDetailView({
  ...
  useEffect(() => {
    ...
    async function loadStyles() {
      ...
      try {
        setStyles(await getIconStyles({ iconName, apiKey, signal: abortController.signal }));
      } finally {
        setIsLoading(false);
      }
    }
...
export default function Command() {
  const { apiKey, gridSize, defaultColor, quickAction } = getPreferenceValues<Preferences>();
  ...
  const loadData = useCallback(async () => {
    const [folders, storedColor, storedGridSize, storedPreviewStyle, storedRecentSearches, storedQuickAction] =
      await Promise.all([
        loadBookmarkFolders(),
    ...
    async ({
      ...
      try {
        ...
        while (nextPage <= resolvedTotalPages || nextPage === page) {
          const pageResponse = await searchIconMetas({
            query: trimmedQuery,
            page: nextPage,
            apiKey,
        ...
        const hydratedItems = await hydrateIconMetas({
  ...
  return (
    <Grid
      ...
      searchBarPlaceholder="Search Hugeicons by name, tag, or category..."
      navigationTitle="Hugeicons"
      pagination={
        searchedText.trim()
          ? {
              hasMore,
    ...
    >
      {showStartState && (
        <Grid.EmptyView
          ...
          description="Type to search, then use the style dropdown for Rounded, Standard, or Sharp variants."
        />
      )}
      ...
      {showFavorites && favoritesFolder && (
        <Grid.Section title="Favorites" subtitle={currentColorName}>
      ...
      {showSearchResults && (
        <Grid.Section
          ...
          subtitle={`${currentColorName} • ${currentPreviewStyleName}${selectedPreviewStyle === DEFAULT_PREVIEW_STYLE ? "" : " filter"} • ${icons.length} loaded${selectedIcons.size > 0 ? ` • ${selectedIcons.size} selected` : ""}${totalPages > 1 ? ` • Source page ${currentPage} of ${totalPages}` : ""}`}
        >
