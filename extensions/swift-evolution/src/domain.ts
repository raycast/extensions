...
export async function fetchProposals(): Promise<ProposalUI> {
  const toUI = (proposals: ProposalJson[]): ProposalUI => {
    const proposalUI = proposals.sort(compareStatus).reduce<ProposalUI>((prev, current) => {
  ...
  const getKeywords = (proposal: ProposalJson) => {
    ...
    return [proposal.id, proposal.status.version, statusKeywords, summaryKeywords, titleKeywords, repoKeywords].flat()
