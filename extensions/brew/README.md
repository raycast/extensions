...
- **Adopt Apps** — find apps already in your Applications folder that Homebrew has a cask for, and hand them to Homebrew with `brew install --adopt` so they upgrade with everything else. Matches are grouped by what vouches for them: **Ready to Adopt** (the cask's bundle identifier matches, or Homebrew will compare the app itself), **Likely Adoptable** (name and version agree, nothing else checks), and **Unlikely Adoptions**. Anything not ready goes through **Preview Adoption** (⌘⇧I) first. Ignore a wrong match with ⌘⇧H; the filter above the list shows ignored apps and restores them
