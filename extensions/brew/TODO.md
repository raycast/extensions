...
- During cask-cache construction, derive a compact app-bundle-name-to-cask map from `artifacts`. The cached records deliberately omit the full `artifacts` arrays, so detection must not restore them just to scan `/Applications`. Exclude casks already installed; when several casks claim one name, show the alternatives instead of selecting one silently.
