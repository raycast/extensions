...
import { AddToListAction, cleanPiholeURL } from "./utils";
...
export default function () {
  ...
  useEffect(() => {
    async function getQueryInfo() {
      ...
      const { top_queries, top_ads } = (await topResponse.json()) as TopQueries;
  ...
  return (
    <List
      isLoading={topQueries?.length == 0 ? true : false}
