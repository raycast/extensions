...
export async function GetTypes(id: string, pokemonStatData: PokemonStatData) {
  ...
  for (const typeEnName in damageResult.from) {
    ...
    if (damageRatio === 0.25 && !(typeCnName in pokemonStatData.damage_relations.quarter_damage_from)) {
      pokemonStatData.damage_relations.quarter_damage_from.includes(typeCnName) ||
        pokemonStatData.damage_relations.quarter_damage_from.push(typeCnName);
    } else if (damageRatio === 0.5 && !(typeCnName in pokemonStatData.damage_relations.quarter_damage_from)) {
      pokemonStatData.damage_relations.half_damage_from.includes(typeCnName) ||
        pokemonStatData.damage_relations.half_damage_from.push(typeCnName);
    } else if (damageRatio === 2 && !(typeCnName in pokemonStatData.damage_relations.quarter_damage_from)) {
      pokemonStatData.damage_relations.double_damage_from.includes(typeCnName) ||
        pokemonStatData.damage_relations.double_damage_from.push(typeCnName);
    } else if (damageRatio === 4 && !(typeCnName in pokemonStatData.damage_relations.quarter_damage_from)) {
      pokemonStatData.damage_relations.quadruple_damage_from.includes(typeCnName) ||
        pokemonStatData.damage_relations.quadruple_damage_from.push(typeCnName);
    } else if (damageRatio === 0 && !(typeCnName in pokemonStatData.damage_relations.quarter_damage_from)) {
      pokemonStatData.damage_relations.none_damage_from.includes(typeCnName) ||
        pokemonStatData.damage_relations.none_damage_from.push(typeCnName);
    } else {
  ...
  for (const typeEnName in damageResult.to) {
    ...
    if (damageRatio < 1 && !(typeCnName in pokemonStatData.damage_relations.weak_damage_to)) {
      ...
      if (pokemonStatData.damage_relations.weak_damage_to.includes(typeCnName)) {
        ...
        pokemonStatData.damage_relations.strong_damage_to.includes(typeCnName) ||
