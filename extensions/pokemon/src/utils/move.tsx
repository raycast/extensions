...
function getMoveLevel(move: MoveVersionDetail) {
  ...
  const tmp =
    move.version_group_details.filter((item) =>
      item.version_group.name === "scarlet-violet" && item.move_learn_method.name === "level-up"
        ? item.level_learned_at
        : -1,
    ) ||
    move.version_group_details.filter((item) =>
      item.version_group.name === "sword-shield" && item.move_learn_method.name === "level-up"
        ? item.level_learned_at
        : -1,
    ) ||
    move.version_group_details.filter((item) =>
      item.version_group.name === "ultra-sun-ultra-moon" && item.move_learn_method.name === "level-up"
        ? item.level_learned_at
        : -1,
    ) ||
    move.version_group_details.filter((item) =>
      item.version_group.name === "sun-moon" && item.move_learn_method.name === "level-up" ? item.level_learned_at : -1,
    );
...
export async function GetMoves(id: string, totalInfo: TotalInfo, pokemonStatData: PokemonStatData) {
  const moveLevelDetails: MoveLevelDetail[] = [];
