...
export function createTimeEntry({
  ...
  const now = new Date();
  return post<TimeEntry>(`/workspaces/${workspaceId}/time_entries`, {
    billable,
...
export function stopTimeEntry({ id, workspaceId }: { id: number; workspaceId: number }) {
  return patch<TimeEntry>(`/workspaces/${workspaceId}/time_entries/${id}/stop`, {});
}
...
export function justTimeEntry(timeEntryId: number) {
  return get<TimeEntry>(`/me/time_entries/${timeEntryId}`);
}
