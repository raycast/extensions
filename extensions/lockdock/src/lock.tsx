...
export default function Command() {
  ...
  return (
    <List
      ...
      navigationTitle={getNavigationTitle(status)}
...
function getNavigationTitle(status?: DockStatus): string {
  if (!status) {
    return "Lock Dock";
  }

  const currentDisplay = getDisplayName(status, status.location);
  if (status.target === undefined) {
    return `Dock unlocked on ${currentDisplay}`;
  }

  return `Dock locked to ${getDisplayName(status, status.target)}`;
}
