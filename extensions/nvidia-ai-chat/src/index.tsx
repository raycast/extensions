...
function RenameForm({ session, onRename }: { session: ChatSession, onRename: (id: string, newTitle: string) => void }) {
  ...
  return (
    <Form
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Save Title" onSubmit={(values) => {
...
function ArchiveList({ sessions, setSessions }: { sessions: ChatSession[], setSessions: any }) {
  const { pop } = useNavigation();
    ...
    <List isShowingDetail={true} searchBarPlaceholder="Search archive...">
      {archivedSessions.map(session => (
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action title="Restore Chat" icon={Icon.Reply} onAction={() => setSessions(sessions.map(s => s.id === session.id ? { ...s, archived: false } : s))} />
              <Action title="Delete Permanently" icon={Icon.Trash} style={Action.Style.Destructive} onAction={() => setSessions(sessions.filter(s => s.id !== session.id))} />
              <Action.CopyToClipboard title="Copy Chat History" shortcut={{ modifiers: ["cmd", "shift"], key: "c" }} content={formatMessages(session.messages)} />
              {archivedSessions.length > 0 && <Action title="Clear Archive" icon={Icon.Trash} style={Action.Style.Destructive} shortcut={{ modifiers: ["cmd", "shift"], key: "delete" }} onAction={clearArchive} />}
            </ActionPanel>
          }
        />
      ))}
...
export default function Command() {
  ...
  const { isLoading: isLoadingModels, revalidate: reloadModels } = usePromise(
    async (apiKeyToFetch) => {
      ...
      const newModels = uniqueModelsRaw.map(m => ({ id: m.id, name: m.id })).sort((a,b) => a.name.localeCompare(b.name));
  ...
  async function sendMessage() {
    ...
    const newSession: ChatSession = {
      id: currentSessionId,
      title: isNewSession ? (userInput.length > 30 ? userInput.substring(0, 30) + '...' : userInput) : (currentSession?.title || "Chat"),
    ...
    try {
      ...
      const errorSession = {
        ...newSession,
        messages: [...updatedMessages, { role: "assistant", content: `Error: ${error instanceof Error ? error.message : "Unknown error"}` }]
      };
      ...
      <List.Item
        ...
        actions={
          <ActionPanel>
            ...
            <Action.Push title="View Archive" shortcut={{ modifiers: ["cmd", "shift"], key: "a" }} icon={Icon.Box} target={<ArchiveList sessions={sessions} setSessions={setSessions} />} />
            <Action title="Refresh Models" onAction={() => reloadModels()} icon={Icon.ArrowClockwise} shortcut={{ modifiers: ["cmd", "shift"], key: "r" }} />
          </ActionPanel>
        }
      />
      ...
      {activeSessions.map(session => (
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action title="Send Message" onAction={sendMessage} icon={Icon.Message} />
              <Action title="Archive Chat" shortcut={{ modifiers: ["cmd"], key: "delete" }} style={Action.Style.Destructive} onAction={() => archiveSession(session.id)} icon={Icon.Box} />
              <Action.Push title="Rename Chat" shortcut={{ modifiers: ["cmd"], key: "r" }} icon={Icon.Pencil} target={<RenameForm session={session} onRename={renameSession} />} />
              <Action.Push title="View Archive" shortcut={{ modifiers: ["cmd", "shift"], key: "a" }} icon={Icon.Box} target={<ArchiveList sessions={sessions} setSessions={setSessions} />} />
              <Action.CopyToClipboard title="Copy Chat History" shortcut={{ modifiers: ["cmd", "shift"], key: "c" }} content={formatMessages(session.messages)} />
              <Action title="Refresh Models" onAction={() => reloadModels()} icon={Icon.ArrowClockwise} shortcut={{ modifiers: ["cmd", "shift"], key: "r" }} />
            </ActionPanel>
