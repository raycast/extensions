...
export class AegisDB {
    ...
    async decryptDb(password: string): Promise<any[]> {
        ...
        for (const slot of slots) {
            ...
            const decipher = crypto.createDecipheriv('aes-256-gcm', kdf, Buffer.from(slot.key_params.nonce, 'hex'));
        ...
        if (masterKey === null) {
            throw new Error("error: unable to decrypt the master key with the given password");
        }
        ...
        const decipher = crypto.createDecipheriv('aes-256-gcm', masterKey, Buffer.from(params.nonce, 'hex'));
