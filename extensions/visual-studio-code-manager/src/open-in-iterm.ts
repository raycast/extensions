...
export const makeScript = ({ command, location }: ItermCommandOpts) => {
...
export function openProjectInNewWindow({ command }: { command: string }) {
  runAppleScript(makeScript({ location: "new-window", command }));
}

export function openProjectInNewTab({ command }: { command: string }) {
  runAppleScript(makeScript({ location: "new-tab", command }));
}
