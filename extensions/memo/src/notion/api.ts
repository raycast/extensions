...
function getBlockFromNotionPage(p: PageObjectResponse): Block | null {
    ...
    if ('title' in p.properties) { // a standalone page
        prop = p.properties['title'] as TitleProperty
    } else if ('Name' in p.properties) { // a page inside a DB
...
export class ApiImpl implements Api {
    ...
    private requestCreatePage(parent: Block, title: string, content: Content): Promise<null | Error> {
        if (parent.type !== NotionBlockType.DATABASE) {
            return Promise.resolve(new Error(`create page fails because parent is not a database:parent=${parent},title=${title}`))
        }
...
function requestWithRetry<A>(
    retry: number,
    requestFunc: () => Promise<Response<A>>
): Promise<A | Error> {
