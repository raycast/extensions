...
function HostForm(props: { config?: SSHHostConfig; configIndex?: number; onSave: () => void }) {
  ...
  function getPortValidationMessage(value: string): string | undefined {
  ...
  async function handleSubmit(values: {
    ...
    try {
      ...
      const newConfig: SSHHostConfig = {
        ...
        port,
        rawBlock: props.config ? updateRawBlock(props.config.rawBlock, { ...values, port }) : "", // Will be generated by saveSSHConfig if empty
      };
  ...
  return (
    ...
    >
      ...
      <Form.TextField
        ...
        placeholder="e.g. 22"
        error={portError}
        onChange={(value) => setPortError(getPortValidationMessage(value))}
      />
    </Form>
  );
}
