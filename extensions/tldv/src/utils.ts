import { Preferences, Workspace, Meeting, DateFilter } from './types';
...
export function getDefaultWorkspace(workspaces: Workspace[], defaultName?: string): Workspace | undefined {
...
export function formatDate(dateString: string, format: 'relative' | 'absolute' = 'relative'): string {
  return format === 'relative' ? formatDateRelative(dateString) : formatDateAbsolute(dateString);
}
...
export function groupMeetingsByDate(meetings: Meeting[]): Map<string, Meeting[]> {
...
export function filterMeetingsByDate(meetings: Meeting[], filter: DateFilter): Meeting[] {
...
export function searchMeetings(meetings: Meeting[], query: string): Meeting[] {
  ...
  return meetings.filter((m) => {
    ...
    const organizerMatch =
      m.organizer?.name?.toLowerCase().includes(lowerQuery) || m.organizer?.email?.toLowerCase().includes(lowerQuery);
    const inviteeMatch = m.invitees?.some(
      (i) => i.name?.toLowerCase().includes(lowerQuery) || i.email?.toLowerCase().includes(lowerQuery),
    );
...
export function searchTranscript(
  ...
  const lowerQuery = query.toLowerCase();
  const results: { speaker: string; text: string; startTime: number; matchIndex: number }[] = [];
...
export function getUniqueParticipants(meetings: Meeting[]): { name: string; email: string; count: number }[] {
  const participantMap = new Map<string, { name: string; email: string; count: number }>();
...
export function filterMeetingsByParticipant(meetings: Meeting[], participantEmail: string): Meeting[] {
  ...
  return meetings.filter((m) => {
    ...
    const isInvitee = m.invitees?.some((i) => i.email?.toLowerCase() === lowerEmail);
...
export function filterMeetingsBySpecificDate(meetings: Meeting[], date: Date): Meeting[] {
