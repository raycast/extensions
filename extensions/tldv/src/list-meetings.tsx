...
import { useWorkspaces, useMeetings, useFilteredMeetings, usePreferences } from './hooks';
...
function EmptyView({ error, onRetry }: { error: string | null; onRetry?: () => void }) {
  return (
    <List.EmptyView
...
function MeetingsListView({ workspace }: { workspace: Workspace }) {
  ...
  const { meetings, isLoading, error, hasMore, loadMore, refresh, clearCache } = useMeetings(workspace);
  const { filteredMeetings, dateFilter, setDateFilter, searchQuery, setSearchQuery } = useFilteredMeetings(meetings);
    ...
    >
      {filteredMeetings.length === 0 && !isLoading ? (
        ...
        sortedGroups.map((group) => (
          <List.Section key={group} title={group} subtitle={`${groupedMeetings.get(group)?.length || 0} meetings`}>
      ...
      {filteredMeetings.length > 0 && (
        <List.Section>
          <List.Item
            title=""
            subtitle={hasMore ? 'Load more meetings...' : `${filteredMeetings.length} meetings loaded`}
            icon={hasMore ? Icon.ArrowDown : Icon.CheckCircle}
            actions={
              <ActionPanel>
                {hasMore && <Action title="Load More" icon={Icon.ArrowDown} onAction={loadMore} />}
                <Action title="Refresh" icon={Icon.ArrowClockwise} onAction={refresh} />
                <Action title="Clear Cache" icon={Icon.Trash} style={Action.Style.Destructive} onAction={clearCache} />
              </ActionPanel>
            }
          />
        </List.Section>
...
function WorkspaceListView() {
  ...
  const selectedWorkspace = workspaces.find((w) => w.name === selectedWorkspaceName);

  if (!selectedWorkspace) {
    return <Detail markdown="# No Workspace Selected\n\nPlease select a workspace from the dropdown." />;
  }
  ...
  return (
    <List
      ...
      searchBarAccessory={
        workspaces.length > 1 ? (
          <List.Dropdown tooltip="Select workspace" value={selectedWorkspaceName} onChange={handleWorkspaceChange}>
...
function MeetingsListContent({ workspace }: { workspace: Workspace }) {
  const prefs = usePreferences();
  const { meetings, isLoading, error, hasMore, loadMore, refresh, clearCache } = useMeetings(workspace);
  const { filteredMeetings, dateFilter, setDateFilter } = useFilteredMeetings(meetings);
