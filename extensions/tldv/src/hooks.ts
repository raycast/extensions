...
import { Meeting, TranscriptResponse, HighlightsResponse, Workspace, Preferences, DateFilter } from './types';
...
// Meetings hook
export function useMeetings(workspace: Workspace | undefined): UseMeetingsResult {
  const prefs = usePreferences();
  ...
  const loadMeetings = useCallback(
    async (page = 1, append = false, forceRefresh = false) => {
      ...
      if (page === 1 && !append && !forceRefresh && !useMockData) {
        const cached = getCachedMeetings<{ meetings: Meeting[]; pages: number }>(cacheKey, cacheTTL);
        if (cached) {
      ...
      if (!useMockData && !isOnline()) {
        const cached = getCachedMeetings<{ meetings: Meeting[]; pages: number }>(cacheKey, Infinity);
        if (cached) {
          setMeetings(cached.meetings);
          setTotalPages(cached.pages);
          setHasMore(false);
      ...
      try {
        const data = await fetchMeetings(workspace?.apiKey || '', page, pageSize, useMockData);
        ...
        if (append) {
          ...
          if (!useMockData) {
            setCachedMeetings(cacheKey, { meetings: data.results, pages: data.pages });
          }
        }
...
// Meeting detail hook
export function useMeetingDetail(meetingId: string, apiKey: string): UseMeetingDetailResult {
  const prefs = usePreferences();
  const useMockData = prefs.useMockData === true;
  ...
  const loadDetails = useCallback(
    async (forceRefresh = false) => {
      ...
      if (!forceRefresh && !useMockData) {
        const cachedTranscript = getCachedTranscript<TranscriptResponse>(meetingId, DEFAULT_DETAIL_CACHE_TTL);
        const cachedHighlights = getCachedHighlights<HighlightsResponse>(meetingId, DEFAULT_DETAIL_CACHE_TTL);
      ...
      try {
        const details = await fetchMeetingDetails(apiKey, meetingId, useMockData);
...
export function useFilteredMeetings(meetings: Meeting[]): UseFilteredMeetingsResult {
  ...
  return { filteredMeetings, dateFilter, setDateFilter, searchQuery, setSearchQuery };
}
...
export function useRecentMeetings(
  ...
  const recentMeetings = useMemo(() => meetings.slice(0, limit), [meetings, limit]);
