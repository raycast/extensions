...
import { Meeting, TranscriptResponse, HighlightsResponse, ExportFormat } from './types';
...
export async function copyToClipboard(content: string, label: string): Promise<void> {
  await Clipboard.copy(content);
  await showHUD(`Copied ${label} to clipboard`);
}

// Export transcript only
export function exportTranscript(transcript: TranscriptResponse, format: 'markdown' | 'txt'): string {
  if (format === 'markdown') {
    let md = '';
    let currentSpeaker = '';
    transcript.data.forEach((s) => {
...
export function exportHighlights(highlights: HighlightsResponse, format: 'markdown' | 'txt'): string {
  if (format === 'markdown') {
    return highlights.data.map((h) => `- \`${formatTime(h.startTime)}\` ${h.text}`).join('\n');
  }
  return highlights.data.map((h) => `[${formatTime(h.startTime)}] ${h.text}`).join('\n');
}

// Save export to file (opens save dialog)
export async function saveExport(content: string, filename: string, format: ExportFormat): Promise<void> {
