...
import { Preferences, Meeting } from './types';
...
describe('searchMeetings', () => {
  const meetings: Meeting[] = [
    createMeeting('1', 'Weekly Standup', '2024-01-15T10:00:00Z', 'John Doe', ['Alice', 'Bob']),
    createMeeting('2', 'Design Review', '2024-01-14T10:00:00Z', 'Jane Smith', ['Charlie']),
    createMeeting('3', 'Sprint Planning', '2024-01-13T10:00:00Z', 'Bob Wilson', []),
  ];
...
describe('getPageSize', () => {
  ...
  it('should clamp to valid range', () => {
    ...
    expect(getPageSize({ pageSize: '5' } as unknown as Partial<Preferences>)).toBe(10);
    expect(getPageSize({ pageSize: '200' } as unknown as Partial<Preferences>)).toBe(100);
  });

  it('should handle invalid input', () => {
    expect(getPageSize({ pageSize: 'invalid' } as unknown as Partial<Preferences>)).toBe(50);
  });
});
...
describe('getCacheTTL', () => {
  ...
  it('should ensure minimum TTL', () => {
    expect(getCacheTTL({ cacheTTL: '0' } as unknown as Partial<Preferences>)).toBe(1);
  });

  it('should handle invalid input', () => {
    expect(getCacheTTL({ cacheTTL: 'invalid' } as unknown as Partial<Preferences>)).toBe(15);
  });
});
...
function createMeeting(
  ...
  return {
    ...
    duration: 1800,
    organizer: { name: organizerName, email: `${organizerName.toLowerCase().replace(' ', '.')}@example.com` },
    invitees: inviteeNames.map((n) => ({ name: n, email: `${n.toLowerCase()}@example.com` })),
  };
}
