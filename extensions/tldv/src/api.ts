import { ApiError, MeetingsResponse, TranscriptResponse, HighlightsResponse } from './types';
import {
...
async function apiRequest<T>(
  ...
  if (!response.ok) {
    ...
    throw new ApiError(`API error: ${response.status} ${response.statusText}`, response.status, response.statusText);
  }
...
export async function fetchMeetings(
  ...
  return apiRequest<MeetingsResponse>(`/meetings?page=${page}&limit=${limit}`, apiKey);
}
...
export async function fetchTranscript(
  ...
  try {
    return await apiRequest<TranscriptResponse>(`/meetings/${meetingId}/transcript`, apiKey);
  } catch (error) {
...
export async function fetchHighlights(
  ...
  try {
    return await apiRequest<HighlightsResponse>(`/meetings/${meetingId}/highlights`, apiKey);
  } catch (error) {
    if (error instanceof ApiError && error.isNotFound) {
      return null;
    }
...
export async function fetchMeetingDetails(
  ...
  meetingId: string,
  useMockData = false,
): Promise<{ transcript: TranscriptResponse | null; highlights: HighlightsResponse | null }> {
