import { MenuBarExtra, Icon, openExtensionPreferences, open, launchCommand, LaunchType } from '@raycast/api';
...
export default function RecentMeetingsMenuBar() {
  ...
  const { meetings, isLoading, error, refresh } = useRecentMeetings(currentWorkspace, MENU_BAR_LIMIT);
  ...
  if (error && meetings.length === 0) {
    return (
      <MenuBarExtra icon={Icon.Video} tooltip="tl;dv Meetings">
        <MenuBarExtra.Item title="Failed to load meetings" icon={Icon.ExclamationMark} />
  ...
  return (
    <MenuBarExtra icon={Icon.Video} isLoading={isLoading} tooltip="tl;dv Meetings">
      {/* Workspace indicator for multiple workspaces */}
      {workspaces.length > 1 && currentWorkspace && (
        <>
          <MenuBarExtra.Item title={`Workspace: ${currentWorkspace.name}`} icon={Icon.Building} />
          <MenuBarExtra.Separator />
        </>
      )}
      ...
      <MenuBarExtra.Section>
        <MenuBarExtra.Item
          ...
          onAction={() => launchCommand({ name: 'list-meetings', type: LaunchType.UserInitiated })}
        />
        <MenuBarExtra.Item
          title="Search Meetings"
          icon={Icon.MagnifyingGlass}
          shortcut={{ modifiers: ['cmd'], key: 's' }}
          onAction={() => launchCommand({ name: 'search-meetings', type: LaunchType.UserInitiated })}
        />
      </MenuBarExtra.Section>
