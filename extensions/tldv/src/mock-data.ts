...
const mockTranscriptData: TranscriptSentence[] = [
  { speaker: 'John Smith', text: 'Good morning everyone, thanks for joining the call.', startTime: 0, endTime: 5 },
  { speaker: 'John Smith', text: "Let's start with a quick status update from each team.", startTime: 5, endTime: 10 },
  {
  ...
  { speaker: 'Bob Wilson', text: "We've reduced response times by about 40% so far.", startTime: 40, endTime: 48 },
  {
    speaker: 'John Smith',
  ...
  { speaker: 'John Smith', text: "Good idea. I'll set that up for Wednesday afternoon.", startTime: 97, endTime: 103 },
];
...
export function getMockMeetingsResponse(page = 1, limit = 50): MeetingsResponse {
...
export function getMockTranscriptResponse(meetingId: string): TranscriptResponse | null {
...
export function getMockHighlightsResponse(meetingId: string): HighlightsResponse | null {
  // Simulate no highlights for some meetings
  if (meetingId === 'mock-meeting-processing' || meetingId === 'mock-meeting-6') {
