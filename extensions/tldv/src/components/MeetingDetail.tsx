import { Detail, ActionPanel, Action, Icon, Color, showToast, Toast, Clipboard } from '@raycast/api';
import { useState, useEffect } from 'react';
import { Meeting, TranscriptResponse, HighlightsResponse } from '../types';
import { useMeetingDetail, usePreferences } from '../hooks';
import { formatDate, formatDuration, formatTime, calculateSpeakerStats, generateDeepLink } from '../utils';
...
export function MeetingDetail({ meeting, apiKey }: MeetingDetailProps) {
  ...
  const { transcript, highlights, isLoading, refresh } = useMeetingDetail(meeting.id, apiKey);
  ...
  const speakerStats = transcript?.data ? calculateSpeakerStats(transcript.data) : null;
      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.Label
            title="Date"
            text={formatDate(meeting.happenedAt, prefs.dateFormat || 'relative')}
            icon={Icon.Calendar}
          />
          <Detail.Metadata.Label title="Duration" text={formatDuration(meeting.duration)} icon={Icon.Clock} />
          {isFav && (
            <Detail.Metadata.Label
              title="Status"
              text="Favorited"
          ...
          <Detail.Metadata.Label
            title="Organizer"
            text={meeting.organizer?.name || meeting.organizer?.email || 'Unknown'}
            icon={Icon.Person}
          />
          {meeting.invitees && meeting.invitees.length > 0 && (
            <Detail.Metadata.TagList title="Participants">
              {meeting.invitees.map((invitee, index) => (
                <Detail.Metadata.TagList.Item key={index} text={invitee.name || invitee.email} color={Color.Blue} />
              ))}
          ...
          {hasSummary && (
            <>
              <Detail.Metadata.Separator />
              <Detail.Metadata.Label title="AI Summary" text={`${topicSummaries.size} topics`} icon={Icon.Stars} />
            </>
          )}
          {hasHighlights && (
            <Detail.Metadata.Label title="Highlights" text={`${highlights.data.length} items`} icon={Icon.Star} />
          )}
          {speakerStats && speakerStats.size > 0 && (
            <>
              <Detail.Metadata.Separator />
              <Detail.Metadata.Label title="Speaker Time" icon={Icon.SpeechBubble} text="" />
...
function extractTopicSummaries(highlights: HighlightsResponse | null): Map<string, string> {
