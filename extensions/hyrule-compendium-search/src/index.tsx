    ...
    if (category?.category == "monsters" || category?.category == "treasure") {
          ...
          return (
            <>
              <Detail.Metadata.Label
                ...
                text={entry?.cooking_effect.replace(/(^|\s)\S/g, (match: string) => match.toUpperCase())}
              />
            </>
          );
        } else {
      ...
      if (entry?.edible == true) {
        if (entry?.hearts_recovered == 0) {
          ...
          if (entry?.cooking_effect == "") {
            return <Detail.Metadata.Label title="Hearts Recovered" text={entry?.hearts_recovered.toString()} />;
          } else {
            return (
              <>
                <Detail.Metadata.Label
        ...
        <Detail.Metadata>
          ...
          <Detail.Metadata.TagList title="Common Locations">
            ...
            {entry?.common_locations === null ? (
              ...
              entry?.common_locations.map((index: string) => <Detail.Metadata.TagList.Item text={index} key={index} />)
            )}
          </Detail.Metadata.TagList>
