...
import { Preferences, SleepStage, SleepMetrics } from "./utils/types";
import { useMetrics, getStateColor, getMetricTitle } from "./utils/api";
...
export default function Command(): JSX.Element {
  ...
  const [sleepMetrics, setSleepMetrics] = useState<SleepMetrics>({
    efficiency: { value: 0, title: "Sleep Efficiency", state_title: "", state: "", unit: "" },
    temperature: { value: 0, title: "Temperature", state_title: "", state: "", unit: "" },
    ...
    tossAndTurns: { value: 0, title: "Tosses & Turns", state_title: "", state: "", unit: "" },
    timeInBed: "",
    actualSleepTime: "",
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(sleepData.object.summary)) {
        sleepData.object.summary.forEach(
          (metric: { score?: number; title: string; state_title?: string; state?: string }) => {
            ...
            else if (title.includes("toss")) targetMetric = "tossAndTurns";
      ...
      if (sleepData.object.toss_turn?.count !== undefined) {
        newMetrics.tossAndTurns = {
          value: sleepData.object.toss_turn.count,
          title: "Tosses & Turns",
          state_title: "Normal",
          state: sleepData.object.toss_turn.state || "good",
          unit: "",
        };
      }
  ...
  const sleepMetricsList = [
    ...
    { key: "tossAndTurns", icon: Icon.Repeat },
  ];
    ...
    return (
      <List isLoading={true}>
        ...
        <List.Section title="Sleep Metrics">
          {sleepMetricsList.map((metric, index) => (
            <LoadingItem key={`loading-${index}`} metric={metric} />
          ))}
        </List.Section>
