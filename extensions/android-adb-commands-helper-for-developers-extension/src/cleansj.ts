...
export default async function main() {
  try {
    ...
    // 构建要执行的adb命令
    const adbCommand = `${adbPath} logcat -s *:E`;
    console.log(`准备执行命令: ${adbCommand}`);

    // 在后台执行adb命令，增加超时和详细输出
    try {
      // 注意：logcat默认会持续运行，所以我们需要限制它的运行时间或方式
      const { stdout, stderr } = await execAsync(`${adbCommand} -d`, {
        timeout: 10000, // 10秒超时
