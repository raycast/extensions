...
export default function Groundhogs() {
    ...
    const groundhogs = !filter ? data?.groundhogs : data?.groundhogs.filter(groundhog => {
        if (filter==="is_groundhog") return groundhog.isGroundhog;
        if (filter==="is_not_groundhog") return !groundhog.isGroundhog;
    })

    return <List isLoading={isLoading} isShowingDetail={isShowingDetail} searchBarAccessory={<List.Dropdown tooltip="Filter" onChange={setFilter}>
        <List.Dropdown.Item title="All" value="" icon={Icon.Dot} />
        <List.Dropdown.Section>
            <List.Dropdown.Item title="IS Groundhog" value="is_groundhog" />
            <List.Dropdown.Item title="IS NOT Groundhog" value="is_not_groundhog" />
        </List.Dropdown.Section>
    </List.Dropdown>}>
        {groundhogs?.map(groundhog => {
            const latestPrediction = groundhog.predictions.at(-1);
        return <List.Item key={groundhog.id} title={groundhog.name} icon={groundhog.predictions.at(-1)?.shadow===0 ? Icon.Sun : Icon.Snowflake} subtitle={isShowingDetail ? undefined : groundhog.region} accessories={isShowingDetail ? undefined : [{tag: groundhog.country}, { text: `${groundhog.predictionsCount} predictions` }]} detail={<List.Item.Detail markdown={`![Illustration](${groundhog.image})
...
`} metadata={<List.Item.Detail.Metadata>
        ...
        <List.Item.Detail.Metadata.Link title="Link" text={`${BASE_URL}groundhogs/${groundhog.slug}`} target={`${BASE_URL}groundhogs/${groundhog.slug}`} />
        ...
        <List.Item.Detail.Metadata.Link title="Source" text={groundhog.source} target={groundhog.source} />
        {groundhog.contact ? <List.Item.Detail.Metadata.Link title="Contact" text={groundhog.contact} target={groundhog.contact.includes("@") ? `mailto:${groundhog.contact}` : groundhog.contact} /> : <List.Item.Detail.Metadata.Label title="Contact" icon={Icon.Minus} />}
        <List.Item.Detail.Metadata.Link title="Current Prediction" text={groundhog.currentPrediction} target={groundhog.currentPrediction} />
        <List.Item.Detail.Metadata.Label title="Is Groundhog" icon={groundhog.isGroundhog ? Icon.Check : Icon.Multiply} />
        <List.Item.Detail.Metadata.TagList title="Type">
            <List.Item.Detail.Metadata.TagList.Item text={groundhog.type} />
        </List.Item.Detail.Metadata.TagList>
        <List.Item.Detail.Metadata.Label title="Active" icon={groundhog.active ? Icon.Check : Icon.Multiply} />
        <List.Item.Detail.Metadata.Label title="Successor" text={groundhog.successor} icon={groundhog.successor ? undefined : Icon.Minus} />
        ...
        <List.Item.Detail.Metadata.Label title="Predictions" text={groundhog.predictionsCount.toString()} />
            ...
            <Action.Push title="View Predictions" icon={Icon.Document} target={<ViewPredictions groundhog={groundhog} />} />
        </ActionPanel>} />})}
    </List>   
}
...
| Year | Shadow | Details |
|------|--------|---------|
${groundhog.predictions.toReversed().map(prediction => {
    const shadowText = prediction.shadow===1 ? "Yes, saw shadow" : prediction.shadow===0 ? "Nope, no shadow" : "";
    const detailsText = prediction.details || (prediction.shadow===1 ? "❄️ More winter" : prediction.shadow===0 ? "🌼 Early spring" : "")
    return `| ${prediction.year} | ${shadowText} | ${detailsText} |`;
}).join("\n")}
`;

    return <Detail navigationTitle={groundhog.name} markdown={markdown} />
}