...
**Show Runner Pools** lists every pool against its owner's GitHub avatar, with how many of its runner slots are busy. Start and stop a pool, pause or resume one pool, change its runner count, and open the log. Press return to see which repositories a pool serves.
