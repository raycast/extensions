...
export default function Command() {
  ...
  if (data.isLoading) {
    return <Detail isLoading={true} markdown={`Loading your journal... \n\n ![Rabbit Hole Loading](rabbit-r1-bunny.gif)`} />;
  }
  ...
  const JournalItem = (item: JournalEntry) => {
    ...
    switch (item.type) {
      case "recording":
        ...
        journalActions = (
          <ActionPanel>
            {item.data.recordingData?.textContent && <Action.CopyToClipboard title="Copy Summarized Notes" content={item.data.recordingData?.textContent} /> }
            {item.data.recordingData?.files[0].transcript && <Action.CopyToClipboard title="Copy Original Transcript" content={item.data.recordingData?.files[0].transcript} /> }
          </ActionPanel>
        )
        break;
      case "note":
        ...
        journalActions = (
          <ActionPanel>
            {item.data.noteData?.textContent && <Action.CopyToClipboard title="Copy Note" content={item.data.noteData?.textContent} /> }
          </ActionPanel>
        )  
        break;
      case "conversation":
        ...
        journalActions = (
          <ActionPanel>
            {item.data.conversationData?.textContent && <Action.CopyToClipboard title="Copy Conversation Response" content={item.data.conversationData?.textContent} /> }
          </ActionPanel>
        )
        break;
      case "search":
        ...
        journalActions = (
          <ActionPanel>
            {item.data.searchData?.textContent && <Action.CopyToClipboard title="Copy Search Result" content={item.data.searchData?.textContent} /> }
          </ActionPanel>
        )
        break;
      case "ai-generated-image":
        markdown += item.data.aiGeneratedImageData?.textContent;
        // markdown += item.data.aiGeneratedImageData?.files[0].url + ' ' as string;
        markdown += RabbitAsset(item.data.aiGeneratedImageData?.files[0].url as string);
        markdown += `\n\n![Generated Image](${item.data.aiGeneratedImageData?.files[0].url})`;
        journalActions = (
          <ActionPanel>
            {item.data.aiGeneratedImageData?.textContent && <Action.CopyToClipboard title="Copy Generated Image Description" content={item.data.aiGeneratedImageData?.textContent} /> }
          </ActionPanel>
        )
        break;
      default:
