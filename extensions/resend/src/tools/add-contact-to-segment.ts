...
const tool = async (input: Input) => {
  ...
  const response = await getResend().contacts.segments.add({
    segmentId: input.segmentId,
    ...(input.contactId ? { contactId: input.contactId } : { email: input.contactEmail as string }),
  });
  return unwrapResponse(response, "add contact to segment");
};

export const confirmation: Tool.Confirmation<Input> = async (input) => ({
  title: "Add Contact to Segment",
  message: "Add this contact to the selected segment?",
  info: [
