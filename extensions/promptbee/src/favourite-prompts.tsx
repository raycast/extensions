...
export default function FavouritePromptsCommand() {
  ...
  async function loadPrompts(token: string) {
    try {
      const res = await fetch(`${API_BASE_URL}/api/v1/prompts/favourites`, {
        headers: {
  ...
  return (
    <List isLoading={false} searchBarPlaceholder="Favourite prompts...">
      {prompts.map((prompt) => (
        <List.Item
          key={prompt.id}
          title={prompt.title}
