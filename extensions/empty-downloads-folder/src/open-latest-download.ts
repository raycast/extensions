...
export default async function main(): Promise<void> {
  ...
  try {
    await open(latestEntry.path);
    await showToast(Toast.Style.Success, "Opening file", latestEntry.name);
  } catch (error) {
    await showToast(Toast.Style.Failure, "Failed to open file", String(error));
  }
}
