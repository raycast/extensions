...
export default function ViewDiskSpeedHistoryCommand() {
  ...
  return (
    <List isLoading={!snapshot} searchBarPlaceholder="Search disks and results…">
      {volumes.length === 0 && snapshot ? (
        <List.EmptyView
          icon={Icon.HardDrive}
          title="No Disk Speed Results"
          description="Run Disk Speed Test to establish a local performance baseline."
          actions={
            <ActionPanel>
              <Action
                title="Run Disk Speed Test"
                icon={Icon.Play}
                onAction={() => launchCommand({ name: "run-disk-speed-test", type: LaunchType.UserInitiated })}
              />
            </ActionPanel>
          }
        />
