# Disk Speed Test — MVP specification

## Product

Build a public, MIT-licensed, macOS-only Raycast Store extension named **Disk Speed Test**. It provides a conservative, explicitly confirmed check of sequential disk performance. It is inspired by the simplicity of dedicated disk-speed applications, but does not copy their branding, visuals, methodology, or promise numerically equivalent results.

The extension exposes two Raycast commands:

1. **Run Disk Speed Test** — runs the benchmark, displays live phase/progress, supports cancellation, and presents results.
2. **View Disk Speed History** — groups bounded local history by volume and manages baselines.
