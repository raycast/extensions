...
export default async function main() {
  ...
  const ahkCode = `
FileAppend VD.getCount(), "${resultPath.replace(/\\/g, "\\\\")}"
`;
  ...
  if (success) {
    try {
      ...
      fs.unlinkSync(resultPath);
      await showHUD(`Total Desktops: ${result}`);
    } catch {
      await showHUD("Desktop count retrieved");
    }
  } else {
    await showToast({ style: Toast.Style.Failure, title: "Failed to get desktop count" });
  }
}
