import { Action, ActionPanel, Detail, Icon, Keyboard, List, getPreferenceValues, openExtensionPreferences } from "@raycast/api";
...
export default function ListWatches() {
  ...
  try {
    ...
    return <Detail markdown={"# Error \n\n Invalid URL"} actions={<ActionPanel><Action icon={Icon.Gear} title="Open Extension Preferences" onAction={openExtensionPreferences} /></ActionPanel>} />
  }
    ...
    <List isLoading={isLoading}>
      {data && Object.entries(data).map(([id, watch]) => {
        ...
        return <List.Item key={id} icon={icon} title={watch.url} subtitle={watch.title ?? undefined} keywords={keywords} accessories={[
          ...
          <Action.Push title="View Details" target={<WatchDetails id={id} />} />
          <Action.OpenInBrowser icon={Icon.ArrowNe} title="Preview" url={`${url}preview/${id}#text`} />
          <Action.OpenInBrowser icon={Icon.ArrowNe} title="Edit" url={`${url}edit/${id}#general`} shortcut={Keyboard.Shortcut.Common.Edit} />
...
history_n: number
  ...
  return <Detail isLoading={isLoading} metadata={data && <Detail.Metadata>
    ...
    <Detail.Metadata.Label title="Last Viewed" text={new Date(data.last_viewed*1000).toString()} />
    ...
    <Detail.Metadata.Label title="Processor" text={data.processor==="restock_diff" ? "Re-stock & Price detection for single product pages" : "Re-stock & Price detection for single product pages"} />
