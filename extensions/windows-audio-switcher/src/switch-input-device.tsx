...
async function setInputDevice(device: AudioDevice, mode: "Both" | "Default" | "Comm"): Promise<boolean> {
  await showToast({
...
export default function Command() {
  ...
  const handleSetDevice = async (device: AudioDevice, mode: "Both" | "Default" | "Comm") => {
    ...
    const success = await setInputDevice(device, mode);
    ...
    <List searchBarPlaceholder="Search input audio devices...">
      {devices.map((device) => (
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action title="Set as Default Input" onAction={() => handleSetDevice(device, "Default")} />
              <Action title="Set as Communication Input" onAction={() => handleSetDevice(device, "Comm")} />
