...
import { CreatePageParameters, QueryDatabaseParameters, UpdatePageParameters } from "@notionhq/client/build/src/api-endpoints"
...
export const QueryAddProject = async (name:string,icon:string, notion:Client|undefined) => {
    ...
    await notion?.pages.create(addProjecctJson(name,icon, idDB) as CreatePageParameters)
...
export const QueryChangeActiveProject = async (id:string, active:boolean, notion:Client|undefined) => {
    await notion?.pages.update(changeActiveProjectJson(active, id) as UpdatePageParameters)
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Project')})})
    return true
}
...
export const QueryDeleteProject = async (project:Project, cache:Cache, notion:Client|undefined) => {
    ...
    const journals = await FetchJournals(['all'],getAPIidFromLink(getPreferenceValues<Pref>().journalAPIID),true, notion)
    if(typeof(journals) === 'string') {
        showToast({title: getAPIError(journals, 'Journals'), style:Toast.Style.Failure})
        return
    }
    const keystones = await FetchKeystones(['all'],getAPIidFromLink(getPreferenceValues<Pref>().keystoneAPIID),true,"", notion)
    ...
    await notion?.pages.update(deleteItemJson(project.id))
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Item')})})
