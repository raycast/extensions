...
const getID = async () => {
  const token = (await LocalStorage.getItem("Journals")) as string;
  return token;
};

export const QueryAddJournal = async (
  text: string,
  date: string,
  projectName: string,
  projectID: string,
  ...
  await notion?.pages
    .create(addJournalJson(text, date, projectName, projectID, name, idDB) as CreatePageParameters)
...
export const QueryChangeDateJournal = async (id: string, start: string, notion: Client | undefined) => {
  await notion?.pages.update(changeDateJson(id, start) as UpdatePageParameters).catch((e: APIResponseError) => {
    return showToast({ title: getAPIError(e.code as string, "Journal") });
  });
  return true;
};
...
export const QuerySetNameJournal = async (journalID: string, name: string, notion: Client | undefined) => {
  await notion?.pages.update(addNewNameJournalJson(name, journalID)).catch((e: APIResponseError) => {
    return showToast({ title: getAPIError(e.code as string, "Journal") });
  });
  return true;
};
...
export const QueryAddTextJournal = async (journalID: string, text: string, notion: Client | undefined) => {
  await notion?.pages.update(addText(journalID, text)).catch((e: APIResponseError) => {
    return showToast({ title: getAPIError(e.code as string, "Journal") });
  });
  return true;
};
...
export const QuerySetTextJournal = async (journalID: string, text: string, notion: Client | undefined) => {
  await notion?.pages.update(setNexTextToJournalJson(text, journalID)).catch((e: APIResponseError) => {
    return showToast({ title: getAPIError(e.code as string, "Journal") });
  });
  return true;
};
