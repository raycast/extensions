...
export const QueryAddKeystone = async (
  name: string,
  date: string,
  projectID: string,
  todos: object[],
  ...
  await notion?.pages
    .create(addKeystoneJson(name, date, projectID, todos, idDB) as CreatePageParameters)
    .catch((e: APIResponseError) => {
      return showToast({ title: getAPIError(e.code as string, "Keystone") });
    });
  return true;
};

export const QueryChangeDateKeystone = async (id: string, date: string, notion: Client | undefined) => {
  await notion?.pages.update(changeDateJson(id, date) as UpdatePageParameters).catch((e: APIResponseError) => {
    return showToast({ title: getAPIError(e.code as string, "Keystone") });
  });
};
