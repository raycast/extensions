...
const getToken = async() => {
    const token = getAPIidFromLink(getPreferenceValues<Pref>().todoAPIID)
    return token
}
...
export const QuerySetTodosUndoneFromJournal = async(journal:Journal, notion:Client|undefined) => {
    journal.todos.forEach(async(todo) => {
        ...
        .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Todo')})})
    })
}
...
export const QueryToogleTodo = async(todo:Todo, notion:Client|undefined) => {
    ...
    if(bool) { 
        ...
        const resJournal = await notion?.databases.query({database_id:jouralAPIID as string,...q} as QueryDatabaseParameters)
        ...
        if(journals.length !==0) {
            ...
            const res = await notion?.pages.create(addJournalJson(" ",new Date().toISOString().slice(0,10), todo?.project.name,todo?.project.id,new Date().toISOString().slice(0,10).toString(), jouralAPIID) as CreatePageParameters)
            await notion?.pages.update(addRelationJournalTodoJson(res?.id as string,todo?.id))
            .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Journal')})})
            return
        }
    } else { 
...
export const QueryAddTodo = async (projects:unknown[], filter:string, search:string, notion:Client|undefined) => {
    ...
    const q = await notion?.pages.create(addTodoJson(search,projectID, token) as CreatePageParameters)
    .then(()=> {return 'ok'})
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Todo')})})
    return q
}

export const QueryAddTodoForm = async (name:string,projectID:string, keystoneID:string, notion:Client|undefined) => {
    const token = await getToken()
    await notion?.pages.create(addTodoWithKeystoneJson(name,projectID, keystoneID,token) as CreatePageParameters)
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Todo')})})
    return true   
}

export const QueryUnlinkTodo = async (id:string, notion:Client|undefined) => {
    await notion?.pages.update(unlinkTodo(id))
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Todo')})})
    return true
}
...
export const QuerySetKeystoneAndTodo = async (keystone:Keystone, todo:Todo, notion:Client|undefined) => {
    await notion?.pages.update(addRelationKeystoneTodoJson(keystone.id,todo.id) as UpdatePageParameters)
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Todo')})})
    return true  
}
