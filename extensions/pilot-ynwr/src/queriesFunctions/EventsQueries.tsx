...
export const QueryAddEvent = async (name:string, start:string, end:string, projectID:string, notion:Client|undefined) => {
    const idDB = await getID()
    await notion?.pages.create(addEventJson(name, start, end, projectID, idDB) as CreatePageParameters)
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Event')})})
    return true
}

export const QueryChangeDateEvent = async (id:string, start:string, end:string, notion:Client|undefined) => {
    await notion?.pages.update(changeDateJson(id, start, end) as UpdatePageParameters)
    .catch((e:APIResponseError) => {return showToast({title:getAPIError(e.code as string, 'Event')})})
    return true
}
