...
export const GetJournalsQuery = (activeProject:boolean, projectFilter:string, onlyLast:boolean, date:string) => {
    ...
    if(onlyLast) {pageSize = 1; sortFilter.push({property:"Date", direction:"descending"})}
...
export const GetEventsQuery = (activeProject:boolean, projectFilter:string, onlyAfterNow:boolean, onlyToday:boolean, today:string) => {
    const andFilter:object[] = []
...
export const filterNextInTimeEvent =(date:string) => { return {
    filter: {
        "and": [
            {
                property:"Date",
