...
import { QueryAddTodo, QuerySetKeystoneAndTodo, QueryToogleTodo, QueryUnlinkTodo } from '../../queriesFunctions/TodosQueries'
...
interface KeystoneItemProps {keystone:Keystone}
...
const TaskManagementView = () => {
    ...
    const onChangeView = (e:string) => {
        if(e===VTEXT.lk) {
            if(keystones===null) {showToast({title:'Please add Keystone before', style:Toast.Style.Failure}); return}
        }
        setView(e)
    } 
    ...
    const onChangeShowDone = (e:boolean) => {setShowDone(e)}
    //#endregion
    ...
    const actions = <ActionPanel>
        {filter==="Nothing"?<Action title='Select a Project' style={Action.Style.Destructive} />:
        addType==="Nothing"?<Action title='Select a Type' style={Action.Style.Destructive} />:
        search===""?<Action title='Enter a Name' style={Action.Style.Destructive} />:
        addType==="todo"?<Action title={"Add New Todo"} onAction={()=>{handleAddTodo()}} />:
        addType==="keystone"?<Action.PickDate title={"Add New "+addType} onChange={(e)=>{handleAddKeystone(e)}}/>:<></>
        }
        <ToogleVueAction/>
        <RefreshAction/>
        <ClearRefreshAction clearRefresh={clearRefresh} setFirst={setFirstLoad} setShow={setShowDetail} /></ActionPanel>

    const accessories = showDetail?[]:[{tag:{value:progbar(todos.filter(t=>t.checkbox).length,todos.length, true)}}]

    return <List.Item id='tm' title={view===VTEXT.at?"Your Tasks":"Your Keystones"} icon={view===VTEXT.at?"📋":"📍"}  
    actions={actions}
    ...
    const YourTaskMetadata = () => {
        ...
        if(view===VTEXT.at) {
            ...
            title = fTodos.length===0?"No todos 😢":progbar(fTodos.filter(t=>t.checkbox===true).length,fTodos.length, true)
        } else {
            const fKeystones = filter==="Nothing"?keystones:keystones.filter(keystone => keystone.project.name === filter)
            let fKDones = 0
        ...
        <List.Item.Detail.Metadata.TagList title='🔁 Views' >
            <List.Item.Detail.Metadata.TagList.Item  text='📋 Todos' color={view===VTEXT.at?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeView(VTEXT.at)} />
            <List.Item.Detail.Metadata.TagList.Item  text='📍 Keystones' color={view===VTEXT.ak?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeView(VTEXT.ak)} />

        </List.Item.Detail.Metadata.TagList>
        <List.Item.Detail.Metadata.TagList title={showDone?"✅ Showing Done":"🟥 Hiding Done"} >
            <List.Item.Detail.Metadata.TagList.Item text={showDone?"Hide":"Show"} color={showDone?Color.PrimaryText:Color.SecondaryText}  onAction={()=>onChangeShowDone(!showDone)} />
        </List.Item.Detail.Metadata.TagList>

        <List.Item.Detail.Metadata.Separator />


        <List.Item.Detail.Metadata.Label title='⏩ Quick Add' text={filter==="Nothing"?'Select a project to add item':addType==="Nothing"?"Select a type to add":search===""?"Enter Name in Search":"Add New "+addType }  />
        <List.Item.Detail.Metadata.TagList title='' >
            {filter==="Nothing"?<List.Item.Detail.Metadata.TagList.Item text='Select a project to add item' color={Color.SecondaryText} />
            : <>
            <List.Item.Detail.Metadata.TagList.Item text='📋 Todo' color={addType==='todo'?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddType('todo')} />
            <List.Item.Detail.Metadata.TagList.Item text='📍 Keystone' color={addType==='keystone'?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddType('keystone')} />
            {addType==="Nothing"?<></>:<List.Item.Detail.Metadata.TagList.Item text='Cancel' color={Color.Red} onAction={()=>{onChangeAddType('Nothing'), setSearch('')}} />}
            </>
            }
        </List.Item.Detail.Metadata.TagList>
    ...
    const AllTodosSection = () => {
        const filteredTodos= search===""?projectFilter(filter,todos):filterTodos(projectFilter(filter,todos) as Todo[], search)
        return todos===null?<></>:
    ...
    const AllKeystonesSection = () => {
        const filteredKeystones= search===""?projectFilter(filter,dateOrder(keystones)):filterKeystones(projectFilter(filter,dateOrder(keystones)) as Keystone[], search)
    ...
    const KeystoneItem = (props:KeystoneItemProps) => {
        ...
        const done = keystone.todos.filter(t=>t.checkbox===true).length
        return <List.Item title={keystone.name} id={keystone.id+"allkey"} icon={keystone.todos.length===0?"0️⃣":done===keystone.todos.length?"✅":"🟥"}
        accessories={[{tag:{value:"📍 "+getDateAndMohth(keystone.date)}}]}
        actions={<ActionPanel>
        <Action title={'Link Todos'} onAction={()=>{setLinkingTodos(!linkingTodos)}} />
        <Action.PickDate title='Change Keystone Date' onChange={(e)=>handleChangePickDateKeystone(e, keystone.id)} />
        <ToogleVueAction/><DeleteAction itemID={keystone.id} /><RefreshAction/></ActionPanel>}
        detail={<List.Item.Detail 
        metadata={<List.Item.Detail.Metadata>
           <List.Item.Detail.Metadata.Label title={keystone.name} text={keystone.todos.length===0?"No Todos 😢":progbar(done, keystone.todos.length, true)} /> 
           ...
           {keystone.todos.map(todo => {
                ...
                return <List.Item.Detail.Metadata.TagList title={todo.name} key={todo.id}>
                    <List.Item.Detail.Metadata.TagList.Item onAction={()=> handleToogleTodo(todo)} text={todo.checkbox?"✅":"🟥"}  color={todo.checkbox?Color.Green:Color.Red} />
                </List.Item.Detail.Metadata.TagList>
           })}
           </>}
           ...
           {todos.map(todo => {
                if(todo.keystone.name.length === 0) {
                    return (<List.Item.Detail.Metadata.TagList title={todo.name} key={todo.id}>
                        <List.Item.Detail.Metadata.TagList.Item color={Color.Blue} icon={"🔗"} onAction={()=> {handleLinkToKeystone(todo, keystone)}} />
                    </List.Item.Detail.Metadata.TagList>)
                }
           })}
           </>}
    ...
    const TodoItem = (p:TodoItemProps) => {
        ...
        const projectAccessorie = filter!=="Nothing"?{text:""}:{tag:{value:todo.project.icon +" "+todo.project.name}}
        const accessories = [(todo.keystone.name.length===0)?{text:""}:{tag:{value:"📍 "+ getDateMounthAndNumber(todo.keystone.date)}},projectAccessorie]
        return <List.Item title={todo.name}  icon={todo.checkbox?"✅":"🟥"} id={'todos'+todo.id}
        accessories={showDetail?[]:accessories}
        detail={<List.Item.Detail metadata={view===VTEXT.lk?<YourTaskMetadata  />:<></>} />}
        actions={<ActionPanel><Action title='Toogle' icon={Icon.Repeat} onAction={()=>handleToogleTodo(todo)}/>
            {todo.keystone.name.length!==0?<Action title='Unlink to Keystone' icon={Icon.PinDisabled} onAction={()=> handleUnlinkKeystone(todo)} />:<></>}
            <ToogleVueAction/>
            <DeleteAction  itemID={todo.id}   /><RefreshAction />
            </ActionPanel>}
        />
    }
    //#endregion
    ...
    const SearchBarAccessories  = () => {
        return <List.Dropdown tooltip="Search todos" onChange={onChangeProjectFilter} value={filter} >
        <List.Dropdown.Item title="Nothing" value="Nothing" />
        <List.Dropdown.Section title="Projects" >
            {projects.map((project:Project,i:number) => {
            return( <List.Dropdown.Item key={i}
            value={project.name}
    ...
    const ToogleVueAction = () => {
        ...
        return <Action title='Toogle Vue' icon={Icon.Eye} shortcut={{modifiers:['cmd'], key:"v"}}
    ...
    const DeleteAction = ({itemID}:{itemID:string}) => {
        return <Action title={"Delete"} icon={Icon.Trash}  shortcut={{modifiers:['cmd'], key:"x"}} style={Action.Style.Destructive} onAction={()=> handleDeleteItem(itemID)}  />
    }
    const RefreshAction = () => {
        return <Action title="Refresh" shortcut={{modifiers:['cmd'], key:"r"}} icon={Icon.ArrowCounterClockwise} onAction={()=> {refresh(['all'])}} />
    }
    ...
    const filterTodos = (todos:Todo[], search:string) => {
        ...
        todos.forEach(todo => {
            if( todo.name.toLowerCase().includes(search) ||
    ...
    const filterKeystones = (keystones:Keystone[], search:string) => {
        ...
        keystones.forEach(keystone => {
            if( keystone.name.toLowerCase().includes(search) ||
    ...
    searchBarPlaceholder={addType==="Nothing"?("Search " +( view===VTEXT.at?'Todos...':'Keystones...')):"Enter New "+addType}
    searchText={search} 
    ...
    >
        ...
        {isLoading && firstLoad? <EmptyView type='Task Manager' /> :<>
        <YourTaskItem />
        {addType!=="Nothing"?<></>:
        view === VTEXT.at? <AllTodosSection />:
        view === VTEXT.ak ? <AllKeystonesSection /> :
        <></>}
