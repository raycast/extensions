...
//#region INTERFACES
interface JournalItemProps {journal:Journal}
interface DeleteActionProps {journal:Journal}
//#endregion
...
const JournalView = () => {
    ...
    const handleAddJournal = async(date:Date|null) => {
        ...
        await QueryAddJournal("",date?.toISOString() as string,filter,projectID as string, date?.toISOString().slice(0,10) as string, notion )
        refresh(['journal'])
    }
    ...
    const MyJournal = () => {
        ...
        return <List.Item title={"📔 "+"My Journal"} id='mj'
        actions={<ActionPanel>
        {filter==="Nothing"? <Action style={Action.Style.Destructive}  title={"Select Project to Add Journal"} />:
    ...
    const sectioningJournals = (journals:Journal[]) => {
        ...
        journals.forEach((journal) => {
            ...
            //YESTERDAY
                ...
                if(sections.some(s=>s.name==="Yesterday")) sections.find(s=>s.name==="Yesterday")?.journals.push(journal)
                else {
            ...
            //THIS WEEK
                ...
                if(sections.some(s=>s.name==="This Week")) sections.find(s=>s.name==="This Week")?.journals.push(journal)
                    else {
            ...
            //THIS MOUNTH
                ...
                if(sections.some(s=>s.name==="This Month")) sections.find(s=>s.name==="This Month")?.journals.push(journal)
                    else {
                    const section:Section={name:"This Month", journals:[journal], order:3,} 
                    sections.push(section)}
            }
            else {
                if(sections.some(s=>s.name==="Before")) sections.find(s=>s.name==="Before")?.journals.push(journal)
                else {
    ...
    const JournalSection = () => {
        ...
        return sectionnedJournal.map(section => {
            return <List.Section title={section.name} key={section.name} >
    ...
    const JournalItem = (p:JournalItemProps) => {
        ...
        return <List.Item title={getDayDateAndMouth(journal.date)} id={journal.id+"page"}
        accessories={[{tag:{value:journal.project.icon+" "+journal.project.name}}]}
        actions={<ActionPanel>
        {addTextJournal === null ? <Action title='Add Text to Journal' onAction={()=> onActiveAddTextJournal(journal.id)}/> : 
        <>
        <Action title={search===""?"Enter New Text in Search":"Add New Text"} style={search===""?Action.Style.Destructive:Action.Style.Regular} 
        onAction={search!==""?()=>handleAddText(journal):()=>{}} />
        <Action title='Quit' style={Action.Style.Destructive} onAction={()=> onActiveAddTextJournal(journal.id)}/>
        </>
        }
        <ChangeDateAction journal={journal} />
        <DeleteAction journal={journal} /></ActionPanel>}
        detail={<List.Item.Detail 
        metadata={addTextJournal !== journal.id?<></>:
            <List.Item.Detail.Metadata>
                <List.Item.Detail.Metadata.TagList title='Command + Enter to' >
                    <List.Item.Detail.Metadata.TagList.Item text='Cancel' color={Color.Red} onAction={()=>onActiveAddTextJournal(journal.id)} />
    ...
    const JournalItemMK = (journal:Journal) => {
        ...
        let mk = ""
        mk = h2(getDayDateAndMouth(journal.date)) + br +
        timeText+ " "+ br +  
        journal.text +" "+ br 
        let journalTodos = ""
        journal.todos.forEach(t=>{journalTodos = journalTodos + code(t.name)+ " "+br+ " "})
    ...
    const filterSearchJournals = (journals:Journal[], search:string) => {
        ...
        journals.forEach(journal => {
            if( journal.date.includes(search) ||
    ...
    const RefreshAction = ()=> {
      ...
      const DeleteAction = (p:DeleteActionProps) => {
        const {journal} = p
        return <Action title='Delete' style={Action.Style.Destructive} shortcut={{modifiers:['cmd'], key:"x"}} onAction={()=>handleDeleteItem(journal)} icon={Icon.Trash}  />
      }
      const ChangeDateAction = ({journal}:{journal:Journal})=> {
        return <Action.PickDate title='Change Date' shortcut={{modifiers:['cmd'], key:"d"}}  onChange={(e)=>handleChangeDateItem(e, journal)} />
      }
    //#endregion
  ...
  return (
    ...
    onSelectionChange={onChangeSelection} searchText={search}
    ...
    >
        {(firstLoad && isLoading) ? <EmptyView type='Journals' /> : 
        <>
        <MyJournal/>
        <JournalSection />
        </>
...
interface Section {
    name:string,journals:Journal[], order:number
}
