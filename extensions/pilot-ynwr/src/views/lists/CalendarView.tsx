...
import { getDateAndMohth, getDateMounthAndNumber, getDayDateAndMouth, getMounthStringByMonthNumber, progbar } from '../../tools/generalTools'
...
const CalendarView = () => {
  ...
  const onChangeItemsView = (item:string) => {
    if(itemViews.length ===2) {setItemViews([item]); return}
  ...
  const onChangeDateViews = (item:string) => {
    if(dateViews.length ===4) {setDateViews([item]); return}
  ...
  const onChangeAddType = (type:string) => {
    if(type===addType || type ==="Nothing") {setAddType("Nothing");setSearch("");setAddTime(null); return}
  ...
  const onSelectionChanged = (id:string|null) => {
    if(id==="mc") {setShowDetail(true), setOpen(false)}
    else if(id?.includes('keystone')) {open?setShowDetail(true):setShowDetail(false)}
    else if(id?.includes('event')) {setShowDetail(false), setOpen(false)}
  }
  ...
  const CalItem = ({item}:{item:CalItem}) => {
    if( item.project.name.toLowerCase().includes(search)||
        item.dateInfos.toLowerCase().includes(search) ||
        item.name.toLowerCase().includes(search)   
    ) return ( <>{item.type === "event" ?
    <CalItemEvent item={item} />
    :<CalItemKeystone item={item} />
    }</>)
  }

  const CalItemEvent = ({item}:{item:CalItem}) => {
    const eventSub = retrieveDateInfos(item.dateInfos, showDetail)
    const accessories = filter!=="Nothing"?[]:!showDetail?[{tag:{value:item.project.icon + " "+item.project.name}}]:[{tag:{value:item.project.icon}}]
    return <List.Item title={item.name} icon={"⌚"} id={"event"+item.id} subtitle={eventSub}
    accessories={accessories}
    actions={<ActionPanel><ChangeDateAction item={item} type={'event'} /><DeleteAction item={item}/></ActionPanel>}
    />
  }

  const CalItemKeystone = ({item}:{item:CalItem})  => {
    const done = item.todos.filter(t => t.checkbox).length
    const projectAccessories = filter!=="Nothing"?[]:!showDetail?[{tag:{value:item.project.icon + " "+item.project.name}}]:[{tag:{value:item.project.icon}}]
    const itemsAccessories = item.todos.length!==0&&!showDetail?[{tag:{value:progbar(done, item.todos.length, true)}}]:[]
    const accessories = [ ...itemsAccessories,...projectAccessories]
    const actions = <ActionPanel>{item.todos.length!==0?<Action icon={Icon.Eye} title={open?"Hide Todos":"Show Todos"} onAction={()=>onToogleOpen()} />:<></>}
    <ChangeDateAction item={item} type={'keystone'} /><DeleteAction item={item} /></ActionPanel>

    const onToogleOpen = () => {setOpen(!open)}

    return <List.Item title={item.name} icon={"📍"} id={"keystone"+item.id} subtitle={showDetail?getDateAndMohth(item.startDate):getDayDateAndMouth(item.startDate)}
    accessories={accessories}
    actions={actions}
  ...
  const CalItemKeystoneMetadata = ({keystone}:{keystone:CalItem}) => {
    ...
    return (<List.Item.Detail.Metadata>
      ...
      {keystone.todos.map(todo => {
        return <List.Item.Detail.Metadata.TagList title={todo.name} key={todo.id} >
        <List.Item.Detail.Metadata.TagList.Item text={todo.checkbox?"✅":"🟥"} color={todo.checkbox?Color.Green:Color.Red} 
  ...
  const CalendarSection = ({calendar}:{calendar:CalSection[]}) => {
    return (calendar.map((section, i) => {
      if(dateViews.includes(section.type)) return  (<List.Section title={section.name} key={i} >
  ...
  //#region  MY CALENDAR
  const MyCalendar = () => {
    return <List.Item title={"My Calendar"} id='mc' icon={"📅"}
    actions={<ActionPanel>
    <AddElementAction />
    <RefreshAction />
    <ClearRefreshAction clearRefresh={clearRefresh} setFirst={setFirstLoad} setShow={setShowDetail} /></ActionPanel>}
    detail={<List.Item.Detail 
    metadata={<MyCalendarMetadata/>}  
    />} />
  }
  const MyCalendarMetadata = () => {
    ...
    <List.Item.Detail.Metadata.TagList title='🗓️ Date filter' >
      <List.Item.Detail.Metadata.TagList.Item  text='Today' color={dateViews.includes('today')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeDateViews('today')} />
      <List.Item.Detail.Metadata.TagList.Item text='This week' color={dateViews.includes('week')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeDateViews('week')}/>
      <List.Item.Detail.Metadata.TagList.Item text='This mounth' color={dateViews.includes('month')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeDateViews('month')}/>
      <List.Item.Detail.Metadata.TagList.Item text='All year' color={dateViews.includes('year')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeDateViews('year')}/>
    </List.Item.Detail.Metadata.TagList>


      <List.Item.Detail.Metadata.TagList title='👁️ Item Filter' >
          <List.Item.Detail.Metadata.TagList.Item  text='📍 Keystones' color={itemViews.includes('keystone')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeItemsView('keystone')} />
          <List.Item.Detail.Metadata.TagList.Item text='🤝 Events' color={itemViews.includes('event')?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeItemsView('event')}/>
        </List.Item.Detail.Metadata.TagList>
      <List.Item.Detail.Metadata.Separator />    

      <List.Item.Detail.Metadata.Label title='🆕 Add New' text={filter==="Nothing"?"Select a project filter":addType==="Nothing"?"Select a type":"Enter "+addType+" Name on Search"} />
      <List.Item.Detail.Metadata.TagList title=''>
        {filter==="Nothing"?<List.Item.Detail.Metadata.TagList.Item text='Select a project filter' color={Color.SecondaryText}/>:
        <><List.Item.Detail.Metadata.TagList.Item text='📍 Keystones' onAction={()=> onChangeAddType("Keystone") } color={addType==="Keystone"?Color.PrimaryText:Color.SecondaryText} />
        <List.Item.Detail.Metadata.TagList.Item text='🤝 Events' onAction={()=>onChangeAddType("Event") } color={addType==="Event"?Color.PrimaryText:Color.SecondaryText} />
        {addType!=="Nothing"?<List.Item.Detail.Metadata.TagList.Item text='Cancel' onAction={()=>onChangeAddType("Nothing")} color={Color.Red} />:<></>}
        </>}
      </List.Item.Detail.Metadata.TagList>

      {addType==="Event"?<List.Item.Detail.Metadata.TagList title='⌛ How many time'  >
        <List.Item.Detail.Metadata.TagList.Item text='15 minutes' color={addTime === ADDTIMES.a?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddTime(ADDTIMES.a)} />
        <List.Item.Detail.Metadata.TagList.Item text='30 minutes' color={addTime === ADDTIMES.b?Color.PrimaryText:Color.SecondaryText}onAction={()=>onChangeAddTime(ADDTIMES.b)} />
        <List.Item.Detail.Metadata.TagList.Item text='1 hour' color={addTime === ADDTIMES.c?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddTime(ADDTIMES.c)}/>
        <List.Item.Detail.Metadata.TagList.Item text='2 hours' color={addTime === ADDTIMES.d?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddTime(ADDTIMES.d)} />
        <List.Item.Detail.Metadata.TagList.Item text='4 hours' color={addTime === ADDTIMES.e?Color.PrimaryText:Color.SecondaryText} onAction={()=>onChangeAddTime(ADDTIMES.e)} />
      </List.Item.Detail.Metadata.TagList>:<></>}
  ...
  return <List.Dropdown tooltip="Search todos" onChange={setFilter} value={filter} >
  ...
  const RefreshAction = ()=> {
    return <Action title='Refresh' onAction={()=>refresh(['all'])} icon={Icon.ArrowCounterClockwise}
    autoFocus={false} shortcut={{modifiers:['cmd'], key:"r"}}
    />
  }
  const DeleteAction = ({item}:{item:CalItem}) => {
    return <Action title='Delete' style={Action.Style.Destructive} shortcut={{modifiers:['cmd'], key:"x"}} onAction={()=>handleDeleteItem(item.id,item.type)} icon={Icon.Trash}  />
  }
  const ChangeDateAction = ({item, type}:{item:object, type:string})=> {
    return <Action.PickDate title='Change Date' shortcut={{modifiers:['cmd'], key:"d"}}  onChange={(e)=>handleChangeDateItem(e, item as CalItem, type)} />
  }
  //#endregion
  ...
  return (
    <List isShowingDetail={showDetail} searchBarPlaceholder={addType==="Nothing"?'Search Keystones, Events...':"Enter "+addType+" Name"}
    searchText={search}
    onSearchTextChange={(e)=>setSearch(e)}
    onSelectionChange={onSelectionChanged}
    searchBarAccessory={isLoading ? <></> :<SearchBarAccessories/>}
    >
      {(isLoading && firstLoad) ? <EmptyView type='Calendar' /> : <> 
      <MyCalendar/>
...
const calendarConverter = (events:Evnt[], keystones:Keystone[], filter:string, search:string) => {
  ...
  if(events!==null) nameSearchFilter(events, search).forEach(e => { 
    const event = e as Evnt
    const item:CalItem = {
      id:event.id,
      name:event.name,
      dateInfos:event.start.slice(0,10)+"-"+event.start.slice(11,16)+ (event.end==null?"":"→"+event.end.slice(11,16)),
  ...
  if(keystones!==null) nameSearchFilter(keystones, search).forEach(k => {
...
const groupByDate = (items:CalItem[]) => {
  ...
  items.forEach((item:CalItem) => {
    ...
    if(new Date(today.toISOString().slice(0,10)).getTime() <= new Date(date.toISOString().slice(0,10)).getTime() || ((item.type==="keystone") && new Date(today.getDate()-1).getTime() < date.getTime()) ) {
      ...
      //SI CETTE SEMAINE
      if(month === today.getMonth() && number-today.getDate() < 7 && today.toISOString().slice(0,10)!== date.toISOString().slice(0,10)) {
        if(sections.some(s=>s.name==="This week")) sections.find(s=>s.name==="This week")?.items.push(item)
        else {
      ...
      if(month === today.getMonth() && number-today.getDate() >= 7) {
        if(sections.some(s=>s.name==="This month")) sections.find(s=>s.name==="This month")?.items.push(item)
          else {
      ...
      if(month !== today.getMonth()) {
        if(sections.some(s=>s.name===getMounthStringByMonthNumber(month))) sections.find(s=>s.name===getMounthStringByMonthNumber(month))?.items.push(item)
          else {
          let order = (month-today.getMonth()) +1
          if(order < 0) order = order +12 
          const section:CalSection={name:getMounthStringByMonthNumber(month), items:[item], order:order, type:'year'} 
          sections.push(section)}
      }
    }
  })
  ...
  sections.map(section =>
    section.items = sortedByDate(section.items)
  )
