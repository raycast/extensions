...
const LinksForm = (p:Props) => {
    ...
    const handleErrors = (v:SubmitForm) => {
        ...
        linkError === undefined)) check = false
        return check
    }
    ...
    const handleSubmit = async (v:SubmitForm, close:boolean) => {
        ...
        if(r) {showToast({title: "Link added successfully", style:Toast.Style.Success}), p.refresh(['project'])}
  ...
  return (
    ...
    >
        ...
        <Form.Dropdown id="projectID" title="Project" error={projectError} value={projectValue} onChange={setProjectValue} >
                {projects.map((item, i) => {
                    return(
                        <Form.Dropdown.Item key={i} value={item.id}  title={item.icon+" "+item.name} />
                )})}
        </Form.Dropdown>
