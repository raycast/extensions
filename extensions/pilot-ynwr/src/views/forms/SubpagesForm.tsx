...
const SubpagesForm = (p:Props) => {
    ...
    const handleErrors = (v:SubmitForm) => {
        ...
        if(NameChecker(v.name)===undefined &&
        IconChecker(v.icon)=== undefined &&
        ProjectChecker(v.projectID)=== undefined) check = false
        return check
    }
    ...
    const handleSubmit = async (v:SubmitForm, close:boolean) => {
        ...
        if(r) {showToast({title:"Subpage added successfully", style:Toast.Style.Success}),p.refresh(['project'])}
  ...
  return (
    ...
    actions={
        ...
        {ICON_LIST.map((icon,i) => {
                return(
                <Form.Dropdown.Item key={i} icon={icon.emoji} title={icon.keywords[0]} value={icon.emoji} />
                )
            })}
        </Form.Dropdown>

        <Form.Dropdown id="projectID" title="Projects" error={projectError} value={projectValue} onChange={setProjectVallue} >
            {projects.map((project, i) => {
                return(
                    <Form.Dropdown.Item key={"l"+i} value={project.id}  title={project.icon+" "+project.name} />
                )
            })}
        </Form.Dropdown>
