...
function parseDateDescription(flyto: string): Date {
  ...
  */
  ...
  if (match1) {
    ...
    if (direction === 1) {
      // 将 next mon,tues,wed,thur,fri,sat,sun转换为相对今天的天数
      const dayOfWeek = {
        mon: 1 + 7,
        tues: 2 + 7,
        wed: 3 + 7,
      ...
      // 将last mon,tues,wed,thur,fri,sat,sun转换为相对今天的天数
      const dayOfWeek = {
        mon: 6,
        tues: 5,
        wed: 4,
      ...
      // match1[2]转换为对应的string
      const dayofweekStr = match1[2].toString() as keyof typeof dayOfWeek;
      let n = todayOfWeek + dayOfWeek[dayofweekStr];
      if (todayOfWeek === 0) {
        n += 7;
      }
      return new Date(today.getFullYear(), today.getMonth(), today.getDate() - n);
