...
export default function Assets({ siteId }: { siteId: string }) {
    ...
    const { isLoading, data: assets = [] } = usePromise(async () => {
    ...
    return <List isLoading={isLoading}>
        {assets.map(asset => <List.Item key={asset.id} icon={asset.hostedUrl} title={`${asset.displayName}`} subtitle={asset.originalFileName} />)}
    </List>
}