...
function provider(
  ...
  control = vi.fn(),
): SourceProvider & { control: ReturnType<typeof vi.fn> } {
  return {
    id,
    displayName: id,
    bundleIds,
...
describe("getMediaSources", () => {
  ...
  it("survives a provider whose getSource rejects", async () => {
    ...
    registerProvider(
      provider("music", ["com.apple.Music"], src({ id: "com.apple.Music", bundleId: "com.apple.Music" })),
    );
    const { sources } = await getMediaSources();
