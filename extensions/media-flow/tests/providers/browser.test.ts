...
describe("parseYouTubeTitle", () => {
  ...
  it("strips notification prefix", () => {
    expect(parseYouTubeTitle("(3) Lo-fi beats - YouTube")).toEqual({
      title: "Lo-fi beats",
    });
  });
...
describe("edge cases", () => {
  ...
  it("uses raw title for soundcloud", async () => {
    ...
    expect(s).toMatchObject({
      title: "A Mix",
      url: "https://soundcloud.com/artist/a-mix",
    });
  });
  ...
  it("has no control method and correct capabilities", () => {
    ...
    expect(safariProvider.capabilities).toEqual({
      control: false,
      artwork: false,
      seek: false,
    });
    expect(chromeProvider.control).toBeUndefined();
    expect(chromeProvider.capabilities).toEqual({
      control: false,
      artwork: false,
      seek: false,
    });
