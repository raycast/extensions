...
# @raycast.title Log Raycast Focus
# @raycast.mode inline
...
# Documentation:
# @raycast.author dinocosta
# @raycast.authorURL https://raycast.com/dinocosta

# Default: use log show
use_file=false
file_path=""
output_file="$(dirname "$0")/log.txt"
...
while IFS= read -r line; do
    ...
    if [[ "$line" == *"Start focus session"* ]]; then
        ...
        if [[ "$output_to_console" = true ]] || [[ -z "$output_file" ]]; then
            echo "$output_line"
        fi
        current_start_timestamp=""
        current_start_goal=""
