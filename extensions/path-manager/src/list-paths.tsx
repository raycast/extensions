...
export default function ListPaths() {
  ...
  async function fetchPaths() {
    try {
      ...
      setPaths(parsedData)
      setFilteredList(
        Object.entries(parsedData).map(([alias, path]) => ({ alias, path }))
      )
    } catch (error) {
      showToast({
  ...
  useEffect(() => {
    const filtered = Object.entries(paths)
      .filter(
        ([alias, path]) =>
          alias.includes(searchText) || path.includes(searchText)
      )
      ...
      primaryAction: {
        ...
        onAction: async () => {
          try {
            ...
            await fs.writeFile(
              STORAGE_PATH,
              JSON.stringify(updatedPaths, null, 2),
              'utf-8'
            )
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={() => handleSubmit(path)}
                icon={Icon.Terminal}
              />
              <Action
                title='Edit Path'
                onAction={() => handleEdit(alias, path)}
                icon={Icon.Pencil}
              />
              <Action
                title='Create Path'
