...
Search previews are screenshot-first: the preview stays blank while the native loading bar is active, then shows the image without first flashing OCR or loading text. Gallery thumbnails also remain blank until ready; unavailable screenshots retain an error indicator. Open the inspector for OCR, accessibility text, OCR boxes or metadata. Use Save Search from search results to retain the current query and filters.
