...
Search previews are screenshot-first: a fixed-height blank area keeps metadata stationary while the native loading bar is active, then shows the image without first flashing OCR or loading text. Recent screenshot paths are reused in memory while their files exist; no screenshots are copied or persisted by this cache. Gallery thumbnails also remain blank until ready; unavailable screenshots retain an error indicator. Open the inspector for the full screenshot, OCR, accessibility text, OCR boxes or metadata. Use Save Search from search results to retain the current query and filters.
