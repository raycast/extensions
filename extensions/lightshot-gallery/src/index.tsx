...
export default function Command() {
  ...
  return isSuccess ? (
    <Grid
      searchBarPlaceholder={screens.length !== 0 ? "Search an image using its publishing date or description" : "There's currently nothing to search for"}
      columns={4}
      isLoading={isLoading}
      navigationTitle={username ? `Logged in as ${username}` : "Logging in..."}
    >
      {screens.length === 0 ? <Grid.EmptyView
          icon={Icon.Globe}
          ...
          actions={
            <ActionPanel>
              <Action.OpenInBrowser
                title={"Open prnt.sc"}
                url={"https://prnt.sc/gallery.html"}
              />
            </ActionPanel>
          }
        />
        : (
          screens.map((screen: Screen) => (
            <Grid.Item
              ...
              keywords={
                screen.description ? [screen.description, screen.id36] : [screen.id36]
              }
              actions={
    ...
    <Detail markdown='## Something went wrong. Your auth key must be the problem. Follow these instructions to get your auth key:
