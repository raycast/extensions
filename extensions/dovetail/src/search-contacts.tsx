...
function getFieldValue(fields: any[], label: string): string | undefined {
...
export default function SearchContacts() {
  const { data, isLoading, onSearchTextChange, onLoadMore, hasMore, numberOfResults } = useContactsSearch();
                ...
                <List.Item.Detail
                  ...
                  metadata={
                    <List.Item.Detail.Metadata>
                      ...
                      <List.Item.Detail.Metadata.Label title="Email" text={Array.isArray(contact?.fields) ? getFieldValue(contact.fields, "Email") || "" : ""} />
                      {contact?.created_at && (
                      ...
                      {Array.isArray(contact?.fields) && contact.fields.length > 0 && (
                        <>
                          ...
                          {contact.fields.map((field: any, idx: number) => {
                              ...
                              return (
                                <List.Item.Detail.Metadata.TagList key={idx} title={field.label}>
                                  {field.value.map((tag: string, tagIdx: number) => (
                                    <List.Item.Detail.Metadata.TagList.Item key={tagIdx} text={tag} color={TAG_COLORS[idx % TAG_COLORS.length]} />
                                  ))}
