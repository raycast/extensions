...
export default async function Command() {
  ...
  try {
    ...
    await convert(selectedModels, "STL");

    // Update the toast notification with the conversion result
    toast.title = `Converted ${selectedModels.length} ${pluralized} to STL`;
    toast.style = Toast.Style.Success;
  } catch (error) {
    // Handle conversion error and show an error toast
    await showErrorToast(`Failed to convert ${selectedModels.length} ${pluralized} to STL`, error as Error, toast);
  } finally {
