...
export default function Command(props: { draftValues?: Values }) {
  ...
  async function handleSubmit(values: Values) {
    ...
    const script = stackScript("install-github-tools.sh");

    if (values.mode === "terminal") {
      await runInTerminal(commandLine(script, args), p.upsaDir);
      return;
    }
  ...
  return (
    ...
    >
      <Form.TextField
        id="target"
        title="Directorio"
        placeholder="/Users/dalonsogomez/Developer/UPSA"
        defaultValue={draft?.target ?? p.upsaDir}
      />
      <Form.Separator />
      <Form.TextField
        id="repo1"
