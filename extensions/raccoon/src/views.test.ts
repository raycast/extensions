...
const viewed = new Set([...registry.slice(0, registry.indexOf("};")).matchAll(/^\t"?([a-z-]+)"?:/gm)].map((m) => m[1]));
...
const hidden = new Set(
	[...(source.match(/const HIDDEN = new Set\(\[([^\]]*)\]\)/)?.[1] ?? "").matchAll(/"([a-z-]+)"/g)].map((m) => m[1]),
);
...
test("every command the CLI offers has a screen of its own", () => {
	...
	const missing = COMMANDS.filter((c) => !viewed.has(viewKey(c.args)) && !hidden.has(viewKey(c.args))).map(
		(c) => `rcc ${c.args.join(" ")}`,
	);
