...
function Rows({ c, actions }: { c: CertsReport; actions: React.ReactNode }) {
	...
	// anyone opens this.
	const sorted = useMemo(() => [...c.certificates].sort(byUrgency), [c.certificates]);
	...
	const clearAll =
		removable.length > 0
			? offered(() => ({
					...
					command: deleteCertificates(removable.map((cert) => cert.sha256)),
				...
				<List.Item
					...
					actions={
						<RowActions
							one={
								isRemovable(cert)
									? offered(() => ({
											...
											command: deleteCertificates([cert.sha256]),
							...
							shared={actions}
						>
							<Action.CopyToClipboard title="Copy Certificate Name" content={cert.name} />
						</RowActions>
					}
				/>
			))}
