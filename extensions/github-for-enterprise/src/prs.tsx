...
export default function Command() {
    ...
    const [open, recentlyClosed] = useMemo(() => partition(data?.user.pullRequests.nodes || [], ({ state }) => /OPEN/.test(state)), [data]);

    const fuseOpen = useMemo(() => new Fuse(open || [], {
        threshold: 0.2,
        ignoreLocation: true,
        keys: ['title', 'number', 'repository.nameWithOwner']
    }), [open]);

    const fuseRecentlyClosed = useMemo(() => new Fuse(recentlyClosed || [], {
        threshold: 0.2,
        ignoreLocation: true,
        keys: ['title', 'number', 'repository.nameWithOwner']
    }), [recentlyClosed]);

    const searchOpen = useCallback((str: string) => {
        if (!str) {
    ...
    const searchRecentlyClosed = useCallback((str: string) => {
        if (!str) {
    ...
    return (
        <List
            ...
            {data && searchOpen(searchText)?.length > 0 && (
                <List.Section
                    title="Open"
                    subtitle={plural(searchOpen(searchText)?.length, 'pull request')}>
