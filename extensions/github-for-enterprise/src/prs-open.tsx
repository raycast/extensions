...
export default function Command() {
    ...
    const fusePRs = useMemo(() => new Fuse(data?.user.pullRequests.nodes || [], {
        threshold: 0.2,
        ignoreLocation: true,
        keys: ['title', 'number', 'repository.nameWithOwner']
    }), [data]);

    const searchPRs = useCallback((str: string) => {
        if (!str) {
    ...
    return (
        <List
            ...
            <List.Section
                title="Pull Requests"
                subtitle={plural(searchPRs(searchText)?.length, 'pull request')}>
                {data && searchPRs(searchText)?.map(({ item }) => (
                    <PRTemplate
                        key={item.id}
                        {...item} />
                ))}
