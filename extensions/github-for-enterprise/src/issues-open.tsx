...
export default function Command() {
    ...
    const fuseIssues = useMemo(() => new Fuse(data?.user.issues.nodes || [], {
        threshold: 0.2,
        ignoreLocation: true,
        keys: ['title', 'number', 'repository.nameWithOwner']
    }), [data]);

    const searchIssues = useCallback((str: string) => {
        if (!str) {
    ...
    if (error) {
        popToRoot();
        showToast(ToastStyle.Failure, 'Could not get issues', error.message);
    }
    ...
    return (
        <List
            ...
            <List.Section
                title="Issues"
                subtitle={plural(searchIssues(searchText)?.length, 'issue')}>
                {data && searchIssues(searchText)?.map(({ item }) => (
                    <IssueTemplate
                        key={item.id}
                        {...item} />
                ))}
