...
import { detectLanguage, detector, supportedDetectors } from "./utils.js";

type LaunchContext = {
  content?: string;
  callbackLaunchOpitons?: LaunchOptions;
};

export default function TextToDetect({ launchContext = {} }: LaunchProps<{ launchContext?: LaunchContext }>) {
  const { content, callbackLaunchOpitons } = launchContext;
  const [languageName, setLanguageName] = useState("");
  const [isDetecting, setIsDetecting] = useState(false);
  const canAccessAi = environment.canAccess(AI);

  useEffect(() => {
    if (content && callbackLaunchOpitons) {
      detectLanguage(content).then(({ languageCode, languageName }) => {
        callbackLaunchCommand(callbackLaunchOpitons, { languageCode, languageName });
      });
    }
  }, []);
