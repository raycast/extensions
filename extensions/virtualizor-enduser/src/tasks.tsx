...
export default function Tasks() {
    ...
    return <List isLoading={isLoading} searchBarPlaceholder="Search tasks" pagination={pagination} isShowingDetail>
        <List.Section title={`${tasks.length} tasks`}>
        {tasks.map(task => {
            const progress = Number(task.progress_num);
            return <List.Item key={task.actid} icon={getProgressIcon(Number(task.progress_num), progress===100 ? Color.Green : Color.Red)} title={task.action || "NO ACTION"} accessories={[{date: timestampToDate(task.time)}]} detail={<List.Item.Detail markdown={`## Data \n\n ${task.data}`} metadata={<List.Item.Detail.Metadata>
