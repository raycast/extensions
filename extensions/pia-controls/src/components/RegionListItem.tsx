...
function icon(region: Region): Image.ImageLike {
  ...
  return region.countryCode ? { source: flagAsset(region.countryCode) } : Icon.Globe;
}
    ...
    <List.Item
      ...
      actions={
        <ActionPanel>
          ...
          {isConnectedHere ? (
            <Action title="Disconnect" icon={Icon.XMarkCircle} onAction={onDisconnect} />
          ) : (
          ...
          />
          <Action.CopyToClipboard title="Copy Region ID" content={region.id} shortcut={Keyboard.Shortcut.Common.Pin} />
          <SettingsActions status={status} cliPath={cliPath} appPath={appPath} onSettingChanged={onSettingChanged} />
        </ActionPanel>
      }
    />
  );
