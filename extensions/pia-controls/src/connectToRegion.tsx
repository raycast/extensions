...
export default function Command() {
  ...
  if (setup.stage === "checking") {
    return <List isLoading searchBarPlaceholder="Loading Private Internet Access…" />;
  }
  ...
  return (
    <List isLoading={isLoading} searchBarPlaceholder="Search regions…">
      ...
      <List.Section title="Quick Connect">
        {renderItem(AUTO_REGION_ENTRY, "auto", "Let PIA pick the fastest region")}
      </List.Section>

      {favoriteRegions.length > 0 && (
        <List.Section title="Favorites">{favoriteRegions.map((r) => renderItem(r, "fav"))}</List.Section>
      )}

      {recentRegions.length > 0 && (
        <List.Section title="Recently used">{recentRegions.map((r) => renderItem(r, "recent"))}</List.Section>
      )}
