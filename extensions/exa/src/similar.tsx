...
export default function Command() {
      ...
      >
        <List.Section title={`${filteredResults.length} Similar Pages`} subtitle={selectedSearch.url}>
              ...
              actions={
                <ActionPanel>
                  ...
                  <Action
                    ...
                    onAction={() => findSimilar(result.url)}
                  />
                  <Action
                    title="Copy URL"
                    icon={Icon.Clipboard}
          ...
          {filteredResults.length === 0 && !pendingSearches[selectedSearch.id] && (
            <List.EmptyView
              icon={Icon.Link}
              title="No similar pages found"
              description={resultsSearchText ? "Try a different search term" : "No similar pages for this URL"}
            />
          )}
        </List.Section>
      </List>
  ...
  return (
    <List
      isLoading={isLoading}
      searchBarPlaceholder="Enter a URL to find similar pages..."
      onSearchTextChange={setSearchText}
      searchText={searchText}
      navigationTitle="Exa Similar"
      actions={
        <ActionPanel>
          <Action title="Find Similar" icon={Icon.Link} onAction={handleSubmit} />
        </ActionPanel>
      }
    >
      {/* New URL item when there's text in the search bar */}
      {searchText.trim() && (
        <List.Item
          title={`Find pages similar to "${searchText}"`}
          icon={Icon.Link}
          actions={
            <ActionPanel>
              <Action title="Find Similar" icon={Icon.Link} onAction={handleSubmit} />
      ...
      <List.Section title="Recent URLs">
        {filteredSearches.map((search) => (
          <List.Item
            key={search.id}
            id={search.id}
            title={search.url}
            icon={pendingSearches[search.id] ? Icon.Clock : Icon.Link}
            accessories={[
            ...
            actions={
              <ActionPanel>
                ...
                <Action
                  ...
                  icon={Icon.Link}
                  shortcut={{ modifiers: ["cmd"], key: "r" }}
                  onAction={() => findSimilar(search.url)}
                />
                <Action
                  title="Copy URL"
                  icon={Icon.Clipboard}
      ...
      {allSearches.length === 0 && !searchText.trim() && (
        <List.EmptyView
          icon={Icon.Link}
          title="Find similar pages with Exa"
          description="Enter a URL in the search bar and press Enter"
        />
      )}
    </List>
  );
