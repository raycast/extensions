...
Phi Stable 2.4.0 or later is required. New Kiosk Window and New Incognito Space require Phi Stable 2.9.0 and scripting API version 2. Phi Canary skips semantic version checks but still requires API version 2 for these commands.
...
New Kiosk Window accepts an optional inline `URL` argument. Leaving it blank opens an empty Kiosk window.
...
1. Build and run a Phi Canary build with scripting API version 2.
2. Run `npm install`.
...
Each command declares its minimum supported Phi Stable and scripting API versions in `src/command-compatibility.ts`. Operations that call Phi execute through `runPhiCommand` or `runPhiCommandAction` so unsupported versions receive an upgrade message. Search History keeps its database query local and applies the compatibility check only when opening a selected result in Phi. Canary ignores semantic-version floors but still enforces scripting API requirements.
...
Before publishing, verify that each required scripting API version is available in the corresponding minimum supported stable Phi release and replace the provisional Raycast author value with the publisher's Raycast username.
