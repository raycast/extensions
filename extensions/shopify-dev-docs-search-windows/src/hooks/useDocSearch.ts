  ...
  useEffect(() => {
    ...
    if (newResponses && newResponses.length > 0) {
      newResponses.forEach((result) => {
        result.object_label =
          result.gid && result.gid.startsWith("-object")
            ? "Object"
            : toTitleCase(result.type);
        result.markdown = `## ${result.title}\n\n${convertHtmlToMarkdown(result.snippet)}`;
        result.pretty_breadcrumbs = formatBreadcrumbs(
          result.breadcrumb,
          result.content_category,
        );
        result.icon = {
          source: result.object_label
            ? result.object_label.toLowerCase() + ".png"
            : "other.png",
...
export function useSearchCategories(query: string) {
  ...
  if (apiData?.hit_counts_by_content_category) {
    const categoryOptions = apiData.hit_counts_by_content_category.map(
      (category) => ({
