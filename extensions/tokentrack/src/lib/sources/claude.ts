...
export async function readClaudeUsage(basePath: string, range: DateRange) {
  ...
  try {
    const files = await findJsonl(projectRoot);
    await runWithConcurrency(files, CLAUDE_READ_CONCURRENCY, (file) =>
      readClaudeFile(file, range, events, seen),
    );
  } catch {
...
export function readClaudeUsageSync(basePath: string, range: DateRange) {
  ...
  try {
    for (const file of findJsonlSync(projectRoot)) {
      readClaudeFileSync(file, range, events, seen);
    }
  } catch {
    errors.push("Claude: read error");
  }
...
async function readClaudeFile(
  ...
  for await (const line of reader) {
    lineNumber += 1;
    pushClaudeLine(file, line, lineNumber, range, events, seen);
  }
}
...
function readClaudeFileSync(
  ...
  readFileSync(file, "utf8")
    .split(/\r?\n/)
    .forEach((line, index) =>
      pushClaudeLine(file, line, index + 1, range, events, seen),
    );
}
...
function findJsonlSync(root: string): string[] {
  return readdirSync(root, { withFileTypes: true }).flatMap((entry) => {
    const path = join(root, entry.name);
    if (entry.isDirectory()) return findJsonlSync(path);
    if (entry.isFile() && entry.name.endsWith(".jsonl")) return [path];
    return [];
  });
}
