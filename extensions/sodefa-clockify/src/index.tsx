...
function NewEntry({ updateTimeEntries }: { updateTimeEntries: () => void }) {
  ...
  useEffect(() => {
    ...
    async function getAllProjectsOnWorkspace(): Promise<void> {
      ...
      const projectsWithTasks = await Promise.all(
        data.map(async (project: { id: string }) => {
          const { data } = await fetcher(
            `/workspaces/${config.workspaceId}/projects/${project.id}/tasks?page-size=1000`
          );
      ...
      setLocalStorageItem("projects", JSON.stringify(projectsWithTasks));
      setIsLoading(false);
    }
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <SubmitFormAction
            title="Start"
            onSubmit={({ description, projectOrTask }) => {
              const data = JSON.parse(projectOrTask);

              if (description && data.projectId) {
                addNewTimeEntry(description, data.projectId, data?.taskId).then(() => updateTimeEntries());
                pop();
              } else {
    ...
    >
      <Form.Dropdown id="projectOrTask" title="Project / Task">
        {projects &&
          projects.map((project: Project) => {
            const title = `${project.name} - ${project?.clientName || "No Client"}`;
            ...
            if (project.tasks?.length === 0) {
              return (
                <Form.Dropdown.Item
                  key={project.id}
                  title={title}
                  value={JSON.stringify({ projectId: project.id })}
            ...
            return (
              <Form.Dropdown.Section
                key={project.id}
                title={`${project.name} - ${project?.clientName || "No Client"} (${project.tasks?.length} Tasks)`}
              >
                <Form.Dropdown.Item
                  key={project.id}
                  title="Book without task"
                  value={JSON.stringify({ projectId: project.id })}
                  icon={{ source: Icon.Circle, tintColor: project.color }}
                />
                {project.tasks.map((task) => (
                  <Form.Dropdown.Item
                    key={task.id}
                    title={task.name}
                    value={JSON.stringify({ projectId: project.id, taskId: task.id })}
                    icon={{ source: Icon.Circle, tintColor: project.color }}
                  />
                ))}
              </Form.Dropdown.Section>
...
async function addNewTimeEntry(description: string, projectId: string, taskId?: string): Promise<void> {
