...
export default function UploadFile() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load workspaces and current selections
      const [workspacesData, currentWorkspace, currentFolder] = await Promise.all([
        razunaAPI.getWorkspaces(),
        getSelectedWorkspace(),
        getSelectedFolder(),
      ]);
      ...
      >
        <List.Dropdown.Section title="Workspaces">
          {workspaces.map((workspace) => (
            <List.Dropdown.Item key={workspace._id} title={workspace.name} value={workspace._id} icon={Icon.Globe} />
          ))}
  ...
  const renderFolderDropdown = () => {
    if (!selectedWorkspace || folders.length === 0) return renderDropdowns();
