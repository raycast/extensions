...
function notePreviewMarkdown(result: NoteSearchResult) {
  ...
  const breadcrumbs = result.note.folder_breadcrumbs
    .map((b) => b.name)
    .join(' / ')
  ...
  return (
    ...
    >
      ...
      {!error && searchText.trim().length < 2 ? (
        <List.EmptyView
          title="Search Notes"
          description="Type at least 2 characters to search your notes."
        />
      ) : null}
      {!error &&
      !isLoading &&
        ...
        <List.Item
              ...
              metadata={
                <List.Item.Detail.Metadata>
                  <List.Item.Detail.Metadata.Label
                    title="Type"
                    text={formatNoteType(result.note.type)}
                  />
                  <List.Item.Detail.Metadata.Label
                    title="Title"
                    text={result.note.title || 'Untitled'}
                  />
                  <List.Item.Detail.Metadata.Label
                    title="Last Modified"
                    text={new Date(
                      result.note.last_modified_date
                    ).toLocaleString()}
                  />
                  <List.Item.Detail.Metadata.Label
                    title="Created"
                  ...
                  {result.note.folder_breadcrumbs.length ? (
                    <List.Item.Detail.Metadata.Label
                      title="Folder"
                      text={result.note.folder_breadcrumbs
                        .map((b) => b.name)
                        .join(' / ')}
                    />
                  ) : null}
          ...
          actions={
            <ActionPanel>
              <Action
                title="Open in nocal"
                onAction={() => openNocalDeepLink(`note?id=${result.note.id}`)}
              />
              <Action.CopyToClipboard
                title="Copy Note ID"
                content={result.note.id}
              />
              <Action
                title="Open Notes View"
                onAction={() => openNocalDeepLink('notes')}
              />
            </ActionPanel>
          }
        />
