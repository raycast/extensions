...
export default function NeedsRsvpCommand() {
  ...
  const handleRsvp = async (
    ...
    const targetId =
      forSeries && item.event.series ? item.event.series : item.event.id
      ...
      {items.map((item) => {
        ...
        const rsvpAction = (
          ...
          isRecurring ? (
            <ActionPanel.Submenu title={title} icon={icon}>
              <Action
                title="Just This Instance"
                onAction={() => void handleRsvp(item, status, false)}
              />
              <Action
                title="All Instances"
                onAction={() => void handleRsvp(item, status, true)}
              />
            </ActionPanel.Submenu>
          ) : (
            <Action
        ...
        const hideAction = isRecurring ? (
          <ActionPanel.Submenu title="Hide This Invite" icon={Icon.EyeDisabled}>
            <Action
              title="Just This Instance"
              onAction={() => void handleHide(item, false)}
            />
            <Action
              title="All Instances"
              onAction={() => void handleHide(item, true)}
            />
          </ActionPanel.Submenu>
        ) : (
          <Action
            title="Hide This Invite"
                ...
                metadata={
                  <List.Item.Detail.Metadata>
                    ...
                    {event.conferencing_details?.platform ? (
                      <List.Item.Detail.Metadata.Label
                        title="Conference"
                        text={event.conferencing_details.platform}
                      />
                    ) : null}
                    {event.recurrence_summary ? (
                      <List.Item.Detail.Metadata.Label
