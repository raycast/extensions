...
const TranslateForm = () => {
  ...
  const doTranslate = React.useMemo(() => {
    const debouncedTranslate = debounce(async (text: string, fromLang: LanguageCode | string, toLang: LanguageCode | string) => {
      ...
      actions={(
        <ActionPanel>
          ...
          <ActionPanel.Section title="Settings">
            <ActionPanel.Submenu shortcut={{ modifiers: ['cmd', 'shift'], key: 'f' }} title="Change From Language" icon={fromLangObj?.flag}>
              {languages.map(lang =>
                <Action key={lang.code} onAction={() => setFromLang(lang.code)} title={lang.name} icon={lang?.flag ?? '🏳️'} />
              )}
            </ActionPanel.Submenu>
            <ActionPanel.Submenu shortcut={{ modifiers: ['cmd', 'shift'], key: 't' }} title="Change To Language" icon={toLangObj?.flag}>
              {languages.map(lang =>
                <Action key={lang.code} onAction={() => setToLang(lang.code)} title={lang.name} icon={lang?.flag ?? '🏳️'} />
              )}
            </ActionPanel.Submenu>
          </ActionPanel.Section>
        </ActionPanel>
