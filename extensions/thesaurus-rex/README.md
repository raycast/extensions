...
- ⌘D opens a detail pane: the word with its IPA where WordNet has one, the definition in full and its example sentences. On a synonym or antonym row it shows what *that* word means.
...
- Inflected forms fall back to their stem, so *running* finds *run* and *carries* finds *carry*. The fallback only fires when the shortened form is really in the dictionary, so *bus* never becomes *bu*.
