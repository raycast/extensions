...
describe('buildChatCompletionsUrl', () => {
  ...
  it('appends chat completions to a v1 base URL', () => {
    expect(buildChatCompletionsUrl(' http://127.0.0.1:11434/v1/ ')).toBe(
      'http://127.0.0.1:11434/v1/chat/completions',
    );
  });

  it('adds v1 chat completions to a server root', () => {
    expect(buildChatCompletionsUrl('http://127.0.0.1:1234')).toBe(
      'http://127.0.0.1:1234/v1/chat/completions',
    );
  });
...
describe('buildInstruction', () => {
  it('joins the preset instruction with trimmed custom instructions', () => {
    expect(buildInstruction('Fix grammar.', '  Keep my voice.  ')).toBe(
...
describe('resolveRevisionRequest', () => {
  ...
  it('parses the temperature string and leaves clamping to reviseText', () => {
    ...
    expect(
      resolveRevisionRequest({ ...preferences, temperature: 'not a number' }, input).temperature,
    ).toBeNaN();
  });
});
...
describe('reviseText', () => {
  ...
  it('sends the expected OpenAI-compatible chat request', async () => {
    ...
    await expect(
      reviseText({ ...baseRequest, apiKey: ' local-token ', temperature: 5 }),
  ...
  it('uses a status fallback when an error payload is not JSON', async () => {
    ...
    await expect(reviseText(baseRequest)).rejects.toThrow(
      'OpenAI-compatible request failed with status 500',
    );
  });
