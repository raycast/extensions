    ...
    {domains.map(domain => {
      ...
      return <List.Item key={domain.domain_id} icon={getFavicon(`https://${domain.domain_name}`, { fallback: Icon.Globe })} title={domain.domain_name} subtitle={isShowingDetail ? undefined : status} accessories={isShowingDetail ? undefined : [
        ...
        <Action icon={Icon.AppWindowSidebarLeft} title="Toggle Details" onAction={() => setIsShowingDetail(prev => !prev)} />
          <Action.Push icon={Icon.List} title="Update Nameservers" target={<UpdateNameservers domain={domain} onUpdate={revalidate} />} />
      </ActionPanel>} />
    })}
  </List>
}
...
const { api_url } = getPreferenceValues<Preferences>();
const { isLoading } = useFetch(
  new URL("domains_api_v1/update_domain_nameservers", api_url).toString() + "?" + new URLSearchParams({ domain_name: domain.domain_name, ...values }).toString(), {
    headers,
    parseResponse,
    execute,
    onData() {
