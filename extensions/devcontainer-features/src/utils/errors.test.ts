...
describe('createAppError', () => {
  ...
  it('includes httpStatus when provided', () => {
    const error = createAppError('SERVER_ERROR', 'Server error', undefined, 503);
...
describe('getUserErrorMessage', () => {
  it('returns user-friendly message for AppError', () => {
    ...
    expect(getUserErrorMessage(error)).toBe('The requested resource was not found.');
  });

  it('returns message for new error types', () => {
    expect(getUserErrorMessage(createAppError('UNAUTHORIZED', 'test'))).toContain('Authentication');
    expect(getUserErrorMessage(createAppError('FORBIDDEN', 'test'))).toContain('Access denied');
    expect(getUserErrorMessage(createAppError('TIMEOUT', 'test'))).toContain('timed out');
    expect(getUserErrorMessage(createAppError('SERVER_ERROR', 'test'))).toContain('Server error');
  });
