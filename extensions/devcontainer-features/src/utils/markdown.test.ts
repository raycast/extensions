...
import { convertRelativeImagePaths, escapeTableCell, formatEnumValues, stripFirstH1, truncateText } from './markdown';
...
describe('convertRelativeImagePaths', () => {
  ...
  it('converts paths without ./ prefix', () => {
    ...
    expect(result).toBe('![Icon](https://raw.githubusercontent.com/devcontainers/features/main/src/python/icon.png)');
  });
  ...
  it('handles multiple images', () => {
    ...
    expect(result).toContain('https://raw.githubusercontent.com/devcontainers/features/main/src/python/a.png');
    expect(result).toContain('https://raw.githubusercontent.com/devcontainers/features/main/src/python/b.png');
  });
});
...
describe('formatEnumValues', () => {
  ...
  it('truncates values exceeding limit', () => {
    expect(formatEnumValues(['a', 'b', 'c', 'd', 'e'], 3)).toBe('a \\| b \\| c, ... (+2 more)');
  });

  it('uses default limit of 3', () => {
    expect(formatEnumValues(['a', 'b', 'c', 'd'])).toBe('a \\| b \\| c, ... (+1 more)');
  });
});
