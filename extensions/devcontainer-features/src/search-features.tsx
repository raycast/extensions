...
import { getUniqueCollections, useFeatureFilters } from './hooks/useFeatureFilters';
import { useFeatures } from './hooks/useFeatures';
import type { Feature } from './types';
import { getFeatureCollectionName, getFeatureGitHubUrl } from './utils/collection';
...
export default function Command() {
  const { features, isLoading, error, progress, cacheTimestamp, failedCollections, refresh } = useFeatures();
  const { favorites, isFavorite, toggleFavorite, isLoading: favoritesLoading } = useFavorites();
  const { filteredFeatures, filters, updateFilter, hasActiveFilters, resetFilters } = useFeatureFilters(
    features,
    favorites,
  );
  ...
  const navigationTitle = cacheTimestamp ? formatCacheTime(cacheTimestamp, filteredFeatures.length) : undefined;
  ...
  const searchBarAccessory = (
    ...
    >
      ...
      <List.Dropdown.Section title="Quick Filters">
        ...
        <List.Dropdown.Item title="Official Features" value="official" icon={Icon.Checkmark} />
        <List.Dropdown.Item title="With Options" value="with-options" icon={Icon.List} />
      </List.Dropdown.Section>
      <List.Dropdown.Section title="By Collection">
        {collections.map((collection) => (
          <List.Dropdown.Item key={collection} title={collection} value={`collection:${collection}`} />
        ))}
      </List.Dropdown.Section>
    </List.Dropdown>
  );
...
function FeatureListItem({ feature, isFavorite, onToggleFavorite, onRefresh }: FeatureListItemProps) {
  ...
  return (
    <List.Item
      ...
      keywords={[feature.id, feature.name, feature.description || '', collectionName]}
      actions={
        <ActionPanel>
          <ActionPanel.Section>
