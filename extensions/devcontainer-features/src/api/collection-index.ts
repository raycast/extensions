...
export async function fetchCollectionIndex(): Promise<CollectionInfo[]> {
  ...
  if (!response.ok) {
    throw createHttpError(response.status, `Failed to fetch collection index: ${response.status}`);
  }
  ...
  try {
    ...
    throw createAppError('INVALID_RESPONSE', 'Failed to parse collection index YAML', err);
  }

  if (!Array.isArray(parsed)) {
    throw createAppError('INVALID_RESPONSE', 'Invalid collection index format: expected array', {
 ...
 */
export function sortCollections(collections: CollectionInfo[]): CollectionInfo[] {
