...
import { clearCache, getCachedFeatures, getCacheTimestamp, setCachedFeatures } from '../api/cache';
...
export function useFeatures(): UseFeaturesResult {
  ...
  const [failedCollections, setFailedCollections] = useState<CollectionFetchResult[]>([]);
  ...
  const loadFeatures = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const { features: allFeatures, failedCollections: failed } = await fetchAllFeatures(
        sortedCollections,
        undefined,
        (completed, total) => {
