...
function generateUsageExample(feature: Feature): string {
  ...
  if (feature.options && Object.keys(feature.options).length > 0) {
    ...
    if (optionDef.default !== undefined) {
      exampleValue = typeof optionDef.default === 'boolean' ? String(optionDef.default) : `"${optionDef.default}"`;
...
function generateScriptsMarkdown(scripts: ScriptFile[], isLoading: boolean): string {
  if (scripts.length > 0) {
    return (
      scripts
        .map((s) => {
...
export function FeatureDetail({ feature }: FeatureDetailProps) {
  ...
  if (content.readme) {
    ...
    readmeBody = convertRelativeImagePaths(readmeBody, feature.collection.sourceInformation, feature.id);
    if (readmeBody) {
      markdown += `## README\n\n${readmeBody}\n\n---\n\n`;
    }
  } else if (isLoading) {
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label
            title="Options"
            text={feature.options ? `${Object.keys(feature.options).length} available` : 'None'}
          />
          {feature.documentationURL && (
            <Detail.Metadata.Link title="Documentation" text="View Docs" target={feature.documentationURL} />
          )}
          <Detail.Metadata.Link title="Source" text="GitHub" target={githubUrl} />
        </Detail.Metadata>
      }
      actions={
        <ActionPanel>
