...
export const getURL = (request: RequestType) => {
  ...
  // Use the reconstruction utility to ensure raw URL exists
  return reconstructUrlFromPostman(request.url)
}
...
export const prepareFinalURL = async (
  ...
  let finalURL = protocol + "://" + host.join(".") + parsePath(payload, path)

  // Substitute environment variables
  try {
    const environment = await getActiveEnvironment()
    if (environment) {
      finalURL = substituteVariables(finalURL, environment)
    }
  } catch (error) {
    console.error("Failed to substitute environment variables:", error)
  }
