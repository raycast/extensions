...
import type { Interval, IntervalType, PomodoroTask } from '../types';
import { clearStatus, endSnooze, setSnooze, setStatus } from './slackApi';

export async function slackCreateInterval(
  ...
  if (intervalType === 'task') {
    if (!taskInput) throw new Error('Task input is required for task intervals');
    return createInterval('task', isFreshStart, taskInput);
  }
...
const getRemainingTime = (interval: Interval): number => {
  ...
  const remainingTime = interval.intervalLength - totalPausedTime;
