...
export default function DNSRecords({domain}: {domain: Domain}) {
  const {isLoading, data: records, mutate} = useCachedPromise(() => alwaysdata.dnsRecords.list({domainId: domain.id}),[],{initialData:[]})

  return <List isLoading={isLoading} isShowingDetail>
    {records.map(record => <List.Item key={record.id} title={record.name} accessories={[{tag: record.type}]} detail={<List.Item.Detail markdown={record.value} />} actions={<ActionPanel>
      <Action.Push icon={Icon.Plus} title="Add DNS Record" target={<AddDNSRecord domain={domain} />} onPop={mutate} />
      <Action icon={Icon.Trash} title="Delete DNS Record" onAction={() => {
        confirmAlert({
          ...
          primaryAction: {
            ...
            async onAction() {
              ...
              try {
                await mutate(
                    alwaysdata.dnsRecords.delete({id: record.id}), {
...
<Action.SubmitForm icon={Icon.Plus} title="Add DNS Record" onSubmit={handleSubmit} />
    ...
    <Form.TextField title="Name" info="The hostname, without adding the domain at the end. Only composed of letters (a-z), numbers (0-9), hyphen (-), underscore (_), dot (.) or leave blank to designate the domain itself. Examples: www, webmail, admin." {...itemProps.name} />
    <Form.Description text={`.${domain.name}`} />
    <Form.Dropdown title="Type" {...itemProps.type}>
      {Object.values(DNSRecordType).map(type => <Form.Dropdown.Item key={type} title={type} value={type} />)}
    </Form.Dropdown>
    <Form.TextField title="Value" {...itemProps.value} />
    <Form.Separator />
    <Form.TextField title="Annotation" info="The annotation is optional and will be displayed in the listing." {...itemProps.annotation} />
  </Form>
}