...
const CommonCardActions = ({ card, removeFromList }: CommonCardActionsProps) => {
  ...
  return (
    <>
      ...
      <Action.CopyToClipboard title="Copy Markdown" icon={Icon.Clipboard} content={card.data.markup} shortcut={{modifiers: ["cmd"], key: "."}}/>
      <Action.CopyToClipboard title="Copy HTML" icon={Icon.Clipboard} content={card.data.html} shortcut={{modifiers: ["cmd", "shift"], key: "."}}/>
      <Action title="Junk Card" icon={Icon.Trash} onAction={() => junk(card.data.id)} shortcut={{modifiers: ["ctrl"], key: "x"}}/>
