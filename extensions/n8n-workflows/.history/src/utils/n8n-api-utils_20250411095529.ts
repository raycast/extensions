...
// Define Tag type (adjust if API response differs)
export interface Tag {
    id?: string; // ID might be useful later
    name: string;
}
...
export async function getAllTagsAPI(): Promise<Tag[]> {
  ...
  try {
    ...
    return response.data || []; // Return empty array if data is missing or request fails gracefully in fetchN8nApi
  } catch (error) {
