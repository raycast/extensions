    ...
    >
      {filteredWorkflows.length === 0 && !loading ? (
         <EmptyView
            ...
            extensionPreferences={false} // Don't show default pref action
            actions={ // Pass custom actions directly
                    [ // Use an array instead of fragment
                        <Action.Push
                            key="set-filters" // Add key for array elements
                            title="Set Trigger Filters..."
                            icon={Icon.Filter}
                        target={<TriggerFilterForm />}
                            shortcut={{ modifiers: ["cmd", "shift"], key: "f" }}
                        />,
                        <Action key="refresh" title="Refresh List" icon={Icon.Repeat} onAction={() => setRefresh(Date.now())} />,
                        <ActionOpenPreferences key="prefs" />
                    ]
            }
         />
      ) : (
                      ...
                      <ActionPanel.Section title="Commands & Filters">
                        {webhookDetails && webhookFullUrl && (
                          <Action.Push
                            ...
                            target={<SaveCommandForm
                                      method={webhookDetails.method}
                                      url={webhookFullUrl}
                                      // Pass empty strings if headers/query/body aren't available here
                                      // Or modify SaveCommandForm to fetch them if needed
                                      headers={undefined}
