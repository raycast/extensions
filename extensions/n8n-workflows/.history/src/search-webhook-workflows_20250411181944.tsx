...
import { getPreferenceValues, List, showToast, Toast, ActionPanel, Action, LocalStorage } from "@raycast/api";
...
import WebhookTriggerForm from "./components/WebhookTriggerForm";
...
export default function SearchWebhookWorkflowsCommand() {
    ...
    async function loadAndFilterWorkflows() {
      ...
      try {
        ...
        if (currentFilters.length > 0) {
          console.log(`Applying ${currentFilters.length} tag filters: ${currentFilters.join(', ')}`);
          const workflowsMatchingFilters = webhookWorkflows.filter(wf => {
            const hasMatchingTag = wf.tags?.some(tag => currentFilters.includes(tag.name)) ?? false;
            console.log(`Workflow ${wf.name} has matching tag: ${hasMatchingTag}`);
            return hasMatchingTag;
          });
          console.log(`Found ${workflowsMatchingFilters.length} workflows matching tag filters`);
  ...
  }, [instanceUrl, apiKey, refresh]);

  console.log("Error message:", error);
  
  // Create an ActionPanel with refresh and filter actions
  const createActionPanel = (workflow?: Workflow) => (
    <ActionPanel>
      {workflow && (
        <Action.Push
          ...
          target={
            <WebhookTriggerForm
              ...
              method={getWebhookDetails(workflow)?.method || "GET"}
              path={getWebhookDetails(workflow)?.path || ""}
      ...
      <Action.Push
        title={`${activeFilters.length > 0 ? "Edit" : "Add"} Tag Filters${activeFilters.length > 0 ? FILTER_APPLIED_INDICATOR : ""}`}
  ...
  return (
    <List
      isLoading={loading}
      navigationTitle={`n8n Webhook Workflows${activeFilters.length > 0 ? FILTER_APPLIED_INDICATOR : ""}`}
      searchBarPlaceholder="Search webhook workflows..."
    >
      {filteredWorkflows.length > 0 ? (
        filteredWorkflows.map((workflow) => (
          <List.Item
            ...
            actions={createActionPanel(workflow)}
          />
        ))
      ) : (
        <EmptyView
