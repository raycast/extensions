...
export default function SearchWorkflowsCommand() {
  ...
  useEffect(() => {
    async function fetchData() { // Renamed function
      ...
      try {
        ...
        console.log("Fetched Workflows Raw:", fetchedWorkflows); // Log raw API response
  ...
  console.log(`Current Filter: ${filter}`); // Log filter
  console.log(`Workflows before filter (${workflows.length}):`, workflows); // Log state before filter
  console.log(`Workflows after filter (${filteredWorkflows.length}):`, filteredWorkflows); // Log state after filter
