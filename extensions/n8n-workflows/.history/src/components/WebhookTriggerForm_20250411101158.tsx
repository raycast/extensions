import { Action, ActionPanel, Form, Icon, showToast, Toast, useNavigation } from "@raycast/api"; // Removed useFormValues
...
export default function WebhookTriggerForm(props: WebhookTriggerFormProps) {
  ...
  async function handleSubmit(values: Form.Values) {
    ...
    // Use state values now for submission logic
    const parsedHeaders = parseHeaders(headers);
    // queryParams and body state variables are already strings

    // Basic validation for JSON body if provided
    if (showBodyField && body) { // Use body state variable
      try {
        JSON.parse(body);
      } catch (e) {
        ...
        // No need to re-assign body state here, just don't pass it to triggerWebhook if not needed
    }

    // Construct final URL with query params
    const finalUrl = queryParams ? `${webhookBaseUrl}?${queryParams}` : webhookBaseUrl; // Use queryParams state

    const toast = await showToast({ style: Toast.Style.Animated, title: "Sending webhook..." });

    try {
      const result = await triggerWebhook(finalUrl, method, parsedHeaders, showBodyField ? body : undefined); // Pass parsed headers and conditional body state
...
// Separate component for the Save action, now receives state values as props
...
function ActionSaveCommand(props: ActionSaveCommandProps) {
  ...
  return (
    <Action
      ...
      icon={Icon.HardDrive} // Changed icon
