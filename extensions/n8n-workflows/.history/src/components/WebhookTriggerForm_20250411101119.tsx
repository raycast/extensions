import { Action, ActionPanel, Form, Icon, showToast, Toast, useNavigation, useFormValues } from "@raycast/api"; // Added useFormValues
...
export default function WebhookTriggerForm(props: WebhookTriggerFormProps) {
  ...
  const { pop, push } = useNavigation(); // Added push
    ...
    <Form
      ...
      actions={
        <ActionPanel>
          ...
          <ActionSaveCommand method={method} webhookBaseUrl={webhookBaseUrl} />
        </ActionPanel>
      }
    >
...
// Separate component for the Save action to easily get form values
function ActionSaveCommand(props: { method: string; webhookBaseUrl: string }) {
  const { method, webhookBaseUrl } = props;
  const { push } = useNavigation();
  // Use useFormValues to get current data without needing submit handler context
  const formValues = useFormValues<Form.Values>();
  ...
  return (
    <Action
      ...
      onAction={() => {
        // Construct the full URL again here
        const queryParams = formValues.queryParams as string || "";
        const finalUrl = queryParams ? `${webhookBaseUrl}?${queryParams}` : webhookBaseUrl;

        push(
          <SaveCommandForm
            method={method}
            url={finalUrl} // Pass the final constructed URL
            headers={formValues.headers as string || ""}
            queryParams={queryParams} // Pass the raw query string
            body={formValues.body as string || ""}
          />
        );
      }}
    />
