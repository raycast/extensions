...
export default function Command() {
 ...
 await showHUD("Table copied to clipboard")
  ...
  useEffect(() => {
    const markDown = csvToMarkdown(input.inputValue, (input.delimiter == "tab") ? "\t" :input.delimiter, input.firstRowIsHeader )
    setResult(markDown)
  })
  ...
  return (
    ...
    >
      ...
      <Form.TextArea id="textarea" title="Text area"   value={input?.inputValue} placeholder="Paste your text here." onChange={pasteAsMarkdownTable} />
      <Form.Separator />
      <Form.Checkbox id="checkbox" title="Row Header" label="First row is not header value" value={input?.firstRowIsHeader} onChange={setFirstRowIsHeader}/>
      <Form.Dropdown id="dropdown" title="Delimiter" value={input.delimiter}  onChange={setDelimiter} >
        ...
        <Form.Dropdown.Item value=" " title="Space" />
            ...
            <Form.TextArea id="pasteArea" title="Text area"  value={result} placeholder="Enter multi-line text" onChange={setResult} />
    </Form>
  );
}
