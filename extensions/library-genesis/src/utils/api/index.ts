...
const parseContentIntoBooks = (content: string, libgenUrl?: string): BookEntry[] => {
...
export const getLibgenSearchResults = async (searchContent: string, results = 100) => {
  ...
  const libgenUrl = preferredLibgenMirror || (await mirror());
  ...
  return parseContentIntoBooks(data, libgenUrl);
};
