...
export default function Webhooks() {
    ...
    async function confirmAndDelete(webhook: GetV2WebhooksData) {
        ...
        try {
            ...
            await mutate(
                attio.webhooks.delete({webhookId}), {
    ...
    return <List isLoading={isLoading}>
        {!isLoading && !webhooks.length && !error ? <List.EmptyView icon="empty/webhook.svg" title="No webhooks" description="Configure webhooks and webhook events" /> : webhooks.map(webhook => <List.Item key={webhook.id.webhookId} title={webhook.targetUrl} accessories={[{icon: {source: Icon.Dot, tintColor: webhook.status==="active" ? Color.Green : undefined}, tag: webhook.status}]} actions={<ActionPanel>
            <Action icon={Icon.Trash} title="Delete" onAction={() => confirmAndDelete(webhook)} style={Action.Style.Destructive} />
        </ActionPanel>} />)}
    </List>
}