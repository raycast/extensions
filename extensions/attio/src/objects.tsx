...
export default function Objects() {
    const {isLoading,data: objects, mutate} = useCachedPromise(async()=> {
    ...
    return <List isLoading={isLoading}>
        {objects.map(object => <List.Item key={object.id.objectId} icon={Icon.Box} title={object.pluralNoun || ""} actions={<ActionPanel>
            ...
            <Action.Push icon={Icon.AppWindowGrid2x2} title="Attributes" target={<Attributes objectId={object.id.objectId} />} />
...
function NewCustomObject() {
    ...
    return <Form actions={<ActionPanel>
        ...
        <Form.TextField title="Identifier / Slug" placeholder="e.g. product" info="Slugs are used to identify your object in the URL and are formatted to only include lowercase letters, numbers, and underscores" {...itemProps.apiSlug} />
