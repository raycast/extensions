...
export default function SwitchCommand() {
  ...
  if (existsSync(brewPath + "/bin/php-config")) {
    current = String(execSync("/opt/homebrew/bin/php-config --version")).slice(0, 3);
  }
    ...
    <List isLoading={false} searchBarPlaceholder="Filter by title...">
      {items().map((version) => (
        <List.Item
          ...
          subtitle={
            version != current
              ? "Switch to PHP " + version + " and get notified once the switch is done."
              : "Attempt to switch to PHP " + version + ", even though this is already linked."
          }
