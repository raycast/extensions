...
export default function Command() {
  ...
  const [sessionID, setSessionID] = useCachedState<string>(
    "add-chapter-marker:sessionID",
    NO_SESSION_VALUE,
  );
  const [subSessionID, setSubSessionID] = useCachedState<string>(
    "add-chapter-marker:subSessionID",
    LATEST_SUBSESSION_VALUE,
  );
  const [tagIDs, setTagIDs] = useCachedState<string[]>(
    "add-chapter-marker:tagIDs",
    [],
  );
  ...
  const { handleSubmit, itemProps } = useForm<AddChapterMarkerValues>({
    onSubmit: submitChapterMarker,
    validation: {
  ...
  async function submitChapterMarker(values: AddChapterMarkerValues) {
    ...
    const targetSessionID = requiredString(
      ...
      "Select a session before adding a chapter marker.",
    );
    ...
    const startDate = dateWithOffset(values.offset, captureDate).toISOString();
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel title="Add Chapter Marker">
          <ActionPanel.Section>
            <Action.SubmitForm
    ...
    >
      ...
      {!pickerSessions.length && !contextState.isLoading ? (
        <Form.Description text="No Marker sessions were loaded. Refresh Marker data or create a session before adding a chapter marker." />
      ) : null}
      <Form.Dropdown
        id="subSessionID"
        title="Sub-session"
...
function resolveSubsessionID(
  ...
  if (selectedSubSessionID !== LATEST_SUBSESSION_VALUE) {
    return requiredString(
      ...
      "Select a sub-session before adding a chapter marker.",
    );
  }
