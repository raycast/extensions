...
export default function Documents({ organization }: { organization: Organization }) {
  ...
  const { isLoading, data: documents, mutate } = useCachedPromise(
    ...
    primaryAction: {
      ...
      async onAction() {
        const toast = await showToast(Toast.Style.Animated, "Deleting", document.name);
            try {
              await mutate(
                papra.documents.delete({organizationId: organization.id, id: document.id}), {
  ...
  return (
    <List isLoading={isLoading} navigationTitle={`Organizations / ${organization.name} / Documents`} onSearchTextChange={setSearchText} throttle>
      {!isLoading && !documents.length ? <List.EmptyView icon={Icon.Cloud} title="No documents" description="There are no documents in this organization yet. Start by uploading some documents." /> : documents.map((document) => (
        <List.Item key={document.id} icon={{source: getDocumentIcon(document.mimeType), tintColor: PAPRA_COLOR}} title={document.name} subtitle={`${filesize(document.originalSize)} - ${document.name.split(".").at(-1)}`} accessories={[{date: new Date(document.createdAt)}]} actions={<ActionPanel>
          <Action.Push icon={Icon.Info} title="Document Details" target={<DocumentDetails organization={organization} document={document} />} />
          <Action.Push icon={Icon.Upload} title="Import Document" target={<ImportDocument organization={organization} />} onPop={mutate} />
...
function ImportDocument({organization}:{organization:Organization}) {
  ...
  return (
    <Form navigationTitle={`Organizations / ${organization.name} / Documents / Upload`}
      actions={
        <ActionPanel>
...
function DocumentDetails({organization, document}:{organization:Organization, document: Document}) {
  const {isLoading, data: content} = useCachedPromise(async(organizationId:string, id:string) => {
    const res = await papra.documents.get({organizationId, id});
    return res.document.content;
  },[organization.id,document.id])
  return (
    <List isLoading={isLoading} isShowingDetail navigationTitle={`... / ${organization.name} / Documents / ${document.name}`}>
      <List.Item icon={Icon.Info} title="Info" detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
      ...
      <List.Item icon={Icon.Text} title="Content" accessories={[{icon: Icon.Info, tooltip: "The content of the document is automatically extracted from the document on upload. It is only used for search and indexing purposes."}]} detail={<List.Item.Detail markdown={content || "_This document has no extracted content, you can set it manually here._"} />} />
    </List>
  );
}
