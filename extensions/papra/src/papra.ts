...
const makeRequest = async <T>(endpoint: string, options?: RequestInit) => {
  ...
  const headers = (options?.body instanceof FormData) ? API_HEADERS : {...API_HEADERS,
...
export const papra = {
  documents: {
    create: (props: {organizationId: string, body: FormData}) =>
      ...
      delete: (props: {organizationId: string, id: string}) => makeRequest(`organizations/${props.organizationId}/documents/${props.id}`, {
          method: "DELETE"
      }),
      get: (props: {organizationId: string, id: string}) => makeRequest<{document: Document & {    content: string,
    ...
    search: (props: { organizationId: string, query: string }) =>
      makeRequest<{ documents: SearchResult[] }>(`organizations/${props.organizationId}/documents/search?searchQuery=${props.query}`),
  },
  organizations: {
      ...
      tags: {
        create: (props: {organizationId: string, tag: Partial<Tag> }) =>
          ...
          delete: (props: {organizationId: string, id: string}) => makeRequest(`organizations/${props.organizationId}/tags/${props.id}`, {
              method: "DELETE"
          }),
      list: (props: { organizationId: string }) =>
