...
import {Preferences, Transcription, TranscriptionCache, TranscriptionResult, TranscriptionStatus} from "../interfaces";
...
async function requestTranscription(filePath: string): Promise<TranscriptionCache> {
    ...
    const transcriptionRequest = {
        ...
        context_prompt: "This is a noisy voice note, either to myself or a discussion with someone else. Please transcribe it into text."
    };
...
async function getTranscription(transcriptionId: string): Promise<TranscriptionResult> {
    try {
        ...
        const textTimestamps = sortedPredictions.map(prediction =>
...
async function generateSummary(summaryType: string, systemPrompt: string, userPrompt: string, text: string): Promise<string> {
    try {
        const response = await mistralClient.chat({
