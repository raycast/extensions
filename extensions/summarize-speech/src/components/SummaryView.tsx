...
async function generateSlackPostSummary(text: string): Promise<string> {
    const systemPrompt = 'You are a helpful assistant generating a summary for a Slack post. It is structured with a bold title, and then explanations. Use thanks, but no other forms of politeness. Use simple sentences and bullet points. Write in an empathetic tone. Use I.';
...
export async function generateCustomSummary(summaryType, customSystemPrompt, customUserPrompt, text: string): Promise<string> {
...
function SummaryView({
                         ...
                         customSystemPrompt,
                         customUserPrompt
                     }: React.FC<SummaryViewProps & {
    onGenerateSummary: any,
    loading: boolean
}>) {
    ...
    const handleGenerateSummary = async (summaryType: SummaryType, customSystemPrompt?: string, customUserPrompt?: string) => {
        setIsLoading(true);
        ...
        switch (summaryType.value) {
            case '1-1':
                ...
                break;
            case 'slack':
                generatedSummary = await generateSlackPostSummary(showTimestamps ? cache?.transcriptionResult.timestamps : cache?.transcriptionResult.noTimestamps);
                break;
            case 'thoughts':
                generatedSummary = await generateStructuringThoughtsSummary(showTimestamps ? cache?.transcriptionResult.timestamps : cache?.transcriptionResult.noTimestamps);
                break;
            case 'custom':
                generatedSummary = await generateCustomSummary(summaryType, customSystemPrompt, customUserPrompt, showTimestamps ? cache?.transcriptionResult.timestamps : cache?.transcriptionResult.noTimestamps);
                break;
            default:
