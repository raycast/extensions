...
export default function Command() {
  ...
  const handleSelectMemo = (memo: VoiceMemo, cache: TranscriptionCache | null) => {
    push(<SelectedMemo selectedMemo={memo} cache={cache} />);
  };
  ...
  const handleTranscribeAll = async () => {
    ...
    const transcribePromises = memosToTranscribe.map((memo, index) => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          handleTranscribe(memo).then(() => resolve());
        }, delay);
      });
    });
      ...
      <List.Item
        ...
        actions={
          <ActionPanel>
            ...
            {transcript && <Action title="Copy Transcript" onAction={() => Clipboard.copy(transcript.noTimestamps)} />}
            <Action
            ...
            />
            <Action title={showArchived ? "Unshow Archived" : "Show Archived"} onAction={handleToggleShowArchived} />
            <Action
              title="Clear Cache"
              onAction={async () => {
    ...
    <List isShowingDetail isLoading={isLoading}>
      <List.Section title="Tools">
        ...
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action title={showArchived ? "Hide Archived" : "Show Archived"} onAction={handleToggleShowArchived} />
            </ActionPanel>
          }
        />
      </List.Section>
