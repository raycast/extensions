...
async function transfer(source: string, destination: string, expected: string) {
  if ((await fileHash(source)) !== expected) throw new Error("The file changed. Refresh and review it again.");
  // COPYFILE_EXCL never overwrites a destination, including a symlink or a racing writer.
  const info = await fs.stat(source);
  await fs.copyFile(source, destination, constants.COPYFILE_EXCL);
  await fs.utimes(destination, info.atime, info.mtime);
  if ((await fileHash(destination)) !== expected || (await fileHash(source)) !== expected)
    throw new Error("The file changed during the move. Both copies were kept for review.");
...
async function moveDocumentUnlocked(store: Store, source: string, folder: string): Promise<Move> {
  ...
  try {
    await transfer(canonicalSource, destination, record.sha256);
  } catch (error) {
    await store.update((d) => {
...
async function undoMoveUnlocked(store: Store, moveId: string) {
  ...
  await transfer(m.destination, m.source, m.sha256);
  await store.update((d) => {
    const found = d.moves.find((x) => x.id === m.id);
    if (found) found.status = "undone";
  });
}
