...
export default function WorkflowDetails({ workflowId, runId }: WorkflowDetailsProps) {
  ...
  return (
    <Detail
      ...
      actions={
        workflow ? (
          <WorkflowActions workflow={workflow} onRefresh={revalidate} showViewDetails={false} />
        ) : undefined
      }
    />
  );
...
function WorkflowMetadata({ workflow }: { workflow: WorkflowInfo }) {
  ...
  return (
    <Detail.Metadata>
      ...
      {workflow.closeTime && (
        <Detail.Metadata.Label title="Closed" text={formatDateTime(workflow.closeTime)} />
      )}
      <Detail.Metadata.Label title="Duration" text={formatDuration(durationMs)} />

      {workflow.historyLength && (
        <Detail.Metadata.Label title="History Events" text={String(workflow.historyLength)} />
      )}
