        ...
        answerProp = await fetch(url, {
            ...
            .then(async (response) => {
                ...
                if (response.status === 400) {
                    return await verifyModelIsInstalled(model)
                        .then(async (modelIsInstalled) => {
...
export default function ResultView(model: string, initialPrompt: string, endPrompt: string, selectText: boolean): JSX.Element {
    const [loading, setLoading]: [boolean, React.Dispatch<React.SetStateAction<boolean>>] = React.useState(false);
    const [answer, setAnswer]: [string, React.Dispatch<React.SetStateAction<string>>] = React.useState("");
    const [answerMetadata, setAnswerMetadata]: [AnswerPropMetadata, React.Dispatch<React.SetStateAction<AnswerPropMetadata>>] = React.useState({} as AnswerPropMetadata);
    let text = "";
            ...
            metadata={
                !loading && (
                    <Detail.Metadata>
                        ...
                        <Detail.Metadata.Label title="Total Inference Duration" text={`${(answerMetadata.total_duration/1e+9).toFixed(2)}s`} />
                        <Detail.Metadata.Label title="Prompt Eval Count" text={`${answerMetadata.prompt_eval_count}`} />
                        <Detail.Metadata.Label title="Prompt Eval Duration" text={`${(answerMetadata.prompt_eval_duration/1e+9).toFixed(2)}s`} />
                        <Detail.Metadata.Label title="Eval Count" text={`${answerMetadata.eval_count}`} />
                        <Detail.Metadata.Label title="Eval Duration" text={`${(answerMetadata.eval_duration/1e+9).toFixed(2)}s`} />
                    </Detail.Metadata>
                )
            }
        />
