...
Translation and rewrite results can be read aloud with Qwen-TTS through Alibaba Cloud Model Studio / DashScope. The default model is `qwen3-tts-flash`; choose `qwen3-tts-instruct-flash` when you want style instructions or slow, teacher-like reading. Gemini TTS remains available as an optional read-aloud engine.
...
- **支持多个模型服务商**：可以按自己的 API key 启用 DeepSeek、小米 MiMo、Gemini、Kimi、OpenAI / ChatGPT。
