...
Translation and rewrite results can be read aloud with Qwen-TTS through Alibaba Cloud Model Studio / DashScope. The default model is `qwen3-tts-flash`; choose `qwen3-tts-instruct-flash` when you want style instructions or slow, teacher-like reading. The multilingual voices (`Cherry`, `Serena`, `Ethan`, `Chelsie`) cover all 11 supported `language_type` values (Auto, Chinese, English, French, German, Italian, Japanese, Korean, Portuguese, Russian, Spanish). Gemini TTS remains available as an optional read-aloud engine on `gemini-3.1-flash-tts-preview` with eight prebuilt voices.
...
- **支持多个模型服务商**：可以按自己的 API key 启用 DeepSeek（`deepseek-v4-flash`/`deepseek-v4-pro`，Anthropic-compatible 路径并默认关闭 thinking）、小米 MiMo（`mimo-v2.5`/`mimo-v2.5-pro`，同上）、Gemini（默认 `gemini-3.5-flash`/`gemini-3.1-pro-preview`）、Kimi（默认 `kimi-for-coding` 走 Kimi Code Plan；可改 base URL 走 Moonshot 直连）、OpenAI / ChatGPT（默认 `gpt-4.1-mini`/`gpt-4.1`，GPT-5.x 推理模型自动加 `reasoning_effort=minimal`）。
