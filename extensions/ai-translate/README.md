...
- **Translation Settings**: adjust provider scope, model tier, prompt profile, translation style, voice provider/model, and custom instructions without digging through every provider setting.
...
| `Translation Settings`        | View    | Configure provider scope, model tier, prompt profile, style, voice, and custom instructions. | 配置 provider 范围、模型档位、提示词场景、翻译风格、朗读声音和自定义说明。 |
...
Provider credentials and durable defaults live in Raycast preferences: API key, base URL, custom model name, timeout, and provider order. Day-to-day choices live in the extension UI: switch between **All Enabled Providers** and a **Single Provider**, choose **Fast**, **Pro**, or **Custom** model tiers, or pick a specific provider model from the action panel. `Custom` uses the model IDs you enter in preferences, which is useful when providers add new models before the extension is updated.
