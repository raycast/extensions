...
- Refreshed the model catalog against official provider docs (DeepSeek, Xiaomi MiMo, Gemini, Kimi, OpenAI, Qwen-TTS) and tightened every request path against the latest spec:
  ...
  - Kept the Kimi base URL default at `https://api.kimi.com/coding` so the Anthropic URL builder produces `.../coding/v1/messages` without a doubled `/v1`; documented the Moonshot pay-as-you-go alternative (`moonshot-v1-8k` etc.) plus the `kimi-for-coding → K2.6` alias, and hardened `anthropicMessagesUrl` to tolerate custom base URLs that already include a trailing `/v1`.
