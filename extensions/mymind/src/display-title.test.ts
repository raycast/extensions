...
test("getObjectDisplayTitle falls back by object type", () => {
  assert.equal(getObjectDisplayTitle(makeObject({ blob: { type: "image/jpeg" } as MyMindObject["blob"] })), "Untitled Image");
  assert.equal(getObjectDisplayTitle(makeObject({ blob: { type: "video/mp4" } as MyMindObject["blob"] })), "Untitled Video");
  assert.equal(
    getObjectDisplayTitle(makeObject({ blob: { type: "application/pdf" } as MyMindObject["blob"] })),
    "Untitled PDF",
  );
  assert.equal(getObjectDisplayTitle(makeObject({ content: { type: "text/markdown", body: "hello" } })), "Untitled Note");
