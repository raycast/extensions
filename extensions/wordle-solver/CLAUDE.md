...
- **Turn 3+**: filter `ANSWERS` through prior guesses via `filterAnswers`, build sigmoid priors over survivors, `bestGuess(candidates, candidates, weights)`. Normally <1s.

The solver is **greedy over the candidate set** — `bestGuess` is always called with the filtered candidates as both the candidate list AND the guess pool, so the suggestion can only ever be a word that's still a possible answer. This is a deliberate departure from the textbook entropy-maximizing solver (which considers the full ~13k guess pool, allowing non-candidate "probe" words like SHALT). The trade-off: slightly more turns on average, but no guess can be wasted on a word that has zero chance of winning. `TURN2_LOOKUP` was regenerated under this rule, so its entries are also candidate-only. The `guessPool` parameter on `bestGuess` is kept for signature stability but in practice always equals `candidates`.
