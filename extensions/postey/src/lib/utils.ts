...
export function groupSocialSetsByTeam(items: SocialSetListItem[]): GroupedSocialSets {
  ...
  for (const socialSet of items) {
    if (!socialSet.teams || socialSet.teams.length === 0) {
      noTeam.push(socialSet);
      continue;
    }

    for (const teamId of socialSet.teams) {
      const teamKey = String(teamId);
      if (!teamMap.has(teamKey)) {
        teamMap.set(teamKey, { name: `Team ${teamKey}`, items: [] });
        teamOrder.push(teamKey);
      }
      teamMap.get(teamKey)?.items.push(socialSet);
    }
  }
