...
export function ActionSetVisibleProperties (props: { title: string | undefined, icon: ImageLike | undefined, databaseProperties: DatabaseProperty[], selectedPropertiesIds: string[] | undefined, onSelect: (propertyId: string) => void, onUnselect: (propertyId: string) => void }): JSX.Element | null {
  ...
  return ( <ActionPanel.Submenu 
    ...
    <ActionPanel.Section>
      {selectedPropertiesIds?.map(function (propertyId) {
        const property = databaseProperties.filter(function (dp) { return dp.id === propertyId })[0]
        if(!property)
          return
        ...
        return (<ActionPanel.Item
          ...
          title={property.name+'  ✓'}
          onAction={function () { onUnselect(property.id)}}/>)
      })}
    </ActionPanel.Section>
    <ActionPanel.Section>
      {databaseProperties?.map(function (dp: DatabaseProperty) {
        if(!selectedPropertiesIds.includes(dp.id)){
          return (<ActionPanel.Item
            ...
            title={dp.name}
            onAction={function () { onSelect(dp.id)}}/>)
        }                      
      })}
    </ActionPanel.Section>
  </ActionPanel.Submenu>)
