...
Keyword search works without a model or API key. Semantic search is optional. It needs a compatible local MLX embedding runtime or an OpenAI-compatible or Ollama embedding endpoint, configured in **Download Docs → Embedding Settings**. The local setup scripts are included in the extension source but are not run automatically. New downloads can be indexed in the background; you can pause and resume vector indexing.
