...
export default function Command() {
  ...
  const { data, isLoading } = useFetch<Record<Currency, string>>(
    `https://min-api.cryptocompare.com/data/price?fsym=SOL&tsyms=${CURRENCIES.join()}`,
  );
  const [shownCurrency, setShownCurrency] = useCachedState<Currency>("shown-currency", "USD");

  const title = data && shownCurrency && `SOL ${formatCurrency(shownCurrency, Number(data[shownCurrency]))}`;

  return (
    <MenuBarExtra isLoading={isLoading} icon={hideIcon ? undefined : Icon.Bolt} title={title}>
      {data &&
        Object.entries(data)
