...
import { ActionPanel, Form, SubmitFormAction, Detail, copyTextToClipboard } from "@raycast/api";
...
export default function Command() {
  ...
  const handleGeneratePassword = (values: any) => {
    ...
    if (Number.isFinite(lengthNumber) && lengthNumber > 4 && lengthNumber < 33) {
  ...
  return (
    <Fragment>
      ...
      {password && !error && (
        <Fragment>
          <Detail markdown={`### Generated Password: **${password}** copied to clipboard!`} />
        </Fragment>
      )}
      {error && (
        <Fragment>
          <Detail markdown={`### Error: please enter a valid number between 5 and 32`} />
        </Fragment>
      )}
    </Fragment>
  );
