...
export function defaultPromptDirectory(): string {
  return join(homedir(), "Library", "Application Support", "Prompt Studio", "Prompts");
}
...
export function resolvePromptDirectory(configured?: string): string {
  ...
  if (!isAbsolute(expanded)) {
    throw new Error("Prompt directory must be an absolute path or start with ~/.");
  }
...
export async function listPromptsReadOnly(directory: string): Promise<PromptLibrary> {
  return listPromptFiles(directory);
}
...
async function listPromptFiles(directory: string): Promise<PromptLibrary> {
  ...
  records.sort((left, right) => right.updatedAt.localeCompare(left.updatedAt) || left.title.localeCompare(right.title));
  invalid.sort((left, right) => left.filePath.localeCompare(right.filePath));
  return { records, invalid };
}

export async function createPrompt(directory: string, draft: PromptDraft): Promise<PromptRecord> {
  ...
  const metadata: PromptMetadata = validateMetadata({
    ...
    ...(draft.missingInformation ? { missingInformation: draft.missingInformation } : {}),
    ...(draft.validationSteps ? { validationSteps: draft.validationSteps } : {}),
  ...
  const filePath = join(directory, `${slug(metadata.title)}--${metadata.id}.md`);
...
export async function updatePrompt(directory: string, id: string, update: PromptUpdate): Promise<PromptRecord> {
  ...
  const archivedAt =
    update.archived === undefined ? current.archivedAt : update.archived ? new Date().toISOString() : undefined;
  const metadata = validateMetadata({
    ...
    ...(update.missingInformation ? { missingInformation: update.missingInformation } : {}),
    ...(update.validationSteps ? { validationSteps: update.validationSteps } : {}),
...
export async function duplicatePrompt(directory: string, id: string): Promise<PromptRecord> {
  const current = await findPrompt(directory, id);
  return createPrompt(directory, {
    ...
    ...(current.missingInformation ? { missingInformation: current.missingInformation } : {}),
    ...(current.validationSteps ? { validationSteps: current.validationSteps } : {}),
...
export async function deletePrompt(directory: string, id: string): Promise<void> {
  const current = await findPrompt(directory, id);
...
export async function listPromptVersions(directory: string, id: string): Promise<PromptRecord[]> {
  ...
  try {
    ...
    return versions.sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
  } catch (error) {
...
export async function rebuildPromptSearchIndex(
  ...
  return rebuildSearchIndex(library.records, path, await collectVersions(directory, library.records));
}
...
export async function ensurePromptSearchIndex(
  ...
  return health.needsRebuild
    ? rebuildSearchIndex(library.records, path, await collectVersions(directory, library.records))
    : health;
}

export async function restorePromptVersion(directory: string, id: string, versionPath: string): Promise<PromptRecord> {
  const expected = resolve(versionDirectory(directory, id), basename(versionPath));
...
export function parsePrompt(source: string, filePath = "<memory>"): PromptRecord {
  ...
  const body = validateBody(rawBody.endsWith("\n") ? rawBody.slice(0, -1) : rawBody);
  return { ...metadata, body, filePath };
}

export function serializePrompt(metadata: PromptMetadata, body: string): string {
...
function validateMetadata(value: unknown): PromptMetadata {
  ...
  if (value.schemaVersion !== 1) throw new Error("Unsupported prompt schema version.");

  const id = requiredString(value.id, "id");
  if (!UUID.test(id)) throw new Error("Prompt id must be a UUID.");

  const target = requiredString(value.target, "target");
  if (!PROMPT_TARGETS.includes(target as PromptTarget)) throw new Error(`Unsupported target: ${target}.`);
  ...
  const metadata: PromptMetadata = {
    ...
    aliases: value.aliases === undefined ? [] : termArray(value.aliases, "aliases"),
  ...
  if (value.archivedAt !== undefined) metadata.archivedAt = timestamp(value.archivedAt, "archivedAt");
  if (value.project !== undefined) metadata.project = projectBinding(value.project);
  if (value.projectFiles !== undefined) metadata.projectFiles = textArray(value.projectFiles, "projectFiles");
  if (value.assumptions !== undefined) metadata.assumptions = textArray(value.assumptions, "assumptions");
  if (value.missingInformation !== undefined) {
    metadata.missingInformation = textArray(value.missingInformation, "missingInformation");
  }
  if (value.validationSteps !== undefined) {
    metadata.validationSteps = textArray(value.validationSteps, "validationSteps");
  }
  if (value.taxonomy !== undefined) metadata.taxonomy = promptTaxonomy(value.taxonomy);
  if (value.sources !== undefined) metadata.sources = promptSources(value.sources);
  if (value.enhancement !== undefined) metadata.enhancement = enhancementProvenance(value.enhancement);
  return metadata;
}
...
function projectBinding(value: unknown): ProjectBinding {
  ...
  if (value.branch !== undefined) project.branch = requiredString(value.branch, "project.branch");
  if (value.commit !== undefined) project.commit = requiredString(value.commit, "project.commit");
  return project;
}
...
function promptSources(value: unknown): PromptSource[] {
  ...
  return value.map((source, index) => {
    if (!isObject(source)) throw new Error(`sources[${index}] must be an object.`);
    const result: PromptSource = {
      title: requiredString(source.title, `sources[${index}].title`),
      retrievedAt: timestamp(source.retrievedAt, `sources[${index}].retrievedAt`),
    };
    if (source.url !== undefined) result.url = requiredString(source.url, `sources[${index}].url`);
    if (source.supports !== undefined) {
      result.supports = textArray(source.supports, `sources[${index}].supports`);
    }
    return result;
  });
}
...
function enhancementProvenance(value: unknown): EnhancementProvenance {
  ...
  if (typeof outputSchemaVersion !== "number" || !Number.isInteger(outputSchemaVersion) || outputSchemaVersion < 1) {
    throw new Error("enhancement.outputSchemaVersion must be a positive integer.");
  }
  return {
    ...
    reasoningEffort: requiredString(value.reasoningEffort, "enhancement.reasoningEffort"),
    compilerVersion: requiredString(value.compilerVersion, "enhancement.compilerVersion"),
    outputSchemaVersion,
...
async function findPrompt(directory: string, id: string): Promise<PromptRecord> {
  // ponytail: O(n) lookup is deliberate until SQLite Activation 1 owns fast ID lookup.
  const prompt = (await listPrompts(directory)).records.find((record) => record.id === id);
  if (!prompt) throw new Error(`Prompt not found: ${id}.`);
  return prompt;
}

async function preserveVersion(directory: string, record: PromptRecord): Promise<void> {
  const filePath = join(
    versionDirectory(directory, record.id),
    `${record.updatedAt.replace(/[:.]/g, "-")}--${randomUUID()}.md`,
  );
  await atomicWrite(filePath, serializePrompt(metadataFrom(record), record.body));
}

async function refreshActiveSearchIndex(directory: string, record: PromptRecord): Promise<void> {
...
async function removeFromActiveSearchIndex(directory: string, id: string): Promise<void> {
  if (!(await sqliteSearchIsActive())) return;
  const path = defaultSearchIndexPath();
  try {
    const library = await listPrompts(directory);
...
async function collectVersions(
  ...
  return new Map(
    await Promise.all(
      records.map(async (record) => [record.id, await listPromptVersions(directory, record.id)] as const),
    ),
  );
}
...
async function sqliteSearchIsActive(): Promise<boolean> {
  ...
  try {
    return getFeatureStatus(await loadFeatureStatuses(), "sqlite-search").effectiveState === "active";
  } catch {
    return false;
  }
}
