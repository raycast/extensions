...
function EmailDelivery() {
  ...
  const { isLoading, data: domains, mutate } = useCachedPromise(
    async () => {
  ...
  return (
    <List isLoading={isLoading}>
      {!isLoading && !domains.length ? <List.EmptyView title="No email domains found." actions={<ActionPanel>
<Action.Push icon={Icon.Plus} title="Create Email Domain" target={<CreateEmailDomain provider={provider} />} onPop={mutate} />
      </ActionPanel>} /> : domains.map((domain) => (
        <List.Item
          key={domain.id}
          icon={{
          ...
          title={domain.name}
          accessories={[
            domain.activeDkimId ?  {tag:{value:"DKIM Signing", color: Color.Green}, tooltip: "Active"} : {tag: {value: "DKIM Signing", color: Color.Orange}, tooltip: "Inactive"},
          ]}
          actions={
            <ActionPanel>
              <Action.Push icon={Icon.Plus} title="Create Email Domain" target={<CreateEmailDomain provider={provider} />} onPop={mutate} />
