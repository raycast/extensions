...
export default function Command() {
  ...
  async function remove(word: string) {
    ...
    if (!okay) return;
    try {
      updateSettings({ custom_words: words.filter((item) => item !== word) });
      setWords((items) => items.filter((item) => item !== word));
    } catch (error) {
  ...
  async function importClipboard() {
    ...
    const seen = new Set(words.map((word) => word.toLocaleLowerCase()));
