 ...
 */
export function XcodeAddSwiftPackageSelectXcodeProject(
  props: {
      ...
      onAction={
        () => {
          ...
          if (xcodeProject.type === XcodeProjectType.swiftPlayground) {
            ...
            showToast(
              ToastStyle.Failure,
              "Swift Package can not be added to a Swift Playground"
            );
          } else {
  ...
  return (
    <List
      ...
      <List.Section
        title={(openedXcodeProjects ?? []).length > 0 ? "All Projects" : undefined}>
        {
          xcodeProjects
            ?.filter(xcodeProject => {
