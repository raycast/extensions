 ...
 */
export function xcodeReleaseListItem(
...
function title(
  ...
  if (xcodeRelease.beta) {
    ...
    const releaseCandidateComponents = [
      "Release Candidate"
    ];
    ...
    if (xcodeRelease.releaseCandidate > 1) {
      ...
      releaseCandidateComponents.push(
        xcodeRelease.releaseCandidate.toString()
      );
    }
...
function subtitle(
  ...
  if (xcodeRelease.swiftVersion) {
    ...
    subtitleComponents.push(
      `(Swift ${xcodeRelease.swiftVersion})`
    );
  }
...
function accessoryTitle(
  ...
  return xcodeRelease
    .sdks
    .map(sdk => [sdk.name, sdk.version].join(" "))
    .join(", ");
}
