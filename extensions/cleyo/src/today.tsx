...
import { List, ActionPanel, Action, Icon, showToast, Toast, Color, Detail } from "@raycast/api";
...
import { ENERGY_OPTIONS, PRIORITY_OPTIONS, CATEGORY_OPTIONS } from "./lib/constants";
            ...
            item.task.priority
              ? {
                  tag: {
                    ...
                    color: PRIORITY_OPTIONS.find((o) => o.value === item.task.priority?.toString())?.color || Color.Red,
                  },
                  tooltip: "Priority",
                }
              : null,
                ...
                <TaskDetail
                  ...
                  actions={
                    <ActionPanel>
                      ...
                      <ActionPanel.Submenu title="Change Priority" icon={Icon.Tag}>
                      ...
                      <ActionPanel.Submenu title="Change Energy" icon={Icon.Bolt}>
                      ...
                      <ActionPanel.Submenu title="Change Category" icon={Icon.Folder}>
                        {CATEGORY_OPTIONS.map((c) => (
                          <Action
                            key={c.value}
                            title={c.title}
                            onAction={() => handleUpdateTask(item.task, { category: c.value })}
                          />
                        ))}
                      </ActionPanel.Submenu>

                      <Action.CopyToClipboard title="Copy Task Title" content={item.task.title} />
                    </ActionPanel>
                  }
                />
              }
            />
            <Action title="Complete Task" icon={Icon.Checkmark} onAction={() => handleCompleteTask(item.task)} />
            <Action.CopyToClipboard title="Copy Task Title" content={item.task.title} />
  ...
  return (
    <List
      ...
      searchBarAccessory={
        ...
        >
          {ENERGY_OPTIONS.map((e) => (
            <List.Dropdown.Item key={e.value} title={e.title} value={e.value} icon={e.icon} />
          ))}
        </List.Dropdown>
      }
    >
        ...
        <>
          ...
          >
            <List.Item
              ...
              icon={Icon.Calendar}
              accessories={[{ date: new Date(plan.date), tooltip: "Plan date" }]}
              actions={
                <ActionPanel>
                  <Action.Push
                    ...
                    target={
                      <Detail
                        ...
                        metadata={
                          <Detail.Metadata>
                            <Detail.Metadata.Label title="Date" text={new Date(plan.date).toLocaleDateString()} />
                          </Detail.Metadata>
                        }
                      />
                    }
          ...
          {plan.must_do.length > 0 && (
            <List.Section title="Must Do" subtitle={`${plan.must_do.length} tasks`}>
              {plan.must_do.map((item) => renderPlannedTask(item, "must_do"))}
            </List.Section>
          )}

          {plan.procrastinated_focus && (
            <List.Section title="Procrastination Focus" subtitle="Face it today!">
              {renderPlannedTask(plan.procrastinated_focus, "procrastinated")}
            </List.Section>
          )}

          {plan.quick_wins.length > 0 && (
            <List.Section title="Quick Wins" subtitle={`${plan.quick_wins.length} tasks`}>
              {plan.quick_wins.map((item) => renderPlannedTask(item, "quick_wins"))}
            </List.Section>
          )}
        </>
      )}
