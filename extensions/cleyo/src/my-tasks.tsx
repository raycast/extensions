...
import { List, ActionPanel, Action, Icon, showToast, Toast, Color, Image, Form, useNavigation } from "@raycast/api";
import { searchTasks, completeTask, snoozeTask, deleteTask, updateTask } from "./lib/api";
import { Task, TaskStatus, TaskCategory } from "./lib/types";
import { useAuth } from "./hooks/useAuth";
import { AuthenticatedView } from "./components/AuthenticatedView";
import TaskDetail from "./components/TaskDetail";
import { STATUS_COLORS, STATUS_LABELS, PRIORITY_OPTIONS, CATEGORY_OPTIONS, ENERGY_OPTIONS } from "./lib/constants";

type SortOption = "created_desc" | "created_asc" | "due_date_asc" | "due_date_desc" | "priority_desc" | "priority_asc";
...
function createFilterState(status: string, category: string, sort: SortOption): FilterState {
...
function FilterDropdowns({
  ...
  return (
    <List.Dropdown tooltip="Filter & Sort" value={currentState} onChange={handleChange}>
      <List.Dropdown.Section title="Status">
        {["active", "all", "done", "delayed", "snoozed", "waiting"].map((status) => (
          <List.Dropdown.Item
            key={`status-${status}`}
...
function EditDescriptionForm({
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Save Description" icon={Icon.Check} onSubmit={handleSubmit} />
  ...
  async function loadTasks() {
    ...
    try {
      const results = await searchTasks({
        ...
        category: categoryFilter === "all" ? undefined : (categoryFilter as TaskCategory),
        limit: 50,
      });
      setTasks(results);
    } catch (error) {
          ...
          accessories={
            [
              task.energy_required
                ? {
                    icon: {
                      source: ENERGY_OPTIONS.find((e) => e.value === task.energy_required)?.icon || Icon.Bolt,
                      tintColor: ENERGY_OPTIONS.find((e) => e.value === task.energy_required)?.color,
                    },
              ...
              task.priority
                ? {
                    tag: {
                      ...
                      color: PRIORITY_OPTIONS.find((o) => o.value === task.priority?.toString())?.color || Color.Red,
                    },
                    tooltip: "Priority",
                  }
                : null,
          ...
          actions={
            <ActionPanel>
                    ...
                    actions={
                      <ActionPanel>
                        <ActionPanel.Section title="Task Actions">
                          ...
                          {task.status === "active" && (
                            <Action title="Snooze Task" icon={Icon.Clock} onAction={() => handleSnoozeTask(task)} />
                          )}
                          <Action.PickDate
                            title="Set Due Date"
                            onChange={(date) =>
                        ...
                        <ActionPanel.Section title="Edit Properties">
                          <Action.Push
                            ...
                            icon={Icon.Pencil}
                            target={<EditDescriptionForm task={task} onSave={handleUpdateTask} />}
                            shortcut={{ modifiers: ["cmd"], key: "e" }}
                          />
                          <ActionWrapper title="Change Priority" icon={Icon.Tag}>
                          ...
                          <ActionWrapper title="Change Category" icon={Icon.Folder}>
                            {CATEGORY_OPTIONS.map((c) => (
                              <Action
                                key={c.value}
                                title={c.title}
                                onAction={() => handleUpdateTask(task, { category: c.value })}
                              />
                            ))}
                          </ActionWrapper>
                        </ActionPanel.Section>
              ...
              <ActionPanel.Section title="Info">
                <Action.CopyToClipboard title="Copy Task Title" icon={Icon.Clipboard} content={task.title} />
                {task.description && (
