...
export async function initAuth(): Promise<LinkInitResponse> {
  ...
  const initResponse = await fetch(`${apiUrl}/api/v1/integrations/raycast/init`, {
    method: "POST",
    headers: {
...
export async function pollForCompletion(token: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const response = await fetch(`${apiUrl}/api/v1/integrations/raycast/poll?token=${encodeURIComponent(token)}`);
