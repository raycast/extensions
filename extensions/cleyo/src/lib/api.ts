...
import { Task, Project, CreateTaskRequest, SearchTasksParams, DailyPlan, PlanningParams } from "./types";
...
async function apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      (errorData.detail as string) || (errorData.message as string) || `API request failed: ${response.statusText}`,
    );
  }
 ...
 */
export async function getTodayPlan(params: PlanningParams = {}): Promise<DailyPlan> {
  const queryParams = new URLSearchParams();
  ...
  const queryString = queryParams.toString();
  const endpoint = queryString ? `/planning/today?${queryString}` : "/planning/today";
 ...
 */
export async function generatePlan(params: PlanningParams = {}): Promise<DailyPlan> {
...
export type FeedbackType = "bug" | "feature" | "general" | "praise" | "complaint";
