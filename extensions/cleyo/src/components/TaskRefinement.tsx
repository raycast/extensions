...
import { PRIORITY_OPTIONS, CATEGORY_OPTIONS, ENERGY_OPTIONS } from "../lib/constants";
...
export default function TaskRefinement({ task }: TaskRefinementProps) {
  ...
  async function handleDone() {
    ...
    try {
      ...
      const existingDueDate = task.due_date ? new Date(task.due_date).toISOString().split("T")[0] : null;
  ...
  return (
    ...
    >
      ...
      <Form.Dropdown id="energy" title="Energy Required" value={energy} onChange={setEnergy}>
        <Form.Dropdown.Item value="" title="None" />
