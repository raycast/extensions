      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.TagList title="Status">
            <Detail.Metadata.TagList.Item text={STATUS_LABELS[task.status]} color={STATUS_COLORS[task.status]} />
          ...
          <Detail.Metadata.Label
            ...
            text={
              task.energy_required
                ? task.energy_required.charAt(0).toUpperCase() + task.energy_required.slice(1)
                : "None"
            }
            icon={{
          ...
          <Detail.Metadata.Label
            title="Deadline"
            text={task.due_date ? new Date(task.due_date).toLocaleDateString() : "None"}
            icon={Icon.Calendar}
          />

          <Detail.Metadata.Label
            title="Scheduled On"
            text={task.scheduled_date ? new Date(task.scheduled_date).toLocaleDateString() : "None"}
            icon={Icon.Clock}
          />

          <Detail.Metadata.Label
            title="Category"
            text={task.category ? task.category.charAt(0).toUpperCase() + task.category.slice(1) : "Uncategorized"}
            icon={Icon.Folder}
          />
