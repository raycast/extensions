...
function Lines({ children }: { children: IChild[] }) {
    return (
        <>
            {children.map(child => <Line child={child} key={child.id} />)}
        </>
    )
}

export default function Command() {
    const { data: favoriteStopPoints } = useSWR("favorite-stop-points", () =>
        getFavoriteStopPoints()
    );

    return (
        <MenuBarExtra
            icon={Icon.Train}
            tooltip="Disruptions in your favorite stop points">
            {favoriteStopPoints &&
                favoriteStopPoints.map(stopPoint =>
                    <Lines
                        key={stopPoint.id}
                        children={stopPoint.children} />
                )
            }
