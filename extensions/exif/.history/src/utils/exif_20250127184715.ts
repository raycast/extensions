...
const decodeUnicodeComment = (value: unknown): string => {
  ...
  if (value.length > header.length && header.every((byte, i) => value[i] === byte)) {
    // Skip the header and create a buffer from the remaining bytes
  ...
  const table = Object.entries(tags)
    ...
    .map(([key, value]) => {
      ...
      if (["ApplicationNotes", "MakerNote"].includes(key)) {
        ...
        const decodedText = decodeUnicodeComment(value);
        return `| ${key} | ${decodedText} | \`${JSON.stringify(value)}\` |`;
      } else if (value instanceof Array) {
        return `| ${key} | ${value.map((v) => v.description).join(", ")} | \`${JSON.stringify(
          value.map((v) => v.value),
        )}\` |`;
      } else if (value instanceof Date) {
        return `| ${key} | ${value.toISOString()} | \`${JSON.stringify(value)}\` |`;
      } else {
