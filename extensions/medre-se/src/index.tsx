...
export default function Command() {
  ...
  const { data, isLoading } = useFetch(
    `https://api.medre.se/search/${searchText}`, { parseResponse: parseFetchResponse });
    ...
    >
      <List.Section title="Search Results" subtitle={data?.entries.length + ""}>
        {data && data.entries.map((entry) => (
          <SearchListItem key={crypto.randomUUID()} entry={entry} />
        ))}
      </List.Section>
    </List>
  );
  ...
  switch (entry.type as ResponseType) {
    case ResponseType.Definition:
      ...
      return <List.Item
        ...
        detail={
          <List.Item.Detail markdown={`## Kelime  
${definition.title}  
## Anlamı  
${definition.meaning}  
...
${definition.reference}` : ""}`} />} />
    case ResponseType.Verse:
      ...
      return <List.Item
        ...
        actions={
          <ActionPanel title="Quick Actions">
            <Action.OpenInBrowser url={`${baseURL}/library/q/${verse.name.toLocaleLowerCase()}/${verse.order}/ayet`} />
            <Action.CopyToClipboard title="Copy to Clipboard Translation" content={`${verse.translation}`} />
          </ActionPanel>
        }
        detail={
          <List.Item.Detail markdown={`## ${verse.name} ${verse.order}. Âyet
### Arapça
${verse.original}
### Türkçe
${verse.translation}`} />} />
    case ResponseType.Book:
      ...
      return <List.Item
        ...
        detail={
          <List.Item.Detail markdown={`# ${book.order}. Cüz
## Başlangıç - Bitiş  
...
Bu cüz \`${book.numberOfVerses}\` âyet içerir.`} />} />
    case ResponseType.Page:
      ...
      return <List.Item
        ...
        detail={
          <List.Item.Detail markdown={`# ${page.no}. Sayfa
## İçerik
This page includes ${page.content.map(content => `
...
`).join(' and ')}`} />} />
    case ResponseType.Chapter:
      ...
      return <List.Item
        ...
        detail={
          <List.Item.Detail markdown={`## Sûre İsmi ![Mekki](../assets/${chapter.revelation === "Mekkî" ? 'mek' : 'medeni'}.png)  
${chapter.name}
## Sûre Bilgi
${chapter.info}`} />} />
    case ResponseType.VerseRange:
      ...
      return <List.Item
        ...
        actions={
          <ActionPanel title="Quick Actions">
              <Action.OpenInBrowser url={`${baseURL}/library/q/${verseRange.name.toLocaleLowerCase()}/${verseRange.from}-${verseRange.to}/ayetler`} />
          </ActionPanel>
        }
        detail={
          <List.Item.Detail markdown={`## ${verseRange.name} ${verseRange.from} - ${verseRange.to}. Âyetler
