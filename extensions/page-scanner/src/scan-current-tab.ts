...
import { scanArgs, type ScanPreferences } from './lib/scan-args';
...
export default async function scanCurrentTab(): Promise<void> {
  const prefs = getPreferenceValues<ScanPreferences>();
  ...
  try {
    let tabs = await runCli<TabsAnswer>([
      'tabs',
      '--wait',
      String(TABS_WAIT_SECONDS),
      ...browserArgs,
    ]);
    if (tabs.exitCode === EXIT.noBrowser || tabs.exitCode === EXIT.notPaired) {
      const repaired = await repairIfBroken(toast);
      if (!repaired) return;
      tabs = await runCli<TabsAnswer>(
        ['tabs', '--wait', String(REPAIR_WAIT_SECONDS), ...browserArgs],
        { timeoutMs: (REPAIR_WAIT_SECONDS + 15) * 1000 },
      );
      if (tabs.exitCode === EXIT.noBrowser) return fail(toast, notConnected());
    }
...
function saveDirectory(prefs: ScanPreferences): string {
...
async function deliver(
  answer: ScanAnswer,
  after: NonNullable<ScanPreferences['afterScan']>,
): Promise<void> {
  const { path } = answer;
  ...
  const name =
    markdown && markdown !== path ? `${basename(path)} and its Markdown` : basename(path);
  if (after === 'editor') {
...
function failure(result: CliResult<unknown>): Toast.Options {
  ...
  return {
    ...
    ...(result.exitCode === EXIT.notPaired || result.exitCode === EXIT.noBrowser
      ? { primaryAction: setUpAction }
      : {}),
  };
}
