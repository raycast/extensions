...
export default function Command() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const games = listInstalledGamesForUser(paths.steamPath, user.steamId64);
        const accs = listSteamUsersSync(paths).sort((a, b) =>
          (a.personaName || a.accountName).localeCompare(b.personaName || b.accountName),
        );
        const cur = user;
  ...
  useEffect(() => {
    ...
    const firstAccount = filteredAccounts[0] ? `acc-${filteredAccounts[0].u.steamId64}` : undefined;
  ...
  return (
    ...
    >
      ...
      {filtered.length === 0 && !isLoading && (
        <List.EmptyView title="No games found" description="Check Steam libraries and retry" />
      )}
            ...
            actions={
              <ActionPanel>
                ...
                {libraryRoots.length <= 1 ? (
                  ...
                  <ActionPanel.Submenu title="Open Game Files" icon={Icon.Folder}>
                    {libraryRoots.map((p) => (
                      <Action
                        key={`open-common-${p}`}
                        title={p}
