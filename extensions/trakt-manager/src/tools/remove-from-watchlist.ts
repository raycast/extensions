...
export default async function tool(input: Input): Promise<Output> {
  ...
  if (type === "movie") {
    await executeToolCall(
      (signal) => removeMovieIdFromWatchlist(toolTraktClient, traktId, { signal }),
      `Failed to remove movie "${title}" (ID: ${traktId}) from watchlist`,
    );
  } else {
    await executeToolCall(
      (signal) => removeShowIdFromWatchlist(toolTraktClient, traktId, { signal }),
      `Failed to remove show "${title}" (ID: ${traktId}) from watchlist`,
    );
  }

  return {
    success: true,
    message: `"${title}" has been successfully removed from your Trakt watchlist.`,
  };
}
