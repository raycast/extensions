...
export default function PlaySelected() {
  ...
  return (
    <List
      ...
      searchBarAccessory={
        <List.Dropdown tooltip='Playlist Kind' value={playlistKind} defaultValue={PlaylistKind.ALL} onChange={s => setPlaylistKind( s as PlaylistKind )}>
    ...
    >
      {Object.entries( playlists ?? {} ).filter( ( [section] ) => section !== 'library' ).map( ( [section, data] ) => (
