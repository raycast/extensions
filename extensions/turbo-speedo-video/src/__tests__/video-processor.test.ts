...
describe("VideoProcessor with FFmpeg", () => {
  ...
  it.each(["0.25", "2", "10", "40"] as const)("encodes %sx video and synchronized audio", async (speed) => {
    ...
    expect(Math.abs(Number(info.format.duration) - 4 / Number(speed))).toBeLessThan(0.3);
  });

  it("honors the custom output path and removes audio", async () => {
    const output = join(directory, "custom output.mp4");
    expect(await processor.processVideo(input, "1", "60", "remove", output)).toBe(output);
    const info = probe(output);
  ...
  it("processes a silent video with Keep Audio selected", async () => {
    ...
    execFileSync("ffmpeg", ["-v", "error", "-i", input, "-an", "-c:v", "copy", silent]);
    await processor.processVideo(silent, "1", "24", "keep");
    expect(probe(join(directory, "silent source_x1_24fps.mp4")).streams).toHaveLength(1);
  });

  it("refuses to overwrite an existing file", async () => {
    const output = join(directory, "existing.mp4");
    await writeFile(output, "do not overwrite");
    await expect(processor.processVideo(input, "1", "30", "keep", output)).rejects.toThrow(
  ...
  it("rejects missing inputs and incompatible output containers", async () => {
    await expect(processor.processVideo(join(directory, "missing.mp4"), "1", "30", "keep")).rejects.toThrow(
      "Input file does not exist"
    );
    await expect(processor.processVideo(input, "1", "30", "keep", join(directory, "output.webm"))).rejects.toThrow(
      "output path"
    );
  });

  it("honors a configured FFmpeg path", async () => {
    jest.mocked(getPreferenceValues).mockReturnValue({ ffmpegPath: "/missing/custom/ffmpeg" });
    await expect(processor.checkFfmpegAvailable()).rejects.toThrow("FFmpeg or ffprobe was not found");
  });
});
