...
const search = async (options: TranslateOption): Promise<string> => {
  ...
  const xNaverClientId = await LocalStorage.getItem<string>(
    PapagoKey['X-Naver-Client-Id'],
  )
  const xNaverClientSecret = await LocalStorage.getItem<string>(
    ...
    .then((response) => {
      if ('errorCode' in response) {
        if (response.errorCode === ErrorCode.AuthorizationError) {
          throw new Error(
            `${L.Setting} -> ${Object.values(PapagoKey).join(', ')}`,
          )
        }
        throw new Error(
