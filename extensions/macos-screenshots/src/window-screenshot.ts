...
export default async function WindowScreenshot() {
  try {
    ...
    await showToast({
      ...
      message: "Click on a window to capture",
    });

    const tempFile = join(tmpdir(), `window-screenshot-${Date.now()}.png`);

    try {
      execSync(`/usr/sbin/screencapture -w "${tempFile}"`, {
        stdio: "pipe",
        timeout: 5000,
        encoding: "utf8",
      });
      ...
      if (errorObj.message?.includes("ETIMEDOUT") || errorObj.signal === "SIGTERM" || errorObj.code === "ETIMEDOUT") {
        await showToast({
          ...
          message: "Window selection took too long. Please click faster.",
        });
        return;
      }
      ...
      if (exitCode === 1) {
        await showToast({
          ...
          message: "Window selection was cancelled",
        });
        return;
      }
    ...
    if (!existsSync(tempFile)) {
      await showToast({
        style: Toast.Style.Failure,
        title: "Screenshot Failed",
        message: "No window was selected",
      });
      return;
    }
    ...
    await showHUD(`Window screenshot uploaded! URL copied to clipboard: ${finalUrl}`);
  } catch (error) {
    await showToast({
      style: Toast.Style.Failure,
      title: "Error",
