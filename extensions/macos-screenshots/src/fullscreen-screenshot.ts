...
export default async function FullscreenScreenshot() {
  try {
    ...
    await showToast({
      ...
      message: "Capturing entire screen",
    });

    const tempFile = join(tmpdir(), `fullscreen-screenshot-${Date.now()}.png`);

    try {
      execSync(`/usr/sbin/screencapture "${tempFile}"`, {
        stdio: "pipe",
        timeout: 3000,
        encoding: "utf8",
      });
      ...
      if (errorObj.message?.includes("ETIMEDOUT") || errorObj.signal === "SIGTERM" || errorObj.code === "ETIMEDOUT") {
        await showToast({
          ...
          message: "Fullscreen screenshot took too long.",
        });
        return;
      }
    ...
    await showHUD(`Fullscreen screenshot uploaded! URL copied to clipboard: ${finalUrl}`);
  } catch (error) {
    await showToast({
      style: Toast.Style.Failure,
      title: "Error",
