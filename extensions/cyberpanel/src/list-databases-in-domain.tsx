...
export default function ListDatabasesInDomain(props: LaunchProps<{ arguments: Arguments.ListDatabasesInDomain }>) {
    ...
    return error ? <ErrorComponent errorMessage={error} /> : <List isLoading={isLoading}>
        {databases && databases.map(database => <List.Item key={database.id} title={database.id + " - " + database.dbName} icon={Icon.Coin} accessories={[
            { tag: `user: ${database.dbUser}` }
        ]}
        actions={<ActionPanel>
            <ActionPanel.Section>
                <Action title="Delete Database" icon={Icon.DeleteDocument} onAction={() => confirmAndDelete(database.dbName)} style={Action.Style.Destructive} />
            </ActionPanel.Section>
        </ActionPanel>} />)}
        {!isLoading && <List.Section title="Actions">
            <List.Item title="Create Database" icon={Icon.Plus} actions={<ActionPanel>
                <Action title="Create Database" icon={Icon.Plus} onAction={() => push(<CreateDatabase initialDatabaseWebsite={databaseWebsite} onDatabaseCreated={getFromApi} />)} />
            </ActionPanel>} />
        </List.Section>}
    </List>
}