import { Action, ActionPanel, Alert, Icon, LaunchProps, List, Toast, confirmAlert, showToast, useNavigation } from "@raycast/api";
...
export default function ListDNSRecordsInDomain(props: LaunchProps<{ arguments: Arguments.ListDnsRecordsInDomain }>) {
    ...
    async function getFromApi() {
        const response = await listDNSRecordsInDomain({ selectedZone, currentSelection: currentSelection.toLowerCase() + "Record" });
    ...
    return error ? <ErrorComponent errorMessage={error} /> : <List isLoading={isLoading} isShowingDetail>
        ...
        {dnsRecords && dnsRecords.map(dnsRecord => <List.Item key={dnsRecord.id} title={dnsRecord.id.toString()} icon={Icon.Document} detail={<List.Item.Detail metadata={
        ...
        actions={<ActionPanel>
            <ActionPanel.Section>
                <Action title="Delete DNS Record" icon={Icon.DeleteDocument} onAction={() => confirmAndDelete(dnsRecord.id)} style={Action.Style.Destructive} />
            </ActionPanel.Section>
        </ActionPanel>} />)}
        </List.Section>
        {!isLoading && <List.Section title="Actions">
            <List.Item title="Create DNS Record" icon={Icon.Plus} actions={<ActionPanel>
                <Action title="Create DNS Record" icon={Icon.Plus} onAction={() => push(<CreateDNSRecord initialSelectedZone={selectedZone} onDNSRecordCreated={getFromApi} />)} />
            </ActionPanel>} />
        </List.Section>}
    </List>
}