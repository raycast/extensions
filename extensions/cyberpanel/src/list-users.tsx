...
export default function ListUsers() {
    ...
    return error ? <ErrorComponent errorMessage={error} /> : <List isLoading={isLoading} isShowingDetail>
        {users && users.map(user => <List.Item key={user.id} title={user.userName} icon={user.acl.includes("admin") ? Icon.PersonCircle : Icon.Person} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
            ...
            <Action title="Modify User" icon={Icon.Pencil} onAction={() => push(<ModifyUser user={user} onUserModified={getFromApi} />)} />
            <Action title="Change User ACL" icon={Icon.Lock} onAction={() => push(<ChangeUserACL user={user} onACLChanged={getFromApi} />)} />
            <ActionPanel.Section>
                <Action title="Delete User" icon={Icon.DeleteDocument} onAction={() => confirmAndDelete(user)} style={Action.Style.Destructive} />
            </ActionPanel.Section>
        </ActionPanel>} />)}
        {!isLoading && <List.Section title="Actions">
            <List.Item title="Create User" icon={Icon.AddPerson} actions={<ActionPanel>
                <Action title="Create User" icon={Icon.AddPerson} onAction={() => push(<CreateUser onUserCreated={getFromApi} />)} />
            </ActionPanel>} />
        </List.Section>}
    </List>
}