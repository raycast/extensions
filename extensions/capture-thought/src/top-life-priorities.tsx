...
import { getLifePriorities } from "./api";
import { NotionThought, Priority } from "./types";

export default function TopLifePriorities() {
  ...
  useEffect(() => {
    async function fetchPriorities() {
      try {
        const lifePriorities = await getLifePriorities();
        setThoughts(lifePriorities);
      } catch (error) {
        showToast({
  ...
  return (
    <List
      isLoading={isLoading}
      searchBarPlaceholder="Search personal priorities..."
    >
      {thoughts.length === 0 && !isLoading && (
        <List.EmptyView
          title="No Personal Priorities Found"
          description="Create some thoughts with Personal category to see them here"
          icon="🏠"
        />
      )}
