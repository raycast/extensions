...
import { getWorkPriorities } from "./api";
import { NotionThought, Priority } from "./types";

export default function TopWorkPriorities() {
  ...
  useEffect(() => {
    async function fetchPriorities() {
      try {
        const workPriorities = await getWorkPriorities();
        setThoughts(workPriorities);
      } catch (error) {
        showToast({
  ...
  return (
    <List
      isLoading={isLoading}
      searchBarPlaceholder="Search work priorities..."
    >
      {thoughts.length === 0 && !isLoading && (
        <List.EmptyView
          title="No Work Priorities Found"
          description="Create some thoughts with Work category to see them here"
          icon="📝"
        />
      )}
