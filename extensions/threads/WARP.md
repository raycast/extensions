...
- Note: package.json includes a prepublishOnly guard to prevent npm publish; use npm run publish for the Raycast Store.
...
- View modes: feed, activity, search, quick-thread are "no-view" commands; new-thread renders a Form view.
...
- Manifest details: command arguments and preferences are declared per-command in package.json and accessed via LaunchProps and getPreferenceValues from @raycast/api.
