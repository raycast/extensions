...
export function getSections(
   ...
   const sections = Array.from(
      new Set(cursorRules.flatMap((cursorRule) => cursorRule.tags)),
   )
   const sectionsWithCursorRules = sections.map((tag) => ({
      name: tag,
      cursorRules: cursorRules.filter((cursorRule) =>
         cursorRule.tags.includes(tag),
      ),
   }))
   ...
   if (sortByPopularity) {
      ...
      return sectionsWithCursorRules
         .map((section) => ({
            ...
            totalCount: section.cursorRules.reduce(
               (sum, cursorRule) => sum + (cursorRule.count || 0),
               0,
            ),
         }))
...
export function parseMarkdownToRule(
   content: string,
   fileName: string,
): CursorRule | null {
   ...
   try {
      ...
      const author: Author = {
         name:
            typeof frontmatterData.author?.name === "string"
               ? frontmatterData.author.name
               : userInfo().username,
         url:
            typeof frontmatterData.author?.url === "string"
               ? frontmatterData.author.url
               : "",
         avatar:
            typeof frontmatterData.author?.avatar === "string"
               ? frontmatterData.author.avatar
               : "",
      }

      return {
         title:
            typeof frontmatterData.title === "string"
         ...
         tags: Array.isArray(frontmatterData.tags)
            ? frontmatterData.tags.map(String)
            : [],
         libs: Array.isArray(frontmatterData.libs)
            ? frontmatterData.libs.map(String)
            : [],
         content: ruleContent.trim(),
         author,
         count:
            typeof frontmatterData.count === "number"
               ? frontmatterData.count
               : null,
         isLocal: true,
      }
   } catch (error) {
...
export async function ensureCursorRulesFile(
   projectPath: string,
): Promise<void> {
...
export async function loadProjects(): Promise<Project[]> {
   try {
      const { projectsDirectory } = getPreferenceValues<PreferenceValues>()
      ...
      if (!projectsDirectory) {
         showToast({
            ...
            message:
               "Please set the projects directory in the extension preferences",
         })
         return []
      }
