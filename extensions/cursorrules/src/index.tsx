...
export default function Command() {
   const { showDetailedView, defaultCursorRulesList } =
      getPreferenceValues<PreferenceValues>()

   const [error, setError] = useState<Error | undefined>(undefined)
   const [showingDetail, setShowingDetail] = useState<boolean>(showDetailedView)
   const [popularOnly, setPopularOnly] = useState<boolean>(
      defaultCursorRulesList === "popular",
   )
   ...
   } = usePromise<(popularOnly: boolean) => Promise<CursorRule[]>>(
      fetchCursorRules,
      [popularOnly],
   )
   ...
   const nonStarredRemoteRules = useMemo(() => {
      return remoteRules
         ? remoteRules.filter((rule) => !starredRules?.includes(rule.slug))
         : []
   }, [remoteRules, starredRules])

   const sections = useMemo(
      () => getSections(nonStarredRemoteRules, popularOnly),
      [nonStarredRemoteRules, popularOnly],
   )
            ...
            actions={
               <ActionPanel>
                  <ActionPanel.Section title="Actions">
                     ...
                     <Action
                        title={
                           isStarred ? "Unstar Cursor Rule" : "Star Cursor Rule"
                        }
                        ...
                        onAction={async () => {
                           try {
                              ...
                              handleError(
                                 error as Error,
                                 "starring/unstarring cursor rule",
                              )
                           }
                        }}
                     />
   ...
   return (
      ...
      >
         {error ? (
            ...
            <>
               <List.Section
                  title="Starred Cursor Rules"
                  subtitle={`${starredRuleItems.length} / ${MAX_STARRED_RULES}`}
               >
                  ...
                  {isLoadingRemote &&
                     starredRuleItems.length < MAX_STARRED_RULES && (
                        <List.Item
                           title="Loading potential starred rules..."
                           icon={Icon.Circle}
                        />
                     )}
               </List.Section>
               ...
               <List.Section
                  title="Local Cursor Rules"
                  subtitle={localRules ? `${localRules.length}` : "0"}
               >
                  {localRules && localRules.length > 0 ? (
                     localRules.map((rule) =>
                        renderRuleItem(
                           rule,
                           starredRules?.includes(rule.slug) || false,
                        ),
                     )
                  ) : (
                     <List.Item
               ...
               {sections.length > 0 &&
                  sections.map((section) => (
                     <List.Section
                        key={section.name}
                        title={section.name}
                        subtitle={`${section.slugs.length}`}
                     >
                        {section.slugs.map((slug) => {
                           const rule = nonStarredRemoteRules.find(
                              (item) => item.slug === slug,
                           )
