...
const isPopularCursorRulesResponse = (
   response: APIResponse,
): response is PopularCursorRulesResponse => {
   return (
      "data" in response &&
      Array.isArray(response.data) &&
      "count" in response.data[0]
   )
}
...
export async function fetchCursorRules(
   popularOnly: boolean,
): Promise<CursorRule[]> {
   const apiUrl = popularOnly ? API_URL_POPULAR : API_URL
   const cachePath = popularOnly
      ? POPULAR_CURSOR_RULES_CACHE_PATH
      : ALL_CURSOR_RULES_CACHE_PATH

   try {
      const { cacheInterval } = getPreferenceValues<{ cacheInterval: string }>()
      const modified_timestamp = await getLastModifiedTime(cachePath)

      if (
         modified_timestamp > 0 &&
         Date.now() - modified_timestamp <
            Number(cacheInterval) * 1000 * 60 * 60 * 24
      ) {
...
export async function fetchLocalRules(): Promise<CursorRule[]> {
   const { exportDirectory } = getPreferenceValues<{
      exportDirectory: string
   }>()
   const expandedPath = expandPath(String(exportDirectory))
