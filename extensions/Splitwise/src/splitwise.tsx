...
export default function Command() {
  const { data, isLoading } = useFetch<GetFriends>(
  ...
  return (
  <List
  searchBarPlaceholder="Search your favorite friend" 
  isLoading={isLoading}
  >
    {data?.friends
...
function FillForm(props: { Friend: Friend} ) {
  ...
  return (
    ...
    >
      ...
      <Form.TextArea id="input" title="Natural Language Input" value={input} onChange={setDescription} placeholder={`I owe ${props.Friend.first_name} 12.82 for cinema...` + "\n" + `${props.Friend.first_name} paid 23 bucks for pizza`} />
    </Form>
  );
}
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (body.valid == true) {
        ...
        let errorString = "";
        let response_error = body.expense.errors;
