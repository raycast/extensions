...
Both commands share the same repo browser. You can **pin repos** to keep your most-used ones at the top (`⌘` + `Shift` + `P` to pin/unpin, then reorder pinned repos with `Ctrl` + `↑`/`↓`). Pins are shared across both commands.
...
- Data (repo scans, workflow runs, workflow files) is cached locally so views load instantly and refresh in the background. Pinned repos are stored in Raycast's local storage.
