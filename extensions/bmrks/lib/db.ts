...
export function getGroup(slug: string, userId: string) {
  return supabase.from("groups").select("id").eq("user_id", userId).eq("slug", slug).single();
}
...
export async function getBookmarksByGroupId(groupId: string) {
  return supabase.from("bookmarks").select("*").eq("group_id", groupId).order("created_at", { ascending: false });
}
