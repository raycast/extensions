...
export { SearchSupersededError } from "./helperProcessClient.ts";
export type { BlumeSearchSession as BlumeSearchClient } from "./searchSession.ts";
...
export async function createBlumeSearchClient(preferred?: Application): Promise<BlumeSearchSession> {
  ...
  const client = new BlumeSearchSession(() => new BlumeSearchClient(helperLaunchForApplication(application), protocol));
  await client.ready();
  return client;
}
