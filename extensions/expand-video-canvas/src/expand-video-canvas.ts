import {
  ...
  async function processVideo(ffmpegPath: string, videoPath: string, edgeColor: string): Promise<string> {
    const outputPath = path.join(path.dirname(videoPath), `${path.basename(videoPath, path.extname(videoPath))}_padded${path.extname(videoPath)}`);
    ...
    try {
      ...
      try {
        ...
        await confirmAlert({
          ...
          message: "ImageMagick is required for this operation. Please install it using Homebrew with the command:\n\nbrew install imagemagick\n\nAfter installation, please restart Raycast and run this command again.",
          primaryAction: {
            title: "Copy Install Command",
            onAction: () => {
