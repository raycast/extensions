...
- **首次正式发布**：将 `## [Unreleased]` 重命名为 `## [Initial Version] - {PR_MERGE_DATE}`，并新建一个空的 `## [Unreleased]` 段。`{PR_MERGE_DATE}` 是 [Raycast 商店约定](https://github.com/raycast/extensions/blob/main/docs/basics/prepare-an-extension-for-store.md#version-history)的占位符，会在 PR 合并时由 Raycast 自动替换为实际日期（YYYY-MM-DD）；这是 Raycast Store 审稿的硬性要求，不得写死具体日期。
- **后续版本发布**：若仍走 GitHub PR 流程，沿用 `## [X.Y.Z] - {PR_MERGE_DATE}` 格式（其中 `X.Y.Z` 遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)），让占位符在合并时被自动替换。
