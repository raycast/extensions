    ...
    switch (sectionType) {
      case "artists":
        if (shouldShowSection("artists") && searchData?.artists?.items?.length) {
          return <ArtistsSection type="list" artists={searchData.artists.items} limit={10} />;
        }
        break;
      case "albums":
        if (shouldShowSection("albums") && searchData?.albums?.items?.length) {
          return <AlbumsSection type="list" albums={searchData.albums.items} limit={10} />;
        }
        break;
      case "tracks":
        if (shouldShowSection("tracks") && searchData?.tracks?.items?.length) {
          return <TracksSection title="Songs" tracks={searchData.tracks.items} limit={10} />;
        }
        break;
      case "playlists":
        if (shouldShowSection("playlists") && searchData?.playlists?.items?.length) {
          return <PlaylistsSection type="list" playlists={searchData.playlists.items} limit={10} />;
        }
        break;
      case "shows":
        if (shouldShowSection("shows") && searchData?.shows?.items?.length) {
          return <ShowsSection type="list" shows={searchData.shows.items} limit={10} />;
        }
        break;
      case "episodes":
        if (shouldShowSection("episodes") && searchData?.episodes?.items?.length) {
          return <EpisodesSection episodes={searchData.episodes.items} title="Episodes" limit={10} />;
        }
        break;
    }
    return null;
  ...
  return (
    <View>
      ...
      >
        ...
        {searchData && getSectionOrder().map((sectionType) => renderSection(sectionType))}
      </List>
    </View>
  );
}
