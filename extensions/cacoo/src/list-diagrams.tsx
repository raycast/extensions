...
export default function Command() {
  ...
  const [organizationKey, setOrganizationKey] = useCachedState<string | null>("organization-id", null);

  const { isLoading: isLoadingOrganizations, data: organizations } = useCachedPromise(
    async () => {
  ...
  const {
    ...
    (organizationKey: string | null, keyword: string) => async (options: { page: number }) => {
