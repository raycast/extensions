...
export interface RequestRescheduleFormValues {
  reason: string;
}
...
interface RequestRescheduleProps {
  ...
  /** Optional callback invoked after a successful reschedule request — useful for
   *  revalidating the Cancelled section, since the original booking ends up there. */
  ...
  const handleRequestAndMutate = async (reason: string) => {
    await mutate(handleRequest(reason), {
      optimisticUpdate: (bookings) => {
        if (!bookings) return;
        // The booking gets cancelled when reschedule is requested — drop it from the source section.
