...
export class NetworkUtils {
  ...
  static async getAvailableNetworkServices(): Promise<string[]> {
    try {
      ...
      const services = stdout
        .split("\n")
        .filter((line) => line.trim() && !line.startsWith("*") && !line.includes("An asterisk"))
        .map((line) => line.trim());
      return services;
    } catch (error) {
  ...
  static async getPrimaryNetworkService(): Promise<string> {
    try {
      ...
      // Try to find the active service by checking which one has an IP address
      for (const service of services) {
        try {
          const { stdout } = await execAsync(`/usr/sbin/networksetup -getinfo "${service}"`);
          if (stdout.includes("IP address:") && !stdout.includes("IP address: none")) {
            return service;
          }
        } catch {
      ...
      const commonServices = ["Wi-Fi", "Ethernet", "USB 10/100/1000 LAN"];
