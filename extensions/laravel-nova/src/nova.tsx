...
export default function SearchNova() {
    ...
    const versionDropdown = (<List.Dropdown tooltip="Select Your Nova Version" defaultValue={novaVersion} onChange={updateVersion}>
        {Object.keys(DOCS).map((version) => {
            return (
                <List.Dropdown.Item value={version} title={version} key={version} />
            );
        })}
    </List.Dropdown>);
