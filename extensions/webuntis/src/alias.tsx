...
import { Detail, List, LocalStorage, Icon, ActionPanel, Action } from "@raycast/api";
...
export default function Command(props: { arguments: { shortcut: string } }) {
  ...
  const fetchAlias = async () => {
    const credentials = JSON.parse((await LocalStorage.getItem("credentials")) || "{}");
    if (Object.keys(credentials).length > 0) {
      const { username, password, school, server } = credentials;
  ...
  return (
    <List searchBarPlaceholder="Kürzel, Name, ID..." searchText={shortcut} navigationTitle={`Alias für: ${shortcut}`}>
