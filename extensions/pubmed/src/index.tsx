  ...
  useEffect(() => {
    async function fetchData() {
      if (!query) {
        ...
        setLoading(true);
        try {
          const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmax=${preferences.retmax.value}&sort=${sortBy}&term=${query}&retmode=json`;
          ...
          if (searchResponse.ok) {
            ...
            if (searchJson.esearchresult.count[0] != "0") {
              ...
              const summaryUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=${idList}&version=2.0&retmode=json`;
  ...
  return (
    ...
    >
      <List.Section
        ...
        subtitle={count ? count.replace(/\B(?=(\d{3})+(?!\d))/g, ".") : ""}
      >
        {entries.map((entry) => {
...
function EntryActions(article: Article, query: string, sortBy: string) {
  if (preferences.scihubinstance.value != undefined && article.doi) {
    return (
      <ActionPanel>
