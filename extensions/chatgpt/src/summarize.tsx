...
export default function Summarize() {
  ...
  const modelLoading = snapshot.isLoading;
  const defaultModel = snapshot.catalog.models.default ?? DEFAULT_MODEL;
  const separateDefaultModel = Object.values(snapshot.catalog.models).filter((model) => model.id !== "default");
  const [cache] = useState(() => new CacheAdapter("select_model"));
  const [selectedModelId, setSelectedModelId] = useState(() => cache.get() ?? "default");
  const currentModel = selectedChatModel(snapshot, selectedModelId);

  useEffect(() => {
    if (modelLoading) return;
    setSelectedModelId(currentModel.id);
    cache.set(currentModel.id);
  }, [currentModel.id, modelLoading]);
