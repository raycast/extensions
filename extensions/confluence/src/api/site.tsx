...
export function hasSiteGotRequiredScopes(site: Site, requiredScopes: string[]) {
  return site.scopes.length >= requiredScopes.length &&
  requiredScopes.every(scope => site.scopes.includes(scope));
}
...
export async function getOrSetDefaultSite(forceRefresh = false) {
  ...
  if (maybeActiveSite && !forceRefresh) {
    ...
    const selectedSite = (maybeActiveSite)
      ? sites.find(s => s.id === maybeActiveSite.id) || sites[0]
      : sites[0];
