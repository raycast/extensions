...
for (const mode of ["always", "desktop-only", "fullscreen-only", "never"]) {
  test(`uses ${mode} preference values directly`, () => {
    assert.deepEqual(fixture({ preferences: { optionOne: mode, optionTwo: mode } }).preferences(), {
      firstMode: mode,
      secondMode: mode,
      closeWindow: false,
    });
...
test("missing preferences use the current defaults", () => {
...
for (const closeWindow of [false, true]) {
  test(`toggle respects closeWindow=${closeWindow} before helper execution`, async () => {
    const f = fixture({ preferences: { closeWindow, optionOne: "desktop-only", optionTwo: "fullscreen-only" } });
    await f.command();
