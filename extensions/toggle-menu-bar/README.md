...
A bundled shell wrapper normalizes arguments and forwards each mutation to a signed universal Objective-C helper as a single native call (`set`, `toggle`, or `cycle`); `status` stays a lock-free read. The helper holds a kernel `flock` across the whole read, target choice, and pulse, so overlapping invocations serialize and can never observe a transient pulse value. The lock is released automatically if the helper dies, so no stale-lock recovery is needed. The helper synchronizes the global preferences and Control Center selection, updates WindowServer through the private SkyLight API, and posts refresh notifications. It briefly traverses all four modes to refresh already-open AppKit full-screen spaces, then restores the requested mode and validates the final preferences before reporting success. On a failed intermediate write it makes a best-effort restoration to the requested mode and reports failure.
