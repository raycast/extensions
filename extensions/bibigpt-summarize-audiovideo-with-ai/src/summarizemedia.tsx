import { Form, ActionPanel, Action, showToast, Toast, Clipboard, Detail, useNavigation, getPreferenceValues } from "@raycast/api";
...
function Command() {
    ...
    return currentTabUrl`;
            ...
            if (currentUrl === "Unsupported browser") {
                return showToast(Toast.Style.Failure, "Browser not supported", "Unable to obtain the active window URL of the browser");
            }
            setUrl(currentUrl);
        } catch (error) {
            return showToast(Toast.Style.Failure, "Unable to obtain browser URL", error instanceof Error ? error.message : "");
        }
    }
    ...
    return (
        <>
            ...
            >
                <Form.TextField id="url" title="Audio and video URL" placeholder="Please enter the audio and video URL to be summarized, press ⌘ + K to view commands" value={url} onChange={setUrl} />
            </Form>
        </>
    );
}
