...
async function testSimpleConnection() {
  ...
  try {
    ...
    const versionResponse = await axios.post(ANKI_CONNECT_URL, {
      action: "version",
      version: 6
    }, {
    ...
    const decksResponse = await axios.post(ANKI_CONNECT_URL, {
      action: "deckNames",
      version: 6
    }, {
      headers: { "Content-Type": "application/json" },
    ...
    const modelsResponse = await axios.post(ANKI_CONNECT_URL, {
      action: "modelNames",
      version: 6
    }, {
      headers: { "Content-Type": "application/json" },
