...
describe("AI Flashcards Generator Tool", () => {
  ...
  it("deve gerar flashcards e adicioná-los ao Anki com sucesso", async () => {
    ...
    expect(FlashcardGenerator.generate).toHaveBeenCalledWith("Texto de teste", expect.objectContaining({
      language: "português",
      minFlashcards: 5,
      maxFlashcards: 20,
      enableTags: true
  ...
  it("deve mostrar erro quando a conexão com o Anki falha", async () => {
    // Mock dos flashcards gerados
    const mockFlashcards = [
      { front: "Pergunta 1", back: "Resposta 1", extra: "Extra 1" }
    ];
