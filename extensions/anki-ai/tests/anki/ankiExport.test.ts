...
describe('Anki Export Tests', () => {
  ...
  test('Deve adicionar flashcards ao Anki', async () => {
    ...
    const response = await AnkiRepository.addFlashcards(
      testFlashcards,
      deckName,
      modelName,
      tags
  ...
  test('Deve lidar com campos vazios apropriadamente', async () => {
    ...
    await expect(
      AnkiRepository.mapFlashcardToModelFields(emptyFlashcard, modelName)
    ).rejects.toThrow('Os seguintes campos estão vazios');
  });
  ...
  test('Deve validar modelo inexistente', async () => {
    ...
    await expect(
      AnkiRepository.mapFlashcardToModelFields(flashcard, modelName)
    ).rejects.toThrow('Erro ao obter campos do modelo');
  });
});
