...
import { convertToAvif, formatBytes, isImageFile, processImages, summarizeResults } from "./utils/image";
...
export default async function Command() {
  ...
  try {
    ...
    const results = await processImages(imagePaths, convertToAvif);
    const summary = summarizeResults(results);

    if (summary.failed === 0) {
      const saved = summary.savedBytes > 0 ? `, saved ${formatBytes(summary.savedBytes)}` : "";
      await showHUD(`✅ ${summary.success} image(s) → AVIF${saved}`);
