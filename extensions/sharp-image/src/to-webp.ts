...
import { convertToWebP, formatBytes, isImageFile, processImages, summarizeResults } from "./utils/image";
...
export default async function Command() {
  ...
  try {
    ...
    const results = await processImages(imagePaths, convertToWebP);
    const summary = summarizeResults(results);

    if (summary.failed === 0) {
      const saved = summary.savedBytes > 0 ? `, saved ${formatBytes(summary.savedBytes)}` : "";
      await showHUD(`✅ ${summary.success} image(s) → WebP${saved}`);
      ...
      if (failedResult?.error) {
        ...
        await showHUD("❌ Conversion failed");
      }
    }
  } catch (e) {
