...
A readable but partial scan cannot replace a complete, non-empty index either. Partial scans can populate an empty index, or refresh an already partial one only when the incoming entry count is at least the saved count. Counts prevent shrinking but do not prove that two scans cover identical paths. A readable complete scan can replace either, including with an empty index to remove stale paths. Setup, the standalone command, and action-panel refresh all apply this policy to both shortcut and shared-folder indexes. The browser uses the retained index's entries, timestamp, and completeness notice. The refresh toast separately explains any scan limit and which previous index was kept.
