...
function updateReplacement(dbPK: number, phrase: string, replacement: string): void {
  execFileSync("/usr/bin/sqlite3", [
    DB_PATH,
    `UPDATE ZTEXTREPLACEMENTENTRY SET ZSHORTCUT=${sql(phrase)}, ZPHRASE=${sql(replacement)}, Z_OPT=Z_OPT+1, ZNEEDSSAVETOCLOUD=1 WHERE Z_PK=${dbPK};`,
...
function ReplacementForm({
  ...
  return (
    ...
    >
      ...
      <Form.TextField id="replacement" title="Expands To" placeholder="the full text" defaultValue={initial?.replacement} />
    </Form>
  );
}
...
export default function SearchReplacements() {
  ...
  function refresh() {
    try {
      ...
      showToast({ style: Toast.Style.Failure, title: "Failed to load replacements", message: String(err) });
    } finally {
      setIsLoading(false);
    }
  }
