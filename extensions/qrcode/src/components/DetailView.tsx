...
export function DetailView({ link, selectedLink, qrCodeUrl, viewType }: DetailViewProps) {
  const t = useTranslation();
  const urlInfo = useMemo(() => parseURL(link?.url), [link?.url]);
  ...
  if (viewType === "qrcode") {
    return (
      <List.Item.Detail
        markdown={
          selectedLink.url === link.url ? (qrCodeUrl ? `![QR Code](${qrCodeUrl})` : t.generateFailed) : t.generateQRCode
        }
      />
    );
  }
