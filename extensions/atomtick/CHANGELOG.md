# AtomTick Changelog

## [Initial Version] - {PR_MERGE_DATE}

- Analog Clock: live SVG watch face with a centered digital `HH:MM:SS` caption, NTP-corrected.
- Copy Atomic Time: copy the NTP-synchronized instant to the clipboard as ISO-8601.
- Automatic system timezone detection, with an optional manual timezone override preference.
- Toast notification when NTP sync fails (clock falls back to the system clock).
