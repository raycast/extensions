...
const { cursorApp } = preferences;
const cursorAppNameShort = cursorApp?.name || "";
const cursorAppCLI = `${cursorApp?.path}/Contents/Resources/app/bin/code`;

const STORAGE = `${homedir()}/Library/Application Support/${cursorAppNameShort}/User/globalStorage/alefragnani.project-manager`;
        ...
        <ActionPanel>
          <ActionPanel.Section>
            {isRemoteProject(path) ? (
              <Action
                title={`Open in ${cursorApp.name} (Remote)`}
                icon={{ fileIcon: cursorApp.path }}
                onAction={() => {
                  visitItem(item);
                  exec(`"${cursorAppCLI}" --remote ${parseRemoteURL(path)}`);
                  closeMainWindow();
                }}
              />
            ) : (
              <Action.Open
                title={`Open in ${cursorAppNameShort}`}
                icon={{ fileIcon: cursorApp.path }}
                target={path}
                application={cursorApp.path}
...
function ExtensionError(detail: string) {
  ...
  return (
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.Label title="VS Code App" text={cursorApp?.name || "(unset)"} />
