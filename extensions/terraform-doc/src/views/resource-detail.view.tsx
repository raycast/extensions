...
import { parseRawDoc, toResourceDocUrl, toResourceRawDocUrl } from "../lib/resource-detail";
      ...
      try {
        if (state === undefined) {
          const markdown = await (
            await fetch(
              toResourceRawDocUrl(props.provider, props.version, props.resource)
            )
          ).text();
      ...
      actions={
        <ActionPanel>
          <Action.OpenInBrowser
            ...
            url={toResourceDocUrl(props.provider, props.version, props.resource)}
            shortcut={{ modifiers:["shift"],key:"enter"}}
          />
          <Action.OpenInBrowser
            title={`Open Raw Doc in Browser`}
            url={toResourceRawDocUrl(props.provider, props.version, props.resource)}
