  ...
  included: z.array(
    z.object({
      ...
      attributes: z.object({
        ...
        "published-at": z.coerce.date().optional().transform(v=>v||new Date(0)),
        tag: z.string().optional().transform(v=>v||""),
        version: z.string().optional().transform(v=>v||""),
      }),
    }),
  ),
});

export type ProviderVersion = z.infer<typeof providerVersionsResponseSchema>["included"][0];
...
`https://registry.terraform.io/v2/providers/${provider.attributes["full-name"]}?include=provider-versions`,
  ...
  return res.included.sort((a, b) => b.attributes["published-at"].getTime() - a.attributes["published-at"].getTime());
}
