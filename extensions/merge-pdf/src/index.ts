...
export default async  function Command(props: { arguments: { fileName:string, deleteOriginalFiles:string } }){
  ...
  try {
    ...
    const nonPdfFiles = selectedFinderItems.filter((item) => path.extname(item.path).toLowerCase() !== '.pdf');
    if (nonPdfFiles.length > 0) {
      await showToast({
        style: Toast.Style.Failure,
        title: "Error",
        message: nonPdfFiles.length + " non-PDF files selected :" + nonPdfFiles.map(item => path.basename(item.path)).join(", ") + ". Please select only PDF files.",
      });
      return;
    }
    ...
    const outputPath = getOutputPath(pdfFiles.map(item => item.path), deleteOriginalFiles, fileName);
...
function getOutputPath(filePaths: string[], deleteOriginalFiles:string, fileName:string): string {
  let finalFileName:string = 'merged';
