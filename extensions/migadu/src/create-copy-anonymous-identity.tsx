...
import { IdentityCreate } from "./utils/types";
...
const NAME_GENERATOR_PREFERENCES: Config = {
...
export default function CreateAnonymousIdentityCommand() {
  const mailboxesQuery = useCachedPromise(async () => {
  ...
  const { handleSubmit, setValue, itemProps } = useForm<FormValue>({
    initialValues: {
      local_part: uniqueNamesGenerator(NAME_GENERATOR_PREFERENCES),
      name: "",
    },
    onSubmit: async ({ mailbox, ...partialIdentity }: FormValue) => {
      ...
      const fullMailbox = mailboxesQuery.data?.data?.find((curr) => curr.address === mailbox);
  ...
  const regenerateLocalPart = () => {
    setValue("local_part", uniqueNamesGenerator(NAME_GENERATOR_PREFERENCES));
  };

  // TODO create action for copy and not copy. also create action to file an issue and reference it in error message
  return (
    <Form
      actions={
        <ActionPanel>
    ...
    >
      ...
      <Form.Separator />
      <Form.TextField
        title="Local part of identity"
        info="The part before the `@`. To create `foo@bar.com`, enter `foo` here."
        placeholder="anonymous"
        {...itemProps.local_part}
      />
      <Form.Dropdown title="Destination Mailbox" storeValue {...itemProps.mailbox}>
        {(mailboxesQuery.data?.data ?? []).map((mailbox) => {
