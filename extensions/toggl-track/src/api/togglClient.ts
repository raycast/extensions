...
async function togglFetch<T>(method: string, endpoint: string, body?: unknown, retryCount = 0): Promise<T | void> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      ...
      return await togglFetch(method, endpoint, body, retryCount + 1);
    }
