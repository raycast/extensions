...
async function QueryGenetics(input: string): Promise<any> {
    try {
        const response = await fetch('https://api.genetics.opentargets.org/graphql', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function renderGeneticsData(input: string): Promise<any> {
    ...
    let entities = ['genes', 'variants', 'studies'];
    const data : any = [];
    entities.forEach(function (item) {
        if (response[item].length > 0) {
            // TODO: show more than 1 result per entity
            const id = item === 'studies' ? response[item][0].studyId : response[item][0].id;
            const name = item === 'genes' ? response[item][0].symbol : item === 'variants' ? response[item][0].id : item === 'studies' ? response[item][0].traitReported : '';
            const entity = item === 'genes' ? 'gene' : item === 'variants' ? 'variant' : item === 'studies' ? 'study' : '';        
