import { ActionPanel, List, Action, Icon, getPreferenceValues, Keyboard } from "@raycast/api";
...
function getSecondaryShortcut(modifier: Preferences.Snippets["snippetSecondaryModifier"]): Keyboard.Shortcut {
  ...
  return { modifiers: modifierMap[modifier] || ["cmd", "shift"], key: "enter" };
}
      ...
      <ActionPanel.Section>
        {isPasteFirst ? (
          <>
            ...
            <Action.CopyToClipboard title="Copy Snippet" content={code} shortcut={secondaryShortcut} />
          </>
        ) : (
          <>
            <Action.CopyToClipboard title="Copy Snippet" content={code} />
            <Action.Paste title="Paste Snippet" content={code} shortcut={secondaryShortcut} />
          </>
        )}
      </ActionPanel.Section>
    </ActionPanel>
        ...
        <List.Section key={category} title={category}>
          {grouped[category].map((snippet) => (
            <List.Item
              ...
              actions={<SnippetActions code={snippet.code} />}
            />
          ))}
        </List.Section>
      ))}
