...
import { Detail, showToast, Toast, LaunchProps, ActionPanel, Action, Icon, getPreferenceValues, openCommandPreferences, LocalStorage, showHUD, Keyboard } from "@raycast/api";
import got from 'got';
...
const API_KEY = "27ac2898-a85a-4f7e-8914-00b74900a4f7";
...
export default function Command(props: LaunchProps<{ arguments: { query: string } }>) {
  ...
  async function streamResponse() {
    ...
    try {
      let stream = await got.stream(`https://api.kapa.ai/query/v1/stream?query=${encodeURIComponent(query)}`, {
          ...
          if (part.trim() !== '') {
            try {
              ...
              if (data.chunk.type === "partial_answer") {
                ...
                let uniqueRelevantSources: RelevantSource[] = Array
                  .from(new Map(data.chunk.content.relevant_sources.map((item: RelevantSource) => [JSON.stringify(item), item])).values()) as RelevantSource[]
                uniqueRelevantSources = uniqueRelevantSources
                  .filter((source: unknown) => (source as RelevantSource).source_url && (source as RelevantSource).source_url.length > 5)
  ...
  const submitFeedback = async (reaction: "upvote" | "downvote") => {
    ...
    try {
      const response = await got.post(`https://api.kapa.ai/query/v1/feedback/upsert`, {
        headers: { "X-API-TOKEN": API_KEY, },
        json: {
  ...
  return (
    <Detail markdown={answer.length > 0 ? answer : "Searching for the best answer..."} isLoading={isLoading} navigationTitle={props.arguments.query} metadata={
      showMetadata && <Detail.Metadata>
        ...
        <Detail.Metadata.Label title="Word Count" text={`${answer.split(/\s+/).filter(word => word.length > 0).length} words`} />
        {isAnswerUncertain !== undefined && (
          <Detail.Metadata.TagList title="AI Answer Confidence">
            <Detail.Metadata.TagList.Item text={isAnswerUncertain ? "Low" : "High"} color={isAnswerUncertain ? "#eed535" : "#37a168"} />
      ...
      <ActionPanel>
        <Action.CopyToClipboard
          ...
          content={answer}
        />
        <ActionPanel.Submenu title="Sources considered for response" icon={Icon.Globe} shortcut={{ modifiers: ["cmd"], key: "/" }}>
        ...
        <ActionPanel.Section>
          ...
          <Action title="Open Preferences" onAction={openCommandPreferences} icon={Icon.Cog} />
        </ActionPanel.Section>

        {questionAnswerId && (<ActionPanel.Section title="Feedback">
