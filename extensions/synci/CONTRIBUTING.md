...
CI runs `npm run lint:source` for ESLint and Prettier checks on both platforms. Run full `npm run lint` before a Store submission to also validate the manifest, icon, and metadata.
