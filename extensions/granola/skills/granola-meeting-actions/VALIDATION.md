...
API 2.5.0 bundles the schema extractor’s TypeScript compiler without its standard-library files at the compiler’s inferred path. Array inputs were reduced to `{}` and extraction failed. The tsconfig explicitly includes the installed `typescript/lib/lib.es2023` declarations alongside Node and React types; build, lint, and a separate TypeScript check pass with the unchanged tool code.
