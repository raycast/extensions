	...
	return (cookie = new Promise((resolve, reject) => {
		exec(
			...
			(err, stdout, stderr) => {
				...
				const robloxSecurity = Object.entries(json).find(([key]) =>
					key.includes("ROBLOSECURITY"),
				)![1] as string;
				if (!robloxSecurity) {
					cookie = undefined;
					return reject(
						new Error("Roblox Studio config file is invalid, please start Roblox Studio & log in."),
					);
				}
