    ...
    >
      {(() => {
        ...
        return (
          <>
            {primaryItems.map((s) => {
              const scr = generated?.[s.type] ?? (loadingTypes[s.type] ? "Generating..." : "");
              return (
                <List.Item
                  key={s.type}
                  id={s.type}
                  title={s.title}
                  detail={<List.Item.Detail markdown={`${scr}`} />}
                  actions={
                    <ActionPanel>
                      <Action.CopyToClipboard content={scr} />
            ...
            {variantItems.length > 0 && (
              <List.Section title="3x3 Variants">
                {variantItems.map((s) => {
                  const scr = generated?.[s.type] ?? (loadingTypes[s.type] ? "Generating..." : "");
                  return (
                    <List.Item
