...
export function parseTimeInput(input: string): { hour: number; minute: number; ampm: Meridiem } | null {
  ...
  if (halfPastMatch) {
    ...
    if (hour >= 1 && hour <= 12) {
      const ampm: Meridiem = ampmChar?.startsWith("p")
        ? "PM"
        : ampmChar?.startsWith("a")
          ? "AM"
          : hour >= 7 && hour <= 11
            ? "AM"
            : "PM";
  ...
  const writtenMinMatch = normalized.match(
    /^(\d{1,2})\s+(fifteen|thirty|fortyfive|forty-five|forty|fifty|five|ten|twenty|twentyfive|twenty-five)\s*(am|pm|a|p)?$/
  );
  if (writtenMinMatch) {
