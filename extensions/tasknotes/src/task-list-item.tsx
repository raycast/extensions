...
function dateAccessories(task: TaskNote, mode: DateAccessoryMode): List.Item.Accessory[] {
  if (mode === "agenda" && agendaDateKind(task) === "scheduled" && task.scheduled) {
...
function agendaDateKind(task: TaskNote): "due" | "scheduled" | undefined {
  if (task.due && task.scheduled) return sortDate(task.scheduled) < sortDate(task.due) ? "scheduled" : "due";
