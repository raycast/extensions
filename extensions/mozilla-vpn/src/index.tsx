...
const Command: React.FC = () => {
  ...
  const toggleVpn = async () => {
    ...
    const toggleTimeout = setTimeout(() => {
      ...
      loadingToast.message = "The operation is taking longer than expected. The VPN might still be changing state.";
    ...
    try {
      ...
      while (attempts < maxAttempts && !statusMatched) {
        ...
        try {
          console.log(`Polling VPN status (attempt ${attempts}/${maxAttempts})...`);
      ...
      if (!statusMatched) {
        console.log("Status didn't match expected after polling, fetching latest data...");
        try {
      ...
      try {
        ...
        console.error('Error fetching data after VPN toggle error:', fetchError);
    ...
    try {
      ...
      while (attempts < maxAttempts && !updated) {
        ...
        try {
          ...
          if (status.serverCity && status.serverCity !== 'Unknown' && 
              status.serverCountry && status.serverCountry !== 'Unknown') {
            
            console.log(`Server info found: ${status.serverCity}, ${status.serverCountry}`);
            ...
            if (status.isActive) {
              try {
                ...
                console.error('Error updating IP after server change:', ipError);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      try {
        ...
        const { isActive, serverCity, serverCountry, isAuthenticated } = await checkVpnStatus();
        console.log(`VPN status retrieved: active=${isActive}, city=${serverCity}, country=${serverCountry}, auth=${isAuthenticated}`);
      ...
      if (statusFailed && ipFailed) {
        await showToast(Toast.Style.Failure, 'Failed to fetch VPN status and IP information.');
  ...
  useEffect(() => {
    ...
    initTimeoutId = setTimeout(() => {
      ...
      if (!errorMessage) {
        setErrorMessage('Initialization timed out. Try refreshing the extension.');
      }
  ...
  switch (currentView) {
    case 'serverSelector':
      return <ServerSelector onServerSelected={handleServerSelected} onBack={navigateBack} />;
              ...
              actions={
                <ActionPanel>
                  ...
                  <Action
                    ...
                    onAction={async () => {
                      await showToast(Toast.Style.Animated, "Refreshing IP information...");
                      try {
                        const ip = await fetchCurrentIP();
                        setCurrentIP(ip);
                        await showToast(Toast.Style.Success, "IP information updated");
                      } catch (error) {
                        await showToast(Toast.Style.Failure, "Failed to update IP information");
                      }
                    }}
                  />
                </ActionPanel>
