...
const Command: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      console.error("Error fetching initial data:", error instanceof Error ? error.message : error);
  ...
  const toggleVpn = async () => {
    ...
    try {
      ...
      setVpnStatus(isActive);
      setServerCity(serverCity);
      showToast(Toast.Style.Success, `VPN ${isActive ? 'activated' : 'deactivated'} successfully.`);
      setTimeout(async () => {
        const newIP = await fetchCurrentIP();
        setCurrentIP(newIP);
      }, 1000);
    } catch (error: unknown) {
      console.error(`Failed to ${action} VPN:`, error instanceof Error ? error.message : error);
      showToast(Toast.Style.Failure, `VPN ${action} failed`, error instanceof Error ? error.message : String(error));
    }
  };
