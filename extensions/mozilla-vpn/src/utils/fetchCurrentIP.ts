...
const fetchWithRetry = (url: string, isHttps = true, maxRetries = 3): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    const makeRequest = () => {
      ...
      const req = httpModule.get(url, { timeout: 8000 }, (res) => {
        ...
        res.setTimeout(8000, () => {
          ...
          if (retries < maxRetries) {
            retries++;
            console.log(`Response timed out. Retrying ${retries}/${maxRetries}...`);
        ...
        res.on('end', () => {
          ...
          if (res.statusCode && (res.statusCode < 200 || res.statusCode >= 300)) {
            ...
            if (retries < maxRetries) {
              retries++;
              console.log(`Retry ${retries}/${maxRetries} after HTTP error ${res.statusCode}`);
      ...
      req.on('error', (error) => {
        console.error(`Request error (attempt ${retries + 1}/${maxRetries + 1}):`, error);
      ...
      req.setTimeout(8000, () => {
        ...
        if (retries < maxRetries) {
          retries++;
          console.log(`Request timed out. Retrying ${retries}/${maxRetries}...`);
          setTimeout(makeRequest, 1500); // Increased wait time
        } else {
...
export async function fetchCurrentIP(): Promise<string> {
  ...
  try {
    const ipData = await fetchWithRetry('https://api.ipify.org?format=json', true);
    const ipResult = JSON.parse(ipData);
    const ip = ipResult.ip;
    
    // Then fetch the geolocation data for the IP
    try {
      const geoData = await fetchWithRetry(`http://ip-api.com/json/${ip}`, false);
    ...
    try {
      const ipData = await fetchWithRetry('https://api.ipify.org?format=text', true);
