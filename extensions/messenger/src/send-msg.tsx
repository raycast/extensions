...
export default function Command({
  ...
  arguments: { contactName: string; messageText: string };
}>) {
  ...
  useEffect(() => {
    if (launchContext?.contactId) {
      ...
      const correspondingContact = contacts?.find((contact) =>
        contact.givenName.toLowerCase().startsWith(contactName.toLowerCase()),
      );
    ...
    return (
      ...
      >
        <Form.Dropdown {...itemProps.contact} title="Contact" storeValue>
          {contacts
            ?.filter((contact) => contact.givenName.toLowerCase().startsWith(contactName.toLowerCase()))
            .map((contact, i) => {
