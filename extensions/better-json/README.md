...
1. Open **Inspect JSON** from **Better JSON Inspector** in Raycast. Valid clipboard JSON opens directly in the browser, with **Entire Document** selected. Nested JSON strings are automatically deserialized throughout the document before any preview is shown.
...
- Arrays and objects load children in pages of 100. Loading another page reuses the children already loaded.
...
- The index contains at most 3,000 nodes and 1,000,000 characters of searchable text. Each path, key, or primitive value contributes at most its first 4,096 characters before case normalization. The interface explicitly labels search as partial whenever a node or text limit is reached. Hierarchical browsing and complete copying remain available beyond these limits.
- Previews longer than 40,000 characters are shortened with a visible notice. Strings and object keys are encoded in small chunks so opening a preview does not serialize an entire large string. Copying still includes the complete selected value.
- Long paths and labels are shortened for display. **Copy As… → Path** retains the complete path, including unusual property names.
...
Numbers retain their precision. Large integers, high-precision decimals, negative zero, and exponents outside JavaScript's numeric range remain JSON numbers with their original digits; they are never silently rounded, changed to strings, or replaced with `null`. This also applies to nested JSON strings and **Restore Original Data Types**.
...
Single-quoted log input can contain one complete escaping layer. Backslashes, quotes, newlines, tabs, and Unicode escapes are decoded together; malformed escapes remain errors.
...
Local development extensions are not updated from the Store. If Better JSON Inspector reports incompatibility or `Missing executable. You might need to build the extension.`, open Raycast v2 and rebuild from this project:
...
After the build succeeds, open **Inspect JSON** in Raycast. You can stop the development watcher with `Ctrl+C`; the built extension remains installed. Use `npm run build` to rebuild it later without starting a watcher.
