...
export default function Command() {
  ...
  const { isLoading: isLoadingList, data } = useFetch<AbbreviationListBase, AbbreviationListBase>(`https://www.vocabula.lat/api/abr/ray/${query}`, {
  ...
  return (
    ...
    >
      {
        data && data?.length > 0 && data?.map((abbr: Abbreviation, index: number) => {
