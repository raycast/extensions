...
export default function Command() {
  ...
  const { isLoading: isLoadingList } = useFetch<string[] | Transformed[], { matchedWords: string[] } | DictionaryWord >(`https://www.vocabula.lat/api/${query}`, {
  ...
  return (
    ...
    >
      {
        wordList && wordList?.length > 0 && wordList?.map((word: string | Transformed, index: number) => {
        ...
        const metaData = firstWord?.grammar && firstWord?.grammar?.map((grammar, grammarIndex) =>
          <GrammarDetails
            grammar={grammar}
