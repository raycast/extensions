...
export default function ListPortals() {
  const { isLoading, data: portals, error, mutate } = useCachedPromise(
    async () => {
  ...
  return (
    <Grid isLoading={isLoading}>
      {!isLoading && !portals.length && !error ? <Grid.EmptyView description="Create self-service help center portals for your customers. Help them find answers quickly, without waiting. Streamline inquiries, boost agent efficiency, and elevate customer support." actions={<ActionPanel>
        <Action.Push icon={Icon.Plus} title="Create Portal" target={<CreatePortal />} onPop={mutate} />
      </ActionPanel>} /> : portals.map((portal) => (
        <Grid.Item
        key={portal.id}
...
type FormValues = {
  ...
  const {handleSubmit,itemProps,values} = useForm<FormValues>({
    async onSubmit(values) {
      ...
      try {
        await chatwoot.portals.create({ portal: {...values, header_text: values.name, color: "#FFFFFF", custom_domain: "", homepage_link: "", page_title: values.name, archived: false, config: {allowed_locales: ["en"], default_locale: "en"}} });
