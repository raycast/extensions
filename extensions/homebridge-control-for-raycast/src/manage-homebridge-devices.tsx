...
export default function Command() {
    ...
    function handleError(error: any, defaultMessage: string, detailedMessage?: string) {
        console.error(error);
        setError(error instanceof Error ? error : new Error(defaultMessage));
    ...
    async function toggleAccessoryState(accessory: Accessory, characteristicType: string) {
        try {
            ...
            showToast(Toast.Style.Success, `Accessory ${newState ? "turned on" : "turned off"}`);
            fetchAccessories(baseUrl);
        } catch (error) {
    ...
    function renderAccessoryControls(accessory: Accessory) {
        ...
        return (
            <ActionPanel>
                ...
                {/*only show if can be turned on off */}
            ...
            <ActionPanel.Section>
                ...
                {brightness !== null && (
                    <>
                        <Action
                            title="20% Brightness"
                            onAction={() => handleBrightnessSubmit({brightness: "25", accessory})}
                        />
                        <Action
                            title="40% Brightness"
                            onAction={() => handleBrightnessSubmit({brightness: "50", accessory})}
                        />
                        <Action
                            title="60% Brightness"
                            onAction={() => handleBrightnessSubmit({brightness: "75", accessory})}/>
                        <Action
                            title="80% Brightness"
                            onAction={() => handleBrightnessSubmit({brightness: "100", accessory})}
                        />
                        <Action
                        ...
                        <Action.Push
                            ...
                            target={<BrightnessForm accessory={accessory}
                                                    handleSubmit={(values) => handleBrightnessSubmit({
...
function AccessoryListItem({ accessory }: { accessory: Accessory }) { // Remove key from the props here
        const isOn = accessory.serviceCharacteristics.find((char) => char.type === "On")?.value;
        const brightness = accessory.serviceCharacteristics.find((char) => char.type === "Brightness")?.value;
        const formattedText =
            accessory.humanType === "Lightbulb" ? `Brightness: ${brightness}` : accessory.serviceCharacteristics.map((char) => `${char.type}: ${char.value}`).join(", ");
    ...
    return (
        <List isLoading={!accessories.length && !error}>
