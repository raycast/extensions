import { Action, ActionPanel, Detail, Form, getPreferenceValues, Icon, Keyboard, List, showToast, Toast, useNavigation } from "@raycast/api";
...
export default function SearchPastes() {
    ...
    const {isLoading, data: pastes, error, mutate } = useFetch(generateApiUrl(), {
    ...
    return <List isLoading={isLoading} searchBarPlaceholder="Search pastes">
        {!isLoading && !pastes.length && !error ? <List.EmptyView title="Pastes" description="You haven't created any pastes…yet!" actions={<ActionPanel>
        ...
        : pastes.map(paste => <List.Item key={paste.id} title={paste.title} subtitle={paste.url} accessories={[
            { tag: paste.language},
            { text: !paste.duration ? "Never expires" : `${paste.duration} min` }
        ]} actions={<ActionPanel>
            ...
            <Action.Push icon={Icon.Plus} title="Create Paste" target={<CreatePaste mutate={mutate} />} shortcut={Keyboard.Shortcut.Common.New} />
            <Action icon={Icon.Trash} title="Delete Paste" style={Action.Style.Destructive} onAction={() => deletePaste(paste.id)} shortcut={Keyboard.Shortcut.Common.Remove} />
        </ActionPanel>} />)}
    </List>
}
...
function PasteDetails({id}: {id: string}) {
    ...
    return <Detail isLoading={isLoading} markdown={paste?.body} metadata={paste && <Detail.Metadata>
        <Detail.Metadata.Label title="Title" text={paste.title || paste.id} />
        <Detail.Metadata.Label title="Duration" text={paste.duration ? `${paste.duration} mins` : undefined} icon={paste.duration ? undefined : Icon.Minus} />
        <Detail.Metadata.TagList title="Language">
            <Detail.Metadata.TagList.Item text={paste.language} />
        </Detail.Metadata.TagList>
    </Detail.Metadata>} actions={paste && <ActionPanel>
...
function CreatePaste({ mutate }: {mutate: MutatePromise<Paste[]>}) {
    ...
    const { itemProps, handleSubmit } = useForm<FormValues>({
        async onSubmit(values) {
            ...
            try {
                await mutate(
                    fetch(generateApiUrl("", params), { method: "POST", body: values.text }).then(r => r.json()).then(res => {
    ...
    return <Form isLoading={isLoading} actions={<ActionPanel>
        ...
        <Form.Dropdown title="Language" {...itemProps.language}>
            {Object.entries(LANGUAGES).map(([val, key]) => <Form.Dropdown.Item key={key} title={key} value={val}  />)}
        </Form.Dropdown>
    </Form>
}