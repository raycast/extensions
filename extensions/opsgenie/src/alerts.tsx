...
export default function AlertList() {
  ...
  return (
    <List isLoading={state.alerts.length === 0} searchBarPlaceholder="Filter alerts..." throttle={true} onSearchTextChange={(text: string) => fetch(text)}>
...
function AlertListItem(props: { alert: Alert }) {
  ...
  const subtitle = alert.acknowledged ? "Acknowledged" : alert.snoozed ? "Snoozed" : alert.status === "closed" ? "Closed" : "Open";
  const icon = alert.priority === "P1" ? "icon-p1.png" : alert.priority === "P2" ? "icon-p2.png" : alert.priority === "P3" ? "icon-p3.png": alert.priority === "P4" ? "icon-p4.png": "icon-p5.png";
