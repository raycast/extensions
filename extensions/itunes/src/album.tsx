...
async function performSearch(searchText: string, signal: AbortSignal) {
  ...
  read.on('line', line => {
    if (line.indexOf('</dict>') >= 0) {
      dictBlock = false
      if (match.filter(entry => {return entry.artist === buffer.artist && entry.album === buffer.album}).length === 0 && buffer.album.length > 0 && buffer.album.toLowerCase().indexOf(searchText.toLowerCase()) >= 0) {
        match = [...match, buffer]
      }
      buffer = {
        album: '',
