...
export default function Command() {
  ...
  const { isLoading: isSearchLoading, data: searchData } = useFetch(
    `${BASE_URL}/api/v1/search.json?q=${encodeURIComponent(searchText)}`,
    {
            ...
            <List.Item
              ...
              actions={
                <ActionPanel title={`Check ${item.attributes.name} Status`}>
                  <Action.OpenInBrowser url={withUtm(item.attributes.isdown_url)} />
                </ActionPanel>
              }
