...
describe("RaycastMcpOAuthProvider", () => {
  ...
  it("registers the Raycast https web callback for dynamic client registration", () => {
    ...
    expect(provider.redirectUrl).toBe("https://raycast.com/redirect?packageName=Extension");
    expect(provider.clientMetadata.redirect_uris).toEqual(["https://raycast.com/redirect?packageName=Extension"]);
  });
  ...
  it("keeps cached clients whose redirect URIs include the web callback", async () => {
    ...
    await expect(provider.clientInformation()).resolves.toMatchObject({ client_id: "current-client" });
    expect(raycastMock.localStorage.removeItem).not.toHaveBeenCalledWith("mobbin.oauth.clientInformation");
  });
  ...
  it("discards cached clients without the web callback redirect URI", async () => {
    ...
    expect(raycastMock.localStorage.removeItem).toHaveBeenCalledWith("mobbin.oauth.clientInformation");
  });
  ...
  it("discards clients written under an older schema version", async () => {
    ...
    await expect(provider.clientInformation()).resolves.toBeUndefined();
    expect(raycastMock.localStorage.removeItem).toHaveBeenCalledWith("mobbin.oauth.clientInformation");
    expect(raycastMock.localStorage.setItem).toHaveBeenCalledWith("mobbin.oauth.clientSchemaVersion", "2");
  });
  ...
  it("starts authorization with Raycast's tracked request object", async () => {
    ...
    const oauthClient = {
      ...
      authorize: vi.fn(async () => ({ authorizationCode: "authorization-code" })),
    };
    ...
    authorizationUrl.searchParams.set("redirect_uri", "https://raycast.com/redirect?packageName=mobbin");
    ...
    expect(raycastMock.localStorage.setItem).toHaveBeenCalledWith("mobbin.oauth.codeVerifier", "raycast-code-verifier");
    expect(oauthClient.authorize).toHaveBeenCalledWith(authorizationRequest);
    expect(provider.redirectUrl).toBe("https://raycast.com/redirect?packageName=mobbin");
