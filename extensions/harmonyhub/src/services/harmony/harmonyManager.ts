...
export class HarmonyManager {
  ...
  public async startDiscovery(
    ...
    try {
      ...
      if (cached && cached.length > 0) {
        Logger.info(`Using ${cached.length} cached hub(s)`);
        onProgress?.(1, `Found ${cached.length} cached hub(s)`);
        return cached;
      }
      Logger.info("No cached hubs found, starting discovery");
    } catch (error) {
    ...
    try {
      ...
      this.discoveryPromise = new Promise<HarmonyHub[]>((resolve, reject) => {
        ...
        // Handle discovered hubs
        this.explorer.on("online", (data: HubDiscoveryData): void => {
          Logger.debug(`Hub discovered: ${data.friendlyName} (${data.ip})`);
          const hub = this.createHubFromDiscovery(data);
        ...
        // Start discovery
        Logger.debug("Starting hub discovery...");
        this.explorer.start();
      });
  ...
  private async getCachedHubs(): Promise<HarmonyHub[] | null> {
    try {
      ...
      let parsedCache: CachedHubs;
      try {
        parsedCache = JSON.parse(cached) as CachedHubs;
      } catch (e) {
      ...
      const { hubs, timestamp } = parsedCache;
      ...
      Logger.debug(`Found ${hubs.length} hub(s) in cache`);
      return hubs;
    } catch (error) {
      Logger.warn("Failed to read cache:", error);
      return null;
