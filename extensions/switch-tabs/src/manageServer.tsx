...
const RELEASE_BASE =
  "https://github.com/ferrocyante/Manage-Server/releases/download/v1";
...
const ASSETS: { name: string; url: string; sha256?: string }[] = [
  ...
  },
  {
    ...
    sha256: "2A49406F8E0298D11E2908797C2487FE58E72C0C00233D27FD3488338CAE3F97",
  },
  {
    name: "ImportWorkspaces.ps1",
    url: `${RELEASE_BASE}/ImportWorkspaces.ps1`,
    sha256: "BC33F524168925106A2622A7D03C4DDFA9C8AB11A00EBB1B631C03116C19874E",
  },
];
...
function checkLoginStartup(): Promise<boolean> {
  return new Promise((resolve) => {
    exec(`reg query "${LOGIN_REG_KEY}" /v "${LOGIN_REG_NAME}"`, (err) =>
      resolve(!err),
    );
  });
}
...
function enableLoginStartup(): Promise<void> {
  return new Promise((resolve, reject) => {
    exec(
      `reg add "${LOGIN_REG_KEY}" /v "${LOGIN_REG_NAME}" /t REG_SZ /d "${exePath}" /f`,
...
function disableLoginStartup(): Promise<void> {
  return new Promise((resolve) => {
    exec(`reg delete "${LOGIN_REG_KEY}" /v "${LOGIN_REG_NAME}" /f`, () =>
      resolve(),
    );
  });
}
...
export default function Command() {
  ...
  const initialHealth: HealthResponse | null = cachedHealth
    ...
    exec(checkCmd, (_err, stdout) => {
      ...
      setActiveWindows((prev) => {
        const isSame =
          ...
          active.every((val, index) => val === prev[index]);
  ...
  const handleToggleLoginStartup = async () => {
    const toast = await showToast({
      style: Toast.Style.Animated,
      title: "Updating startup setting...",
    });
    try {
      if (loginStartup) {
  ...
  const serverStatusIcon = isServerRunning
    ? Icon.Livestream
    ...
    <List searchBarPlaceholder="Filter tasks...">
      ...
      <List.Section title="Bridge Server">
        <List.Item
          ...
          accessories={[
            ...(isServerRunning && health
              ? [{ text: formatUptime(health.uptime), icon: Icon.Clock }]
              : []),
        ...
        <List.Item
          ...
          subtitle={
            loginStartup
              ? "Server launches automatically on Windows startup"
              : "Manual start only"
          }
          icon={{
