...
import { usePersistentState, useFilterValidation } from "./hooks/useFilterState";
import { DisplayTab } from "./types";
import { ConnectionErrorView, ServerDownView } from "./components/TabSwitchStatusViews";
...
export default function Command() {
  ...
  const [manualSearchMode, setManualSearchModeState] = useState<"filter" | "search">(() => {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => cache.get("browser_bridge_collapsed") === "true");

  const browserTarget = browserFilter === "all" ? lastActiveBrowser : browserFilter;
  const windowTarget = currentWindowFilter === "all" ? undefined : currentWindowFilter;
  ...
  const activateTab = useCallback((tab: DisplayTab) => originalActivateTab(tab), [originalActivateTab]);
  ...
  const ToggleAction = useCallback(
    () => <ToggleCollapseAction isCollapsed={isCollapsed} setIsCollapsed={setIsCollapsed} />,
    [isCollapsed],
  );
  const HistoryActionComponent = useCallback(
    ...
    [browserFilter, windowTarget, sendToSocket, requestData, navigateCurrentTab, unfilteredMergedTabs],
  );
  ...
  const CycleBrowserActionComponent = useCallback(
    () => (
      <CycleBrowserAction
        ...
        shortcut={getActionShortcut("cycleBrowser") || { modifiers: [], key: "`" }}
      />
    ),
    [
      availableBrowsers,
  ...
  const CycleWindowActionComponent = useCallback(
    () => (
      <CycleWindowAction
        ...
        includeAllWindows={includeAllWindows}
        setWindowFilterForBrowser={setWindowFilterForBrowser}
        shortcut={getActionShortcut("cycleWindow") || { modifiers: [], key: "tab" }}
      />
    ),
    [browserFilter, windowsByBrowser, currentWindowFilter, includeAllWindows, setWindowFilterForBrowser],
  );

  if (connectionError) return <ConnectionErrorView error={connectionError} reinitialize={reinitialize} />;
