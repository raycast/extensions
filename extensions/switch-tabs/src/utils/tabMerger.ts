...
function enrichTab(tab: DisplayTab, extensionData: ExtensionData | null, idx = 0): void {
  ...
  if (tab.windowType === "popup") {
    const isFirst = idx === 0;
    acc.push({
      icon: { source: Icon.Desktop, tintColor: isFirst ? Color.Blue : Color.SecondaryText },
      tooltip: "Popup Tab",
    });
  }
  if (tab.windowState === "fullscreen" && tab.isActive) {
    acc.push({ icon: { source: Icon.Maximize, tintColor: Color.Blue }, tooltip: "Fullscreen" });
  }

  // B. Media & Audio
  if (tab.audible) acc.push({ icon: Icon.SpeakerHigh, tooltip: "Playing Audio" });

  // D. Status Indicators
  if (tab.pinned) {
    acc.push({ icon: { source: RESOLVED_ICONS.pinned, tintColor: RESOLVED_COLORS.pinned }, tooltip: "Pinned Tab" });
  }

  if (tab.discarded) {
    acc.push({
      icon: { source: RESOLVED_ICONS.discarded, tintColor: RESOLVED_COLORS.discarded },
      tooltip: "Discarded (Memory Released)",
    });
  }
  if (tab.frozen && !tab.discarded) {
    acc.push({ icon: { source: RESOLVED_ICONS.sleeping, tintColor: RESOLVED_COLORS.sleeping }, tooltip: "Sleeping" });
  }

  // E. Reader Mode
  if (isReaderMode) {
    acc.push({ icon: { source: Icon.Mobile, tintColor: Color.Orange }, tooltip: "Reader Mode" });
  }

  // F. Identity Indicators
  if (tab.isActive && tab.windowType !== "popup") {
    acc.push({ icon: { source: Icon.Dot, tintColor: Color.Blue }, tooltip: "Active Tab" });
  }

  // G. Group Tags
  const matchedGroup = tab.groupId && tab.groupId !== -1 ? extensionData?.groups[tab.groupId] : null;
  if (matchedGroup) {
    acc.push({
      tag: {
...
export function mergeTabs(extensionData: ExtensionData | null, limit?: number, skipPrune = false): DisplayTab[] {
  ...
  const merged = processingList.map((et, idx) => {
    ...
    if (nextObj.url?.startsWith("read:")) {
      ...
      if (urlParamIdx !== -1) {
        try {
          domain = getHostnameFast(decodeURIComponent(nextObj.url.slice(urlParamIdx + 5)));
        } catch {
          /* ignore */
        }
      }
      // Fallback: strip read: prefix and extract domain directly
      if (!domain) domain = getHostnameFast(nextObj.url.replace(/^read:[/]*/, ""));
      if (domain) nextObj.favIconUrl = `https://www.google.com/s2/favicons?domain=${domain}&sz=32`;
    }
    ...
    if (existing) {
      ...
      const isIdentical =
        ...
        (nextObj.windowType !== "popup" || (existing.browserIndex === 0) === (nextObj.browserIndex === 0)) &&
        Math.floor(existing.currentTime || 0) === Math.floor(nextObj.currentTime || 0) &&
