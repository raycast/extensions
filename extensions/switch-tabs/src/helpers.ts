...
export function getTabGroupColor(
  colorName: string,
...
// Note: Raycast restarts the extension process on preference changes,
// so this frozen snapshot is always current within a session.
...
export function formatBytes(bytes: number | undefined | null): string {
  if (bytes === undefined || bytes === null || bytes <= 0 || bytes === -1)
    return "0 B";
  const k = 1024;
...
export function getDownloadIcon(
