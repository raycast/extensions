...
export function DownloadsView({
  ...
  const history = filteredDownloads.filter(
    (d) => d.state === "complete" || d.state === "interrupted",
  );
  ...
  const historyGroupsMap = history.reduce(
    (
      groups: Record<
        string,
        { groupName: string; sortTime: number; items: DownloadItem[] }
      >,
      item,
    ) => {
  ...
  return (
    ...
    >
      {inProgress.length > 0 && (
        <List.Section
          title="In Progress"
          subtitle={`${inProgress.length} items`}
        >
          {inProgress.map((item) => (
            <DownloadListItem
              key={item.id}
...
function DownloadPreview({ item }: { item: DownloadItem }) {
  const filename =
    item.filename?.split(/[\\/]/).pop() || item.filename || "Unknown";
  ...
  if (isMissing) {
    ...
    const iconSource =
      typeof itemIcon === "string"
        ? itemIcon
        : (itemIcon as { source: string | Icon }).source;
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label
            title="Size"
            text={formatBytes(item.fileSize || item.totalBytes)}
          />
          <Detail.Metadata.Label
            title="Status"
          ...
          {item.endTime && (
            <Detail.Metadata.Label
              title="Finished"
              text={new Date(item.endTime).toLocaleString()}
            />
          )}
          <Detail.Metadata.Separator />
          <Detail.Metadata.Label
            title="Mime Type"
            text={item.mime || "Unknown"}
          />
          <Detail.Metadata.Label
            title="Path"
...
function DownloadListItem({
  ...
  return (
    <List.Item
      ...
      icon={itemIcon}
      quickLook={
        item.exists && item.filename
          ? { path: item.filename, name: filename }
          : undefined
      }
            ...
            {item.state === "complete" ? (
              item.exists ? (
                <Action.Trash
                  ...
                  onTrash={() => {
                    sendToSocket({
                      type: "ERASE_DOWNLOAD_HISTORY",
                      id: item.id,
                    });
                    showToast({
                      title: `Moved to Recycle Bin: ${filename}`,
                      style: Toast.Style.Success,
                    });
                  }}
                />
