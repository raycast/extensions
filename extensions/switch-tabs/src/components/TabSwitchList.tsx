...
import { List, getPreferenceValues, showToast, Toast, Icon } from "@raycast/api";
...
import { DisplayTab, CollapsedListItem, ExtensionGroup, BookmarkItem, HistoryItem, BridgeMessage } from "../types";
...
export const TabSwitchList = React.memo((props: TabSwitchListProps) => {
  const { isLoading: isSearchLoading, results: searchResults, searchText, setSearchText } = useSearch();
  ...
  const isLoading = isSearching ? isSearchLoading : hasTabsToShow ? false : isEdgeLoading;
  ...
  const handleSearchTextChange = useCallback(
    (text: string) => {
      ...
      if (text === toggleKey) {
        setManualSearchMode(currentSearchMode === "filter" ? "search" : "filter");
        setSearchText("");
        return;
      }
      if (text.endsWith(clearKey)) {
        ...
        showToast({
          ...
          title: currentSearchMode === "filter" ? "Filter cleared" : "Search cleared",
        });
        return;
      }
  ...
  }, [serverStatus, isConnecting, availableBrowsers, isSearchMode, activeBrowserName]);

  const { finalSearchResults } = useLocalSearch(searchText, searchResults, props.browserTarget);
  ...
  const navigationTitle = useMemo(() => {
    ...
    if (titleBarContent === "tabCount") {
      ...
      const parts: string[] = [];
      if (tabCount > 0) parts.push(`${tabCount} ${tabCount === 1 ? "tab" : "tabs"}`);
      if (groupCount > 0) parts.push(`${groupCount} ${groupCount === 1 ? "group" : "groups"}`);
      return parts.length > 0 ? `${modeLabel} • ${parts.join(" • ")}` : modeLabel;
    }
    return modeLabel;
  }, [titleBarContent, isSearchMode, clock, props.allDisplayTabs, props.groupsArray]);
  ...
  return (
    <List
      ...
      searchBarAccessory={
        ...
        (showWindowFilter && availableBrowsers.some((b: string) => (windowsByBrowser[b] || []).length > 1)) ? (
          <SearchBarFilter
    ...
    >
      {isLuckyOnboarding ? (
        ...
        <SearchResultsSection {...searchProps} searchResults={finalSearchResults} setSearchText={setSearchText} />
      ) : (
