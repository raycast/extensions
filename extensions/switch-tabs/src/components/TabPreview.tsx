...
export function TabPreview({
          ...
          try {
            ...
            try {
              const oldFiles = fs
                ...
                .filter(
                  (f: string) =>
                    f.startsWith(`snap_${safeId}_`) && f.endsWith(".jpg"),
                );
  ...
  const getStatusLabel = (): { text: string; color: string } => {
    ...
    if (!snapshot && !snapshotPath)
      return { text: "Unavailable", color: Color.Orange };
    if (isCached === false) return { text: "Active", color: Color.Blue };
    if (isCached === true)
      return { text: "Cached", color: Color.SecondaryText };
