...
export const SearchModeAction = (props: SearchModeActionProps) => {
  const { currentSearchMode, setManualSearchMode, searchText, setSearchText } = props;
  ...
  return (
    <>
      {hasText && setSearchText && (
        <Action
          title={currentSearchMode === "filter" ? "Clear Filter Text" : "Clear Search Text"}
          icon={{ source: Icon.Eraser, tintColor: Color.Magenta }}
          shortcut={getActionShortcut("clearSearch") || { modifiers: [], key: "'" }}
          onAction={() => {
            setSearchText("");
            showToast({
      ...
      <Action
        ...
        icon={{ source: isSearch ? Icon.Filter : Icon.MagnifyingGlass, tintColor: Color.Magenta }}
