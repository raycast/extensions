...
import { getBookmarkIcon, formatTimeAgo, getActionShortcut, forceCopy } from "../helpers";
import { BookmarkFolderPicker } from "./BookmarkFolderPicker";
import { RenameBookmarkForm } from "./RenameBookmarkForm";
import { useState, useMemo, useEffect } from "react";
import { subscribeToBookmarks, getCurrentBookmarks } from "../context/BrowserStore";
...
interface BookmarksViewProps {
  ...
  onDeleteBookmark: (id: string, isFolder: boolean, browserType?: string) => void;
...
export function BookmarksView({
  ...
  const targetFolderTitle = (prefs.bookmarksRedirectFolder || "Bookmarks Bar").toLowerCase();
  ...
  const bookmarksBarFolder = useMemo(() => {
    ...
    return liveBookmarks.find((b: BookmarkItem) => {
      ...
      return titleLower.includes(targetFolderTitle) || (targetFolderTitle === "bookmarks bar" && rawId === "1");
    });
  ...
  const breadcrumb = useMemo(() => {
    const displayTitle = folderId === "root" && !isShowingRoot && bookmarksBarFolder ? bookmarksBarFolder.title : title;
  ...
  const filteredBookmarks = useMemo(() => {
    ...
    if (searchText) {
      ...
      list = list.filter(
        (b: BookmarkItem) => b.title.toLowerCase().includes(lower) || (b.url && b.url.toLowerCase().includes(lower)),
      );
    }
      ...
      {filteredBookmarks.length === 0 ? (
        ...
        filteredBookmarks.map((item: BookmarkItem) => (
          <BookmarkRow
            ...
            isRedirectedRoot={folderId === "root" && effectiveFolderId !== "root"}
            path={[
              ...path,
              folderId === "root" && !isShowingRoot && bookmarksBarFolder ? bookmarksBarFolder.title : title,
            ]}
          />
        ))
      )}
...
function BookmarkRow({
  ...
  return (
    <List.Item
      ...
      accessories={
        isFolder
          ? [{ text: `${item.children?.length || 0} items`, icon: Icon.Folder, tooltip: item.url }]
          ...
          <ActionPanel.Section title="Organize">
            {!isRootFolder && (
              <Action.Push
                ...
                target={<RenameBookmarkForm bookmark={item} onRename={onRenameBookmark} />}
              />
            )}
            <BookmarkFolderPicker
              ...
              actionTitle={isFolder ? "Move Folder to..." : "Move Bookmark to..."}
