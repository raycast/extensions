import { Action, ActionPanel, Form, useNavigation, Icon, Color } from "@raycast/api";
...
interface CreateGroupFormProps {
  tab: DisplayTab;
  allTabs: DisplayTab[];
  onCreate: (tab: DisplayTab, groupName: string, color: string, tabIds?: string[]) => void;
}

export function CreateGroupForm({ tab, allTabs, onCreate }: CreateGroupFormProps) {
  ...
  return (
                ...
                tabIds = allTabs
                  .filter((t) => {
                    try {
                      ...
                      return tHost === hostname && t.browserType === tab.browserType && t.windowId === tab.windowId;
                    } catch {
                      return false;
                    }
                  })
    ...
    >
      <Form.Description text={`Create a new group for tab: ${tab.title} (${browserType})`} />
      <Form.TextField id="groupName" title="Group Name" placeholder="My Awesome Group" />
      ...
      {hostname && <Form.Checkbox id="groupDomain" label={`Group all tabs from ${hostname}`} defaultValue={false} />}
    </Form>
  );
}
