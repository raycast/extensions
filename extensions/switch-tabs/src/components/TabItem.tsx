    ...
    <ActionPanel>
        ...
        <Action
          ...
          shortcut={
            currentSearchMode === "search"
              ? { modifiers: ["ctrl", "shift"], key: "enter" }
              : undefined
          }
        />
      )}
      ...
      <ActionPanel.Section title="Tab Management">
        ...
        <Action
          ...
          shortcut={
            hasMedia
              ? { modifiers: ["shift"], key: "arrowLeft" }
              : { modifiers: [], key: "arrowLeft" }
          }
          onAction={async () => {
            const targetTab =
              allTabs.find((t) => t.isActive && t.windowFocused) ||
              allTabs.find((t) => t.isActive) ||
              tab;
        ...
        <Action
          title={
            tab.windowType === "popup" ? "Re-Attach Tab" : "Focus Tab (Popup)"
          }
              ...
              if (tab.url?.startsWith("read:")) {
                ...
                if (urlParamIdx !== -1) {
                  try {
                    navigateTab(
                      tab,
                      decodeURIComponent(tab.url.slice(urlParamIdx + 5)),
                    );
                    return;
                  } catch {
                    /* fallback */
        ...
        <Action.Push
          ...
          target={
            <RenameTabForm tab={tab} onRename={(title) => rename(tab, title)} />
          }
        />
        <Action
          title="Duplicate Tab"
          ...
          {groups
            .filter(
              (group) =>
                ...
                group.browserType === tab.browserType,
            )
            .map((group) => (
              <Action
                key={group.id}
        ...
        >
          {Array.from(
            ...
            const windowActiveTab =
              allTabs.find(
                (t) => String(t.windowId) === windowId && t.isActive,
      ...
      <ActionPanel.Section title="Danger Zone">
        <Action
          ...
          onAction={() => {
            if (tab.windowId) {
              sendToSocket({
                type: "TOGGLE_MINIMIZE_WINDOW",
                windowId: tab.windowId,
                browser: tab.browserType,
              });
...
export const TabItem = React.memo((props: TabItemProps) => {
  ...
  const accessories = useMemo(() => {
    ...
    if (
      ...
      if (
        typeof tab.playbackRate === "number" &&
        Math.abs(tab.playbackRate - 1.0) > 0.01
      ) {
      ...
      const timeAcc = {
        tag: {
          value: displayValue,
          color: tab.paused ? Color.SecondaryText : Color.Green,
        },
  ...
  return (
    <List.Item
      ...
      icon={useMemo(
        () => getTabIcon(tab),
        [tab.url, tab.favIconUrl, tab.title],
      )}
