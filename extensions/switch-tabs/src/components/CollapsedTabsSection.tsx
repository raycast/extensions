...
import { CollapsedListItem, DisplayTab, ExtensionGroup, BookmarkItem, BridgeMessage } from "../types";
...
interface CollapsedTabsSectionProps {
  ...
  createTabGroup: (tab: DisplayTab, title: string, color: string, tabIds?: string[]) => void;
      ...
      <Action.Push
        ...
        target={
          <GroupDetailView
            folderId={item.id}
            type={
              (typeof item.id === "string" && item.id.includes("-")) || typeof item.id === "number"
                ? "group"
                : "hostname"
            }
            title={item.title}
...
export const CollapsedTabsSection = React.memo((props: CollapsedTabsSectionProps) => {
  const {
    collapsedViewItems,
    browserIconMap,
    activateTab,
      ...
      {collapsedViewItems.map((item, index) =>
        item.type === "folder" ? (
          <List.Item
            ...
            title={item.title}
            icon={{
              source: (item.browserType && browserIconMap[item.browserType]) || Icon.Folder,
              tintColor: getTabGroupColor(item.color, item.browserType),
            }}
            accessories={[
              ...(item.isActive
                ? [{ icon: { source: Icon.Dot, tintColor: Color.Blue }, tooltip: "Active Tab inside" }]
                : []),
