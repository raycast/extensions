...
export function TargetInputSearchView({
  ...
  useEffect(() => {
    ...
    if (globalSocket && globalSocket.readyState === WebSocket.OPEN) {
      globalSocket.send(
        JSON.stringify({ type: "INIT_SEARCH_TARGETING", tabId: tab.id }),
      );
    }
    ...
    return () => {
      ...
      if (globalSocket && globalSocket.readyState === WebSocket.OPEN) {
        globalSocket.send(
          JSON.stringify({ type: "STOP_SEARCH_TARGETING", tabId: tab.id }),
        );
      }
    };
  }, [tab.id]);
  ...
  return (
    <List
      ...
      onSearchTextChange={(text: string) => {
        const clearKey =
          (getPreferenceValues() as { clearSearchKey?: string })
            .clearSearchKey || "'";
      ...
      searchBarPlaceholder={
        isTargetingLoading
          ...
          : searchInputs.find((i) => i.id === selectedInputId)?.label ||
            `Search ${tab.displayTitle}...`
      }
      throttle={false}
      searchBarAccessory={
        searchInputs.length > 1 ? (
          <List.Dropdown
            tooltip="Select Target Search Box"
    ...
    >
      ...
      {searchText.trim().length > 0 && (
        <List.Item
          ...
          title={searchText}
          subtitle={
            hasTarget ? `Search ${searchContext.label}` : `No Input Found`
          }
          accessories={
            hasTarget
              ? [{ tag: { value: "Surgical Target", color: Color.Green } }]
              : []
          }
          actions={
