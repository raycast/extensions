...
export function GroupDetailView({
  ...
  const folderTabs = useMemo(() => {
    if (type === "group") {
      ...
      return allDisplayTabs.filter(
        (t) => String(t.groupId) === String(folderId),
      );
    } else {
      ...
      return allDisplayTabs.filter((t) => {
        ...
        if (folderId === "Ungrouped")
          return isUngrouped && (!t.subtitle || t.subtitle === "");
  ...
  const matches = useMemo(() => {
    return query
      ? folderTabs.filter((t: DisplayTab) =>
  ...
  return (
    <List
      ...
      searchBarPlaceholder="Filter group..."
    >
