import { List, ActionPanel, Action, Icon, Color, open, getPreferenceValues } from "@raycast/api";
...
function getExePath(): string | null {
  try {
    const prefs = getPreferenceValues<{ serverDir?: string }>();
    if (!prefs.serverDir) return null;
    return path.join(prefs.serverDir, SERVER_EXE_NAME);
  } catch {
    return null;
  }
}
  ...
  return (
    <List>
      <List.EmptyView
        ...
        description={
          exePath
            ? "Start the server to connect your browser tabs."
            : "Configure the server path in Extension Preferences to enable quick start."
        }
        ...
        actions={
          <ActionPanel>
            ...
            <Action
              ...
              onAction={() => open("raycast://extensions/water_tear/switch-tabs/manageServer")}
            />
          </ActionPanel>
        }
      />
...
export function WaitingForDataView({
  ...
  return (
    <List isLoading={true} searchBarPlaceholder={searchBarPlaceholder} onSearchTextChange={setSearchText}>
      <List.EmptyView
...
export function ConnectionErrorView({ error, reinitialize }: { error: string; reinitialize: () => void }) {
  return (
    <List>
      <List.EmptyView
...
export function HangingServerView({
  ...
  return (
    <List isLoading={true} searchBarPlaceholder={searchBarPlaceholder} onSearchTextChange={setSearchText}>
      <List.EmptyView
        title={serverStatus === "LISTENING" ? "Waiting for Browser" : "Initializing..."}
        description={
          serverStatus === "LISTENING"
            ? "Server is ready. Open a supported browser to see your tabs."
            : "Starting the bridge server..."
        }
        icon={{ source: serverStatus === "LISTENING" ? Icon.CheckCircle : Icon.Circle, tintColor: Color.Blue }}
        actions={
          <ActionPanel>
            <Action
