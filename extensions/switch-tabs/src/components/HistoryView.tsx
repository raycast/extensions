  ...
  const historyGroups = React.useMemo(() => {
    ...
    const groupsMap = filteredHistory.reduce(
      (
        groups: Record<
          string,
          { groupName: string; sortTime: number; items: HistoryItem[] }
        >,
        item,
      ) => {
...
function HistoryListItem({
  ...
  return (
    <List.Item
      ...
      accessories={[
        { text: formatTimeAgo(item.lastVisitTime), tooltip: "Visit Time" },
      ]}
      actions={
        <ActionPanel>
          <ActionPanel.Section title="Primary">
              ...
              onAction={async () => {
                if (item.url) {
                  if (sendToSocket) {
                    ...
                    const idStr = item.id?.toString() || "";
                    const browserName = idStr.includes("-")
            ...
            {navigateCurrentTab && (
              <Action
                ...
                onAction={() =>
                  item.url && navigateCurrentTab(item.url, allTabs || [])
                }
              />
            )}
            ...
            <ActionPanel.Section title="History Management">
              ...
              <Action
                ...
                onAction={() => {
                  ...
                  showToast({
                    style: Toast.Style.Success,
                    title: "Cleared entire browser history",
                  });
                }}
              />
            </ActionPanel.Section>
