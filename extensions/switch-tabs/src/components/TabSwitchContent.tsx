...
import { List, ActionPanel, Action, Icon, Color, getPreferenceValues, open } from "@raycast/api";
...
function limitSections<T>(sections: Record<string, T[]>, limit: number): Record<string, T[]> {
...
export const TabSwitchContent = React.memo((props: TabSwitchContentProps) => {
  ...
  if (allDisplayTabs.length === 0 && (!globalSessions || globalSessions.length === 0)) {
    return (
      <List.EmptyView
    ...
    <React.Fragment key={viewReturnTick}>
      {isCollapsed ? (
        <CollapsedTabsSection
          collapsedViewItems={isLimitedView ? collapsedViewItems.slice(0, 10) : collapsedViewItems}
        ...
        <NormalTabsSections
          sections={isLimitedView ? limitSections(normalSections, 15) : normalSections}
          groups={isLimitedView ? groupsArray.slice(0, 3) : groupsArray}
          bookmarks={bookmarks || []}
          activateTab={activateTab}
          activateTabBackground={activateTabBackground}
