...
function loadCachedStats(): Record<
  string,
  { tabCount: number; groupCount: number }
> {
  try {
...
function saveCachedStats(
  stats: Record<string, { tabCount: number; groupCount: number }>,
) {
  try {
...
export function WorkspacesView() {
  const lastKnownStats =
    useRef<Record<string, { tabCount: number; groupCount: number }>>(
      loadCachedStats(),
    );
  const prevStatsHash = useRef<string>("");

  const [roster, setRoster] = useState<WorkspaceInfo[]>(() =>
    getCurrentWorkspaces(),
  );
  const [workspaceStats, setWorkspaceStats] = useState<WorkspaceStat[]>([]);

  const { pop } = useNavigation();
  const workspacesShortcut = getActionShortcut("workspaces") || {
    modifiers: ["shift"],
    key: "w",
  };
  ...
  const computeStats = useCallback(
    (currentRoster: WorkspaceInfo[], currentTabs: DisplayTab[]) => {
      const stats: Record<
        string,
        { tabCount: number; groupIds: Set<string | number>; tabs: DisplayTab[] }
      > = {};
  ...
  const activateTab = (tab: DisplayTab) => {
    if (globalSocket && globalSocket.readyState === 1) {
      globalSocket.send(
        JSON.stringify({ type: "ACTIVATE_TAB", tabId: tab.id }),
      );
      closeMainWindow();
    }
  };
    ...
    wsTabs.forEach((t) => {
      if (t.windowId) {
        ...
        if (!seen.has(winIdStr)) {
          seen.add(winIdStr);
          if (globalSocket && globalSocket.readyState === 1) {
            const rawWinId = winIdStr.includes("-")
              ? winIdStr.split("-").slice(1).join("-")
              : winIdStr;
        ...
        {workspaceStats.map((ws) => {
          const subtitle = (() => {
            if (ws.isActive) {
              return (
                `${ws.tabCount} tabs` +
                (ws.groupCount > 0 ? ` • ${ws.groupCount} groups` : "")
              );
            }
            if (ws.lastTabCount > 0) {
              return (
                `${ws.lastTabCount} tabs` +
                (ws.lastGroupCount > 0 ? ` • ${ws.lastGroupCount} groups` : "")
              );
            }
            return "Inactive";
          })();
          ...
          return (
            <List.Item
              ...
              icon={{
                source: Icon.Map,
                tintColor:
                  ensureReadable(ws.color) ||
                  (ws.isActive ? Color.Blue : Color.SecondaryText),
              }}
              accessories={[
                ws.isActive
                  ? {
