...
export function usePersistentState(): FilterState {
  ...
  const [browserFilter, setBrowserFilter] = useState<string>(() => cache.get("browser_filter") || "all");
  ...
  const setWindowFilterForBrowser = useCallback((browser: string, windowId: string) => {
...
export function useFilterValidation(
  ...
  const { browserFilter, setBrowserFilter, currentWindowFilter, includeAllWindows, setWindowFilterForBrowser } = state;

  // Safety Guard: Reset browser filter if selected browser is no longer available
  useEffect(() => {
    if (browserFilter !== "all" && availableBrowsers.length > 0 && !availableBrowsers.includes(browserFilter)) {
      console.log(`[Switch Tabs] Selected browser '${browserFilter}' is offline. Resetting filter to 'all'.`);
  ...
  useEffect(() => {
    ...
    const isCurrentValid =
      currentWindowFilter === "all" ? includeAllWindows : windows.some((w) => w.id === currentWindowFilter);
