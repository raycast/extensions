...
export function useSearch(externalSearchText?: string) {
  ...
  const { luckyProvider, showSearchImages } = useMemo(
    () => getPreferenceValues<Preferences.TabSwitch>(),
    [],
  );
  ...
  useEffect(() => {
    ...
    const actualQuery = (
      isLucky ? searchText.substring(1).trim() : query
    ).toLowerCase();
    ...
    async function fetchSuggestions() {
      ...
      try {
        ...
        let baseSuggestions: SearchResult[] = chromeSuggestions.map(
          (item: string, index: number) => {
            const type = chromeTypes[index];
            const isNavigation = type === "NAVIGATION";
            let itemUrl = isNavigation
              ? item
        ...
        baseSuggestions = baseSuggestions.filter(
          (v: SearchResult, i: number, a: SearchResult[]) =>
          ...
          if (entityMap.size > 0 && !signal.aborted) {
            ...
            setSuggestions((prev) =>
              prev.map((s) => {
                ...
                if (entityMeta.zs && !isLucky)
                  repIcon = {
                    source: entityMeta.zs,
