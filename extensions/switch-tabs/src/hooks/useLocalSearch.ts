...
export function useLocalSearch(
  ...
  const finalSearchResults = useMemo(() => {
    ...
    const cleanSearchText = isSpecialTrigger
      ? lowerSearchText.substring(1).trim()
      : lowerSearchText;
    ...
    const browserPageMatches = browserPages
      .map((page: BrowserPage) => {
        ...
        else if (page.keywords.some((k: string) => k === cleanSearchText))
          score += 12;
        else if (
          page.keywords.some((k: string) => k.startsWith(cleanSearchText))
        )
          score += 8;
        else if (page.keywords.some((k: string) => k.includes(cleanSearchText)))
          score += 5;

        return { ...page, score };
      })
      .filter(
        (p: BrowserPage & { score: number }) =>
          p.score > 0 || (isSpecialTrigger && !cleanSearchText),
      )
      .sort(
        (
          a: BrowserPage & { score: number },
          b: BrowserPage & { score: number },
        ) => b.score - a.score,
      );
    ...
    if (matchedBrowserPages.length > 0) {
      combinedResults.splice(
        1,
        0,
        ...(matchedBrowserPages as unknown as SearchResult[]),
      );
    }
