...
export function useBrowser(
  ...
  const [structuralVersion, setStructuralVersion] = useState(
    globalStructuralVersion,
  );
  ...
  const updateLocalState = useCallback(() => {
    ...
    setDataVersion((prev) =>
      prev === globalDataVersion ? prev : globalDataVersion,
    );
    setStructuralVersion((prev) =>
      prev === globalStructuralVersion ? prev : globalStructuralVersion,
    );
    ...
    setServerStatus((prev) => {
      ...
      const hasData =
        globalExtensionData && globalExtensionData.tabs.length > 0;
  ...
  // Delayed 1000ms after mount so it doesn't compete with initial render.
  useEffect(() => {
    if (isSearchActive) return;
    ...
    const startDelay = setTimeout(() => {
      ...
      ticker = setInterval(() => {
        ...
        const hasPlayingTab = globalExtensionData.tabs.some(
          (t) => t.paused === false && t.currentTime !== undefined,
        );
  ...
  const allDisplayTabs = useMemo(() => {
    ...
    return nextTabs.filter((t) => {
      ...
      const matchWindow =
        wFilter === "all" ||
        t.windowType === "popup" ||
        String(t.windowId) === String(wFilter);
      return matchBrowser && matchWindow;
    });
  }, [
    extensionData,
    structuralVersion,
    deferredBrowserFilter,
    deferredWindowFilter,
    isSearchActive,
  ]);
  ...
  const { normalSections, collapsedViewItems } = useMemo(() => {
    ...
    Object.entries(sections).forEach(([title, tabs]) => {
      const filtered = tabs.filter(
        (t) =>
          t.searchTitle.includes(lowerSearch) ||
          t.searchUrl.includes(lowerSearch),
      );
    ...
    const filteredItems = items.filter((item) => {
      if (item.type === "tab") {
        return (
          item.tab.searchTitle.includes(lowerSearch) ||
          item.tab.searchUrl.includes(lowerSearch)
        );
      }
      return (
    ...
    return {
      normalSections: filteredSections,
      collapsedViewItems: filteredItems,
    };
  ...
  const {
    ...
    allBrowsersTabs.forEach((t) => {
      ...
      if (!browsers.includes(bType)) {
        browsers.push(bType);
        iconMap[bType] =
          browserIconMapSource[bType] ||
          ICON_SEQUENCE[browsers.length % ICON_SEQUENCE.length];
      }
    ...
    const windowsByBrowser: Record<
      string,
      { id: string; name: string; tabCount: number; isPopup?: boolean }[]
    > = {};
    const browserToWindows = new Map<
      string,
      Map<string | number, DisplayTab[]>
    >();
    ...
    browserToWindows.forEach((wMap, browserType) => {
      ...
      windowsByBrowser[browserType] = sortedIds
        ...
        .map((wId, index) => {
          const tabsInWindow = wMap.get(wId)!;
          const activeTab =
            tabsInWindow.find((t) => t.isActive) || tabsInWindow[0];
          const useSmartNaming =
            (preferences.smartWindowNaming as boolean) || false;
    ...
    return {
      ...
      activeBrowserName:
        browsers.length === 1
          ? browserNames[browsers[0]] || "Browser"
          : browserNames[recentB] || "All",
      totalTabsCount: allBrowsersTabs.length,
      browserIconMap: iconMap,
      windowsByBrowser,
    };
  }, [structuralVersion, memoizedMergedTabs]);
  ...
  return {
    ...
    serverDown,
    groups: useMemo(
      () => Object.values(extensionData?.groups || EMPTY_GROUPS),
      [extensionData?.groups],
    ),
