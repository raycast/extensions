...
export default function ({ entry }: { entry: string }): JSX.Element {
  ...
  const getProgressColor = (remaining: number, period = 30): Color => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Only regenerate OTP codes when period resets, without calling gopass.show again
      const currentOtpUrls = Object.keys(otpCodes);
      let shouldRegenerate = false;

      currentOtpUrls.forEach((url) => {
        const { period } = parseOtpauthUrl(url);
        const remaining = getOTPRemainingTime(period);
        // Regenerate when we hit exactly the period boundary (new cycle)
        if (remaining === period) {
          shouldRegenerate = true;
        }
      });

      if (shouldRegenerate && currentOtpUrls.length > 0) {
        updateOTPCodes(currentOtpUrls);
      }
    }, 1000);
  ...
  return (
    <List isLoading={loading}>
      <List.Section title={"/" + entry}>
        ...
        {Object.keys(otpCodes).map((otpauthUrl, index) => {
        ...
        {details.filter(isOtpauthUrl).map((otpauthUrl, index) => (
          <List.Item
            ...
            actions={
              <ActionPanel>
                <Action
                  ...
                  icon={Icon.Clipboard}
                  onAction={() => copy("OTP Auth URL", otpauthUrl)}
                />
                <Action
                  title="Paste to Active App"
                  icon={Icon.Document}
                  onAction={() => paste("OTP Auth URL", otpauthUrl)}
                />
              </ActionPanel>
            }
          />
