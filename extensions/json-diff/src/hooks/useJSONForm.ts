...
export const useJSONForm = (onCompare: (original: unknown, modified: unknown) => void) => {
  ...
  const formatJSON = async () => {
    ...
    if (!origOk && !modOk) {
      ...
      await showToast({ style: Toast.Style.Success, title: "Formatted both JSONs" });
    } else {
      await showToast({ style: Toast.Style.Success, title: `Formatted ${origOk ? "Original" : "Modified"} JSON` });
    }
  };
