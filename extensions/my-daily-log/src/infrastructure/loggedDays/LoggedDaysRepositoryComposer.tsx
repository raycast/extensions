...
export class LoggedDaysRepositoryComposer implements LoggedDaysRepository {
  ...
  getLoggedDays(): LoggedDay[] {
    const allLoggedDays = this.repositories
      .map((repository) => repository.getLoggedDays())
      .flat();
    return allLoggedDays.filter((loggedDay, index, self) =>
      index === self.findIndex((t) => (
        t.date.getTime() === loggedDay.date.getTime()
      ))
    );
  }
}
