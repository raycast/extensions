...
test("rejects unsupported and duplicate recipients", () => {
  ...
  assert.throws(() => parseBulkRecipients("255712345678, +255712345678"), InputError);
});

test("requires a UUID sender ID", () => {
  assert.equal(requireSenderId("6addad95-f6c8-5929-8b5a-c8ef52067ea6"), "6addad95-f6c8-5929-8b5a-c8ef52067ea6");
  assert.throws(() => requireSenderId("MYBRAND"), InputError);
  assert.equal(requireSenderId("6addad95-f6c8-0929-0b5a-c8ef52067ea6"), "6addad95-f6c8-0929-0b5a-c8ef52067ea6");
});
...
test("reads the numbers header used by the contact sample CSV", async (context) => {
  ...
  await writeFile(file, "names,numbers\nAsha,255712345678\nBaraka,+255713456789\n");

  assert.deepEqual(await parseRecipientFile(file), ["+255712345678", "+255713456789"]);
});
...
test("reads a Raycast-style file URL and a suffixless cached CSV", async (context) => {
  ...
  assert.deepEqual(await parseRecipientFile(pathToFileURL(csv).href), ["+255712345678"]);
  assert.deepEqual(await parseRecipientFile(cachedCopy), ["+255712345678"]);
});

test("reads a headerless first column from a CSV file", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "notify-africa-sms-"));
  context.after(() => rm(directory, { recursive: true, force: true }));
  const file = join(directory, "recipients.csv");
  await writeFile(file, "712345678\n713456789\n");

  assert.deepEqual(await parseRecipientFile(file), ["+255712345678", "+255713456789"]);
});

test("reads quoted CSV values", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "notify-africa-sms-"));
  context.after(() => rm(directory, { recursive: true, force: true }));
  const file = join(directory, "recipients.csv");
  await writeFile(file, 'name,phone\n"Asha, Customer","+255712345678"\n');
...
test("explains how to use an Apple Numbers document", async () => {
  await assert.rejects(() => parseRecipientFile("/tmp/contact-import.numbers"), /Export the file as CSV/);
});

test("rejects XLSX files", async () => {
  await assert.rejects(() => parseRecipientFile("/tmp/contact-import.xlsx"), /XLSX files are not supported/);
});
