...
export function normalizeTanzanianPhone(value: string): string {
  ...
  if (!/^\+255\d{9}$/.test(phone)) {
    throw new InputError("Use a Tanzanian number such as 0712345678, 255712345678, or +255712345678.");
  }
...
export function validateContent(value: string): string {
  ...
  if (content.length > MAX_SMS_LENGTH) {
    throw new InputError(`Message text must be ${MAX_SMS_LENGTH} characters or fewer.`);
  }
  return content;
}
...
export function parsePersonalizedMessages(value: string): SmsMessage[] {
  ...
  const messages = lines.map((line, index) => {
    ...
    if (separator < 1) {
      throw new InputError(`Line ${index + 1} must use phone number | message.`);
    }
