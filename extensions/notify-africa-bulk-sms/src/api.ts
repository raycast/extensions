...
export function sendSingle(apiKey: string, senderId: string, message: SmsMessage): Promise<SingleSendResponse> {
...
export function sendBulk(apiKey: string, senderId: string, messages: SmsMessage[]): Promise<BulkSendResponse> {
  return request("/api/messages/send-bulk", apiKey, { senderId, messages });
}

async function request<T>(path: string, apiKey: string, body: unknown): Promise<T> {
  ...
  if (!response.ok) {
    throw new NotifyApiError(errorMessage(payload, response.status), response.status);
  }
