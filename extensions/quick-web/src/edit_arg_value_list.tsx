...
export default function EditArgValueList({argName, argValueList}: {
    argName: string,
    argValueList: string[] | undefined
}) {
    const items = [
        <List.Item key="addNewArg" title="Add New Arg Value" icon={"add.png"} actions={
            <ActionPanel>
        ...
        items.push(
            <List.Item key={argValue} title={argValue} icon={"list-item.png"} actions={
                <ActionPanel>
                    ...
                    <Action title="Delete Arg" onAction={
                        async () => {
