...
import {queryArgValueList, queryArgValueListErrIfNotExistOrEmpty} from "./arg_value_repository";
...
export default function Command(props: LaunchProps<{ arguments: Input }>) {
    ...
    urlConfigs?.urlConfigs.forEach((value, webName) => {
        ...
        items.push(
            <List.Item key={webName} title={webName} icon={"list-item.png"} actions={
                <UrlAction urlBuilder={builder}/>
            }/>
        )
    })
...
function NextList({argIndex, urlBuilder}: { argIndex: number; urlBuilder: UrlBuilder }) {
    ...
    for (const argValue of argValueList) {
        items.push(
            <List.Item key={argValue} title={argValue} icon={"list-item.png"} actions={
                <ActionPanel>
                    <Action.Push title={"Next Arg"}
    ...
    return <List searchBarPlaceholder={argName}>
        {items}
    </List>
}
...
function LastList(index: number, urlBuilder: UrlBuilder) {
    ...
    return <List searchBarPlaceholder={argName}>
        {
            valueList.map(value => (
                <List.Item key={value} title={value} icon={"list-item.png"} actions={
                    <ActionPanel>
...
class UrlBuilder extends UrlPattern {
    ...
    build() {
        ...
        urlPatternCopy = urlPatternCopy.replace("$$", encodeURI(this.customizedInput))
