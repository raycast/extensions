...
final class ScannerDelegate: NSObject, NSApplicationDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, NSWindowDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    do {
      if let iconPath, let appIcon = NSImage(contentsOfFile: iconPath) {
...
func runScanner(iconPath: String?) throws -> String? {
  let app = NSApplication.shared
  let delegate = ScannerDelegate(iconPath: iconPath)
...
func scanQRCode(iconPath: String?) throws -> String? {
  try runScanner(iconPath: iconPath)
}

func parseIconPath(from args: [String]) -> String? {
  var index = 0
  while index < args.count {
    if args[index] == "--icon-path" {
...
func main() {
  do {
    ...
    switch command {
    case "scanQRCode":
      let result = try scanQRCode(iconPath: parseIconPath(from: commandArgs))
