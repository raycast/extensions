...
if [ $# -ne 2 ]; then
  echo "Usage: $0 <gram_db_path> <target_version>"
  exit 1
fi
...
echo "Creating fixture for Gram version $TARGET_VERSION"

# Directory for fixtures
FIXTURES_DIR="test/fixtures"
mkdir -p "$FIXTURES_DIR"

NEW_DB="$FIXTURES_DIR/gram-db-v${TARGET_VERSION}.sqlite"
rm -f "$NEW_DB"
...
for i in $(seq 0 $TARGET_VERSION); do
  ...
  if [ -n "$MIGRATION" ]; then
    ...
    FIXED_MIGRATION=$(echo "$MIGRATION" | sed "s/\"\"/''/g" | sed "s/\"ssh\"/'ssh'/g")

    run_sqlite "$NEW_DB" "$FIXED_MIGRATION" || echo "⚠ Warning: Failed to apply step $i"
  fi
done
