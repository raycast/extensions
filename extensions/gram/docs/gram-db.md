# Gram Database Documentation

This document describes the Gram editor's SQLite database structure, specifically the workspace-related tables used by
this extension.

## Database Location

The Gram database is located at:

- **macOS:** `~/Library/Application Support/Gram/db/{version}/db.sqlite`

Where `{version}` corresponds to the Gram build:
...
Since Gram is an active application, the SQLite database is frequently locked or in WAL (Write-Ahead Logging) mode.

### Database Concurrency

Gram uses SQLite's WAL mode. When querying the database while Gram is open, you may encounter ```Database is locked``` errors during heavy write operations.

**The Solution**: The extension implements a fallback that assumes the version is supported if a lock is encountered, preventing UI stutters.

###### CLI Tip: Always use the --init /dev/null flag with the sqlite3 CLI to prevent the local .sqliterc from interfering with the output format.
...
SELECT MAX(step)
FROM migrations
WHERE domain = 'WorkspaceDb';
```

### Supported Versions

This extension supports **v30+** of the WorkspaceDb schema.
The v30 schema uses the `remote_connections` table, and includes the `paths_order` field for multi-folder workspace
support.
...
Stores SSH and WSL connection information.
...
Acts as a general-purpose settings and state repository
...
### Implementation Logic: Reconstructing Workspace Folders

To display folders in the correct UI order, follow this logic:

1) Split the ```paths``` string by the newline character ```\n```.

2) Split the ```paths_order``` string by commas ```,```.

3) Map the indices to the paths.

#### TypeScript Example:

```ts
const pathList = row.paths.split("\n");
const order = row.paths_order.split(",").map(Number);

// Reconstruct the ordered array
const orderedPaths = order.map(index => pathList[index]);
```
...
Gram does not automatically prune the workspaces table. This means the query will return workspaces from months ago. The extension relies on ```ORDER BY timestamp DESC``` to keep the most relevant projects at the top. Projects without a ```window_id``` are considered "closed" or "recent," while those with an active ```window_id``` (matched via ```session_window_stack```) are "active."
...
```
**Note**: If Gram crashes, ```session_window_stack``` may still contain IDs for windows that are no longer open. The extension handles this by checking if the ```session_id``` is still current.