...
export default function createAlias(props: LaunchProps<{ arguments: DomainArgs }>) {
  ...
  const { isLoading: isCreating, error: createError, revalidate: create } = useFetch(API_URL + `domains/${values.domain}/aliases`, {
    method: "POST",
    headers: API_HEADERS,
    body: JSON.stringify({
      alias: values.alias,
  ...
  return error ? (
    ...
    >
      <Form.Dropdown
        ...
        {...itemProps.domain}
      >
        {data?.domains.filter((domain) => !domain.banned && domain.active)
          .map((domain) => (
