...
export function RolesAndDatabases({ projectId, branchId }: { projectId: string, branchId: string }) {
    ...
    return <List isLoading={isLoading}>
      ...
      <List.Section title="Databases">
        {databases.map(database => <List.Item key={database.id} icon={Icon.Coin} title={database.name} actions={<ActionPanel>
          <Action.Push icon={{ source: "table.svg", tintColor: Color.PrimaryText }} title="View Schema" target={<DatabaseSchema projectId={projectId} database={database} />} />
  ...
  function DatabaseSchema({projectId, database}: {projectId: string, database: Database}) {
    ...
    return <Detail isLoading={isLoading} markdown={sql} actions={<ActionPanel>
