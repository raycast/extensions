...
export function ListComputes({ projectId, branchId }: { projectId: string, branchId: string }) {
  ...
  {endpoints?.map(endpoint => <List.Item key={endpoint.id} icon={Icon.ComputerChip} title={endpoint.id} accessories={[
    { text: `${endpoint.autoscaling_limit_max_cu} CU`, tooltip: `Size: ${endpoint.autoscaling_limit_max_cu} vCPU` },
