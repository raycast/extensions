import { Action, ActionPanel, Alert, Clipboard, Color, confirmAlert, Form, Icon, Keyboard, List, showToast, Toast, useNavigation } from "@raycast/api";
...
export default function ListAPIKeys() {
    ...
    return <List isLoading={isLoading}>
        {data.map(key => <List.Item key={key.id} icon={Icon.Key} title={key.name} accessories={[{icon: key.created_by.image || getAvatarIcon(key.created_by.name.split(" ")[0]), date: new Date(key.created_at), tooltip: `By ${key.created_by.name} on ${formatDate(key.created_at)}`}, { text: `Last used: ${key.last_used_at ? formatDate(key.last_used_at) : "never"}` }]} actions={<ActionPanel>
            ...
            {/* eslint-disable-next-line @raycast/prefer-title-case */}
            <Action shortcut={Keyboard.Shortcut.Common.Remove} icon={{ source: Icon.Warning, tintColor: Color.Yellow }} title="Revoke API Key" onAction={() => confirmAndRevoke(key.id)} />
        </ActionPanel>} />)}
    </List>
}
...
validation: {
    ...
    return <Form isLoading={isLoading} actions={<ActionPanel>
        ...
        <Form.TextField title="Key name" placeholder="mykey" info="A user-specified API key name. This value is required when creating an API key." {...itemProps.key_name} />
    </Form>
}