      ...
      {branches.map((branch) => {
        ...
        return (
          <List.Item
            ...
            actions={<ActionPanel>
              <Action.Push icon={Icon.Coin} title="View Roles & Databases" target={<RolesAndDatabases projectId={id} branchId={branch.id} />} />
              <Action.Push icon={Icon.ComputerChip} title="View Compute Endpoints" target={<ListComputes projectId={id} branchId={branch.id} />} />
            </ActionPanel>}
          />
        );
      })}
...
function UpdateProject({project, mutate}: {project: ProjectListItem, mutate: MutatePromise<ProjectListItem[]>}) {
  ...
  return <Form actions={<ActionPanel>
    ...
    <Form.Separator />
    <Form.Checkbox label="Logical Replication" info="Logical replication lets you replicate data changes from Neon to external data services and platforms." {...itemProps.enable_logical_replication} />
    <Form.Description title="⚠️" text="Enabling logical replication:
