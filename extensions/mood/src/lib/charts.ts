...
export function generateSparkline(entries: MoodEntry[], width = 600, height = 300) {
  ...
  const margin = { top: 10, right: 10, bottom: 25 };
  ...
  const points = values.map((value, index) => {
    ...
    const y = margin.top + chartHeight - ((value - min) / range) * chartHeight;
    return [x, y];
  });

  // Calculate where the neutral line should be (4 on our scale)
  const neutralY = margin.top + chartHeight - ((4 - min) / range) * chartHeight;
...
export function generateMoodDistribution(entries: MoodEntry[], width = 600, height = 300) {
  ...
  const barWidth = chartWidth / (sortedMoods.length || 1);
