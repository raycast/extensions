...
* you can find all error code in here. (https://ai.youdao.com/DOCSIRMA/html/自然语言翻译/API文档/文本翻译服务/文本翻译服务-API文档.html)`;
  ...
  if (content.split(" ").length == 1) {
    return <List isLoading={translate_result.errorCode === "" && translate_result.isWord}>
    ...
    return <Detail isLoading={translate_result.errorCode === "" && !translate_result.isWord} markdown={result} actions={
      <TranslateResultActionPanel copy_content={result}
                                  url={translate_result.webdict && translate_result.webdict.url ? translate_result.webdict.url : undefined} />
    } />;
  }
}
      ...
      <Form actions={
        <ActionPanel title="Translate">
          <SubmitFormAction title="Translate" onSubmit={(input: translateRequest) => {
            push(
                <Translate content={input.content} to_language={input.to_language}
                           from_language={input.from_language} />
            );
          }} />
        </ActionPanel>
      }>
        <Form.TextArea title="Content" id="content"
