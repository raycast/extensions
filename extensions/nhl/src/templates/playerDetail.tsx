...
import { PlayerDetailResponse, GoalieStats, SkaterStats } from "../utils/types";
...
const summaryStats = function(
	...
	if(player.position === 'G' && isGoalieStats(stats)) {
		...
		table += `|---|---|---|---|---|---|\n`;
		table += `| ${stats.gamesPlayed} | ${stats.wins} | ${stats.losses} | ${stats.shutouts} | ${(stats.goalsAgainstAvg).toFixed(3)} | ${(100 * stats.savePctg).toFixed(2)}% |`;
	}
...
export default function PlayerDetail({ id }: { id: number }) {
	...
	if (playerData.isLoading) return <Detail isLoading={true} markdown={userInterface.loading[getLanguageKey()]}/>;

	if (!playerData.data) return <Unresponsive />;

	const player = playerData.data;

	const heroImage = player.heroImage ? `![${player.firstName.default} ${player.lastName.default}](${player.heroImage})` : '';

	let playerMarkdown = `\n # ${player.firstName.default} ${player.lastName.default} \#${player.sweaterNumber}, ${player.position} <img src="${player.headshot}" width="50" height="50" /> ${player.inHHOF ? '<img src="https://assets.nhle.com/badges/hockey_hof.svg" width="50" height="50" />' : ''} ${player.inTop100AllTime ? '<img src="https://assets.nhle.com/badges/100_greatest_players.svg" width="50" height="50">' : ''} \n`;
	...
	const birthday = function(player: PlayerDetailResponse): string {
		let birthday = '';
		birthday += player.birthDate ? `${playerTitleStrings.born[lang]}: ${player.birthDate}` : '';
