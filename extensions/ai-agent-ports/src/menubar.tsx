  ...
  if (error && isCliNotFound(error)) {
    return (
      <MenuBarExtra
        ...
        tooltip="ports CLI not found — setup required"
        isLoading={isLoading}
      >
        <MenuBarExtra.Section title="ports CLI Not Found">
          <MenuBarExtra.Item
            title="Copy Install Command"
            subtitle={PORTS_INSTALL_COMMAND}
            icon={Icon.Clipboard}
            onAction={async () => {
              await Clipboard.copy(PORTS_INSTALL_COMMAND);
              await showHUD("Copied — paste in Terminal to install");
            }}
          />
          <MenuBarExtra.Item
            title="Open portscli.com"
            icon={Icon.Globe}
            onAction={() => open(PORTS_WEBSITE)}
          />
        </MenuBarExtra.Section>
        <MenuBarExtra.Section>
          <MenuBarExtra.Item
            title="Set Custom Binary Path…"
            icon={Icon.Gear}
