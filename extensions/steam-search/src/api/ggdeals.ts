...
export async function batchFetchGGDeals(appIds: number[], apiKey: string, region: string): Promise<void> {
  ...
  try {
    ...
    const data = await res.json() as { data?: Record<string, { prices?: { currentKeyshops?: string } }> };

    for (const id of missing) {
      const game = data?.data?.[String(id)];
      if (game?.prices) {
        const keyshop = parseFloat(game.prices.currentKeyshops ?? "");
        ggDealsCache.set(`${id}-${region}`, !isNaN(keyshop) ? `🔑 ${keyshop.toFixed(2).replace(".", ",")}${CURRENCY_SYMBOLS[region] ?? "€"}` : null);
      } else {
