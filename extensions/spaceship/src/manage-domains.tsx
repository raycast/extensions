...
function CreateDNSRecord({domain, onRecordAdded}: { domain: DomainInfo, onRecordAdded: () => void }) {
  ...
  const { handleSubmit, itemProps, values } = useForm<{
    ...
    validation: {
      ...
      preference(value) {
        if (values.type==="MX" && (!value || !Number(value) || +value<0 || +value>65535)) return "Enter a value between 0 and 65535, only integer";
      },
    }
  })

  const TYPE_SPECIFIC_FIELDS: { [type: string]: { value: { placeholder: string, info: string } } } = {
    A: {
      value: {
  ...
  return <Form isLoading={isLoading} actions={<ActionPanel>
    ...
    <Form.Separator />
    <Form.TextField title="Host" placeholder="@" info="Enter @ to create a record for yourdomain.com. Enter blog to create a record for blog.yourdomain.com." {...itemProps.name} />
    <Form.TextField title="Value" placeholder={TYPE_SPECIFIC_FIELDS[values.type].value.placeholder} info={TYPE_SPECIFIC_FIELDS[values.type].value.info} {...itemProps.value} />
    {values.type==="MX" && <Form.TextField title="Preference" placeholder="0 - 65535" info="Preference (distance) number of mail server" {...itemProps.preference} />}
    <Form.Dropdown title="Time To Live (TTL)" info="Specifies the amount of time in seconds that a DNS record should be cached by a resolver or a caching server before it expires and needs to be refreshed from the authoritative DNS servers" {...itemProps.ttl}>
