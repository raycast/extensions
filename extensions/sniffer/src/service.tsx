...
const getPerformanceColor = (performance: number): string => {
  const normallizedPerformance = Math.min(255, Math.max(0, performance));
  const red = Math.min(255, Math.max(0, 255 - normallizedPerformance));
  const green = Math.min(255, Math.max(0, normallizedPerformance));
  ...
  try {
    ...
    const script = `
      with timeout of 5 seconds
        do shell script "/usr/sbin/arp -a -i bridge100 || ip neigh"
      end timeout
    `;
...
const estimateNetworkPerformance = async (ip: string): Promise<number> => {
  const script = `
      with timeout of 5 seconds
        do shell script "ping -c 5 -i 0.2 -q ${ip} -I bridge100 || ping -c 5 -i 0.2 -q ${ip}"
      end timeout
    `;
  const stdout = await runAppleScript<string>(script);
  const lines = stdout.split("\r");
...
const extractBridgeAddresses = (lines: string[]) => {
  const regex = /\?\s+\(((?:[0-9]{1,3}\.){3}[0-9]{1,3})\)\s+at\s+([0-9a-f:]+)\s+on\s+bridge\d+\s+ifscope\s+\[bridge\]/i;
