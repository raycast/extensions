...
type Input = {
  /**
   * The amount of money received. MUST be a number or string number (e.g., "50000", "1500000").
   * Do NOT use shortcuts like '50k' or '3tr'. Convert them to full numeric values.
   */
  amount: string;
  /**
   * Description of the income (e.g., "salary", "lương tháng 12").
   */
  description: string;
  /**
...
export default async function addIncome(input: Input) {
  ...
  const incomeTags = allTags.filter((t) => t.type === "income");
  ...
  if (categoryId) {
    const found = incomeCategories.find((c) => c.id === categoryId);
    if (found) {
  ...
  if (tagIds) {
    const ids = tagIds
      ...
      .filter(Boolean);
    for (const id of ids) {
      const found = incomeTags.find((t) => t.id === id);
      if (found) {
  ...
  return {
    success: true,
    message: `Đã thêm thu nhập: ${description} - ${formatDisplayAmount(parsedAmount, currency)}`,
    ...
    availableCategories: incomeCategories.map((c) => ({ id: c.id, name: c.name })),
