...
export default function AWSProfileDropdown({ onProfileSelected }: Props) {
  ...
  // Inject MFA session credentials if available
  const { isValid: hasMfaSession } = useMfaSession();
  ...
  useEffect(() => {
    // Skip profile-based auth when MFA session is active
    if (hasMfaSession) {
    ...
    onProfileSelected?.();
  }, [selectedProfile, isUsingAwsVault, hasMfaSession]);
