...
export function useChat<T extends Chat>(props: T[]): ChatHook {
  ...
  async function handleStream(
    ...
    tools: any[],
    chatAnthropic: any,
    setData: (fn: (prev: Chat[]) => Chat[]) => void,
    setStreamData: (chat: Chat) => void
  ) {
        ...
        switch (event.type) {
          ...
          case "content_block_stop":
            if (currentContentBlock?.type === "tool_use") {
              ...
              messages.push({
                role: "assistant",
                content: [currentContentBlock] as any,
              });
              ...
              await handleStream(messages, chat, model, tools, chatAnthropic, setData, setStreamData);
              return;
            }
            currentContentBlock = null;
            break;
  ...
  async function ask(question: string, model: Model) {
    ...
    if (useStream) {
      ...
      try {
        ...
        await handleStream(initialMessages, chat, model, tools, chatAnthropic, setData, setStreamData);
        setLoading(false);
