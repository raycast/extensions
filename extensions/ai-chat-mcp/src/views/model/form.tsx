...
export const ModelForm = (props: { model?: Model; use: { models: ModelHook }; name?: string }) => {
  ...
  const [selectedModel, setSelectedModel] = useState(model?.model_id ?? "claude-3-haiku-20240307");
  ...
  const { handleSubmit, itemProps, setValue } = useForm<Model>({
    ...
    initialValues: {
      ...
      model_id: model?.model_id ?? "claude-3-haiku-20240307",
      prompt: model?.prompt ?? "",
      pinned: model?.pinned ?? false,
    },
  });

  const MODEL_OPTIONS = use.models.data.map((model) => ({
    ...
    >
      ...
      <Form.Dropdown
        ...
        onChange={(newValue) => {
          ...
          setValue("model_id", newValue);
        }}
      >
        {MODEL_OPTIONS.map((option) => (
          <Form.Dropdown.Item
            value={option.value}
            title={`${option.title}${option.description ? ` - ${option.description}` : ""}`}
            key={option.value}
          />
        ))}
      </Form.Dropdown>
