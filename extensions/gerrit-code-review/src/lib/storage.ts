...
export const saveGerrit = async (gerritList: Gerrit[]) => {
  await LocalStorage.setItem(GerritKey, JSON.stringify(gerritList));
};

export const listGerrit = async (): Promise<Gerrit[]> => {
  const gerritList = await LocalStorage.getItem(GerritKey);
  return JSON.parse(gerritList ? gerritList.toString() : "[]") as Gerrit[];
};
...
export const addGerrit = async (gerrit: Gerrit) => {
