...
export class GerritAPI {
  ...
  public async getChanges(q: string) {
    const instanceUrl = new URL(this.gerrit.url);
    instanceUrl.pathname += this.gerrit.authorized
      ? "/a/changes/"
      : "/changes/";
    // If user is authenticated with API, return results mimicking personal dashboard
    if (!q && this.gerrit.authorized) {
      instanceUrl.search =
        "q=(owner:self AND is:open) OR (attention:self) OR (reviewer:self AND is:open)";
    }
    ...
    GerritChangesParameters.map((gerritParam: string) =>
      instanceUrl.searchParams.append("o", gerritParam)
    );
    ...
    return openChangees.map((change: Change) => {
      ...
      change.url =
        `${this.gerrit.url}/c/${change.project}/+/${change._number}`.replace(
          /([^:]\/)\/+/g,
          "$1"
        );
      ...
      change.committer =
        change.revisions[change.current_revision].commit.committer;
      ...
      change.mergeable =
        change.mergeable === undefined || change.mergeable ? true : false;
      ...
      requirementsStatus.indexOf("NOT_APPLICABLE") !== -1 &&
        requirementsStatus.splice(
          requirementsStatus.indexOf("NOT_APPLICABLE"),
          1
        );
      change.submitRequirementsMet =
        requirementsStatus.length > 0
          ? requirementsStatus.length == 1 &&
            requirementsStatus[0] == "SATISFIED" &&
  ...
  public async getProjects(projectQuery: string) {
    ...
    for (const project in projects) {
      projects[project].url =
        `${this.gerrit.url}/admin/repos/${projects[project].id}`.replace(
          /([^:]\/)\/+/g,
          "$1"
        );
  ...
  public async getProjectBranches(project: Project) {
    ...
    return listBranches.map((branch: ProjectBranch) => {
      ...
      branch.name =
        remoteRefs.length > 1 ? remoteRefs[remoteRefs.length - 1] : branch.ref;
      return branch;
    });
  }
  ...
  async request(url: URL, init?: RequestInit) {
    ...
    if (this.gerrit.password) {
      ...
      headers.append(
        ...
        "Basic " +
          Buffer.from(
            `${this.gerrit.username}:${this.gerrit.password}`
          ).toString("base64")
      );
    }
