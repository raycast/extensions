...
import { Gerrit } from "../lib/gerrit";
import { addGerrit, deleteGerrit, listGerrit } from "../lib/storage";
import { useState, useCallback, useEffect } from "react";
import { format } from "date-fns";

export function gerritMenu(menuAction: string): JSX.Element {
  const [isLoading, setIsLoading] = useState(true);
  const [gerritList, setGerritList] = useState<Gerrit[]>([]);
...
function GerritItem(props: {
  gerrit: Gerrit;
  setGerritList: (f: (v: Gerrit[]) => Gerrit[]) => void;
  actionItem: string;
}) {
