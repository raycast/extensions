...
function Leads({gaViewId}: {gaViewId: number}) {
    ...
    const {isLoading, data: leads, pagination} = useFetch(options => API_URL + `leads?ga_view_id=${gaViewId}&page=${options.page+1}`, {
        headers,
        async parseResponse(response) {
            const result = await response.json();
            if (!response.ok) throw new Error((result as {message: string}).message);
    ...
    return <List isLoading={isLoading} isShowingDetail={isShowingDetail} pagination={pagination}>
{leads.map(lead => <List.Item key={lead.id} icon={lead.logo} title={lead.name} subtitle={isShowingDetail ? undefined : `${lead.city}, ${lead.state}, ${lead.country}`} accessories={isShowingDetail ? undefined : [{icon: Icon.Eye, text: lead.pages_viewed.toString(), tooltip: `Views: ${lead.pages_viewed}`}, {icon: Icon.Clock, text: lead.time_on_page, tooltip: `Time spent: ${lead.time_on_page}`}]} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
    ...
    <List.Item.Detail.Metadata.Label icon={Icon.Calendar} title="Last Visited" text={formatDateString(lead.last_visited_at)} />
    ...
    {lead.website ? <List.Item.Detail.Metadata.Link title="Website" text={lead.website} target={lead.website} /> : <List.Item.Detail.Metadata.Label icon={Icon.Globe} title="Website" text="N/A" />}
    <List.Item.Detail.Metadata.TagList title="Industry">
        {lead.industry.map(i => <List.Item.Detail.Metadata.TagList.Item key={i} text={i} />)}
    </List.Item.Detail.Metadata.TagList>
