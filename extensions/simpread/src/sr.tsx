import { ActionPanel, OpenInBrowserAction, Icon, ImageMask, List, PushAction, Detail,
        getLocalStorageItem, getPreferenceValues,
...
function getDetail( unread: Unread ) {
    ...
    unread.annotations && unread.annotations.forEach( ( annote: Annote ) => {
        let tags    = '',
            content = '';
            ...
            try {
                const favicon = 'https://simpread-1254315611.cos.ap-shanghai.myqcloud.com/mobile/apple-icon-180x180.png',
                    ...
                    .map( ( unread: Unread ) => {
                        ...
                        return { id: unread.idx, title: unread.title, icon: icon, lastModifiedAt: now, desc: unread.desc, url: unread.url, markdown: getDetail( unread ) };
                    })
                            ...
                            actions={
                                    <ActionPanel>
                                        <ActionPanel.Section>
                                            ...
                                            />
                                            <OpenInBrowserAction title="Open Local File" url={ 'http://localhost:7026/reading/' + info.id } icon={ Icon.Desktop } shortcut={{ modifiers: [ 'cmd' ], key: 'l' }}/>
                                        </ActionPanel.Section>
                                    </ActionPanel>
                            }
                        />
