import { LocalStorage, List, ActionPanel, Action, Icon, launchCommand, LaunchType, useNavigation, Form, Image, confirmAlert, Alert, closeMainWindow } from "@raycast/api";
import { installDefaultWeights } from "./defaults";
import { useEffect, useState } from "react";
import { getFavicon } from "@raycast/utils"

const EditDestinationForm = (props: { destination: string, weight: number, onSubmit: (destination: string, weight: number) => void }) => {
  ...
  return <Form
    actions={
      <ActionPanel>
        <Action.SubmitForm onSubmit={async (values: { urlPathField: string, weightField: string }) => {
      ...
      actions={
        <ActionPanel>
          <ActionPanel.Section title="This Destination">
            <Action title="Jump" icon={Icon.ArrowRightCircle} shortcut={{ modifiers: ["cmd"], key: "j" }} onAction={async () => {
              await closeMainWindow()
              await launchCommand({ name: "jump", type: LaunchType.Background, arguments: { destination: target }})
            }} />

            <Action title="Edit Destination" icon={Icon.Pencil} shortcut={{ modifiers: ["cmd"], key: "e" }} onAction={() => push(<EditDestinationForm destination={target} weight={weight} onSubmit={async () => {
              setDestinations(await LocalStorage.allItems())
              pop()
            }} />)} />

            <Action title="Delete Destination" icon={Icon.Trash} shortcut={{ modifiers: ["cmd"], key: "d" }} style={Action.Style.Destructive} onAction={async () => {
          ...
          <ActionPanel.Section title="General">
            <Action title="New Destination" icon={Icon.PlusCircle} shortcut={{ modifiers: ["cmd"], key: "n" }} onAction={() => push(<EditDestinationForm destination="" weight={1} onSubmit={async () => {
              setDestinations(await LocalStorage.allItems())
              pop()
            }} />)} />
          </ActionPanel.Section>
