...
async function getHAWSCurrentUser(): Promise<HAUser | undefined> {
  ...
  const picture = person?.picture && person?.picture.startsWith("/") ? apex.urlJoin(person?.picture) : person?.picture;
  const r: HAUser = {
        ...
        <List.Section title="Conversation">
          {conversations?.map((c, i) => (
            <List.Item
              ...
              title={c.text}
              icon={{
                value: c.author === Author.Assist ? "apex-connect.png" : userPicture(),
