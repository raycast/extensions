...
export function FanSpeedControlAction(props: { state: State }): JSX.Element | null {
  ...
  const handle = async (pvalue: number) => {
    await apex.callService("fan", "turn_on", { entity_id: state.entity_id, percentage: `${pvalue}` });
  };
  ...
  return (
    <ActionPanel>
      <ActionPanel.Section title="Controls">
        <Action
          title="Toggle"
          onAction={async () => await apex.toggleFan(props.state.entity_id)}
          icon={{ source: "toggle.png", tintColor: Color.PrimaryText }}
        />
        <Action
          title="Turn On"
          shortcut={{ modifiers: ["cmd"], key: "o" }}
          onAction={async () => await apex.turnOnFan(props.state.entity_id)}
          icon={{ source: "power-btn.png", tintColor: Color.Green }}
        />
        <Action
          title="Turn Off"
          shortcut={{ modifiers: ["cmd"], key: "f" }}
          onAction={async () => await apex.turnOffFan(props.state.entity_id)}
