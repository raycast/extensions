...
function MediaPlayerPlayPauseMenubarItem(props: { state: State }): JSX.Element | null {
  const s = props.state;
  const onAction = s.state === "off" || s.state === "unavailable" ? undefined : () => apex.playPauseMedia(s.entity_id);
  return (
    <MenuBarExtra.Item
...
function MediaPlayerNextMenubarItem(props: { state: State }): JSX.Element | null {
  ...
  return <MenuBarExtra.Item title="Next" icon={Icon.Forward} onAction={() => apex.nextMedia(props.state.entity_id)} />;
}
...
function MediaPlayerPreviousMenubarItem(props: { state: State }): JSX.Element | null {
  ...
  return (
    <MenuBarExtra.Item title="Previous" icon={Icon.Rewind} onAction={() => apex.previousMedia(props.state.entity_id)} />
  );
}
