...
export function BrightnessControlAction(props: { state: State }): JSX.Element | null {
  ...
  const handle = async (bvalue: number) => {
    await apex.callService("light", "turn_on", { entity_id: state.entity_id, brightness_pct: `${bvalue}` });
  };
...
export function ColorTempControlAction(props: { state: State }): JSX.Element | null {
  ...
  const handle = async (K: number) => {
    await apex.callService("light", "turn_on", { entity_id: state.entity_id, kelvin: `${K}` });
  };
...
export function ColorRgbControlAction(props: { state: State }): JSX.Element | null {
  ...
  const handle = async (color_rgb: RGB) => {
    await apex.callService("light", "turn_on", {
  ...
  return (
    <ActionPanel>
      <ActionPanel.Section title="Controls">
        <Action
          title="Toggle"
          onAction={async () => await apex.toggleLight(props.state.entity_id)}
          icon={{ source: "toggle.png", tintColor: Color.PrimaryText }}
        />
        <Action
          title="Turn On"
          shortcut={{ modifiers: ["cmd"], key: "o" }}
          onAction={async () => await apex.turnOnLight(props.state.entity_id)}
          icon={{ source: "power-btn.png", tintColor: Color.Green }}
        />
        <Action
          title="Turn Off"
          shortcut={{ modifiers: ["cmd"], key: "f" }}
          onAction={async () => await apex.turnOffLight(props.state.entity_id)}
