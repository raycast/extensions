...
export default function ListDomains() {
  ...
  return <List isLoading={isLoading}>
    {domains.map(domain => <List.Item key={domain.id} icon={getFavicon(`https://${domain.domain}`, { fallback: Icon.Globe })} title={domain.domain} accessories={[ {tag: {value: "AUTO_RENEW", color: domain.auto_renew ? Color.Green : Color.Red}} ]} actions={<ActionPanel>
...
function ViewDomainZone({domain}: {domain: Domain}) {
  ...
  return <Detail isLoading={isLoading} markdown={`Domain: ${domain.domain}` + (!zone ? "" : `\n\n Name Servers: \n\n ${zone.name_servers.join("\n")}`)} metadata={zone && <Detail.Metadata>
