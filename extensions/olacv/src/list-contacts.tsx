...
export default function ListContacts() {
  ...
  const { isLoading, data: contacts, revalidate } = useFetch(API_URL + "contacts", {
    headers: API_HEADERS,
  ...
  return <List isLoading={isLoading} isShowingDetail={isShowingDetail}>
    {!isLoading && !contacts.length && <List.EmptyView description="No Results" actions={<ActionPanel>
      <Action.Push icon={Icon.AddPerson} title="Create Contact" target={<CreateContact onCreate={revalidate} />} />
    </ActionPanel>} />}
    {contacts.map(contact => <List.Item key={contact.id} icon={Icon.Person} title={contact.name} subtitle={isShowingDetail ? undefined: contact.email} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
      <List.Item.Detail.Metadata.Label title="ID" text={contact.id} />
      <List.Item.Detail.Metadata.Label title="Name" text={contact.name} />
      <List.Item.Detail.Metadata.Link title="Email" text={contact.email} target={`mailto:${contact.email}`} />
...
function CreateContact({ onCreate }: {onCreate: () => void}) {
  ...
  return <Form isLoading={isLoading} actions={<ActionPanel><Action.SubmitForm icon={Icon.AddPerson} title="Create Contact" onSubmit={handleSubmit} /></ActionPanel>}>
