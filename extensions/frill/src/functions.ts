...
export function generateAnnouncementMarkdown(content: AnnouncementContentObject[], excludeTitle=false) {
    const arrContent = content.map(c => {
    ...
    if (excludeTitle)
        return arrContent.slice(1).join("\n\n");
    else
