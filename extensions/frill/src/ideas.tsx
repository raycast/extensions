    ...
    <List.Dropdown.Section title="Status">
      {Object.entries(IDEA_FILTERS).map(([key, val]) => <List.Dropdown.Item key={key} title={val} value={key} />)}
        ...
        {filteredIdeas?.map(idea => <List.Item key={idea.idx} title={idea.name} icon={{ source: Icon.Circle, tintColor: idea.status?.color || undefined }} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
            ...
            <List.Item.Detail.Metadata.Label title="Is Private" icon={idea.is_private ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Is Bug" icon={idea.is_bug ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Is Within Roadmap" icon={idea.show_in_roadmap ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Is Archived" icon={idea.is_archived ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Is Shortlisted" icon={idea.is_shortlisted ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Is Completed" icon={idea.is_completed ? Icon.Check : Icon.Multiply} />
            <List.Item.Detail.Metadata.Label title="Approval Status" text={idea.approval_status} />
            <List.Item.Detail.Metadata.Label title="Created At" text={idea.created_at || ""} icon={idea.created_at ? undefined : Icon.Minus} />
            <List.Item.Detail.Metadata.Label title="Updated At" text={idea.updated_at || ""} icon={idea.updated_at ? undefined : Icon.Minus} />
            <List.Item.Detail.Metadata.Label title="Excerpt" text={idea.excerpt} />
            <List.Item.Detail.Metadata.Label title="Is Pinned" icon={idea.is_pinned ? Icon.Check : Icon.Multiply} />

            <List.Item.Detail.Metadata.Label title="Created By" text={idea.created_by || ""} icon={idea.created_by ? undefined : Icon.Minus} />
            ...
            <List.Item.Detail.Metadata.Link title="Author Email" text={idea.author.email} target={`mailto:${idea.author.email}`} />
            ...
            {idea.status ? <Fragment>
                ...
                <List.Item.Detail.Metadata.Label title="Status Color" text={idea.status.color} icon={{ source: Icon.CircleFilled, tintColor: idea.status.color }} />
                <List.Item.Detail.Metadata.Label title="Status Is Completed" icon={idea.status.is_completed ? Icon.Check : Icon.Multiply} />
            </Fragment> : <List.Item.Detail.Metadata.Label title="STATUS" icon={Icon.Minus} />}
            
            <List.Item.Detail.Metadata.Separator />
            <List.Item.Detail.Metadata.Label title="TOPICS" />
            <List.Item.Detail.Metadata.Label title="---" />
            {idea.topics.map((topic, topicIndex) => <Fragment key={topic.idx}>
            ...
            <Action.Push title="Update Idea" icon={Icon.Pencil} target={<UpdateIdea initialIdea={idea} onIdeaUpdated={getIdeasFromApi} />} />
            <Action title="Delete Idea" style={Action.Style.Destructive} icon={Icon.DeleteDocument} onAction={() => confirmAndDeleteIdea(idea)} />
            <ActionPanel.Section>
                <Action.Push title="Create New Idea" icon={Icon.Plus} target={<CreateNewIdea onIdeaCreated={getIdeasFromApi} />} shortcut={{ modifiers: ["cmd"], key: "n" }} />
            </ActionPanel.Section>
        </ActionPanel>} />)}
        {!isLoading && <List.Section title="Actions">
            <List.Item title="Create New Idea" icon={Icon.Plus} actions={<ActionPanel>
                <Action.Push title="Create New Idea" icon={Icon.Plus} target={<CreateNewIdea onIdeaCreated={getIdeasFromApi} />} shortcut={{ modifiers: ["cmd"], key: "n" }} />
            </ActionPanel>} />
        </List.Section>}
    </List>
...
export function CreateNewIdea({ onIdeaCreated }: CreateNewIdeaProps) {
    ...
    const { handleSubmit, itemProps } = useForm<CreateIdeaRequest>({
      ...
      return <Form isLoading={isLoading} actions={<ActionPanel>
        ...
        <Form.TextField title="Name" placeholder="New Idea" {...itemProps.name} info="One sentence that summarizes your Idea" />
        <Form.Dropdown title="Author" {...itemProps.author_idx}>
            {followers?.map(follower => <Form.Dropdown.Item key={follower.idx} title={follower.name} value={follower.idx} icon={follower.avatar || getAvatarIcon(follower.name)} />)}
        </Form.Dropdown>
        <Form.TextField title="Description" placeholder="Description of New Idea" {...itemProps.description} info="Why your idea is useful, who would benefit and how it should work?" />
        <Form.Separator />
...
function UpdateIdea({ initialIdea, onIdeaUpdated }: UpdateIdeaProps) {
  ...
  const { handleSubmit, itemProps } = useForm<UpdateIdeaRequest>({
    async onSubmit(values) {
      const params = values;
      if (!values.topic_idxs) params.topic_idxs = [null];
  ...
  return <Form isLoading={isLoading} actions={<ActionPanel><Action.SubmitForm title="Submit" icon={Icon.Check} onSubmit={handleSubmit} /></ActionPanel>}>
    <Form.TextField title="Name" placeholder="New Idea" {...itemProps.name} info="One sentence that summarizes your Idea" />
    <Form.Dropdown title="Author" {...itemProps.author_idx}>
        {followers?.map(follower => <Form.Dropdown.Item key={follower.idx} title={follower.name} value={follower.idx} icon={follower.avatar || getAvatarIcon(follower.name)} />)}
    </Form.Dropdown>
