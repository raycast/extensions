...
import { deleteFollower, getFollowers } from "./api";
...
export default function Followers() {
    ...
    return error ? <ErrorComponent error={error} /> : <List isLoading={isLoading} isShowingDetail>
        {followersResponse?.data.map(follower => <List.Item key={follower.idx} title={follower.name} icon={follower.avatar || getAvatarIcon(follower.name)} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
