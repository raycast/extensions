...
// The current synthetic corpus contains high-confidence but semantically wrong
// outputs, so token confidence is not yet safe for automatic delivery. Keep all
// results behind review until a real paper/PDF calibration corpus is available.
export const DEFAULT_MEAN_CONFIDENCE = 0.995;
export const DEFAULT_MIN_CONFIDENCE = 0.9;
export const REQUIRE_REVIEW_UNTIL_CALIBRATED = true;
...
export function postProcessLatex(input: string): string {
  ...
  return output.replace(/\uE000(\d+)\uE000/g, (_, index: string) => protectedParts[Number(index)]).trim();
}
...
export function getReviewReasons(input: {
  ...
  if (REQUIRE_REVIEW_UNTIL_CALIBRATED) reasons.push("Automatic delivery is gated until confidence is calibrated");
  ...
  if (input.meanTokenProbability < DEFAULT_MEAN_CONFIDENCE) reasons.push("Average token confidence is low");
