...
import { ProjectType, SourceRepo } from "./types";
const execp = promisify(exec);

export async function buildCache(paths: string[], maxDepth: number): Promise<SourceRepo[]> {
...
function parseRepoPaths(repoPaths: string[]): SourceRepo[] {
  return repoPaths.map((path) => {
    if (path.endsWith("package.json")) {
      ...
      return { name: name, icon: "node-js.png", fullPath: fullPath, type: ProjectType.NODE };
    } else if (path.endsWith("pom.xml")) {
      const fullPath = path.replace("/pom.xml", "");
      const name = fullPath.split("/").pop() ?? "unknown";
      return { name: name, icon: "maven.png", fullPath: fullPath, type: ProjectType.MAVEN };
    } else if (path.endsWith("build.gradle")) {
      const fullPath = path.replace("/build.gradle", "");
      const name = fullPath.split("/").pop() ?? "unknown";
      return { name: name, icon: "gradle.png", fullPath: fullPath, type: ProjectType.GRADLE };
    }

    return { name: "unknown", icon: "unknown.png", fullPath: path, type: ProjectType.UNKNOWN };
  });
}
