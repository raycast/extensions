...
func typeText(_ text: String) {
    guard AXIsProcessTrusted() else {
        fail(3, "缺少辅助功能权限：请在系统设置 → 隐私与安全性 → 辅助功能中允许 Raycast。")
    }
...
func detectBarcodes(_ imagePath: String) {
    ...
    do {
        ...
        fail(6, "条码识别失败：\(error.localizedDescription)")
    }
...
guard arguments.count >= 2 else {
    fail(2, "用法：goose-2fa-helper type <text> | qr <image>")
}

switch arguments[1] {
case "type":
    guard arguments.count >= 3 else { fail(2, "缺少要输入的文本。") }
    typeText(arguments[2])
case "qr":
    guard arguments.count >= 3 else { fail(2, "缺少图片路径。") }
    detectBarcodes(arguments[2])
default:
    fail(2, "未知子命令：\(arguments[1])")
}
