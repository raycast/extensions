...
export async function getSites(): Promise<SiteData[]> {
  ...
  try {
    ret = await readCache();
    if (ret.length === 30) throw "Cache with only 30 sites"; //temp fix to force reloading more sites - this can be removed in a future PR when most users should have ALL sites loaded
  } catch {
    try {
