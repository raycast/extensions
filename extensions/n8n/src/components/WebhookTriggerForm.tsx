...
import SaveCommandForm from "./SaveCommandForm"; // Import SaveCommandForm
  ...
  return (
    <Action
      ...
      onAction={() => {
        ...
        console.log("ActionSaveCommand: Pushing SaveCommandForm");

        try {
          // Create the SaveCommandForm with all required props
          const saveCommandForm = (
            <SaveCommandForm
              method={method}
              url={finalUrl}
              headers={headers || ""}
          ...
          push(saveCommandForm);
        } catch (error) {
          console.error("Error creating SaveCommandForm:", error);
        }
      }}
    />
  );
