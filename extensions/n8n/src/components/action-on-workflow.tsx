...
import { DETAIL_VIEW_KEY } from "../utils/constants";
import { getWebhookDetails } from "../utils/workflow-utils"; // Import webhook helper
import WebhookTriggerForm from "./WebhookTriggerForm"; // Import the form
...
export function ActionOnWorkflow(props: {
  ...
  const toggleDetailAction = (
    <Action
      ...
      onAction={async () => {
        await LocalStorage.setItem(DETAIL_VIEW_KEY, !showDetail);
  ...
  return (
    <ActionPanel>
      <ActionPanel.Section title="Workflow Actions">
        {/* Conditionally render the webhook trigger action */}
        {triggerWebhookAction}
        {/* Always render activate/deactivate and toggle detail */}
        {activateDeactivateAction}
        {toggleDetailAction}
      </ActionPanel.Section>

      {/* Keep Open Preferences in its own section */}
