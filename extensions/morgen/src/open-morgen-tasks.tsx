...
export default async function Command() {
  try {
    ...
    try {
      await open("morgen://open-sidebar-tasks");
      await showHUD("Opening Tasks in Morgen");
      return;
    } catch (error) {
    ...
    try {
      ...
      const script = `
        tell application "Morgen"
          activate
          delay 1
          open location "morgen://open-sidebar-tasks"
        end tell
      `;
    ...
    await open("Morgen");
    await showHUD("Opened Morgen (Task view not available)");
  } catch (error) {
