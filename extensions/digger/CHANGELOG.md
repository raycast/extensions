...
- A DNS section that couldn't be checked no longer reads as a host with no records. If the first record type came back empty — normal, plenty of hosts publish no AAAA or MX — and the resolver then failed on everything after it, only that first harmless result was kept and the section reported "no records found" for a lookup that never completed.
