...
export default function TenantsConfiguration() {
  ...
  const onDeleteTenant = (tenant: TenantConfiguration) => () =>
    deleteTenant(tenant)
      .catch(showError);

  return <List isLoading={tenantsLoading}>
...
const TenantsListSection = ({tenants, onEditTenant, onDeleteTenant}: {
    ...
    <List.Item
      ...
      actions={
        <ActionPanel>
          <Action.Push
            title={"Add Tenant"}
            target={<TenantConfigurationForm onSubmit={onCreateTenant}/>}/>
        </ActionPanel>
      }
    />
  </List.Section>
