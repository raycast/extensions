  ...
  .then(
    nodes => (nodes ?? [])
      .map(node => {
        ...
        const {id, url, author, reviewRequests, reviews, number, title, comments} = node;
        ...
        return {
          ...
          reviews: (reviews?.nodes || []).map(node => !node ? null : ({
            id: node.id,
          ...
          comments: (comments.nodes || []).map(node => !node ? null : ({
            user: {login: node.author?.login, avatarUrl: node.author?.avatarUrl},
            url: node?.url,
            createdAt: node?.createdAt,
          })).filter(comment => comment !== null),

          requestedReviewers: (reviewRequests?.nodes || []).map(node => {
              if (!node) {
                return null;
              }
