...
import { menuBarLimit, menuBarPrefs, usesCli } from "./lib/preferences";
...
export default function Command() {
  ...
  if (needsSetup) {
    ...
    return (
      <MenuBarExtra icon={icon} isLoading={isLoading} tooltip="GitHub Review Requests — setup required">
        <MenuBarExtra.Section title={title}>
          <MenuBarExtra.Item
            title={usesCli() ? `Run \`${fix}\` in a terminal` : "Open Extension Preferences"}
            subtitle="Check access for the selected authentication method"
            onAction={() =>
              usesCli()
                ? launchCommand({ name: "pull-requests", type: LaunchType.UserInitiated })
                : openExtensionPreferences()
            }
          />
          <MenuBarExtra.Item
  ...
  return (
    <MenuBarExtra
      ...
      tooltip="GitHub Review Requests"
    >
