...
function PullRequests() {
  ...
  async function ignoreAuthor(login: string) {
    ...
    await showToast({
      ...
      title: `Hiding @${login}`,
      message: "Undo it from Configure GitHub Review Requests",
    });
  }
      ...
      <List.EmptyView
        ...
        actions={
          <ActionPanel>
            <Action.Push
              icon={Icon.Gear}
              title="Configure GitHub Review Requests"
              target={<SettingsView />}
              onPop={refreshAll}
            />
...
function emptyDescription(categoryCount: number): string {
  if (categoryCount === 0) {
    return "Turn the built-in categories back on, or add a saved filter, from Configure GitHub Review Requests.";
  }
