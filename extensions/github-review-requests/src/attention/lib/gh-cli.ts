...
import { prefs, usesCli } from "./preferences";
...
let tokenCache: { value: string; at: number; host: string } | undefined;
const TOKEN_TTL_MS = 5 * 60 * 1000;

/** Returns the OAuth token for `host` via `gh auth token`. */
export async function token(host: string): Promise<string> {
  if (!usesCli()) {
    const value = (prefs().token ?? "").trim();
    if (!value)
      throw new GhError(
        "Personal Access Token is missing",
        "Open extension preferences and enter a PAT, or explicitly select GitHub CLI.",
      );
    return value;
  }
  if (tokenCache && tokenCache.host === host && Date.now() - tokenCache.at < TOKEN_TTL_MS) {
  ...
  tokenCache = { value, at: Date.now(), host };
  return value;
}
