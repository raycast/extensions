 ...
 */
import { prepareAuthCache } from "./auth-cache";
...
export async function checkGhStatus(): Promise<GhStatus> {
  ...
  try {
    await prepareAuthCache(await token(currentHost));
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (!usesCli()) return { state: "not-authenticated", detail: message };
  ...
  // Fine-grained PATs do not report classic OAuth scopes. Let API permissions
  // determine access, without ever invoking gh for a PAT user.
  if (!usesCli()) return { state: "ready", login, scopes: [], missingScopes: [] };
