...
import { host, usesCli } from "../lib/preferences";
  ...
  switch (status.state) {
    case "not-installed":
      return {
        ...
        body: [
          "GitHub Review Requests talks to GitHub through the **GitHub CLI** and borrows its token — it has no login of its own and never stores a token.",
          "",
    ...
    case "ready":
      ...
      return {
        ...
        body: [
          ...
          "",
          ...REQUIRED_SCOPES.filter(s => status.missingScopes.includes(s.name)).map(s => `- \`${s.name}\` — ${s.why}`),
          "",
          "```sh",
...
export function SetupRequired({ status, onRecheck }: SetupRequiredProps) {
  if (!usesCli()) {
    const detail = "detail" in status ? status.detail : "Check the token permissions.";
    return (
      <Detail
        navigationTitle="Check GitHub Access"
        markdown={[
          "# Check your Personal Access Token",
          "",
          detail,
          "",
          "Open extension preferences and enter a valid Personal Access Token. To reuse your existing GitHub CLI login instead, explicitly select **GitHub CLI** as the authentication method.",
          "",
          "For a classic PAT accessing an SSO organization, go to GitHub Settings → Developer settings → Personal access tokens → Tokens (classic) → Configure SSO → Authorize. The organization must allow classic PATs. Fine-grained tokens require the correct resource owner, repositories and permissions, and may require administrator approval.",
          "",
          "If access previously worked, check token expiry, repository permissions, your network/VPN, and organization policies. The extension will not switch credentials automatically.",
        ].join("\n")}
        actions={
          <ActionPanel>
            <Action title="Open Extension Preferences" icon={Icon.Gear} onAction={openExtensionPreferences} />
            <Action title="Check Again" icon={Icon.ArrowClockwise} onAction={onRecheck} />
            <Action.OpenInBrowser title="Manage Personal Access Tokens" url="https://github.com/settings/tokens" />
          </ActionPanel>
        }
      />
    );
  ...
  const markdown = [
    ...
    "",
    "> Everything below is between you, `gh`, and GitHub — none of it is a setting in this extension. GitHub Review Requests has no login of its own and no opinion about which kind of credential you use; it reads whatever `gh auth token` returns. It's documented here only because this is where the symptom shows up.",
    "",
    ...
    "",
    "You aren't stuck. Those restrictions govern OAuth *apps*, and a **classic personal access token isn't one** — but the organization may also restrict PATs. Check its token policy before choosing this option.",
    "",
    PAT_FALLBACK_STEPS,
    "",
