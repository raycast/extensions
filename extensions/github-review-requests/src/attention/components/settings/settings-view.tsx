 ...
 * saved filters.
 */
export function SettingsView({ initialSelection }: { initialSelection?: string } = {}) {
  const [selectedItemId, setSelectedItemId] = useState<string | null>(initialSelection ?? null);
  ...
  return (
    <List
      isLoading={isLoading}
      navigationTitle="Configure GitHub Review Requests"
    ...
    >
          ...
          actions={
            <ActionPanel>
              ...
              <Action
                ...
                onAction={() => launchCommand({ name: "watch", type: LaunchType.UserInitiated })}
              />
            </ActionPanel>
          }
        />
      ...
      <List.Section title="Menu Bar">
        <List.Item
          id="show-menu-bar"
          icon="icon.png"
          title="Show in Menu Bar"
          subtitle="Show the GitHub icon and your pull requests in the macOS menu bar"
          actions={
            <ActionPanel>
              <Action
                title="Show in Menu Bar"
                ...
                onAction={async () => {
                  try {
                    ...
                    await showToast({ style: Toast.Style.Success, title: "Menu Bar Command Started" });
                  } catch {
                    await showToast({
                      style: Toast.Style.Failure,
                      title: "Enable My Pull Requests in Raycast Settings",
                      message: "The menu-bar command may be disabled.",
                    });
      ...
      <List.Section title="Account & Data">
        <List.Item
          ...
          icon={Icon.Key}
          subtitle="Choose Personal Access Token or your existing GitHub CLI login"
          accessories={[{ text: usesCli() ? "GitHub CLI" : "Personal Access Token" }]}
          actions={
            <ActionPanel>
              <Action title="Change Authentication Method" icon={Icon.Key} onAction={openExtensionPreferences} />
            </ActionPanel>
          }
        />
        <List.Item
          ...
          title="Signed in as"
          subtitle={viewer ? `@${viewer.login}` : "Checking GitHub access…"}
        ...
        <List.Item
          ...
          subtitle="Authentication method, activity tracking, GitHub host and results per category"
          actions={
            <ActionPanel>
              <Action icon={Icon.Gear} title="Open Extension Preferences" onAction={() => openExtensionPreferences()} />
            </ActionPanel>
