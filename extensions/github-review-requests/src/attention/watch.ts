...
export default async function Command() {
  if (!prefs().trackingEnabled && environment.launchType === LaunchType.Background) return;
  try {
    ...
    await updateCommandMetadata({ subtitle: "Couldn't reach GitHub — check authentication in extension preferences" });
  }
}
