...
- The isolated development extension compiled and started successfully.
- Native visual and real organization-access checks above remain pending.
- Installed Node is 22.16.0; npm reports that the Raycast API requires >=22.22.2. These checks passed locally, but use the required version for final validation.

## Automated checks

Run `npm test`, `npm run typecheck`, `npm run lint`, and `npm run build`. The tests exercise actual domain modules with a fake Raycast storage API, controlled HTTP responses, and a temporary fake CLI executable. They do not establish real organization access or prove native Raycast rendering.

`UPSTREAM.json` pins the original extension snapshot. The repository's initial commit is the unmodified source baseline, making `git diff HEAD` useful for review. The original icon asset is unchanged.
...
- This is a local integration candidate, not an updated store PR. Keep the preview harness and local review files separate from the final product diff as appropriate.
