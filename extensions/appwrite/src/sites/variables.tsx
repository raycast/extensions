import { List, Icon, ActionPanel, Action, useNavigation, showToast, Toast, Form, Keyboard, confirmAlert, Alert } from "@raycast/api";
...
export default function Variables({siteId}:{siteId:string}) {
  ...
  const {isLoading,data:variables,mutate} = useCachedPromise(async(siteId)=> {
  ...
  async function confirmAndDelete(variable: sdk.Models.Variable) {
    ...
    const toast = await showToast(Toast.Style.Animated, "Deleting Variable", variable.key);
            try {
              await mutate(
                sites.deleteVariable({siteId, variableId: variable.$id}), {
  ...
  return <List isLoading={isLoading}>
    {!isLoading && !variables.length ? <List.EmptyView icon={Icon.PlusCircle} title="Create a variable to get started" actions={<ActionPanel>
<Action.Push icon={Icon.PlusCircle} title="Create Variable" target={<CreateVariable siteId={siteId} mutate={mutate} />} />
    </ActionPanel>} /> : variables.map(variable => <List.Item key={variable.$id} icon={Icon.Code} title={variable.key} accessories={[
      variable.secret ? {tag: "SECRET"} : {text: variable.value}
    ]} actions={<ActionPanel>
      ...
      <Action.Push icon={Icon.PlusCircle} title="Create Variable" target={<CreateVariable siteId={siteId} mutate={mutate} />} shortcut={Keyboard.Shortcut.Common.New} />
      <Action icon={Icon.Trash} title="Delete Variable" onAction={() => confirmAndDelete(variable)} shortcut={Keyboard.Shortcut.Common.Remove} style={Action.Style.Destructive} />
    </ActionPanel>} />)}
  </List>
}
