...
export default function Users() {
  ...
  const { isLoading, data: users, error, mutate } = useCachedPromise(
    async () => {
  ...
  return (
    <List isLoading={isLoading}>
      {!isLoading && !users.length && !error ? <List.EmptyView title="Create your first user" description="Need a hand? Learn more in our documentation." actions={<ActionPanel>
        <Action.Push icon={Icon.AddPerson} title="Create User" target={<CreateUser mutate={mutate} />} />
        <Action.OpenInBrowser title="Documentation" url="https://appwrite.io/docs/references/cloud/server-nodejs/users" />
      </ActionPanel>} /> : users.map((user) => (
        <List.Item
          key={user.$id}
...
function CreateUser({mutate}: {mutate: MutatePromise<sdk.Models.User<sdk.Models.Preferences>[]>}) {
  type FormValues = {
    ...
    return <Form actions={<ActionPanel>
      ...
      <Form.TextField title="User ID" placeholder="Enter ID" info="Allowed characters: alphanumeric, non-leading hyphen, underscore, period" {...itemProps.userId} />
      <Form.TextField title="Name (optional)" placeholder="Enter name" info="User name. Max length: 128 chars." {...itemProps.name} />
      <Form.TextField title="Email (optional)" placeholder="Enter email" {...itemProps.email} />
      <Form.TextField title="Phone (optional)" placeholder="Enter phone" info="Format this number with a leading '+' and a country code, e.g., +16175551212." {...itemProps.phone} />
      <Form.PasswordField title="Password (optional)" placeholder="Enter password" info="Plain text user password. Must be at least 8 chars." {...itemProps.password} />
    </Form>
  }