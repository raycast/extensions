  ...
  return (
    <List isLoading={isLoading}>
      {!isLoading && !buckets.length && !error ? (
        <List.EmptyView title="Create your first bucket" description="Need a hand? Learn more in our documentation." actions={<ActionPanel>
        ...
        buckets.map((bucket) => <List.Item key={bucket.$id} icon={Icon.Folder} title={bucket.name}
        accessories={[
...
function CreateBucket({mutate}: {mutate: MutatePromise<sdk.Models.Bucket[]>}) {
  ...
  const {handleSubmit,itemProps} = useForm<FormValues>({
    async onSubmit(values) {
      ...
      try {
        await mutate(
          storage.createBucket(bucketId, name)
        )
  ...
  return <Form actions={<ActionPanel>
    ...
    <Form.TextField title="Bucket ID" placeholder="Enter ID" info="Allowed characters: alphanumeric, non-leading hyphen, underscore, period" {...itemProps.bucketId} />
  </Form>
}