...
it("filters speech, safety and inactive models, and handles Compound capabilities", async () => {
  const entry = { id: "openai/gpt-oss-120b", active: true, context_window: 131072, owned_by: "OpenAI" };
  const fetcher = vi.fn<typeof fetch>().mockResolvedValue(
    Response.json({
      data: [
        entry,
        { ...entry, id: "groq/compound" },
        { ...entry, id: "qwen/qwen3.8-27b" },
        ...[
          "whisper-large-v3",
  ...
  expect(await createModelProvider({ apiKey: "test-key", fetch: fetcher }).getModels()).toMatchObject([
    {
      ...
      capabilities: { tools: { supported: true }, reasoningEffort: { options: ["low", "medium", "high"] } },
    },
    { id: "groq/compound", capabilities: { tools: { supported: false } } },
    { id: "qwen/qwen3.8-27b", capabilities: { vision: { mediaTypes: ["image/png", "image/jpeg", "image/webp"] } } },
  ]);
});

it("forwards a supported GPT OSS reasoning effort", async () => {
  const fetcher = vi.fn<typeof fetch>().mockResolvedValue(completion());
  await collect(
    await createModelProvider({ apiKey: "test-key", fetch: fetcher }).streamCompletion(model, {
      messages: [{ role: "user", content: [{ type: "text", text: "Hi" }] }],
  ...
  expect(JSON.parse(String(fetcher.mock.calls[0][1]?.body)).reasoning_effort).toBe("high");
});
