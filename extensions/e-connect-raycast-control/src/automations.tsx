...
function formatLastExecution(automation: AutomationResponse) {
  ...
  const prefix =
    automation.last_execution.status === "success"
      ? "Last run succeeded"
      : "Last run failed";
...
export default function Command() {
  ...
  const toggleAutomation = useCallback(
    async (automation: AutomationResponse) => {
      ...
      try {
        const updated = await updateAutomationEnabledState(
          automation,
          nextEnabled,
        );
        toast.style = Toast.Style.Success;
        toast.title = updated.is_enabled
          ? "Automation running"
          : "Automation stopped";
        toast.message = updated.is_enabled
          ? "The automation is now enabled."
          : "The automation is now stopped and moved to the Stopped section.";
        await load();
      } catch (toggleError) {
        toast.style = Toast.Style.Failure;
        toast.title = nextEnabled
  ...
  if (error && automations.length === 0) {
    return (
      <ConnectionErrorView title="Unable to load automations" message={error} />
    );
  }

  const webUiBaseUrl = getWebUiBaseUrl();
  const runningAutomations = automations.filter(
    (automation) => automation.is_enabled,
  );
  const stoppedAutomations = automations.filter(
    (automation) => !automation.is_enabled,
  );
    ...
    <List isLoading={isLoading} searchBarPlaceholder="Search automations">
      {runningAutomations.length > 0 ? (
        <List.Section title="Running">
          {runningAutomations.map(renderAutomationItem)}
        </List.Section>
      ) : null}
      {stoppedAutomations.length > 0 ? (
        <List.Section title="Stopped">
          {stoppedAutomations.map(renderAutomationItem)}
        </List.Section>
      ) : null}
    </List>
  );
