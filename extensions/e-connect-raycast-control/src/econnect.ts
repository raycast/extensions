...
function resolveServerAddress(
  ...
  try {
    parsed = new URL(
      rawAddress.includes("://") ? rawAddress : `http://${rawAddress}`,
    );
  } catch {
    throw new Error(
      "Invalid Server IP:Port. Use values like 192.168.1.25:8000 or https://192.168.1.25:3443.",
    );
  }
  ...
  if (parsed.search || parsed.hash) {
    throw new Error(
      "Enter only the server IP:Port or the API base URL, without query parameters.",
    );
  }
  ...
  if (normalizedPath && normalizedPath !== "/api/v1") {
    throw new Error(
      "Use only Server IP:Port like 192.168.1.25:8000 or the direct API URL ending in /api/v1.",
    );
  }
  ...
  return {
    ...
    port:
      parsed.port ||
      (protocol === "https" ? DEFAULT_HTTPS_PORT : DEFAULT_HTTP_API_PORT),
  };
}
...
export function getWebUiBaseUrl(preferences = getPreferences()) {
  ...
  const webUiPort =
    protocol === "https" && port !== DEFAULT_HTTP_API_PORT
      ? port
      : DEFAULT_HTTPS_PORT;
...
function mapNetworkError(error: unknown) {
  ...
  if (
    ...
    (lowerMessage.includes("wrong version number") ||
      lowerMessage.includes("ssl3_get_record"))
  ) {
    ...
    const request = transport.request(
      apiUrl,
      {
        ...
        ...(apiUrl.protocol === "https:"
          ? { rejectUnauthorized: !preferences.allowInsecureTls }
          : {}),
      },
      (response) => {
        const chunks: Buffer[] = [];
...
export async function updateAutomationEnabledState(
...
export async function triggerAutomation(automationId: number) {
  return requestJson<TriggerResponse>(`automation/${automationId}/trigger`, {
    method: "POST",
  });
}
...
export async function sendDeviceCommand(
  ...
  return requestJson<DeviceCommandResponse>(
    `device/${encodeURIComponent(deviceId)}/command`,
    {
      method: "POST",
      body: payload,
    },
