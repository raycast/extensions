  ...
  (json as Array<any>).map((item: any) => {
    ...
    if (searchText.substring(0, 1) == "!") {
      if (searchText.charAt(searchText.length - 1) == " " && searchText.split(" ").length == 2) {
        // "!g "
        bang = searchText.substring(1, searchText.length - 1);
        if (bang in BANGS) {
          results.push({
            id: nanoid(),
            query: item.phrase,
      ...
      } else if (searchText.charAt(searchText.length - 1) != " " && searchText.split(" ").length < 1) {
        // "!g"
        bang = searchText.substring(1, searchText.length);
        results.push({
          id: nanoid(),
          query: item.phrase,
      ...
      } else if (searchText.substring(1, searchText.length).length > 2) {
