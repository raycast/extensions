...
const VariablesView: React.FC<VariablesViewProps> = ({ collectionId, accessToken, fileKey }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        setVariables(Object.values(data.meta?.variables || {}).filter((variable: any): variable is Variable => variable.variableCollectionId === collectionId) as Variable[]);
        ...
        const modesMap: ModesMap = data.meta?.variableCollections[collectionId]?.modes.reduce((acc: ModesMap, mode) => {
...
const showVariableDetails = (variable: Variable) => {
  ...
  return (
    <List isLoading={isLoading}
      searchBarAccessory={
    <List.Dropdown
      tooltip="Select Syntax Type"
      value={selectedSyntaxType}
      onChange={setSelectedSyntaxType}
    >
    ...
    <List.Item
      ...
      actions={
        <ActionPanel>
          <Action
            title="View Details"
            onAction={() => showVariableDetails(variable)}
          />
