...
export function TaskActions({
  ...
  // A read-only list answers every change with 403 — so the changes are not offered.
  const editable = context.canEdit(task);
          ...
          <Action.PickDate
            ...
            onChange={(date) =>
              run(
                ...
                // `allDay` travels only with a date: sent on a clear, it would turn a
                // time-blocked task all-day while removing nothing but its due date.
                () => updateTask(task.id, date ? { dueDate: toFloatingDay(date), allDay: true } : { dueDate: null }),
                onChange,
              )
            }
          />
