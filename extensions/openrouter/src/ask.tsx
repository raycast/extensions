import { ActionPanel, clearSearchBar, getPreferenceValues, List, LocalStorage, useNavigation } from "@raycast/api";
...
export default function Ask(props: { conversation?: Conversation; initialQuestion?: string }) {
  ...
  // Load last selected model from storage - only once at component mount
  useEffect(() => {
    async function loadLastSelectedModel() {
      if (props.conversation) {
        setIsInitialLoad(false);
        return;
      } // Don't load if we have a conversation

      const storedModelId = await LocalStorage.getItem("lastSelectedModelId");
      if (storedModelId) {
        console.debug("loadLastSelectedModel", storedModelId);
        setSelectedModelId(storedModelId as string);
      }
      setIsInitialLoad(false);
    }
  ...
  // Save selected model ID when it changes
  useEffect(() => {
    // Skip saving during initial load to prevent race conditions
    if (isInitialLoad) return;
    // Always save the selected model ID, even if it's "default"
    LocalStorage.setItem("lastSelectedModelId", selectedModelId);
    console.debug("selectedModelId: ", selectedModelId);
  }, [selectedModelId, isInitialLoad]);
