...
export function raycastForKiroURI(uri: string) {
  return `kiro://tonka3000.raycast/${uri}`;
}

export async function openURIinKiro(uri: string) {
  await open(raycastForKiroURI(uri));
}
