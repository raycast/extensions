...
export default function Main(props: { arguments: ExportArguments }) {
  ...
  rows?.forEach((rowText, index) => {
    if (rowText != "" && rowText.toLowerCase().includes(query?.toLowerCase() || "")) {
        ...
        <List.Item
            ...
            actions={
            <ActionPanel>
                <Action title="Go To Row" icon={Icon.ArrowRightCircle} shortcut={{ modifiers: ["cmd"], key: "g" }} onAction={async () => await runAppleScript(`tell application "Bike"
                    activate
                    select document 1 at row ${index + 1} of document 1
                end tell`)} />
                <Action title="Copy URL" icon={Icon.CopyClipboard} shortcut={{ modifiers: ["cmd"], key: "c" }} onAction={async () => await runAppleScript(`tell application "Bike"
                    set theURL to row ${index + 1} of document 1
                    set the clipboard to theURL
                end tell`)} />
                <Action title="Delete Row" icon={Icon.Trash} style={Action.Style.Destructive} shortcut={{ modifiers: ["cmd"], key: "d" }} onAction={async () => {
                    if (await confirmAlert({ title: "Are you sure?", primaryAction: { title: "Delete", style: Alert.ActionStyle.Destructive }})) {
                        await runAppleScript(`tell application "Bike"
  ...
  return (
    <List isLoading={rows === undefined} searchBarPlaceholder={rows?.length ? `Search ${rows.length} rows...` : `Search rows...`}>
