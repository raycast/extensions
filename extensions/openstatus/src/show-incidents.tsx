...
export default function ShowIncidents() {
  ...
  const { isLoading, data: incidents, mutate } = useFetch("https://api.openstatus.dev/v1/incident", {
    headers: {
  ...
  async function updateIncident(incident: Incident, {action}: {action: "acknowledge" | "resolve" }) {
    const toast = await showToast(Toast.Style.Animated, action==="acknowledge" ? "Acknowledging" : "Resolving", incident.id.toString());
    try {
  ...
  return (
    <List isLoading={isLoading || isWhoamiLoading}>
      {incidents.map((incident) => {
        ...
        if (incident.acknowledgedAt) accessories.push({ icon: acknowledgeIcon, date: new Date(incident.acknowledgedAt), tooltip: `Acknowledged At: ${incident.acknowledgedAt}` });
        if (incident.resolvedAt) accessories.push({ icon: resolveIcon, date: new Date(incident.resolvedAt), tooltip: `Resolved At: ${incident.resolvedAt}` });
        return(
        <List.Item
          key={incident.id}
          icon={
          ...
          actions={
            <ActionPanel>
              ...
              />
              {!incident.acknowledgedAt && <Action icon={acknowledgeIcon} title="Acknowledge Incident" onAction={() => updateIncident(incident, { action: "acknowledge" })} />}
              {incident.acknowledgedAt && !incident.resolvedAt && <Action icon={resolveIcon} title="Resolve Incident" onAction={() => updateIncident(incident, { action: "resolve" })} />}
            </ActionPanel>
          }
        />
      )})}
