...
If you wish to use your own local Bitwarden CLI, you can do so by entering the corresponding path in the `Bitwarden CLI Path` field (tip: use the `which bw` (or `where.exe bw` on Windows) command to find the path).
