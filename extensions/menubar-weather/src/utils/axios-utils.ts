...
import { cityName, precipitationUnits, tempUnits } from "./weather-utils";
...
export async function getOpenMeteoWeather(lon: string, lat: string) {
  const axiosResponse = await axios({
    ...
    params: {
      ...
      longitude: lon,
      hourly:
        "temperature_2m,relativehumidity_2m,apparent_temperature,precipitation,rain,weathercode,surface_pressure,visibility,winddirection_120m",
      models: "best_match",
