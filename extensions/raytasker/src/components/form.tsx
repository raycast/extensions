...
export function EditTaskForm(props: FormProps) {
  ...
  return (
            ...
            onSubmit={async (values) => {
              ...
              if (props.createNew) {
                ...
                if (props.task && props.currentList != values.list) {
                  ...
                  const res = await google.patchTask(
                    props?.task?.id ? props.task.id : "",
                    patchTask.list ? patchTask.list : props.currentList,
                    patchTask as Task
                  );
    ...
    >
      ...
      />
      <Form.Dropdown id="list" title="List" onChange={() => setIsChanged(true)} defaultValue={props.currentList}>
        {props.lists.map((list) => (
          <Form.Dropdown.Item value={list.id} key={list.id} title={list.title} icon={Icon.Dot} />
        ))}
      </Form.Dropdown>
    </Form>
  );
