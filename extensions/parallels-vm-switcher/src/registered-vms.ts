...
import {
  ParallelsHostError,
  createMacOSParallelsHost,
  type ActivationTarget,
  type ParallelsHost,
} from "./internal/parallels-host";
...
function wrapHostError(error: unknown, fallbackCode: RegisteredVMErrorCode, message: string): RegisteredVMError {
  ...
  if (error instanceof ParallelsHostError && error.code === "activation-failed") {
    return new RegisteredVMError("activation-failed", error.message, {
      cause: error,
      details: error.details,
    });
  }
...
class RegisteredVMsModule implements RegisteredVMs {
  ...
  async #findByID(id: string): Promise<RegisteredVM> {
    ...
    const sameNameVMs = vms.filter(
      (candidate) => candidate.id !== vm.id && normalizedQuery(candidate.name) === normalizedQuery(vm.name),
    );
    if (sameNameVMs.length > 0) {
      throw new RegisteredVMError(
        "vm-name-ambiguous",
        `Cannot safely focus “${vm.name}” because another registered virtual machine has the same name. Rename one VM in Parallels Desktop and try again.`,
        {
          details: {
            vmID: vm.id,
            conflictingVMIDs: sameNameVMs.map((candidate) => candidate.id),
          },
        },
      );
    }
  ...
  async #performOpen(id: string): Promise<OpenOutcome> {
    ...
    try {
      ...
      await this.#host.activate(target);
    } catch (error) {
