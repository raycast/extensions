...
**Focus Proxy**:
The unique regular Parallels Dock Helper for one VM UUID. Requesting activation from it moves macOS to that VM's Space; the Host Adapter then raises the target Parallels Console window and verifies its stable focused-window identity.
...
- Open or Switch completes only after the exact target Parallels Console window is onscreen and stably owns both frontmost and displayed menu-bar focus.
- Duplicate normalized Registered VM names fail before activation; a title match alone is never accepted as sufficient identity evidence.
