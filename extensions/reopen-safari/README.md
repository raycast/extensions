...
The command activates Safari, then clicks the "Reopen All Windows from Last Session" item in the History menu. The menu item is located by its English title, with a locale-independent structural lookup (its fixed position below the "Recently Closed" submenu) on non-English systems — so it works regardless of your system language.
