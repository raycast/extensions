...
export function pairSubstitutions(
  ...
  // Per direction: a sub who came on in a paired swap can still go off later
  // in an unpaired one. No re-entry in football, so id + direction is unique.
  const pairedIn = new Set(paired.map((e) => e.swap![0].id));
  const pairedOut = new Set(paired.map((e) => e.swap![1].id));
  ...
  const unpairedLines: SubLine[] = [
    ...ins
      .filter((e) => !pairedIn.has(e.id))
      .map((e) => ({ ...e, direction: "in" as const })),
    ...outs
      .filter((e) => !pairedOut.has(e.id))
