...
test("pairSubstitutions keeps a paired sub's later unpaired exit", () => {
  // in-1 comes on at 62' in a complete swap, then goes off at 85' in one
  // without swap data. Id-only filtering would drop the 85' exit.
  const swaps = [{ time: 62, swap: [{ id: "in-1" }, { id: "out-1" }] }];
  const lines = pairSubstitutions(
    swaps,
    [
  ...
  assert.deepEqual(lines, [
    { time: 62, inId: "in-1", outId: "out-1" },
    { time: 85, id: "in-2", direction: "in" },
    { time: 85, id: "in-1", direction: "out" },
  ]);
});
