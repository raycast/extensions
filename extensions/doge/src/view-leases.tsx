...
export default function Command() {
  ...
  const formatSquareFt = (value: number) => `${value.toLocaleString()} sq.ft.`;

  // Choose icon based on savings amount
  const getLeaseIcon = (lease: Lease) => {
  ...
  const getTerminationDescription = (description: string) => {
  ...
  const getTerminationType = (description: string) => {
    if (description.includes("Mass Mod")) {
      return {
  ...
  const getLeaseDetailMarkdown = (lease: Lease) => {
    return `
# ${lease.location}
...
- Savings per Square Foot: ${formatCurrency(lease.savings / lease.sq_ft)} per sq.ft.
  ...
  return (
    <List
      ...
      throttle
      searchBarAccessory={
        <List.Dropdown tooltip="Sort By" storeValue={true} onChange={(newValue) => setSortBy(newValue as SortOption)}>
