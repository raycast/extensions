...
export default function SearchThings() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // here matches what the user sees inside NewTwos.
  ...
  async function setCompleted(thing: TwosThing, completed: boolean) {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.kind === "thing" && r.thing.id === thing.id
            ? { ...r, thing: { ...r.thing, completed } }
            : r,
        ),
      );
  ...
  const listRows = useMemo(
    () => rows.filter((r): r is ListRow => r.kind === "list"),
    [rows],
  );
  const thingRows = useMemo(
    () => rows.filter((r): r is ThingRow => r.kind === "thing"),
    [rows],
  );
