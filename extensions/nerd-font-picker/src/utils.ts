...
export let glyphPool: Glyph[] = []
export const updateGlyphPool = (glyphs: Glyph[]) => {
  glyphPool = glyphs
}
...
export const buildCache = (): Glyph[] => {
  ...
  for (let i = 0; i < font.glyphs.length; i++) {
    ...
    glyphs.push({
      ...
      name: (g.name ?? `uni${code.toString(16).toUpperCase()}`).replace(
        /\./g,
        "-",
      ),
...
export const glyphToDetailUri = (g: Glyph, color: string): string => {
  ...
  return buildSvgUri(
    ...
    color,
    `0 0 ${w} ${h}`,
    `translate(${pad - bb.x1} ${pad + bb.y2}) scale(1 -1)`,
    150,
  )
}

export const search = (query: string): Glyph[] => {
  if (!query.trim()) return glyphPool.slice(0, MAX_DISPLAY)
  const q = query.toLowerCase()
  const results: Glyph[] = []
  for (const g of glyphPool) {
