...
export function loadNotes(vault: Vault): Note[] {
  ...
  // Get preferences
  const preferences = getPreferenceValues<SearchNotePreferences>();
  const maxFileSizeMB = parseFloat(preferences.maxFileSizeMB || "2");
  const maxFileSizeBytes = maxFileSizeMB > 0 ? maxFileSizeMB * 1024 * 1024 : Infinity;

  // Get max note count from preferences
  const maxNoteCount = parseInt(preferences.maxNoteCount || "20000");
  const maxNotes = maxNoteCount > 0 ? maxNoteCount : Infinity;

  // Add option to load content lazily
  const lazyLoadContent = preferences.lazyLoadContent !== false; // default to true
  const maxContentPreview = 500; // Only load first 500 chars for preview
  ...
  for (const filePath of filePaths) {
    ...
    if (processedFiles >= maxNotes) {
      console.log(`Reached maximum note limit (${maxNotes}), stopping...`);
      break;
    }
    ...
    try {
      ...
      if (lazyLoadContent && stats.size > 50000) {
        // Lazy load for files > 50KB
        // Only read first part of the file for metadata
        ...
        content = content.substring(0, maxContentPreview) + "...\n[Content truncated for performance]";
      } else {
        content = getNoteFileContent(filePath, false);
        tags = tagsForString(content);

        // Truncate content for very large files even when fully loaded
      ...
      // Log progress for large vaults
      if (processedFiles % 1000 === 0) {
        console.log(`Processed ${processedFiles} notes...`);
      }
    } catch (error) {
  ...
  if (skippedFiles > 0) {
    console.log(`Skipped ${skippedFiles} files due to size limit (>${maxFileSizeMB} MB)`);
  }
  if (filePaths.length > maxNotes) {
    console.log(`Note: Only loaded first ${maxNotes} notes out of ${filePaths.length} total files.`);
  }
