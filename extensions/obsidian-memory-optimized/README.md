...
# Obsidian (Memory Optimized)

**A Raycast extension for Obsidian - specially optimized for large vaults with 20,000+ notes.**

This is an optimized version of the Obsidian Raycast extension that solves memory issues when working with large note collections. It includes smart memory management, configurable file limits, and lazy loading to handle vaults of any size.

## 🚀 Memory Optimizations

- **Configurable file limits**: Set maximum number of notes to index (default: 20,000)
- **File size filtering**: Skip files larger than specified size (default: 2MB) 
- **Lazy loading**: Only load partial content for large files to reduce memory usage
- **Smart content truncation**: Limit content preview while preserving functionality
- **Progress tracking**: See loading progress for large vaults

## 📋 Quick Setup for Large Vaults

1. **Vault Path**: Set your Obsidian vault path
2. **Max File Size (MB)**: Set to `2` or smaller (skips large files that cause memory issues)
3. **Max Note Count**: Keep at `20000` (or adjust based on your needs)
4. **Lazy Load Content**: Keep enabled
5. **Excluded Folders**: Add template folders, archives, attachments

This extension is based on the original Obsidian extension with extensive memory optimizations.
