...
export const Assignment = (props: assignment) => {
  ...
  useEffect(() => {
    async function load () {
      if (typeof props.description == 'function') {
        let output = props.description();
        if (output instanceof Promise) output = await output;
        setMarkdown(output);
      } else {
        setMarkdown(props.description);
      }
      const apiAssignment = await api.courses[props.course_id].assignments[props.id].searchParams({
        'include[]': 'submission'
      }).get();
  ...
  return (
    <List.Item
          ...
          <Action.Push
            ...
            target={
              <Detail
                ...
                metadata={
                  <Detail.Metadata>
                    <Detail.Metadata.Label title="Due Date" text={props.date.toDateString() + ' at ' + props.pretty_date.substring(7)} />
                    ...
                    {apiAssignment?.submission?.graded_at && (
                      <Detail.Metadata.Label title="Score" text={apiAssignment.submission?.grade == 'complete' ? `Complete/${apiAssignment.points_possible} point${apiAssignment.points_possible !== 1 ? 's' : ''}` : `${apiAssignment.submission.score}/${apiAssignment.points_possible} point${apiAssignment.points_possible !== 1 ? 's' : ''}`} />
                    )}
                   </Detail.Metadata>
                }
              />
      ...
      accessories={props?.time ? [{ text: props.pretty_date, ...((apiAssignment?.submission?.submitted_at) ? { icon: Icon.Checkmark, tooltip: 'Submitted' } : {}) }] : [{ text: props.pretty_date, icon: Icon.Calendar }]}
    />
  );
};
