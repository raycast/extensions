...
export default function ConfigureModelCommand() {
    ...
    (async () => {
      ...
      if (p !== "raycast" && key) {
        try {
          ...
          setModels(fetched);
          if (model && !fetched.find((m) => m.id === model)) {
  ...
  async function handleSubmit(values: {
    ...
    const finalModel = (values.customModel as string)?.trim() || values.modelId;
      ...
      {showModelFields && (
        <>
          ...
          <Form.TextField
            ...
            placeholder="e.g. anthropic/claude-3-haiku:nitro or openrouter/auto"
            value={customModel}
            onChange={setCustomModel}
            info="If set, this overrides the dropdown above. Use for :nitro, :floor, or any routing alias."
          />
        </>
      )}
    </Form>
