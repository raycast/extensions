...
export async function uploadFile(filePath: string): Promise<UploadedFile> {
  ...
  // `fileTypeFromFile` dynamically imports `strtok3` at runtime. Raycast bundles
  // the static dependency graph, so that runtime import is unavailable in the
  // installed extension. We already need the file contents for the upload, so
  // detect its type from the buffer instead.
