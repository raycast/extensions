...
function BrowseOrganizations() {
  ...
  useEffect(() => {
    API.get<InfomaniakResponse<Organization>>('/1/accounts').then((response) => {
    ...
    <List isLoading={loading}>
      {organizations.map((organization) => (
        <List.Item
          ...
          subtitle={organization.legal_entity_type === 'association' ? 'Association' : organization.legal_entity_type === 'individual' ? 'Individual' : 'Company'}
          actions={
            <ActionPanel>
              <Action.Push title="Browse organization" icon="domain.png" target={<BrowseOrganizationProducts organization={organization} />} />
            </ActionPanel>
          }
        /> 
      ))}
