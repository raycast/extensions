...
export default function EmailAccounts() {
    ...
    return <List isLoading={isLoading}>
        {data?.map(account => <List.Item key={account.email} title={account.email} icon={Icon.Envelope} subtitle={account.login} accessories={[
            ...
            <Action.Push title="View Disk Information" target={<ViewEmailAccountDiskInformation emailAccount={account.email} />} icon={Icon.Coin} />
        </ActionPanel>} />)}
    </List>
}
...
function ViewEmailAccountDiskInformation({ emailAccount }: { emailAccount: string }) {
    ...
    <Detail.Metadata.TagList title="Restrictions">
        ...
        <Detail.Metadata.TagList.Item text="Outgoing" color={!account.suspended_outgoing ? Color.Green : Color.Red} />
