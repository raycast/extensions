    ...
    return <List isLoading={isLoading}>
        {data && <>
            ...
            <List.Section title="Addon Domains">
                {data.addon_domains.map(domain => <Domain key={domain} domain={domain} />)}
            </List.Section>
            <List.Section title="Sub Domains">
                {data.sub_domains.map(domain => <Domain key={domain} domain={domain} showAction={false} />)}
            </List.Section>
            <List.Section title="Parked Domains">
                {data.parked_domains.map(domain => <Domain key={domain} domain={domain} />)}
            </List.Section>
        </>}
    </List>
}
...
function Domain({ domain, showAction=true }: { domain: string, showAction?: boolean }) {
    return <List.Item title={domain} icon={getFavicon(`https://${domain}`, { fallback: DEFAULT_ICON })} actions={showAction && <ActionPanel>
