...
import { List, ActionPanel, Action, Icon, Detail, Form, Color, useNavigation } from "@raycast/api";
import { useState, useEffect } from "react";
import { dirname } from "path";
import {
  getPreferences, listRecipes, friendlyName, runCook, recipeToMarkdown,
  validateRecipePath, DirEntry,
} from "./utils";
...
export default function Command() {
  ...
  //   setLoading = false
  useEffect(() => {
    if (!validateRecipePath()) { setLoading(false); return; }
    try {
  ...
  return (
    <List isLoading={loading} searchBarPlaceholder="Search recipes by name…" throttle>
      ...
      {entries.map((e) => (
        <List.Item
          ...
          subtitle={e.isDir ? undefined : dirname(e.fullPath).replace(getPreferences().recipePath, "") || "root"}
          icon={e.isDir ? Icon.Folder : e.name.endsWith(".menu") ? Icon.Calendar : Icon.Document}
          accessories={e.isDir ? [{ tag: { value: "dir", color: Color.Blue } }] : []}
          actions={
            <ActionPanel>
              {/* DIRECTORY → push a SubDirView to navigate deeper */}
              {e.isDir ? (
                <Action.Push title="Open" icon={Icon.Folder}
                  target={<SubDirView base={e.fullPath} />} />
              ) : (
                /* RECIPE FILE → push RecipeDetail (parsed + formatted) */
                <Action.Push title="View Recipe" icon={Icon.Eye}
...
function SubDirView({ base }: { base: string }) {
  ...
  useEffect(() => {
    try { setEntries(listRecipes(base)); } catch { /* empty folder */ }
    setLoading(false);
  }, [base]);

  return (
    <List isLoading={loading} navigationTitle={base} searchBarPlaceholder="Filter…">
      {entries.map((e) => (
        <List.Item
          key={e.fullPath}
          title={friendlyName(e.name)}
          ...
          actions={
            <ActionPanel>
              {e.isDir ? (
                <Action.Push title="Open" icon={Icon.Folder} target={<SubDirView base={e.fullPath} />} />
              ) : (
                <Action.Push title="View Recipe" icon={Icon.Eye} target={<RecipeDetail filePath={e.fullPath} />} />
              )}
            </ActionPanel>
          }
        />
...
function RecipeDetail({ filePath }: { filePath: string }) {
  ...
  useEffect(() => { loadRecipe(1); }, [filePath]);
  ...
  return (
    <Detail
      ...
      actions={
        <ActionPanel>
          <Action.Push title="👨‍🍳 Cooking Mode" icon={Icon.Play} target={<CookingMode filePath={filePath} scale={scale} />} />
          <Action title="Scale ½x" icon={Icon.Minus} onAction={() => loadRecipe(0.5)} />
          <Action title="Scale 1x" icon={Icon.Circle} onAction={() => loadRecipe(1)} />
          <Action title="Scale 2x" icon={Icon.Plus} onAction={() => loadRecipe(2)} />
          <Action title="Scale 4x" icon={Icon.Plus} onAction={() => loadRecipe(4)} />
          <Action.Push title="Custom Scale…" icon={Icon.Pencil}
...
function ScaleForm({ onScale }: { onScale: (s: number) => void }) {
  ...
  function submit(v: { factor: string }) {
    ...
    if (!isNaN(f) && f > 0) { onScale(f); pop(); }
  }
  return (
    <Form actions={<ActionPanel><Action.SubmitForm title="Apply Scale" onSubmit={submit} /></ActionPanel>}>
      <Form.TextField id="factor" title="Scale Factor" placeholder="2" defaultValue="2" />
