...
macOS 전용 Raycast extension. 클립보드 이미지·Finder 파일을 SSH로 원격 서버에 보내고 원격 경로를 클립보드에 복사한다 — 원격 Claude Code/CLI 세션에 스크린샷을 붙여넣기 위한 도구. 반대 방향(Pull)도 지원.
...
| Runtime (side effect) | `src/runtime/` | `system.ts` — ssh/scp spawn, Keychain, askpass, `~/.ssh` config 쓰기, 클립보드 PNG 추출 전부. `store.ts` — LocalStorage (recents, host별 authMode) |
...
- `send-file-to-server` (view): 공유 서버 셀렉터 겸 Finder 파일 전송. `parseSelectorContext`(`src/lib/launchContext.ts`)가 비신뢰 context를 정규화. 파일 목록은 절대 context로 받지 않고 실행 시점 Finder 선택으로만 읽는다.
...
- **비밀번호는 argv·디스크·LocalStorage에 절대 노출 금지**: Keychain + `SSH_ASKPASS` helper로 조회, 1회용 PW는 FIFO(`withPasswordPipe`)로 전달.
- **경로 검증**: 원격 경로·basename은 `validateRemotePath`/`isSafeRemoteDir`/`isSafeBasename`(`src/lib/validate.ts`)로 metachar·제어문자·`..`·루트/홈 전체 pull 거부. 원격 shell 명령은 `shQuote` escaping, `~/` prefix는 quote 밖에 유지(`shQuotePath`).
- **`~/.ssh` 쓰기**: `writeFileAtomicNoSymlink`(symlink 거부 + temp 후 atomic rename)만 사용. 메인 config에는 Include 1줄만, 사용자 동의 + 백업 후 추가.
- 바이너리는 절대경로(`/usr/bin/ssh` 등) 고정, `scp`는 SFTP 프로토콜 전제(macOS 13+).

### Auth 모드

host별 `"key" | "keychain"`이 LocalStorage에 저장(`store.ts`). key 모드는 `BatchMode=yes`, keychain 모드는 askpass + `PubkeyAuthentication=no`. ssh/scp 옵션 조합은 `src/lib/transferArgs.ts`의 `authOpts`가 단일 소스.
