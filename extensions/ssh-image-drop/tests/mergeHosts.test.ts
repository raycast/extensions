...
import { mergeHosts, parseAdditionalHosts } from "../src/lib/mergeHosts";

describe("mergeHosts", () => {
  it("orders recent → managed → config → additional, dedup keeps first occurrence", () => {
    ...
    expect(out).toEqual([
      ...
      { name: "user@192.0.2.4", source: "additional" },
    ]);
  });
...
describe("parseAdditionalHosts", () => {
  it("splits by comma, trims, drops empties and invalid hosts", () => {
    expect(parseAdditionalHosts(" a , user@b ,, -bad , c\nd ")).toEqual([
      "a",
      "user@b",
    ]);
  });
  it("returns empty for empty input", () => {
    expect(parseAdditionalHosts("")).toEqual([]);
  });
});
