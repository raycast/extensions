...
export function parseSelectorContext(raw: unknown): SelectorContext {
  ...
  // 원격 클립보드 주입 — 전송 대상은 context로 받지 않는다. 선택 텍스트는 LocalStorage
  // 핸드오프로 넘어온다(store.takePendingSelection): context는 비신뢰라 조작된 딥링크가
  // 공격자 문자열을 "사용자가 지정한 텍스트"로 위장해 원격에 주입할 수 있다.
  if (o.payload === "remote-clipboard") return { payload: "remote-clipboard" };
  if (o.payload === "pull") {
