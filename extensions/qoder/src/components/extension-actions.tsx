...
export function OpenExtensionByIDInQoderAction(props: {
  extensionID: string;
  ...
  return (
    <Action.OpenInBrowser
      title="Open in Qoder"
      url={`qoder:extension/${props.extensionID}`}
      icon={"icon.png"}
      onOpen={(url) => {
        showHUD("Open Qoder Extension");
        if (props.onOpen) {
          props.onOpen(url);
        }
      }}
...
export function OpenExtensionByIDInBrowserAction(props: { extensionID: string }): JSX.Element {
  ...
  return (
    <Action.OpenInBrowser
      ...
      onOpen={() => {
        showHUD("Open Qoder Extension in Browser");
      }}
    />
  );
}
