...
describe("ai-by-vercel model provider", () => {
...
it("discovers chat models with catalog capabilities and excludes other output types", async () => {
  const entry = {
    id: "vendor/chat",
    name: "Chat",
    type: "language",
  ...
  const fetcher = vi.fn<typeof fetch>().mockResolvedValue(
    Response.json({
      data: [
        entry,
        { ...entry, id: "embed", type: "embedding", context_window: 0 },
        { ...entry, id: "image", modalities: { input: ["text"], output: ["text", "image"] } },
      ],
    }),
  );
  ...
  expect(fetcher.mock.calls[0][0]).toBe("https://ai-gateway.vercel.sh/v1/models");
});
