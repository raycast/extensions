...
export default function Laureates(props: LaunchProps<{ arguments: Arguments.NobelPrizes }>) {
    ...
    const { isLoading, data, error, pagination } = useFetch(
        (options) =>
            ...
            new URLSearchParams({ offset: String((options.page) * DEAULT_LIMIT), limit: DEAULT_LIMIT.toString(), nobelPrizeYear, nobelPrizeCategory }).toString(),
        {
    ...
    return error ? <Detail navigationTitle="Error" markdown={JSON.stringify(error)} /> : <List navigationTitle={navigationTitle} isLoading={isLoading} isShowingDetail pagination={pagination} searchBarPlaceholder="Filter Nobel Prizes (locally)">
        {data.map((prize, prizeIndex) => <List.Item key={prizeIndex} title={`${prize.awardYear} - ${prize.category.en}`}
            ...
            detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
                ...
                <List.Item.Detail.Metadata.Label title="Top Motivation (en)" text={prize.topMotivation?.en || "N/A"} />
                ...
                {prize.laureates?.map(laureate => <Fragment key={laureate.id}>
                    {("orgName" in laureate) ? 
