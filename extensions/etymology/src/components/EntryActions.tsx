...
export function EntryActions(props: Props) {
  const { entry, view, onToggleView, onReload, isFavorite, onToggleFavorite, renderAncestor } = props;
  ...
  const followable = dedupe(
    entry.sections.flatMap((s) => (s.tree ? ancestors(s.tree) : [])).filter((n) => n.hasPage && n.title),
  );
  ...
  return (
    <ActionPanel>
      ...
      <ActionPanel.Section title="Copy">
        {chain && (
          <Action.CopyToClipboard title="Copy Chain" content={chain} shortcut={Keyboard.Shortcut.Common.Copy} />
        )}
        ...
        />
        <Action.CopyToClipboard title="Copy Etymology" content={plainSummary(entry)} shortcut={COPY_PROSE} />
        <Action.CopyToClipboard title="Copy Wiktionary Link" content={entry.pageUrl} shortcut={COPY_LINK} />
      </ActionPanel.Section>

      <ActionPanel.Section title="Open">
        <Action.OpenInBrowser title="Open in Wiktionary" url={entry.pageUrl} shortcut={Keyboard.Shortcut.Common.Open} />
      ...
      <ActionPanel.Section>
        <Action
          ...
          onAction={() => onToggleFavorite({ term: entry.term, lang: entry.lang, langName: entry.langName })}
        />
        <Action
