...
- Added punctuation preservation support to Capital Case transformation
- Added `Keyboard.Shortcut.Common.Copy` shortcut for Copy to Clipboard action
- Improved code organization and readability:
  - Extracted shared action handler logic to reduce duplication
  - Memoized case conversions for better performance
  - Moved async initialization logic into proper `useEffect` hooks
  - Simplified `upperFirst` and `lowerFirst` implementations
  - Removed module-level preference caching in favor of per-call retrieval
