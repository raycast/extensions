...
"Open Commit in Browser", "Copy Commit URL", and "Copy Commit SHA" link a version row to the `raycast/extensions` commit that added it. The commits come from a billed `api.github.com/…/commits?path=extensions/{slug}/CHANGELOG.md` call in `src/hooks/useChangelogCommits.ts`. That call is cached for 15 minutes per extension, because the fetch re-runs on every mount and paging through changelogs would otherwise bill each visit. It is skipped during the shared rate-limit cooldown, starts that cooldown on a genuine limit, and fails soft to no actions. `pairVersionCommits()` in `src/utils/changelog.ts` matches rows to commits by date. A stamped heading's commit merged within two days of the start of its date (UTC): CI stamps in UTC or US Pacific time, sometimes a day behind the merge, and never ahead of it in the samples. The function walks both lists newest-first, reversing an oldest-first file. A row that cannot be paired has no commit actions, and heading text is never compared with commit messages. Measured 2026-09-24 against the commit diffs: of 157 headings in 30 extensions, 144 were correct, 10 unpaired, and 3 wrong. All three carried a hand-typed date. A fresh 144 headings came out 135 correct, 9 unpaired, and 0 wrong. A one-Pacific-day window was tried and was strictly worse. Pairing only by position fails on any commit that edits the file without adding a version.
