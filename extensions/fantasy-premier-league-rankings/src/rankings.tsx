...
export default function getUserRankings() {
  ...
  const userTeam = useMemo(() => userTeamByGameweek?.picks?.map((pick) => {
    const player = bootstrapData?.elements?.find((player) => player.id === pick.element);
    const position = bootstrapData?.element_types?.find((position: { id: number | undefined; }) => position.id === player?.element_type);
            ...
            <List.Item
                ...
                <List.Item.Detail
                  metadata={
                    <List.Item.Detail.Metadata>
                      ...
                      <List.Item.Detail.Metadata.Label
                        title="Current Starting Team"
                      />
                      <List.Item.Detail.Metadata.Label
                        title="GK"
                        text={userTeam?.filter((player) => player.positionName === 'GKP' && player.benched === false).map((player) => `${player.name}`).toString()}
                      />
                      <List.Item.Detail.Metadata.Label
                        title="Defence"
                        text={userTeam?.filter((player) => player.positionName === 'DEF' && player.benched === false).map((player) => ` ${player.name?.split(',')[0]}`).toString()}
                      />
                      <List.Item.Detail.Metadata.Label
                        title="Midfield"
                        text={userTeam?.filter((player) => player.positionName === 'MID' && player.benched === false).map((player) => ` ${player.name?.split(',')[0]}`).toString()}
                      />
                      <List.Item.Detail.Metadata.Label
                        title="Fowards"
                        text={userTeam?.filter((player) => player.positionName === 'FWD' && player.benched === false).map((player) => ` ${player.name?.split(',')[0]}`).toString()}
                      />
                      <List.Item.Detail.Metadata.Label
                        title="Bench"
                        text={userTeam?.filter((player) => player.benched === true).map((player) => ` ${player.name?.split(',')[0]}`).toString()}
                      />
                    </List.Item.Detail.Metadata>
              ...
              actions={
                <ActionPanel>
                  ...
                  />
                  <Action.OpenInBrowser url={`https://fantasy.premierleague.com/entry/${result.entry}/event/${user?.current_event}`}
