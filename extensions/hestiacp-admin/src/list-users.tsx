...
import ListWebDomainsComponent from "./components/ListWebDomainsComponent";
import ListDatabasesComponent from "./components/ListDatabasesComponent";
import ListMailDomainsComponent from "./components/ListMailDomainsComponent";
import ListItemDetailComponent from "./components/ListItemDetailComponent";
import { getUserAuthLog, getUserLogs, getUserNotifications, getUserPackages, getUserStats, getUsers } from "./utils/hestia";
...
export default function ListUsers() {
    ...
    return error ? <ErrorComponent error={error} /> : <List isLoading={isLoading} isShowingDetail>
        {users && <List.Section title={`${Object.keys(users).length} users`}>
            {Object.entries(users).map(([user, data]) => <List.Item key={user} title={user} icon={{ source: data.ROLE==="admin" ? Icon.PersonCircle : Icon.Person, tintColor: data.SUSPENDED==="yes" ? Color.Red : Color.Green }} detail={<ListItemDetailComponent data={data} />} actions={<ActionPanel>
            ...
            <ActionPanel.Section>
                <ActionPanel.Submenu title="Go To" icon={Icon.ArrowRight}>
                    <Action.Push title="Web Domains" icon={Icon.Globe} target={<ListWebDomainsComponent user={user} />} />
                    <Action.Push title="Databases" icon={Icon.Coin} target={<ListDatabasesComponent user={user} />} />
                    <Action.Push title="Mail Domains" icon={Icon.Envelope} target={<ListMailDomainsComponent user={user} />} />
                </ActionPanel.Submenu>
            </ActionPanel.Section>
            <ActionPanel.Section>
                ...
                <Action.Push title="View Login History" icon={Icon.Binoculars} target={<ViewUserAuthLog user={user} />} />
                <Action.Push title="View Notifications" icon={Icon.Bell} target={<ViewUserNotifications user={user} />} />
            </ActionPanel.Section>
...
export function ViewUserStats({ user }: ViewUserStatsProps) {
    ...
    return <List navigationTitle="View User Stats" isLoading={isLoading} isShowingDetail>
        {stats && Object.entries(stats).map(([date, data]) => <List.Item key={date} title={date} detail={<ListItemDetailComponent data={data} />} accessories={[{date: new Date(date)}]} />)}
    </List>
}
...
export function ViewUserLogs({ user }: ViewUserLogsProps) {
    ...
    const filteredLogs = !logs ? undefined : !filter ? logs : Object.fromEntries(Object.entries(logs).filter(([, data]) => {
    ...
    return <List navigationTitle="View User Logs" isLoading={isLoading} isShowingDetail searchBarAccessory={<List.Dropdown tooltip="Filter" onChange={(val) => setFilter(val)}>
        <List.Dropdown.Item title="All" value="" icon={Icon.Dot} />
        {logs && <>
            <List.Dropdown.Section title="Level">
                <List.Dropdown.Item title="Info" value="level_Info" icon={{ source: Icon.Info, tintColor: Color.Blue }} />
                <List.Dropdown.Item title="Warning" value="level_Warning" icon={{ source: Icon.Warning, tintColor: Color.Yellow }} />
                <List.Dropdown.Item title="Error" value="level_Error" icon={{ source: Icon.ExclamationMark, tintColor: Color.Red }} />
            </List.Dropdown.Section>
            <List.Dropdown.Section title="Category">
                {Array.from(new Set(Object.values(logs).map(log => log.CATEGORY).flat())).map(category => <List.Dropdown.Item key={category} title={category} value={`category_${category}`} />)}
            </List.Dropdown.Section>
        </>}
    </List.Dropdown>}>
        <List.Section title={filteredLogs && logs && `${Object.keys(filteredLogs).length} of ${Object.keys(logs).length} lines`}>
            ...
            </ActionPanel>} icon={getLogIconByLevel(data.LEVEL)} />)}
        </List.Section>
    </List>
}
...
export function ViewUserAuthLog({ user }: ViewUserAuthLogProps) {
    ...
    return <List navigationTitle="View User Auth Log" isLoading={isLoading} isShowingDetail>
        ...
        .map(([line, data]) => <List.Item key={line} title={line} detail={<List.Item.Detail markdown={data.USER_AGENT} metadata={<List.Item.Detail.Metadata>
            {Object.entries(data).map(([key, val]) => {
                if (val==="success") return <List.Item.Detail.Metadata.TagList key={key} title={key}>
                    <List.Item.Detail.Metadata.TagList.Item text={val} color={val==="success" ? Color.Green : Color.Red} />
        ...
        </List.Item.Detail.Metadata>} />} accessories={[{date: new Date(data.DATE)}]} actions={<ActionPanel>
        ...
        </ActionPanel>} icon={data.STATUS==="success" ? { source: Icon.CheckCircle, tintColor: Color.Green } : { source: Icon.MinusCircle, tintColor: Color.Red }} />)}
    </List>
}
...
export function ViewUserNotifications({ user }: ViewUserNotificationsProps) {
    ...
    return <List navigationTitle="View User Notifications" isLoading={isLoading} isShowingDetail>
        {notifications && Object.entries(notifications)
        // .sort(([keyA], [keyB]) => keyB - keyA) // we reverse so latest log is at top
        .map(([line, data]) => <List.Item key={line} title={line} detail={<List.Item.Detail markdown={data.NOTICE} metadata={<List.Item.Detail.Metadata>
            {Object.entries(data).map(([key, val]) => {
                const { text, icon } = getTextAndIconFromVal(val);
                return <List.Item.Detail.Metadata.Label key={key} title={key} text={text} icon={icon} />
            })}
            ...
            <Action.CopyToClipboard title={`Copy Line#${line} to Clipboard`} content={JSON.stringify(data)} />
            <Action.CopyToClipboard title={`Copy All to Clipboard as JSON`} content={JSON.stringify(notifications)} />
        </ActionPanel>} icon={data.ACK==="yes" ? Icon.BellDisabled : Icon.Bell} />)}
    </List>
}
...
function AddUser({ onUserAdded }: AddUserProps) {
    ...
    return <Form navigationTitle="Add User" isLoading={isLoading} actions={<ActionPanel>
        ...
        <Form.Dropdown title="Package" {...itemProps.package}>
            {userPackages && Object.keys(userPackages).map(packageName => <Form.Dropdown.Item key={packageName} title={packageName} value={packageName} />)}
        </Form.Dropdown>
