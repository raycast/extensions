  ...
  return (
    <List isLoading={isLoading}>
      {tokens.map(token => <List.Item key={token.id} icon={token.status==="unpause" ? { value: {source: Icon.Dot, tintColor: Color.Green }, tooltip: "Active" } : {value: {source: Icon.Dot, tintColor: Color.Red}, tooltip: "Inactive"}} title={token.name} subtitle={generateSubtitle(token)} accessories={[
        ...
        <Action.Push icon={Icon.TextCursor} title="Rename Token" target={<RenameToken token={token} mutate={mutate} />} />
        {token.status==="pause" ? <Action icon={Icon.Play} title="Unpause Token" onAction={() => toggle(token)} /> : <Action icon={Icon.Pause} title="Pause Token" onAction={() => toggle(token)} />}
      </ActionPanel>} />)}
    </List>
  );
}
