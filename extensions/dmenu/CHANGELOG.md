...
- Fixed the picker going stale if you took longer than 20 seconds to choose: the CLI now waits 30 seconds, and if it still gives up, the Raycast list now shows a "timed out" message and closes itself instead of sitting there and failing with "Socket disconnected" on your next click.
- Regenerated `package-lock.json` so a clean install actually resolves `@raycast/api` to the supported `2.2.1`, matching `package.json`.
- Improved the direct-launch setup experience with clearer installation, PATH, and usage instructions.
- Improved the timeout state with clearer messaging and automatic dismissal after the CLI receive timeout.
...
- Added actions to copy the PATH configuration and dmenu usage example from the setup screen.
