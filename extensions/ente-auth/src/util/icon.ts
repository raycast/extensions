...
export function icon(otp: Service, color = "#000000") {
  const icon: SimpleIcon = icons.Get(otp.name.toLocaleLowerCase());
  if (icon) {
    const coloredSvg = changeIconColor(icon.svg, color);
