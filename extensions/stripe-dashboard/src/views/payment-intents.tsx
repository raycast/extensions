...
const PaymentIntents = () => {
  const { isLoading, data: resp } = useStripeApi(ENDPOINTS.PAYMENT_INTENTS);
  const { dashboardUrl } = useStripeDashboard();
  const paymentIntentsResp = get(resp, "data", []) as PaymentIntentResp[];
  const formattedPaymentIntents = paymentIntentsResp.map(resolvePaymentIntent);
...
export default withPropsContext(PaymentIntents);
