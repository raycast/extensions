...
const ConnectedAccounts = () => {
  const { isLoading, data: resp } = useStripeApi(ENDPOINTS.CONNECTED_ACCOUNTS);
  const { dashboardUrl } = useStripeDashboard();
  const connectedAccountsResp = get(resp, "data", []) as ConnectedAccountResp[];
  const formattedConnectedAccounts = connectedAccountsResp.map(resolveConnectedAccount);
...
export default withPropsContext(ConnectedAccounts);
