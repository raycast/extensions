...
export default function Command(props: { draftValues?: TagValues }) {
  ...
  useExec(
    "aws ecr describe-images --repository " +
      repository +
  ...
  useExec("aws ecr describe-repositories --query 'repositories[*].repositoryName'", {
    shell: true,
    onData: (data) => {
...
function UpdateECRTagAction() {
  ...
  const { isLoading, data } = useExec(
    "aws ecr put-image --repository-name " +
      ...
      " --image-tag " +
      formValues.newTag +
      ' --image-manifest "$(aws ecr batch-get-image --repository-name ' +
      formValues.repository +
      " --image-ids imageTag=" +
      formValues.currentTag +
...
function GetImagesList(props: {
  imageList: ImageDetailList;
  currentTag: string;
...
function ImageListItem(props: { imageDigest?: string; imageTag: string }) {
  ...
  return <Form.Dropdown.Item value={imageTag} title={imageTag} />;
}
