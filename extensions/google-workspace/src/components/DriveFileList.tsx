...
export default function DriveFileList({ initialQuery = "", downloadFirst }: DriveFileListProps) {
  ...
  const { data, isLoading } = useCachedPromise(
    async (queryType: QueryTypes, scopeType: ScopeTypes, query: string, parentId?: string) => {
      // A pasted Drive link resolves to the file it points to
      const linkedFileId = getFileIdFromLink(query);
      return linkedFileId
        ? { files: [await getFileById(linkedFileId)] }
  ...
  const isLink = !!getFileIdFromLink(query);
  ...
  return (
    <List
      ...
      searchBarPlaceholder="Search in Drive or paste a link"
      searchBarAccessory={
        <List.Dropdown
          tooltip="Search mode"
    ...
    >
      ...
      {data?.files && data.files.length > 0 && email && (
        <List.Section title={isLink ? "Linked File" : getSectionTitle(queryType)} subtitle={`${data.files.length}`}>
          {data.files.map((file) => (
            <FileListItem
              file={file}
              key={file.id}
