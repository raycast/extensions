...
export default function Command() {
  ...
  const prefs = getPreferenceValues<{ baseUrl: string }>();
  ...
  return (
    <List isLoading={isLoading} navigationTitle="FreshRSS Feeds" searchBarPlaceholder="Search feeds...">
      {!isLoading && feeds.length === 0 ? (
        <List.EmptyView icon={Icon.Rss} title="No Feeds Found" description="Make sure your FreshRSS instance has subscriptions" />
      ) : null}
      {groups.length > 0 ? (
        <List.Section title="Groups">
          {groups.map((group) => (
            <List.Item
              ...
              actions={
                <ActionPanel>
                  <Action.Push title="Browse Group" icon={Icon.Folder} target={<ArticleList streamId={group.id} streamTitle={group.label} />} />
                  <Action.OpenInBrowser url={prefs.baseUrl} title="Open Freshrss" shortcut={{ modifiers: ["cmd", "shift"], key: "f" }} />
                </ActionPanel>
              }
            />
          ))}
      ...
      <List.Section title="Feeds">
        {feeds.map((feed) => (
          <List.Item
            ...
            actions={
              <ActionPanel>
                <Action.Push title="Browse Articles" icon={Icon.Document} target={<ArticleList streamId={feed.id} streamTitle={feed.title} />} />
                ...
                {feed.htmlUrl ? <Action.OpenInBrowser url={feed.htmlUrl} shortcut={{ modifiers: ["cmd", "shift"], key: "o" }} /> : null}
                <Action.OpenInBrowser url={prefs.baseUrl} title="Open Freshrss" shortcut={{ modifiers: ["cmd", "shift"], key: "f" }} />
              </ActionPanel>
            }
          />
