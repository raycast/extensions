...
class FreshRSSClient {
  ...
  async searchWeb(
    ...
    try {
      while (matchedItems.length < targetCount) {
        ...
        const response = await this.fetchWithWebAuth(`${this.baseUrl}/i/?a=rss&search=${encodeURIComponent(query)}&nb=${pageSize}&offset=${offset}`);
  ...
  private articleMatchesFilters(
    ...
    if (stream === "user/-/state/com.google/starred" && !article.categories.includes("user/-/state/com.google/starred")) {
      return false;
    }
    if (stream && !stream.startsWith("user/-/state/com.google/") && !stream.startsWith("user/-/label/") && article.origin?.streamId !== stream) {
      return false;
    }
