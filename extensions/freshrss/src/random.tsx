    ...
    <ArticleDetail
      ...
      onToggleRead={(updatedArticle, markRead) => {
        setArticle({
          ...updatedArticle,
          categories: markRead
            ? [...updatedArticle.categories.filter((c) => c !== "user/-/state/com.google/read"), "user/-/state/com.google/read"]
      ...
      onToggleStar={(updatedArticle, markStarred) => {
        setArticle({
          ...
          categories: markStarred
            ? [...updatedArticle.categories.filter((c) => c !== "user/-/state/com.google/starred"), "user/-/state/com.google/starred"]
            : updatedArticle.categories.filter((c) => c !== "user/-/state/com.google/starred"),
        });
      }}
      extraActions={
        <>
          <Action title="Try Another Random Article" icon={Icon.Shuffle} onAction={loadRandom} shortcut={{ modifiers: ["cmd"], key: "r" }} />
          <Action
