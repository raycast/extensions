...
export default function ArticleDetail({ article, onToggleRead, onToggleStar, extraActions }: ArticleDetailProps) {
  ...
  useEffect(() => {
    if (!read) {
      api.markAsRead(article.id).then(() => {
        setRead(true);
    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label title="Published" text={article.published ? new Date(article.published * 1000).toLocaleString() : "—"} />
          ...
          <Detail.Metadata.Link title="URL" target={url || ""} text={url ? (url.length > 40 ? url.slice(0, 40) + "..." : url) : "—"} />
        </Detail.Metadata>
      }
      actions={
        <ActionPanel>
          <ActionPanel.Section>
            ...
            {url ? <Action.CopyToClipboard content={url} title="Copy URL" shortcut={{ modifiers: ["cmd", "shift"], key: "c" }} /> : null}
          </ActionPanel.Section>
          <ActionPanel.Section title="Reading">
            ...
            />
            <Action title={starred ? "Unstar" : "Star"} icon={Icon.Star} onAction={handleToggleStar} shortcut={{ modifiers: ["cmd"], key: "s" }} />
          </ActionPanel.Section>
