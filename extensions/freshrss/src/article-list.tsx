...
export function cleanTitle(title: string): string {
  return title
    .replace(/\uFFFC/g, "")
    .replace(/^(?:(?:\p{Extended_Pictographic}(?:\uFE0F|\u200D\p{Extended_Pictographic})*)|(?:\p{Regional_Indicator}{2})|(?:[#*0-9]\uFE0F?\u20E3))+\s*/u, "")
    .replace(/\s+/g, " ")
    .trim();
}
...
export default function ArticleList({
  ...
  const navigationTitle =
    statusFilter === "unread" && serverUnreadCount !== null ? `${baseNavigationTitle} • ${serverUnreadCount} unread` : baseNavigationTitle;
  ...
  return (
    ...
    >
      {showLoadingRow ? <List.Item id="loading-search" title="Searching…" subtitle="Fetching results from FreshRSS" icon={Icon.MagnifyingGlass} /> : null}
      {showEmptyView ? (
        <List.EmptyView
          title={searchText.trim() ? "No Results" : "No Articles"}
          description={searchText.trim() ? "Nothing matched your search." : "There are no articles for the selected filters."}
        />
      ) : null}
          ...
          <List.Item
            ...
            accessories={
              [
                ...
                {
                  icon: isStarred ? { source: Icon.Star, tintColor: Color.Yellow } : { source: Icon.Star, tintColor: Color.SecondaryText },
                    ...
                    <ArticleDetail
                      ...
                      onToggleRead={(targetArticle, markRead) => {
                        updateArticle(targetArticle.id, (currentArticle) => ({
                          ...currentArticle,
                          categories: markRead
                            ? [...currentArticle.categories.filter((category) => category !== "user/-/state/com.google/read"), "user/-/state/com.google/read"]
