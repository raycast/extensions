...
export default function Command() {
    ...
    const handleOnTextChange = (value = "") => {
        ...
        const rgbMatch = value.match(/^rgb(a)?\((\d{1,3}),(\s)?(\d{1,3}),(\s)?(\d{1,3})(,(\s)?(?<alpha>\d+\.\d+|\.\d+))?\)$/i);
        if (rgbMatch) {
        ...
        const hslMatch = value.match(/^hsl(a)?\((\d{1,3}),(\s)?(\d{1,3})(%)?,(\s)?(\d{1,3})(%)?(,(\s)?(?<alpha>\d+\.\d+|\.\d+))?\)$/i);
        if (hslMatch) {
            ...
            <List.Section>
                ...
                {hsla &&
                    <List.Item
                        ...
                        actions={
                            <ActionPanel title="Copy">
                                <Action.CopyToClipboard title="Copy to clipboard" content={`hsla(${hsla[0]}, ${hsla[1]}%, ${hsla[2]}%, ${hsla[3]})`} />
                            </ActionPanel>
                        }
                    />
                }
