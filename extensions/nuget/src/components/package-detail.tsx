...
export default function PackageDetail(props: PackageDetailProps): JSX.Element {
  ...
  function renderVersions(): JSX.Element[] {
    ...
    revs.map((v) => {
      _versions.push(<Detail.Metadata.Link
        key={v.version}
  ...
  return (
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.Label title={props.package.id} text={props.package.title} icon={props.package.iconUrl || join(environment.assetsPath, "command-icon.png")} />
          ...
          <Detail.Metadata.TagList title="Tags">
            {
              props.package.tags.map((t) => <Detail.Metadata.TagList.Item key={t} text={t} />)
            }
          </Detail.Metadata.TagList>
          <Detail.Metadata.Separator />
          <Detail.Metadata.Link
            title="About"
            target={nugetBaseUrl + props.package.id}
            text="View in Nuget Website"
          />
          <Detail.Metadata.Link
            title=""
            target={props.package.projectUrl}
