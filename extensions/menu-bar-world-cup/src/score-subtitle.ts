import { updateCommandMetadata, LocalStorage, showHUD } from "@raycast/api";
import {
  getMatches,
  liveMatch,
  formatLine,
...
export default async function Command() {
  ...
  const prev = JSON.parse(
    (await LocalStorage.getItem<string>(STORE_KEY)) ?? "{}",
  ) as ScoreMap;
  ...
  for (const m of matches) {
    ...
    if (a > was.a) await notifyGoal(m, "away");
    if (h > was.h) await notifyGoal(m, "home");
  }

  await LocalStorage.setItem(STORE_KEY, JSON.stringify(current));
}

async function notifyGoal(m: Match, side: "home" | "away") {
  ...
  await showHUD(goalHud(m, scorerCode, userCountry()));
}
