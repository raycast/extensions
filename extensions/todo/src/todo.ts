...
export function create_todo(todo: Todo): Action {
  return (todos: Todo[]) => insert_todo_in_todo_list(
    todo,
    [...(todos || [])]
  )
}
...
export function has_text(text: string): (todo: Todo) => boolean {
  ...
  function subquery_applies(subquery: string): ((todo: Todo) => boolean) {
    ...
    if (matches[1].startsWith("u") )
      return (todo: Todo) => todo.urgent == value
    if (matches[1].startsWith("i") )
      return (todo: Todo) => todo.important == value
