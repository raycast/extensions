...
export default async function Command() {
  const prefs = getPreferenceValues<Preferences.CustomAction1>();

  if (!prefs.customPrompt?.trim()) {
    await closeMainWindow();
    await showHUD(
      "❌ No prompt set — configure it in Raycast Settings → Extensions → Tweak → Custom Action 1",
    );
    return;
  }

  const label = prefs.actionTitle?.trim() || "Custom Action 1";
