...
export default function Command() {
	...
	const { isLoading, data, revalidate } = useFetch(index_url);
	...
	$categories.map((index, value) => {
		...
		let array = $links.map((index, tvalue) => {
	...
	return (
		...
		{categories.map((categoryItem) => (
			...
			{filteredList.filter((elem) => elem.category == categoryItem.key).map((item) => (
				<List.Item
				key={item.link}
				title={item.name} 
				icon={{ 
