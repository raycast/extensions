import { ActionPanel, Action, Icon, useNavigation, showToast, Toast, Form, List, Detail, popToRoot } from "@raycast/api";
...
export default function AccountTokensTab({tokens}: {tokens: ApiTokenResource[]}) {
      ...
      return <List.Item icon={Icon.Key} title="API Tokens" detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
        {tokens.map((token, index) => <List.Item.Detail.Metadata.Label key={token.id} title={`${index+1} - ${token.name}`} text={new Date(token.created_at).toDateString()} />)}
    ...
    async onSubmit(values) {
      const toast = await showToast(Toast.Style.Animated, "Generating token", values.name);
            try {
              ...
              reset();
              push(<Detail markdown={`API Token was added \n\n Copy and securely store your API token. It won't be shown again for security reasons. \n\n---\n\n ${result.access_token}`} actions={<ActionPanel>
                <Action.CopyToClipboard title="Copy & Close" content={result.access_token} concealed onCopy={() => popToRoot()} />
              </ActionPanel>} />)
            } catch (error) {
