...
async function fetchPullRequests(): Promise<PullRequest[]> {
  const token = preferences.token.value as string | undefined;
  ...
  try {
    ...
    if ("errors" in json) {
      throw new Error(json.errors[0]?.message ?? "Could not load Pull Requests");
    } else if ("message" in json) {
      throw new Error(json.message ?? "Could not load Pull Requests");
    }
    ...
    const extractPR = (pr: PullRequest, forceIncludeCondition: boolean) => {
      ...
      if (
        ...
        ignoredRepos.includes(pr.repository.nameWithOwner) ||
