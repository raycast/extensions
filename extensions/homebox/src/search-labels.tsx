...
export default function SearchLabels() {
    ...
    return <List isLoading={isLoading}>
        {labels.map(label => <List.Item key={label.id} icon={{source: Icon.Tag, tintColor: label.color}} title={label.name} subtitle={label.description} actions={<ActionPanel>
            ...
            <Action icon={Icon.Trash} title="Delete" onAction={() => confirmAlert({
                  ...
                  primaryAction: {
                    ...
                    async onAction() {
                      ...
                      try {
                        await mutate(
                          homebox.labels.delete(label.id), {
