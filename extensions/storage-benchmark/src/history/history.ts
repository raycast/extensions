...
export class BenchmarkHistory {
  ...
  async recordSuccess(input: BenchmarkRunInput): Promise<void> {
    await this.update((document) => {
      ...
      volume.successfulRuns = [input, ...volume.successfulRuns.filter((run) => run.id !== input.id)].sort(
      ...
      if (!baseline) {
        ...
        baseline.runId !== input.id &&
        resultsAreClose(baseline.result, input.result)
      ) {
  ...
  async deleteVolume(volumeId: string): Promise<void> {
    await this.update((document) => {
      if (!document.volumes[volumeId]) return false;
      delete document.volumes[volumeId];
    });
  }

  private async update(operation: (document: HistorySnapshot) => void | false): Promise<void> {
    await this.store.runExclusive(async () => {
      const document = await this.load();
      if (operation(document) !== false) await this.save(document);
    });
  }
