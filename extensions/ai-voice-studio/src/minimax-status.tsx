...
export default function MinimaxMenuBarStatus() {
  const [prefRate, setPrefRate] = useState(1);
  const [state, setState] = useState<NowPlayingState | null | undefined>(undefined);
  const [override, setOverride] = useState<number | null>(null);

  useEffect(() => {
    Promise.all([getNowPlaying(), getSpeedOverride(), getMinimaxSettings()]).then(([s, r, settings]) => {
      setState(s);
      setOverride(r);
  ...
  return (
    <MenuBarExtra icon={icon} title={title} isLoading={isLoading} tooltip="MiniMax Status">
      {isActive && state ? (
        <>
      ...
      <MenuBarExtra.Section>
        <MenuBarExtra.Item
          title="Read with MiniMax Voice"
          icon={Icon.SpeakerHigh}
          onAction={launch("minimax-read-with-voice")}
        />
        <MenuBarExtra.Item
          title="Set MiniMax Quick Read Voice"
          icon={Icon.Star}
          onAction={launch("minimax-select-voice")}
        />
