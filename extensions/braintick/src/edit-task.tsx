...
import { Project, Task, UpdateTaskInput } from "./types";
...
export default function EditTask({ task, onTaskUpdated }: EditTaskProps) {
  ...
  const [syncWithHuly, setSyncWithHuly] = useState(task.sync_with_huly);
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Update Task" icon={Icon.Pencil} onSubmit={handleSubmit} />
        </ActionPanel>
      }
    >
