...
- A sheet file that is not valid JSON, or is missing a field Keysi requires — an id, a name, a title on a group or item — is skipped rather than failing the command, so one hand-written mistake does not hide every other sheet
