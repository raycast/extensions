import { List, ActionPanel, Action, Icon, LocalStorage, showToast, updateCommandMetadata, Toast } from "@raycast/api";
...
import { abbreviateName } from "./helpers/abbriviations";
...
export default function Command() {
  ...
  const fetchAndSetArrivals = useCallback(async () => {
    ...
    const sortedArrivals = arrivals.sort((a, b) => new Date(a.expectedArrival).getTime() - new Date(b.expectedArrival).getTime());
  ...
  const handleDeselect = (item: MatchedStop) => setSelectedItems((prevSelected) =>
  ...
  return (
    ...
    >
      ...
      <List.Section title="Next arrivals">
        {sortedArrivals.slice(0, 5).map(renderArrival)}
      </List.Section>
      )}
        ...
        <List.Section title="Selected stops">
          {selectedItems.map((item) => (
            <List.Item
              key={item.id}
              title={item.name}
              accessories={[{ icon: Icon.Checkmark }, ...item.modes.map((mode) => ({ tag: { value: mode, color: getModeColor(mode) } }))]}
              actions={
                <ActionPanel>
        ...
        {(data?.matches || []).map((item) => {
          ...
          return (
            <List.Item
              ...
              accessories={
                isSelected
                  ? [{ icon: Icon.Checkmark }, ...item.modes.map((mode) => ({ tag: { value: mode, color: getModeColor(mode) } }))]
