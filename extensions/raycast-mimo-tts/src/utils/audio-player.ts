...
const PID_FILE = join(tmpdir(), "raycast-mimo-tts.pid");
const STOP_FILE = join(tmpdir(), "raycast-mimo-tts.stop");
...
export class AudioPlayer {
  ...
  private flushPcmBufferToFile(): void {
    ...
    const fileName = `raycast-mimo-tts-pcm-${randomUUID()}.wav`;
  ...
  private saveTempFile(base64Audio: string, format: string): string {
    ...
    const fileName = `raycast-mimo-tts-${randomUUID()}.${extension}`;
    const filePath = join(tmpdir(), fileName);
