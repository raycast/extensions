...
import NewThreadAction from "./new-thread-action";
...
export default function SessionList({
  projectPath,
  initialScope = "Interactive",
}: {
  projectPath?: string;
  initialScope?: ThreadMode;
}) {
  const [scope, setScope] = React.useState<SessionScope>(initialScope);
  ...
  const { data, isLoading, revalidate } = useCachedPromise(loadThreads, [mode, databaseSearch, projectPath], {
  ...
  const atSearchCap = mode !== "Interactive" && data?.truncated;
  ...
  const projectActions = projectPath ? (
    <ActionPanel>
      <NewThreadAction projectPath={projectPath} />
    </ActionPanel>
  ) : undefined;
        ...
        : rows.length > 0
          ? [
              <List.Section
                ...
                subtitle={
                  atSearchCap
                    ? "Showing the newest 200 sessions — refine your search for more specific results."
                    : undefined
                }
              >
                {rows.map((row) => (
  ...
  return (
    <List
      actions={projectActions}
      navigationTitle={projectPath ? projectName(projectPath) : undefined}
      searchBarPlaceholder={projectPath ? `Search sessions in ${projectName(projectPath)}` : "Search sessions"}
      isLoading={isLoading}
      isShowingDetail={isShowingDetail}
      ...
      searchBarAccessory={
        <List.Dropdown tooltip="Session scope" value={scope} onChange={onScopeChange} storeValue={!projectPath}>
          {scopes.map((value) => (
    ...
    >
      {listChildren}
      {rows.length === 0 ? <EmptyState scope={scope} degraded={degraded} actions={projectActions} /> : null}
    </List>
  );
}
