...
export default function ListApps() {
    ...
    return <List isLoading={isLoadingApps || isLoadingServices} isShowingDetail>
        {apps.map(app => <List.Section key={app.id} title={app.name}>
            {services.filter(service => service.app_id===app.id).map(service => <List.Item key={service.id} icon={SERVICE_TYPE_ICONS[service.type]} title={service.name} detail={<List.Item.Detail metadata={<List.Item.Detail.Metadata>
                <List.Item.Detail.Metadata.TagList title="Type">
                    <List.Item.Detail.Metadata.TagList.Item text={service.type} />
                </List.Item.Detail.Metadata.TagList>
                <List.Item.Detail.Metadata.Link title="Domain" text={app.domains[0].name} target={`https://${app.domains[0].name}`} />
