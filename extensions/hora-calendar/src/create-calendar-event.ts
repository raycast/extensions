...
export default async function Command(props: { arguments: Arguments.CreateCalendarEvent }) {
  const { calendarName, closeWindowOnAdd, dateLocale } = getPreferenceValues<Preferences.CreateCalendarEvent>();
  ...
  try {
    ...
    // online.
    const when = formatWhen(event, dateLocale);
    toast.message = event.synced ? when : `${when} · waiting to sync`;
  } catch (error) {
    await showFailure(error, "Could not add the event");
  }
}

function formatWhen(event: { start: string; isAllDay: boolean }, locale: string): string {
  const start = new Date(event.start);
  return event.isAllDay
    ? start.toLocaleDateString(locale, { weekday: "long", month: "long", day: "numeric" })
    : start.toLocaleString(locale, { weekday: "long", hour: "numeric", minute: "2-digit" });
}
