...
async function getHeaders() {
  ...
  if (!token) {
    throw new Error("Not authenticated. Please connect your device or add an API token.");
  }
...
export async function initDeviceLink(deviceId: string, deviceName: string): Promise<DeviceLinkResponse> {
...
export async function checkDeviceStatus(code: string): Promise<DeviceStatusResponse> {
  const res = await fetch(`${BASE_URL}/auth/device/status?code=${encodeURIComponent(code)}`, {
    method: "GET",
  });
...
export async function getBookmarks(since?: string): Promise<Bookmark[]> {
  const url = since ? `/bookmarks?since=${encodeURIComponent(since)}` : "/bookmarks";
  const data = await request<{ bookmarks?: Bookmark[]; data?: Bookmark[] } | Bookmark[]>(url);
  // Handle various response shapes
  if (Array.isArray(data)) return data;
  if ("bookmarks" in data && Array.isArray(data.bookmarks)) return data.bookmarks;
...
export async function getFolders(parentId?: string): Promise<Folder[]> {
  ...
  const data = await request<{ folders?: Folder[]; data?: Folder[] } | Folder[]>(url);
  if (Array.isArray(data)) return data;
...
export async function moveBookmarksToFolder(folderId: string, bookmarkIds: string[]): Promise<void> {
