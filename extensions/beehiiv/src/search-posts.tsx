...
function PostListItem({ post }: { post: Post }) {
  ...
  let markdown = `![Thumbnail](${post.thumbnail_url})`;
  return (
    <List.Item
      id={post.id}
      title={post.title}
      accessories={[
        { icon: post.status === "confirmed" ? (post.publish_date * 1000 < new Date().getTime() ? Icon.CheckCircle : Icon.Clock) : 
          (post.status === "archived" ? Icon.CircleDisabled : Icon.Circle) },
      ]}
      actions={
        <ActionPanel>
          {post.status === "confirmed" && (<Action.OpenInBrowser title="Open Performance" url={`https://app.beehiiv.com/posts/${post.id.replace("post_","")}/analytics`} />)}
          {post.status === "confirmed" && (<Action.OpenInBrowser title="Open Published" url={post.web_url} />)}
          <Action.OpenInBrowser title="Open Overview" url={`https://app.beehiiv.com/posts/${post.id.replace("post_","")}`} />
          <Action.OpenInBrowser title="Edit Post" icon={Icon.Pencil} url={`https://app.beehiiv.com/posts/${post.id.replace("post_","")}/edit`} />
          <Action.CopyToClipboard title="Copy URL" content={post.web_url} icon={Icon.Link} shortcut={{ modifiers: ["cmd"], key: "." }} />
          <Action.CopyToClipboard title="Copy Title" content={post.title} icon={Icon.Text} shortcut={{ modifiers: ["cmd"], key: "c" }} />
        ...
        <List.Item.Detail
          ...
          metadata={
            <List.Item.Detail.Metadata>
              <List.Item.Detail.Metadata.TagList title="Status">
                {(
                  post.status == "confirmed" ? 
                    ...
                    <List.Item.Detail.Metadata.TagList.Item text={parseStatus(post.status)} icon={Icon.CheckCircle} color={"green"} /> : 
                    ...
                    <List.Item.Detail.Metadata.TagList.Item text={parseStatus(post.status)} icon={Icon.CircleDisabled} color={"gray"} />
                )}
              </List.Item.Detail.Metadata.TagList>
              <List.Item.Detail.Metadata.Label title="Date" text={parseDate(post.displayed_date || post.publish_date)} icon={Icon.Calendar} />
              ...
              <List.Item.Detail.Metadata.TagList.Item text={post.stats.email.recipients.toString()} icon={Icon.Envelope} />
              <List.Item.Detail.Metadata.TagList.Item text={post.stats.email.unique_opens.toString()} icon={Icon.Eye} />
              <List.Item.Detail.Metadata.TagList.Item text={post.stats.email.clicks.toString()} icon={Icon.ArrowNe} />
              </List.Item.Detail.Metadata.TagList>
              )}
              {post.stats?.web && (
              ...
              <List.Item.Detail.Metadata.TagList.Item text={post.stats.web.clicks.toString()} icon={Icon.ArrowNe} />
              </List.Item.Detail.Metadata.TagList>
              )}
            </List.Item.Detail.Metadata>
          }
