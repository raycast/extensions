...
export function ConvertTime(searchText: string): TimeItem[] {
  ...
  if (searchText.length == 10 && searchText.match(/^\d{10}/)) {
    ...
    items = [
      { id: 1, val: dateString, icon: "📅", isCurrent: true, tType: "date", ctime: dateString }
    ];
    ...
    if (currentTime == false) {
      // bugs
      items = [
        { id: 1, val: "wrong time format", icon: "🐞️", isCurrent: true, tType: "wrong", ctime: "" }
      ];
    }else{
      const dateString = convertTimestamp(currentTime);
      console.log(currentTime)
      items = [
        { id: 1, val: currentTime.toString(), icon: "⏱️", isCurrent: true, tType: "time", ctime: dateString },
        { id: 2, val: dateString, icon: "📅", isCurrent: true, tType: "date", ctime: dateString }
      ];
    }
    ...
    // bugs
    items = [
       { id: 1, val: "waiting...... ", icon: "⌨️", isCurrent: true, tType: "wrong", ctime: "" }
     ];
  }
