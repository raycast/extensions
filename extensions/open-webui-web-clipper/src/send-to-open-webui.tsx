...
import { DEFAULT_MODEL_KEY, LAST_FOLDER_KEY, SEND_FORM_STATE_KEY, shouldOpenAfterMainSend } from "./settings";
import type { CaptureMode, ChatRecord, Folder, ModelInfo, PreferencesShape, PromptMode, SendMode } from "./types";
...
function isPromptMode(value: unknown): value is PromptMode {
  return value === "none" || value === "discuss" || value === "summarize" || value === "research";
}
...
function parsePersistedFormState(raw?: string): PersistedFormState {
  ...
  try {
    ...
    return {
      captureMode: isCaptureMode(value.captureMode) ? value.captureMode : DEFAULT_FORM_STATE.captureMode,
      sendMode: isSendMode(value.sendMode) ? value.sendMode : DEFAULT_FORM_STATE.sendMode,
      promptMode: isPromptMode(value.promptMode) ? value.promptMode : DEFAULT_FORM_STATE.promptMode,
      generate: typeof value.generate === "boolean" ? value.generate : DEFAULT_FORM_STATE.generate,
    };
  } catch {
...
export default function Command() {
  ...
  const client = useMemo(() => new OpenWebUIClient(prefs), [prefs.baseUrl, prefs.apiKey]);
  const [folders, setFolders] = useState<Folder[]>([]);
  const [models, setModels] = useState<ModelInfo[]>([]);
  const [chats, setChats] = useState<ChatRecord[]>([]);
  const [captureMode, setCaptureMode] = useState<CaptureMode>(DEFAULT_FORM_STATE.captureMode);
  const [folderId, setFolderId] = useState("");
  const [modelId, setModelId] = useState("");
  const [chatId, setChatId] = useState("");
  const [sendMode, setSendMode] = useState<SendMode>(DEFAULT_FORM_STATE.sendMode);
  const [promptMode, setPromptMode] = useState<PromptMode>(DEFAULT_FORM_STATE.promptMode);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [folderList, modelList, lastFolderId, defaultModelId, savedFormRaw] = await Promise.all([
          client.getFolders(),
          client.getModels(),
        ...
        const rememberedFolder = lastFolderId && folderList.some((folder) => folder.id === lastFolderId)
          ? lastFolderId
          : folderList[0]?.id ?? "";
        setFolderId(rememberedFolder);

        const preferredModel = defaultModelId && modelList.some((model) => model.id === defaultModelId)
          ? defaultModelId
          : modelList[0]?.id ?? "";
        setModelId(preferredModel);
      } catch (error) {
        await showToast({ style: Toast.Style.Failure, title: "Could Not Connect to Open WebUI", message: String(error) });
      } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setChats(list);
          setChatId((current) => (current && list.some((chat) => chat.id === current) ? current : (list[0]?.id ?? "")));
        }
      } catch (error) {
        if (!cancelled) {
          setChats([]);
          await showToast({ style: Toast.Style.Failure, title: "Could Not Load Chats", message: String(error) });
        }
      }
    })();
    return () => {
  ...
  async function finishSuccessfulSubmit(destinationUrl?: string): Promise<void> {
  ...
  async function handleSubmit(values: FormValues) {
    const toast = await showToast({ style: Toast.Style.Animated, title: "Capturing Content…" });
    try {
      if (!values.folderId) throw new Error("No Open WebUI folder is available. Create at least one folder first.");
      ...
      if (values.sendMode === "save") {
        ...
        await finishSuccessfulSubmit(shouldOpenAfterMainSend(prefs.openAfterSendMode) ? client.folderUrl(values.folderId) : undefined);
        return;
      }

      const prompt = buildPrompt(clip, values.promptMode ?? promptMode, values.customInstruction ?? customInstruction);
      if (values.sendMode === "chat") {
        ...
        toast.style = Toast.Style.Success;
        toast.title = result.warnings.length ? "Chat Created With Warning" : "Chat Created, File Attached";
        toast.message = result.warnings[0] || result.chat.title || chatTitleFromClip(clip);
        await finishSuccessfulSubmit(shouldOpenAfterMainSend(prefs.openAfterSendMode) ? client.chatUrl(result.chat.id) : undefined);
        return;
      }

      const selectedChatId = values.chatId || chatId;
      if (!selectedChatId) throw new Error("Choose an existing chat.");
      const result = await client.appendToChat(selectedChatId, prompt, clip, values.generate ?? generate);
      toast.style = Toast.Style.Success;
      toast.title = result.warnings.length ? "Content Added With Warning" : "Content and File Added to Chat";
  ...
  const sortedFolders = [...folders].sort((a, b) => folderLabel(a, folders).localeCompare(folderLabel(b, folders), "en"));
  return (
    <Form
      isLoading={loading}
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Send to Open WebUI" onSubmit={handleSubmit} />
        </ActionPanel>
      }
    >
      <Form.Dropdown id="captureMode" title="Capture" value={captureMode} onChange={(value: string) => setCaptureMode(value as CaptureMode)}>
      ...
      <Form.Dropdown id="sendMode" title="Action" value={sendMode} onChange={(value: string) => setSendMode(value as SendMode)}>
        <Form.Dropdown.Item value="chat" title="Create Chat in Folder" />
        <Form.Dropdown.Item value="save" title="Save as Folder Source Without Chat" />
        <Form.Dropdown.Item value="append" title="Append to Existing Chat" />
      </Form.Dropdown>

      <Form.Dropdown id="folderId" title="Folder" value={folderId} onChange={setFolderId}>
        {sortedFolders.map((folder) => (
          <Form.Dropdown.Item key={folder.id} value={folder.id} title={folderLabel(folder, folders)} />
        ))}
      </Form.Dropdown>

      {sendMode === "chat" ? (
        <Form.Dropdown id="model" title="Model" value={modelId} onChange={setModelId}>
          {models.map((model) => (
            <Form.Dropdown.Item key={model.id} value={model.id} title={model.name || model.id} />
          ))}
        </Form.Dropdown>
      ) : (
        <Form.Description title="Model" text={sendMode === "append" ? "The selected chat's model will be used." : "Saving a folder source does not run a model."} />
      )}

      {sendMode === "append" && (
        <Form.Dropdown id="chatId" title="Chat" value={chatId} onChange={setChatId}>
          {chats.map((chat) => (
            <Form.Dropdown.Item key={chat.id} value={chat.id} title={chat.title || chat.id} />
          ))}
        </Form.Dropdown>
      )}
      ...
      {sendMode !== "save" && (
        <>
          <Form.Dropdown id="promptMode" title="Prompt" value={promptMode} onChange={(value: string) => setPromptMode(value as PromptMode)}>
