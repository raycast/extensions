...
export default async function Command() {
  ...
  if (!folderId) {
    await showToast({
      ...
      message:
        "Run “Send Web Content” once and choose a folder. Save Page and Save Selection will reuse it automatically.",
    });
    return;
  }
  ...
  const toast = await showToast({
    ...
    title: "Saving Page and Starting Model…",
  });
  try {
    const client = new OpenWebUIClient(prefs);
    const clip = await capture("page");
    ...
    const result = await client.startQuickChat(
      folderId,
      model,
      prompt,
    ...
    toast.title = result.warnings.length
      ...
      : "Page Saved, Model Started";
    toast.message =
      result.warnings[0] || result.chat.title || chatTitleFromClip(clip);

    if (shouldOpenAfterQuickSave(prefs.openAfterSendMode))
      await open(client.chatUrl(result.chat.id));
  } catch (error) {
    toast.style = Toast.Style.Failure;
    toast.title = "Could Not Save Page";
