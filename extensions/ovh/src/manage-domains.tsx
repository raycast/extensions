...
export default function ManageDomains() {
  ...
  return <List isLoading={isLoading} isShowingDetail={isShowingDetail}>
    {domains.map(domain => <List.Item key={domain.serviceId} icon={getFavicon(`https://${domain.domain}`, {fallback: Icon.Globe})} title={domain.domain} accessories={!isShowingDetail ? generateDomainAccessories(domain) : undefined} detail={<List.Item.Detail markdown={domain.nameServers.map(ns => ns.nameServer).join(`\n\n`)} metadata={<List.Item.Detail.Metadata>
      <List.Item.Detail.Metadata.Label title="General information" />
      <List.Item.Detail.Metadata.TagList title="DNS servers">
        <List.Item.Detail.Metadata.TagList.Item text={domain.nameServerType==="external" ? "Custom" : domain.nameServerType} />
      ...
      <List.Item.Detail.Metadata.TagList title="Protection against domain name transfer">
        {domain.transferLockStatus==="locked" ? <List.Item.Detail.Metadata.TagList.Item icon={Icon.Check} text="Enabled" color={Color.Green} /> : <List.Item.Detail.Metadata.TagList.Item icon={Icon.Xmark} text="Disabled" color={Color.Red} />}
      </List.Item.Detail.Metadata.TagList>
      <List.Item.Detail.Metadata.TagList title="Secured Delegation - DNSSEC">
        {domain.dnssecState==="enabled" ? <List.Item.Detail.Metadata.TagList.Item icon={Icon.Check} text="Enabled" color={Color.Green} /> : <List.Item.Detail.Metadata.TagList.Item icon={Icon.Xmark} text="Disabled" color={Color.Red} />}
      </List.Item.Detail.Metadata.TagList>
      ...
      <Action shortcut={Keyboard.Shortcut.Common.ToggleQuickLook} icon={Icon.AppWindowSidebarLeft} title="Toggle Details" onAction={() => setIsShowingDetail(show => !show)} />
    </ActionPanel>} />)}
  </List>
}
...
<Form.Dropdown title="Type" {...itemProps.fieldType}>
  {Object.entries(types).map(([section, items]) => <Form.Dropdown.Section key={section} title={section}>
