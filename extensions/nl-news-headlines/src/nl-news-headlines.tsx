...
const checkCacheValidity = (cachedData: string | undefined, cachedLanguage: string | undefined, currentLanguage: string): boolean => {
...
const fetchNewsWithCache = async (sources: NewsSource[], language: string, page: number): Promise<NewsItem[]> => {
  ...
  await Promise.allSettled(sourcesToFetch.map(async (source) => {
    try {
...
export default function Command() {
  ...
  const fetchNews = useCallback(async (forceRefresh = false, page = 1) => {
  ...
  const filteredItems = useMemo(() => selectedSource 
