...
import { CometActions } from "./CometActions";
...
function HistoryItem({
  ...
  return (
    <List.Item
      ...
      icon={getFavicon(url)}
      actions={
        <CometActions.TabHistory title={title} url={url} profile={profile} />
      }
    />
  );
}
...
function TabListItem(props: {
  ...
  return (
    <List.Item
      ...
      actions={
        <CometActions.TabList tab={props.tab} onTabClosed={props.onTabClosed} />
      }
      icon={
        props.useOriginalFavicon
          ? props.tab.realFavicon()
          : props.tab.googleFavicon()
      }
    />
  );
}
