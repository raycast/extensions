...
import { authorize, logoutAndClearToken } from "./oauth";
...
export default function Command() {
  ...
  const { handleSubmit, itemProps } = useForm<EventFormValues>({
    initialValues: {
      eventName: "",
      eventStartTime: null,
      eventDuration: "30",
    ...
    async onSubmit(values) {
      try {
        ...
        await createGoogleEvent(values, accessToken);
  ...
  return (
    ...
    >
      <Form.TextField title="Title" placeholder="Enter Event Title" {...itemProps.eventName} />
      <Form.DatePicker title="Start Time" {...itemProps.eventStartTime} />
      <Form.Dropdown title="Duration" {...itemProps.eventDuration}>
        {[15, 30, 45, 60, 75, 90, 120, 150, 180, 210, 240, 270, 300].map((duration) => {
          ...
          return <Form.Dropdown.Item value={duration.toString()} title={title} key={duration} />;
        })}
      </Form.Dropdown>
      <Form.Dropdown title="Color" {...itemProps.eventColor}>
      ...
      <Form.TextArea title="Description" placeholder="Add a description (optional)" {...itemProps.eventDescription} />
      <Form.Dropdown title="Availability" {...itemProps.eventAvailability}>
        <Form.Dropdown.Item value="busy" title="Busy" />
        <Form.Dropdown.Item value="free" title="Free" />
      </Form.Dropdown>
    </Form>
  );
}
