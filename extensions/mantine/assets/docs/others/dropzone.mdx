...
function Demo() {
  return <Dropzone accept={["image/png", "image/jpeg", "image/sgv+xml", "image/gif"]}>{/* children */}</Dropzone>;
}
```
...
function Demo() {
  return (
    <Dropzone accept={[MIME_TYPES.png, MIME_TYPES.jpeg, MIME_TYPES.svg, MIME_TYPES.gif]}>{/* children */}</Dropzone>
  );
}
```
