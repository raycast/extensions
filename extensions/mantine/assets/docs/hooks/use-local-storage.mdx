...
Hook works exactly the same as `useState`, but also writes the value to local storage:
...
function ColorSchemeToggle() {
  ...
  return <ActionIcon onClick={toggleColorScheme}>{colorScheme === "dark" ? <Sun /> : <MoonStars />}</ActionIcon>;
}
```
