...
import { ButtonDemos, SliderDemos, TabsDemos, AccordionDemos, CalendarDemos, ThemeDemos } from "@mantine/demos";
...
<DataTable
  ...
  data={Object.keys(SliderStylesApi).map((name) => [name, `.mantine-slider-${name}`, SliderStylesApi[name]])}
/>
...
<DataTable
  head={["Name", "Static selector", "Description"]}
  data={Object.keys(ButtonStylesApi).map((name) => [name, `.mantine-button-${name}`, ButtonStylesApi[name]])}
/>
