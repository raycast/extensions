...
function Demo() {
  return (
    <MantineProvider theme={{ primaryShade: { light: 6, dark: 8 } }} withGlobalStyles withNormalizeCSS>
      <App />
    </MantineProvider>
  );
}
...
<DataTable
  ...
  data={Object.keys(DEFAULT_THEME.breakpoints).map((size) => [size, `${DEFAULT_THEME.breakpoints[size]}px`])}
/>
