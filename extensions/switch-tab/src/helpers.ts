...
export function getTabIcon(tab: { url?: string; title?: string; favIconUrl?: string }) {
  ...
  return {
    source: `https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${encodeURIComponent(tab.url || "")}&size=128`,
    fallback: Icon.Globe,
...
export function getBookmarkIcon(url: string | undefined) {
  ...
  return {
    source: `https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${encodeURIComponent(url)}&size=128`,
    fallback: Icon.Globe,
    mask: Image.Mask.RoundedRectangle,
  };
}
