...
const fetchDashboard = () =>
  useFetch<GocdBoard>(`${GoCDBaseUrl}/go/api/dashboard`, {
    headers: { ...GOCDAcceptHeaders.v3, ...getBearerTokenHeader(GocdPAT) }
  })

const fetchPipelineStatus = (pipelineName: string) =>
  useFetch<GocdPipelineStatus>(`${GoCDBaseUrl}/go/api/pipelines/${pipelineName}/status`, {
    headers: { ...GOCDAcceptHeaders.v1, ...getBearerTokenHeader(GocdPAT) }
  })

const fetchPipelineHistory = (pipelineName: string) =>
  useFetch<GocdPipelineHistory>(`${GoCDBaseUrl}/go/api/pipelines/${pipelineName}/history`, {
    headers: { ...GOCDAcceptHeaders.v1, ...getBearerTokenHeader(GocdPAT) }
  })
