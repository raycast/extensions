...
} from '../../apis/types/grafana.type'
import {
  buildGrafanaClient, GrafanaClient,
} from '../../apis/grafana'
import { isEmpty } from 'lodash'
import { withConfig } from "../../utils/configurationCenter";
import { useCachedPromise } from "@raycast/utils";

export const GrafanaCommand: FC = withConfig(
  ({ configurations: { grafanaBaseUrl, grafanaPAT } }) => {
    const [keyword, setKeyword] = useState('')
    const [selectedFolder, setFolder] = useState('')
    const grafanaClient = useMemo(
      () => buildGrafanaClient(grafanaBaseUrl, grafanaPAT),
      [grafanaBaseUrl, grafanaPAT]
    );

    const { data: folders = [] as GrafanaSimpleFolders } = useCachedPromise(async (client: GrafanaClient) => {
    ...
    const {
      isLoading,
      data: dashboardMap = {}
    } = useCachedPromise(async (folders: GrafanaSimpleFolders, client: GrafanaClient) => {
