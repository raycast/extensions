...
export function PomodoroRunning({ pomodoroId }: Props) {
  ...
  function refresh() {
    ...
    setTimer((prev) => {
      ...
      if (
        ...
        prev.pomodoroPhase === t.pomodoroPhase &&
        prev.pomodoroCycle === t.pomodoroCycle
      )
        return prev;
      return t;
    });
  ...
  function handlePause() {
    pauseTimer(pomodoroId);
    refresh();
    showHUD("⏸ Pomodoro paused");
  }
  function handleResume() {
    resumeTimer(pomodoroId);
    refresh();
    showHUD("▶ Pomodoro resumed");
  }
  ...
  async function changeVolume(v: number) {
    setVolumeState(v);
    await setVolume(v);
    updateTimerSound(pomodoroId, sound, v);
  }
  async function changeSoundFn(id: string) {
    setSoundState(id);
    await setSound(id);
    updateTimerSound(pomodoroId, id, volume);
  }
      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.Label
            ...
            text={`${phaseLabel}`}
            icon={{
              source: phase === "work" ? Icon.Bolt : Icon.Circle,
              tintColor: phase === "work" ? Color.Red : Color.Green,
            }}
          />
          ...
          <Detail.Metadata.Label
            title="Cycles"
            text={
              (timer.pomodoroMaxCycles ?? 0) > 0
                ? `${cycle} / ${timer.pomodoroMaxCycles}`
                : `${cycle} (∞)`
            }
            ...
            <Action
              ...
              onAction={() =>
                push(
                      ...
                      <ActionPanel>
                        <Action.SubmitForm
                          ...
                          onSubmit={(v: { cycles: string }) => {
                            ...
                            if (v.cycles && (isNaN(n) || n < 1)) {
                              showHUD(
                                "Enter a number ≥ 1, or leave empty for infinite",
                              );
                              return;
                            }
                            const state = readState();
                            ...
                            if (t) {
                              ...
                              if (n > 0 && n < cur) {
                                showHUD(
                                  `Already on cycle ${cur} — can't set lower`,
                                );
                                return;
                              }
                  ...
                  >
                    <Form.Description
                      text={`Current cycle: ${cycle}${(timer.pomodoroMaxCycles ?? 0) > 0 ? ` / ${timer.pomodoroMaxCycles}` : " (infinite)"}`}
                    />
                    <Form.TextField
                      id="cycles"
                      title="Max cycles"
                      ...
                      defaultValue={
                        (timer.pomodoroMaxCycles ?? 0) > 0
                          ? String(timer.pomodoroMaxCycles)
                          : ""
                      }
                    />
                  </Form>,
