  ...
  async function handleMarkAsRead(link: Link) {
    ...
    try {
      ...
      if (!result.success) {
        ...
        showFailureToast(new Error(result.error || "Please try again"), { title: "Failed to mark as read" });
      }
    } catch (error) {
      // If it failed, add the link back and show error
      setLinks((currentLinks) => [...currentLinks, link]);
  ...
  async function handleDelete(link: Link) {
    ...
    try {
      ...
      if (!result.success) {
        // If it failed, add the link back and show error
        setLinks((currentLinks) => [...currentLinks, link]);
        showFailureToast(new Error(result.error || "Please try again"), { title: "Failed to delete link" });
      }
    } catch (error) {
      // If it failed, add the link back and show error
      setLinks((currentLinks) => [...currentLinks, link]);
      showFailureToast(error, { title: "Failed to delete link" });
    }
  }
