...
- Report an unreadable Arc window as a transient state and keep polling, and only reinterpret it as Little Arc if the detection deadline expires without a single poll ever having read a URL — a lone unreadable sample, even the one crossing the deadline, proves nothing. Adapters can now refine an expired deadline into a more specific error through a new optional `describeTimeout` hook, so a genuinely unscriptable window still gets its actionable message rather than a bare timeout.
