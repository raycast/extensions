...
export function FormModel(props: props): React.JSX.Element {
  ...
  const InfoServer = "AI Server or Provider.";
  ...
  async function Submit(values: FormData): Promise<void> {
    const getServerSettings = async (serverName: string): Promise<OllamaServer | undefined> => {
      if (isCustomServer(serverName)) {
        return undefined;
      }
      return await GetOllamaServerByName(serverName);
    };

    const serverSettings: Map<string, OllamaServer | undefined> = new Map();
    serverSettings.set(values.serverMain, await getServerSettings(values.serverMain));
    if (values.serverVision && !serverSettings.has(values.serverVision))
      serverSettings.set(values.serverVision, await getServerSettings(values.serverVision));
    if (values.serverTools && !serverSettings.has(values.serverTools))
      serverSettings.set(values.serverTools, await getServerSettings(values.serverTools));
    let chat = props.Chat;
    if (chat && props.ChatNameIndex !== undefined) {
      chat.models.main = {
        server_name: values.serverMain,
        server: serverSettings.get(values.serverMain),
        tag: values.modelMain,
        thinking: values.thinkingMain === "false" ? false : (values.thinkingMain as ThinkingEffortOllama),
        keep_alive: !isCustomServer(values.serverMain) && CheckboxMainAdvanced ? values.keepAliveMain : undefined,
      };
      if (values.serverVision && values.modelVision) {
        chat.models.vision = {
          server_name: values.serverVision,
          server: serverSettings.get(values.serverVision),
          tag: values.modelVision,
          thinking: values.thinkingVision === "false" ? false : (values.thinkingVision as ThinkingEffortOllama),
          keep_alive:
            !isCustomServer(values.serverVision) && CheckboxVisionAdvanced ? values.keepAliveVision : undefined,
        };
      ...
      if (values.serverTools && values.modelTools) {
        chat.models.tools = {
          ...
          server: serverSettings.get(values.serverTools),
          tag: values.modelTools,
          thinking: values.thinkingTools === "false" ? false : (values.thinkingTools as ThinkingEffortOllama),
          keep_alive: !isCustomServer(values.serverTools) && CheckboxToolsAdvanced ? values.keepAliveTools : undefined,
        };
          ...
          tools:
            CheckboxTools && values.serverTools && values.modelTools
              ? {
                  ...
                  thinking: values.thinkingTools === "false" ? false : (values.thinkingTools as ThinkingEffortOllama),
                  keep_alive:
                    !isCustomServer(values.serverTools) && CheckboxToolsAdvanced ? values.keepAliveTools : undefined,
                }
              : undefined,
        },
        messages: [],
