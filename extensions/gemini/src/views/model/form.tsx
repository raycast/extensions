...
import { getConfiguration } from "../../hooks/useGemini";
...
export const ModelForm = (props: { model?: Model; use: { models: ModelHook }; name?: string }) => {
  ...
  const { handleSubmit, itemProps, setValue } = useForm<Model>({
    ...
    initialValues: {
      ...
      option: model?.option ?? "gemini-2.0-flash",
    ...
    >
      ...
      {showAwesomePrompts && (
        <Form.Dropdown
          ...
          onChange={setPrompt}
        >
          <Form.Dropdown.Item value="none" title="Choose an Awesome Gemini Prompts" icon={"⭐"} />
          {(data || []).map((prompt) => (
