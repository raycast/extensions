...
export default function Roll(props: LaunchProps<{ arguments: RollArguments; }>) {
    ...
    return <List
        ...
        searchBarAccessory={<List.Dropdown tooltip="Some actions" value={searchText} onChange={value => value.length > 0 ? handleRoll(value) : undefined}>
            <List.Dropdown.Item title="Choose Dice" value="" />
            <List.Dropdown.Item title="1d20" value="1d20" />
        </List.Dropdown>}
        actions={<ActionPanel>
            <Action title="Roll Dice" onAction={() => handleRoll(searchText.length > 0 ? searchText : undefined)}></Action>
        </ActionPanel>}
    >
        {!searchText && results.map((result, i) => <List.Item
            key={i}
        ...
        />)}
        {results.length < 1
            ? <List.EmptyView title="No dice rolled" icon={Icon.Box} description="Type in the dice you want to roll in the search bar!" />
