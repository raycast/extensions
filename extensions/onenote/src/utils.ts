...
const ONENOTE_USER_INFO_CACHE = resolve(homedir(),
...
export async function openNote(item: OneNoteItem) {
    var cmd = (await getUrl(item));
    cmd = 'open ' +  cmd?.replaceAll(" ", "%20")
                        .replaceAll("&", "%26")
                        .replaceAll("{", "%7B")
                        .replaceAll("}", "%7D")
    console.log(cmd);
    exec(cmd, (err, sdtout, stderr) => {closeMainWindow();});
  }

async function getUrl(item: OneNoteItem) {
    var userid:string = (await get_user_uid());

    if (item.Type == PAGE)
        return `onenote:#page-id=${item.GUID}`;
    ...
    if (item.Type == NOTEBOOK)
        return `onenote:https://d.docs.live.net/${userid}/Documents/${item.Title}`;
}

export function getAncestorsStr(item: OneNoteItem|undefined, separator: string, includeSelf: boolean=true) {
    // TODO BUG HERE: WEIRD ORDER WITH NESTED SECTION GROUPS
    if (item == undefined )
        return ""
    var gpGOIDs: string[] = split_grandparents(item);
    var ancestors: string[] = gpGOIDs.map(x=>getCachedTitle(x))
    if (item.ParentGOID)
        ancestors.push(getParentTitle(item));
    if (includeSelf)
        ancestors.push(item.Title);
...
export function parseDatetime(datetime:number): string {
    ...
    var corrected = datetime/10000-11644470000000;
    
    var now = Date.now();
    var val = new Date(corrected);
    var diff = now.valueOf() - val.valueOf()
    if (diff > 1000*3600*24*90)
        return String(val.getFullYear());
    if (diff > 1000*3600*24*7)
        return dateFormat(val, "d mmm");
...
export function getIcon(item: OneNoteItem) {
    ...
    if (item.Type == SECTION)
        return { source: Icon.Folder, tintColor: "#C35BE2" }
    if (item.Type == GROUP)
        return { source: Icon.ChevronRight, tintColor: "#C35BE2" }
    if (item.Type == NOTEBOOK)
        return { source: Icon.Tray, tintColor: "#641996" }
};

async function get_user_uid(): Promise<string> {
    var ONENOTE_USER_UID = (await 
        LocalStorage.getItem<string>("onenote-user-uid")) as string;
    if (ONENOTE_USER_UID != undefined) {
        return ONENOTE_USER_UID;
    }    
    const files = readdirSync(ONENOTE_USER_INFO_CACHE);
    for (const file of files){
        var indx = file.indexOf('LiveId.db');
        if (indx != -1) {
...
export function split_grandparents(item: OneNoteItem) {
    var parts = item.GrandparentGOIDs?.split("}");
    var newIds = []
    var i=0;
...
export function getParentTitle(item: OneNoteItem): string {
    if (item.ParentGOID)
        return getCachedTitle(item.ParentGOID);
    else
        return "";
}