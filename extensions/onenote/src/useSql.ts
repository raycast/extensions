...
const ONENOTE_FULL_SEARCH_PATH = resolve(homedir(),"Library/Containers/com.microsoft.onenote.mac/Data/Library/Application Support/Microsoft User Data/OneNote/15.0/FullTextSearchIndex");
...
const create_or_update_db = async (force_update = false) => {
  ...
  var lastOneNoteModification = (await LocalStorage.getItem<number>("onenote-db-time")) as number;
  if (lastOneNoteModification == undefined) lastOneNoteModification = 0;
  var NEEDUPDATE = force_update;

  var ALL_DB_PATHS: string[] = []
  var ALL_DB: Database[] = []
  var ALL_DB_NAMES: string[] = []
  ...
  for (const file of db_files){
      if (file.indexOf('.db') != -1 && file.indexOf('journal') == -1 ) { 
          var filepath = resolve(ONENOTE_FULL_SEARCH_PATH, file);
  ...
  try {
      const merged_file = await readFile(ONENOTE_MERGED_DB); 
  } catch (error) {        
      NEEDUPDATE = true;
  }    
  ...
  for (const db_file of ALL_DB_PATHS) {        
      ...
      var dbname = db_t.exec("select file from pragma_database_list where name='main';")[0].values[0][0];
  ...
  for (var index in ALL_DB) {
      ...
      db.run("INSERT INTO Entities (\
              ...
              (select group_concat(text, '\n\n') FROM db"+ index + ".PageElements as PE2 WHERE PE2.EntityRowId = E.rowid) \
              FROM db"+ index + ".Entities as E;");      
  }
  
  // CACHING PARENTS' TITLE :
  var results = db.exec("SELECT DISTINCT GOID, Title FROM Entities WHERE Type > 1");   
  ...
  // console.log(`MERGED DATABASE CREATED in ${(Date.now()-start)} ms`);

  // CLOSE DBs
  for (var _db of ALL_DB) {
    _db.close();
  }
  db.close();
  return true;
