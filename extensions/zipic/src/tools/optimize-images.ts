...
type Input = {
  ...
  /**
   * Subfolder name when addSubfolder is true
   * @default "zipic"
   */
  subfolder?: string;

  /**
   * Whether to overwrite the original file when saving to the original location
   * @default true
   */
  overwrite?: boolean;

  /**
   * Whether to encode JPEG output as progressive
   * @default true
   */
  progressive?: boolean;
  ...
  /**
   * When saveLocation is "custom", whether to use the Zipic app's default save
   * directory instead of the explicit `saveDirectory`. When true, `saveDirectory`
   * is ignored.
   * @default false
   */
...
export default async function tool({
  ...
  try {
    ...
    urlParams += `overwrite=${overwrite}&`;
    urlParams += `progressive=${progressive}&`;
    urlParams += `ratio=${keepAspectRatio}&`;
