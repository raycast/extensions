      ...
      {journeys.map((journey, index) => {
        ...
        return (
          <List.Item
            ...
            detail={
              <List.Item.Detail
                markdown={getDetailMarkdown(journey)}
                metadata={getDetailMetadata(journey)}
              />
            }
            actions={
  ...
  return (
    <List.Item.Detail.Metadata>
      {journey.sections.map((section, index) => {
        ...
        const readableBaseDepartureTime = section.baseDepartureTime ? dateToReadableDate(section.baseDepartureTime) : null;
        const readableBaseArrivalTime = section.baseArrivalTime ? dateToReadableDate(section.baseArrivalTime) : null;

        const hasDisruptions = readableBaseDepartureTime && readableBaseArrivalTime && (readableDepartureTime !== readableBaseDepartureTime || readableArrivalTime !== readableBaseArrivalTime);

        console.log('section', section)
      
        return (
          <React.Fragment key={index}>
            <List.Item.Detail.Metadata.TagList title={hasMultipleSections ? `${section.from} -> ${section.to}` : "Detail"}>
              <List.Item.Detail.Metadata.TagList.Item text={hasDisruptions ? "Delayed" : "In time"} color={hasDisruptions ? '#ff0000' : '#00ff00'} />
