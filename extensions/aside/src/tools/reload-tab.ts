import { reloadAsideTabById } from "../lib/applescript";
...
/** Reload an Aside tab using the exact live ID returned by a tab read tool. */
export default async function tool(input: Input) {
  const tabId = requireNonEmpty(input.tabId, "Tab ID");
  return { tabId, status: await reloadAsideTabById(tabId) };
}
