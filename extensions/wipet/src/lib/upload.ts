...
export async function inspectFile(path: string): Promise<LocalImage> {
  ...
  if (!mimeType) {
    throw new Error(
      `${basename(path)}: unsupported file type (${ext || "no extension"})`,
    );
  }
  ...
  if (st.size > MAX_BYTES) {
    throw new Error(
      `${basename(path)}: too large (max ${Math.round(MAX_BYTES / 1024 / 1024)}MB)`,
    );
  }
...
export async function uploadFiles(
  ...
  const uploads = await presignUploads(
    images.map((i) => ({ mimeType: i.mimeType, sizeBytes: i.sizeBytes })),
  );
  ...
  await Promise.all(
    uploads.map(async (u, i) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Upload failed for ${basename(file.path)}: ${txt || res.statusText}`,
        );
      }
    }),
  );
