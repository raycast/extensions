...
export default function GenCommand(props: LaunchProps<{ arguments: SearchArguments }>) {
  ...
  const format = data?.format?.replace(/\[img\](.*?)\[\/img\]/, '<img src="$1" width="100" />').replace(/\n/g, "  \n");

  const titleMatch = data?.format?.match(/◎片\s+名\s+(.+?)\n/);
  const title = isLoading ? "Loading..." : titleMatch ? titleMatch[1] : "未知标题";
  ...
  return (
    <Detail
      ...
      actions={
        <ActionPanel>
          <Action.CopyToClipboard title="Copy Content" content={data?.format ?? ""} />
