...
export type SQLChat = Omit<ChatParticipant, "is_group"> & {
  guid: string;
  chat_row_id: number;
...
export function buildChatQuery({
  ...
  return `
    SELECT
      ...
      CASE
        WHEN chat.chat_identifier LIKE '%chat%'
