import { Action, ActionPanel, Icon, List, getPreferenceValues, openExtensionPreferences, Toast, showToast } from '@raycast/api';
...
export default function Command() {
  ...
  const { data, isLoading } = useFetch<ObjectsList>(`https://api.cosmicjs.com/v3/buckets/${bucketSlug}/objects?query=%7B%22type%22:%22bookmarks%22%7D&read_key=${readKey}&depth=1&props=`);
      ...
      <List.Item.Detail
        ...
        metadata={
          <List.Item.Detail.Metadata>
            ...
            <List.Item.Detail.Metadata.Label title='Status' text={data?.object.metadata.read ? 'Read' : 'Unread'} />
          </List.Item.Detail.Metadata>
        }
      />
    )
...
async function deleteBookmark(id: string, title: string, bucketSlug: string, readKey: string, writeKey: string) {
  const cosmic = createBucketClient({
    bucketSlug: bucketSlug,
    readKey: readKey,
    writeKey: writeKey,
  ...
  try {
    await showToast({ style: Toast.Style.Animated, title: `Deleting ${title}` });
