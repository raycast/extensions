...
export function generateApiUrl(endpoint: string, params: { [key: string]: string }) {
  try {
...
function useDirectAdmin<T>(endpoint: string, options: UseDirectAdminOptions<T> = { params: {}, animatedToastMessage: "", userToImpersonate: "" }) {
    try {
        ...
        if (error instanceof FetchError) {
          ...
          message = "Invalid URL - make sure the DirectAdmin URL is valid";
        }
        showFailureToast(message, { title: "DirectAdmin Error" });
        return { isLoading: false, error: { name: "", message } }
      }    
}

export function useLegacyDirectAdmin<T>(cmd: string, options: UseDirectAdminOptions<T> = { params: {}, animatedToastMessage: "", userToImpersonate: "" }) {
  return useDirectAdmin<T>(`CMD_API_${cmd}`, {...options, params: {
    ...options.params,
    json: "yes"
  }});
}

export function useJsonDirectAdmin<T>(cmd: string, options: UseDirectAdminOptions<T> = { params: {}, animatedToastMessage: "", userToImpersonate: "" }) {
