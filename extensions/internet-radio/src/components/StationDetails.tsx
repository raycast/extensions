...
${showOtherImages ? "## Images:\n" + data.otherImages.map((imageURL) => `![Station Image](${imageURL})`) : ""}`}
      metadata={
        <List.Item.Detail.Metadata>
          ...
          {showAltSites ? data.alternateSites.map((siteData: WebsiteData) => <List.Item.Detail.Metadata.Link title={siteData.title} text={siteData.url} target={siteData.url} />) : null}
          ...
          {showAltStreams ? data.alternateStreams.map((streamURL, index) => <List.Item.Detail.Metadata.Link title={`Alternate Stream #${index}`} text={streamURL} target={streamURL} />) : null}
          ...
          {showSocialProfiles ? data.socialProfiles.map((socialProfile: SocialProfileData) => <List.Item.Detail.Metadata.Link title={socialProfile.type} text={socialProfile.handle} target={socialProfile.url} />) : null}
          ...
          {showCallsigns ? data.socialProfiles.map((socialProfile: SocialProfileData) => <List.Item.Detail.Metadata.Link title={socialProfile.type} text={socialProfile.handle} target={socialProfile.url} />) : null}
          ...
          {showContacts ? data.contacts.map((contactData: ContactData) => <List.Item.Detail.Metadata.Label title={contactData.title} text={contactData.value} icon={contactIcons[contactData.type]} />) : null}

          {showPolicies ? <List.Item.Detail.Metadata.Separator /> : null}
          {showPolicies ? <List.Item.Detail.Metadata.Label title="Policies" /> : null}
          {showPolicies ? data.policies.map((policyData: PolicyData) => <List.Item.Detail.Metadata.Link title={policyData.title} text={policyData.url} target={policyData.url} />) : null}
        </List.Item.Detail.Metadata>
      }
    />
  );
