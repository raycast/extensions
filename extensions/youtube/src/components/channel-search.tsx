...
import { ListOrGrid, ListOrGridEmptyView, ListOrGridSection } from "./list-or-grid";
import * as cache from "./cached-channels";

const { gridColumns, showRecentChannels } = getPreferenceValues<Preferences>();
...
export function SearchChannelList() {
  ...
  return data ? (
    <ListOrGrid isLoading={isLoading} columns={gridColumns} onSearchTextChange={setSearchText} throttle={true}>
      {data.map((c) => (
