...
import { ArrOrObj, DomainDNSGetHostsResult, DomainDNSGetListResult, DomainGetListResult, ErrorCall, NCResponse } from "./types";
...
const generateUrl = (command: string, params?: Record<string, string>) => ApiUrl + "?" + new URLSearchParams({
    ...preferences,
...
const parseResponse = async (response: Response) => {
    ...
    const result: NCResponse<any> = await xml2js.parseStringPromise(xml, { explicitArray: false, attrValueProcessors: [parseBooleans] });
    if (isErrorCall(result)) throw new Error(result.ApiResponse.Errors.Error._, { cause: result.ApiResponse.Errors.Error.$.Number });
...
export const useListDomainDNSServers = (SLD: string, TLD: string) => useFetch(generateUrl("domains.dns.getList", { SLD, TLD }), {
    parseResponse,
...
export const useListDomainDNSHosts = (SLD: string, TLD: string) => useFetch(generateUrl("domains.dns.getHosts", { SLD, TLD }), {
    parseResponse,
