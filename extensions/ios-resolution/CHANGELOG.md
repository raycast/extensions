# iOS Resolution Changelog

## [2025–2026 Device Update] - {PR_MERGE_DATE}

- Add both iPhone Duo displays, iPhone 18 Pro and Pro Max, and iPhone 17e
- Add iPad (A16), iPad Air with M3 and M4, and iPad Pro with M5
- Add Apple Watch SE 3, Series 11 and 12, and Ultra 3 and 4
- Add MacBook Neo, MacBook Air with M5, and MacBook Pro with M5 Pro and M5 Max
- Correct recent MacBook Air display data and the M5 MacBook Pro release date
- Clearly mark unverified logical resolutions and scale factors and omit their copy actions

## [Addition of Device Type (MacBook)] - 2026-02-06

- Addition of New Device type (MacBook)
- Browse all MacBook models
- View MacBook specifications: logical/physical resolution, PPI, scale factor, screen diagonal, aspect ratio

## [Initial Version] - 2026-01-23

- Browse all devices (iPhone, iPad, iPod touch, Apple Watch)
- View device specifications: logical/physical resolution, PPI, scale factor, screen diagonal, aspect ratio
- Filter devices by family (iPhone, iPad, iPod touch, Apple Watch)
- Sort devices by release date (newest first)
- Copy resolution data to clipboard
- Detailed metadata view for each device
