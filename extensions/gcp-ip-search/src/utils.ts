...
export async function searchIPInProject(
  ...
  const results = await Promise.all([
    ...
    // Search in GLOBAL forwarding rules
    (async (): Promise<SearchResult[]> => {
      try {
        const { stdout } = await execAsync(
          `"${gcloud}" compute forwarding-rules list --global --project "${projectId}" --filter="IPAddress='${ip}'" --format="json(name,IPAddress,IPProtocol,ports,target,loadBalancingScheme,description,creationTimestamp)" 2>/dev/null`,
        );
        const rules = JSON.parse(stdout);
        const fwResults: SearchResult[] = [];
  ...
  const [
    ...
    addressResults,
    forwardingResults,
    globalForwardingResults,
  ] = results;

  // Deduplicate: If we found a Real Address (in addressResults), remove the Synthetic Address (in instanceResults) for the same IP.
  const realIPs = new Set(addressResults.map((r) => r.ipAddress));

  const filteredInstanceResults = instanceResults.filter((r) => {
    ...
    return true; // Keep instance resources
  });

  // Flatten results
  // Order matters: Instances first, then Addresses, then Forwarding Rules
  return [
...
export function getResourceTypeName(
  ...
  switch (resourceType) {
    case "forwarding-rules":
      return t.resourceTypeForwardingRule;
    case "addresses":
      return t.resourceTypeAddress;
    case "instances":
      return t.resourceTypeComputeInstance;
    case "routers":
      return t.resourceTypeRouter; // Add this key to locales
    default:
      return t.resourceTypeResource;
  }
}
