...
import { exportToIosFiles } from "./export-handlers";
...
export default async function main() {
  ...
  try {
    ...
    // 检查iOS输出路径是否已配置
    if (!preferences.iosOutputPath) {
      await showHUD("❌ 错误: 未设置iOS输出路径");
      return;
    }
    ...
    const success = await exportToIosFiles(data, preferences.iosOutputPath);
