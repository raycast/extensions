import { ActionPanel, Form, Action, showToast, ToastStyle, Detail, AI, Clipboard, open } from "@raycast/api";
import { useState } from "react";

export default function AskRaycastAI() {
  const [userInput, setUserInput] = useState("");
  const [extractedUrl, setExtractedUrl] = useState(null); // State to hold the extracted URL
  const [isLoading, setIsLoading] = useState(false);

  const defaultPrompt = `# Task: Generate a Marketplace URL
  ...
  **Do not come up with new IDs that are not available in the provided JSON attachments.**
  ...
  -  Estimated Views (mediaProject_seoMetric.estimatedViews)
  -  Search Traffic (mediaProject_seoMetric.sourceSearch)
  -  Direct Traffic (mediaProject_seoMetric.sourceDirect)
  -  Social Traffic (mediaProject_seoMetric.sourceSocial)
  ...
  **KEEP in mind that filters are put into "mediaProject.seoMetric" but columns and sorting options are in "mediaProject_seoMetric" and always check for it.**
  ...
  # User Request to Generate an URL for
  ...
  const handleSubmit = async () => {
    if (!userInput) {
      showToast(ToastStyle.Failure, "Please enter a request");
      return;
    }

    setIsLoading(true);
    try {
      const fullPrompt = `${defaultPrompt}${userInput}`;
      const aiResponse = await AI.ask(fullPrompt, { model: AI.Model["OpenAI_GPT4o-mini"] }); // Call AI.ask with the user input

      // Regular expression to extract the first URL from the response
      const urlRegex = /(https?:\/\/[^\s]+)/;
      const match = aiResponse.match(urlRegex);

      if (match) {
        Clipboard.copy(match[0]);
        showToast(ToastStyle.Success, "Marketplace link has been copied to the clipboard");
        open(match[0]);
        setExtractedUrl(match[0]); // Set the extracted URL
      } else {
        showToast(ToastStyle.Failure, "No URL found in the response");
      }
    } catch (error) {
      showToast(ToastStyle.Failure, "An error occurred", error.message || "Unknown error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (extractedUrl) {
    // Render the Detail component if there's an extracted URL
    return (
      <Detail
  ...
  // Render the form if there's no extracted URL
  return (
    <Form
      isLoading={isLoading}
      actions={
    ...
    >
      <Form.TextArea
        id="userInput"
        title="What do you promote, and through which media?"
        placeholder="Provide details such as the country, preferred metrics, price per placement, and information on what you want to promote..."
        value={userInput}
