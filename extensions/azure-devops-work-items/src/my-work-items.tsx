...
function useProcessedDescription(
...
function useProcessedHtmlList(
  htmls: (string | undefined)[],
  key: string,
): ProcessedDescription | null {
  const [media, setMedia] = useState<ProcessedDescription | null>(null);
  useEffect(() => {
...
async function writeAppSettings(settings: AppSettings): Promise<void> {
  await Promise.all([
    LocalStorage.setItem(
      ...
      JSON.stringify(settings.project),
    ),
    ...
    LocalStorage.setItem(
      STORAGE_KEYS.defaultRepo,
      JSON.stringify(settings.defaultRepo),
    ),
    LocalStorage.setItem(
      STORAGE_KEYS.defaultBaseBranch,
      JSON.stringify(settings.defaultBaseBranch),
    ),
  ...
  return (
    ...
    >
      ...
      {groupKeys.map((key) => {
        ...
        const groupSelected = groupItems.filter((i) =>
        ...
        return (
          <List.Section
            ...
            subtitle={
              groupSelected > 0
                ? `${groupItems.length} · ${groupSelected} selected`
...
function WorkItemView({
  ...
  const { data: rawAttachments, isLoading: loadingAttachments } =
    useCachedPromise(getWorkItemAttachments, [current.id], {
      initialData: [],
      keepPreviousData: false,
      onError: (err) => {
        showToast({
  ...
  const extraAttachments = fileAttachments.filter(
    (a) => !inlineUrls.has(a.originalUrl),
  );
    ...
    >
      <List.Section title="Item">
        <List.Item
          ...
          detail={
            <WorkItemDetail item={current} withChildren={false} media={media} />
          }
          actions={
            <ActionPanel>
              <ActionPanel.Section>
      ...
      <List.Section
        title="Attachments"
        subtitle={attachmentsLoading ? "loading…" : String(images.length)}
      >
        ...
        {images.map((img, i) => {
          ...
          return (
            <List.Item
              ...
              subtitle={
                img.alt && img.alt !== img.filename ? img.alt : undefined
              }
...
function WorkItemDetail({
  ...
  const ownMedia = useProcessedDescription(
    providedMedia === undefined ? item.description : undefined,
  );
  ...
  const descriptionMd =
    media?.markdown ??
    (item.description ? stripHtml(item.description) : "_No description_");
...
function SetupView({
  ...
  const { data: fetchedProjects, isLoading: loadingProjects } =
    useCachedPromise(getProjects, [], { initialData: [] });
  ...
  const { data: allRepos, isLoading: loadingRepos } = useCachedPromise(
    getAllRepositories,
    [],
  ...
  const selectedRepo = repoList.find(
    (r) => r.name.toLowerCase() === defaultRepo.toLowerCase(),
  );
  ...
  const repoInList =
    !defaultRepo || repoList.some((r) => r.name === defaultRepo);
  const baseBranchInList =
    !defaultBaseBranch || branchList.includes(defaultBaseBranch);
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm
            title={firstRun ? "Get Started" : "Save"}
            icon={Icon.Check}
    ...
    >
      ...
      >
        ...
        {!repoInList && (
          <Form.Dropdown.Item value={defaultRepo} title={defaultRepo} />
        )}
        {repoList.map((r) => (
          <Form.Dropdown.Item
            key={r.id}
            value={r.name}
            title={`${r.name}  ·  ${r.projectName}`}
          />
        ))}
      </Form.Dropdown>
      <Form.Dropdown
...
function CreateBranchForm({ item }: { item: WorkItem }) {
  ...
  const [defaultBaseBranchSetting, setDefaultBaseBranchSetting] =
    useState<string>("");
  ...
  useEffect(() => {
    ...
    const byPref = wantedNorm
      ? (repoList.find((r) => norm(r.name) === wantedNorm) ??
        repoList.find((r) => norm(r.name).includes(wantedNorm)))
      : undefined;
    const byProject = repoList.find(
      (r) =>
        r.projectName === item.project && norm(r.name) === norm(item.project),
    );
    ...
    if (wanted && !byPref) {
      const sample = repoList
        ...
        .join(", ");
      const more =
        repoList.length > 8 ? ` … (+${repoList.length - 8} more)` : "";
      showToast({
        style: Toast.Style.Failure,
  ...
  useEffect(() => {
    ...
    const exists = (name: string) =>
      branchList.find((b) => b.toLowerCase() === name.toLowerCase());
  ...
  return (
    ...
    >
      <Form.Description
        text={`Branch will be created from "${baseBranch || "…"}" and linked to #${item.id}.`}
      />
      <Form.Dropdown
        id="repo"
        title="Repository"
      ...
      >
        <Form.Dropdown.Item
          value=""
          title={repoList.length === 0 ? "Loading repos…" : "—"}
        />
        {repoList.map((r) => (
          <Form.Dropdown.Item
            key={r.id}
      ...
      <Form.Dropdown
        id="baseBranch"
        title="Base Branch"
        value={safeBaseBranch}
        onChange={handleBaseBranchChange}
      >
        <Form.Dropdown.Item
          value=""
          title={branchList.length === 0 ? "Loading branches…" : "—"}
        />
        {branchList.map((b) => (
          <Form.Dropdown.Item
            key={b}
            value={b}
            title={b === repoDefault ? `${b}  (default)` : b}
          />
        ))}
      </Form.Dropdown>
      <Form.TextField
...
function CreateWorkItemForm({
  ...
  useEffect(() => {
    readAppSettings().then(({ settings }) =>
      setAppDefaultProject(settings.project),
    );
  }, []);
...
function BulkChangeStateView({
  ...
  const uniquePT = Array.from(
    new Map(
      items.map((i) => [
        `${i.project}|${i.workItemType}`,
        [i.project, i.workItemType] as const,
      ]),
    ).values(),
  );
  ...
  const { data: states, isLoading } = useCachedPromise(
    async (key: string) => {
      ...
      const sets = await Promise.all(
        uniquePT.map(([p, t]) => getWorkItemStates(p, t).catch(() => [])),
      );
        ...
        {items.map((i) => {
          ...
          return (
            <List.Item
              ...
              subtitle={`#${i.id}`}
              accessories={[
                { tag: { value: i.state, color: getStateColor(i.state) } },
              ]}
            />
          );
        })}
