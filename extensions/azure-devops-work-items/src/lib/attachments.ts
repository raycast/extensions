...
function getAuthHeader(): string {
  ...
  return `Basic ${Buffer.from(`:${prefs.pat}`).toString("base64")}`;
}
...
export async function fetchAttachmentByName(
  url: string,
  name: string,
...
export async function processDescription(
  ...
  if (!html || !html.trim())
    return { markdown: "_No description_", images: [] };
