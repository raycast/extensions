...
export async function getWorkItemTypes(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch work item types (${res.status}): ${await res.text()}`,
    );
  }
 ...
 */
export async function getAllStatesForProject(
  project: string,
): Promise<string[]> {
  const types = await getWorkItemTypes(project);
  const lists = await Promise.all(
    types.map((t) => getWorkItemStates(project, t.name).catch(() => [])),
  );
...
export async function getAllRepositories(): Promise<Repository[]> {
  ...
  const lists = await Promise.all(
    projects.map((p) =>
      getRepositories(p.name).catch(() => [] as Repository[]),
    ),
  );
...
export async function getBranches(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch branches (${res.status}): ${await res.text()}`,
    );
  }
...
export async function getWorkItemAttachments(
  ...
  return (data.relations ?? [])
    ...
    .map((r) => ({
      ...
      name:
        r.attributes?.name ??
        decodeURIComponent(r.url.split("/").pop() ?? "attachment"),
...
export async function createWorkItem(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to create work item (${res.status}): ${await res.text()}`,
    );
  }
  const data = (await res.json()) as AzureWorkItem;
  return mapWorkItem(data);
