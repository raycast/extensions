...
export default async function Command(props: LaunchProps<{ arguments: Arguments.PasteFiglet }>) {
  ...
  if (figletRender) {
    await Clipboard.paste(figletRender);
    await showToast({ title: "Success", message: "FIGlet pasted to active application" });
  }
}
