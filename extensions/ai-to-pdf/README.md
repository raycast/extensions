...
- A file that is already open in Illustrator is converted from what is on disk and left open afterwards. If it has unsaved changes, the conversion stops with a message instead of exporting stale artwork.
- Multi-artboard documents export to one multi-page PDF by default; tick **Save every artboard as its own PDF** for separate files.
