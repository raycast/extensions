...
export default function (props: LaunchProps<{ arguments: CurrentDateArguments; }>) {
    ...
    function updateCurrentTime(queryText: string) {
        ...
        if (!queryText || queryText === "") {
            ...
            var parsedTime = queryText.split(" ")[0];
            var opt = queryText.split(" ")[1];
            var num = Number(queryText.split(" ")[2]);
            var freq = (queryText.split(" ")[3] as dayjs.ManipulateType);
            switch (opt) {
                case "+": { setCurrentTime(dayjs(parsedTime).add(num, freq).format('YYYY-MM-DD')); break; };
                case "-": { setCurrentTime(dayjs(parsedTime).subtract(num, freq).format('YYYY-MM-DD')); break; };
            }
        } else {
        ...
        >
            {
                ...
                formatTime(currentTime ? currentTime.toString() : new Date().toString())
                    .map((time, index) => (
