  ...
  $(".gs_ri").each((i, el) => {
    ...
    try {
      ...
      if (metaParts.length > 1) {
        ...
        if (!potentialPublication.match(/^([a-zA-Z0-9\-_]+\.)+[a-zA-Z]{2,}$/) && potentialPublication.length > 5) {
          publication = potentialPublication;
        } else if (metaParts.length > 2) {
          const thirdPartAsPublication = metaParts[2].replace(/,\s*$/, "");
          if (
            !thirdPartAsPublication.match(/^([a-zA-Z0-9\-_]+\.)+[a-zA-Z]{2,}$/) &&
            thirdPartAsPublication.length > 5
          ) {
      ...
      if (publication === "Unknown" && authors !== "Unknown" && authors !== processedMetaInfo) {
        const remainingText = processedMetaInfo
          ...
          .replace(/^\s*-\s*/, "")
          .trim();
        if (remainingText && !remainingText.match(/^([a-zA-Z0-9\-_]+\.)+[a-zA-Z]{2,}$/) && remainingText.length > 5) {
