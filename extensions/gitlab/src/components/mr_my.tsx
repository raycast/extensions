...
export function useMyMergeRequests(
  ...
  const {
    ...
    `mymrs_${scope}_${state}_${labels ? labels.join(",") : "[]"}_${params ? JSON.stringify(params) : ""}`,
    async (): Promise<MergeRequest[] | undefined> => {
      return await gitlab.getMergeRequests({ state, scope, ...(labels && { labels }), ...(params || {}) });
    },
    {
      deps: [project, scope, state, labels, JSON.stringify(params)],
      secondsToRefetch: 10,
